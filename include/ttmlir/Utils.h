...
template <template <typename...> typename VectorType, typename IntType,
          typename... Rest>
inline llvm::SmallVector<IntType>
calculateStrides(VectorType<IntType, Rest...> const &shape,
                 IntType elementSize = 1) {
  llvm::SmallVector<IntType> strides(shape.size());
  IntType stride = elementSize;
...
IntType volume(Container<IntType, Rest...> const &shape, IntType stride = 1) {
  for (auto dim : shape) {
    stride *= dim;
  }
  return stride;

...
// where a single integer can apply to all dimensions or a specific 2D configuration can be provided
inline llvm::SmallVector<int32_t, 2> parseAttrToTwoElementVector(mlir::Attribute baseAttr) {
    llvm::SmallVector<int32_t, 2> result;

    if (const auto attr = mlir::dyn_cast_if_present<mlir::IntegerAttr>(baseAttr)) {

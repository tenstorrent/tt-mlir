...
struct MemoryLayoutAnalysisResult {
  ...
  MemoryLayoutAnalysisResult(
      const llvm::DenseMap<Operation *, std::vector<TTNNLayoutAttr>>
          &legalLayouts,
      const std::unordered_set<Edge> &memReconfigEdges, const std::vector<Operation *> &spillToDramOps)
      : legalLayouts(legalLayouts), memReconfigEdges(memReconfigEdges), spillToDramOps(spillToDramOps) {}
};

...
struct Conv2dConfigSearchSpace {
  std::optional<llvm::SmallVector<tt::DataType>> dtype;
  std::optional<llvm::SmallVector<tt::DataType>> weightsDtype;
  ...
  void setSearchDtype(llvm::SmallVector<tt::DataType> &&values) {
    dtype = std::move(values);
  }
  void setSearchWeightsDtype(llvm::SmallVector<tt::DataType> &&values) {

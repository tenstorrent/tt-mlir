...
enum class TensorPageLayout { RowMajor = 0, Tiled = 1, kNumValues = 2 };
...
using TensorTypeLayoutsForMemoryLayout =
    std::array<std::vector<TTNNLayoutAttr>,
               static_cast<std::size_t>(TensorMemoryLayoutIndex::kNumValues)>;
...
using TensorTypeLayoutsForPageLayout =
    std::array<TensorTypeLayoutsForMemoryLayout,
               static_cast<std::size_t>(TensorPageLayout::kNumValues)>;
...
inline std::size_t getPageLayoutIndex(Layout layout) {
  assert(layout == Layout::Tile ||
         layout == Layout::RowMajor && "Invalid layout type");

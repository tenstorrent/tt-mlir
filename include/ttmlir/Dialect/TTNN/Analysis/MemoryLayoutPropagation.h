...
private:
  ...
  /// -> the shared reshard result (a ToMemoryConfigOp, or a ToLayoutOp when the
  /// page layout changes). Lets consumers requesting the same reshard (e.g.
  /// q/k/v slices off a fused-QKV matmul) reuse one op. Keying on the target
  /// layout is what makes the op kind consistent per entry: whether a retile is
  /// needed is a pure function of the producer layout and the target layout.

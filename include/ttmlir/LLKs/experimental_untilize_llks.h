...
STRINGIFY(
    ...
    namespace experimental {
      UNPACK(
          template <bool first_pass = true> ALWI void llk_unpack_untilize_pass(
              std::uint32_t operand, std::uint32_t block_tile_cols,
            ...
            UNPACK((_llk_unpack_untilize_pass_<first_pass>(
                base_address + (start_tile_index * page_bytes),
                block_tile_cols)));
          }
          ...
          ALWI void llk_unpack_untilize(std::uint32_t operand,
                                        ...
                                        std::uint32_t start_tile_index = 0) {
            WAYPOINT("UPUW");
            UNPACK((llk_unpack_untilize_pass<true>(operand, block_c_tiles,
                                                   start_tile_index)));
            UNPACK((llk_unpack_untilize_pass<false>(operand, block_c_tiles,
                                                    start_tile_index)));
            WAYPOINT("UPUD");
          });

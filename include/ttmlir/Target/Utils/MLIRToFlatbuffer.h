...
// NOTE: toNative(T) produces a FlatBuffers native C++ object (e.g.
//       Conv2dConfigT) without a builder; toFlatbuffer(cache, T) then calls
//       Pack(fbb, &t) to write it into the buffer. Not all types follow this
...
template <typename>
struct is_fb_optional : std::false_type {};
template <typename T>
struct is_fb_optional<::flatbuffers::Optional<T>> : std::true_type {};

template <typename T>
auto toNative(const std::optional<T> &opt) {
  using R = decltype(toNative(std::declval<T>()));
  using Result = std::conditional_t<is_fb_optional<R>::value, R,
                                    ::flatbuffers::Optional<R>>;
  if (!opt) {
    return Result{};
  }
  return Result{toNative(*opt)};
}
...
toNative(ttnn::CoreRangeSetAttr coreRangeSetAttr) {
  ...
  ::tt::target::ttnn::CoreRangeSetT coreRangeSet;

  for (const auto &core_range : coreRangeSetAttr.getCoreRanges().vec()) {
    coreRangeSet.core_ranges.push_back(toNative(core_range));
  }
...
toNative(ttnn::MatmulMultiCoreReuseProgramConfigAttr matmulConfigAttr) {
  ::tt::target::ttnn::MatmulMultiCoreReuseProgramConfigT matmulConfig;
  ...
  matmulConfig.in0_block_w = matmulConfigAttr.getIn0BlockW();
  matmulConfig.out_subblock_h = matmulConfigAttr.getOutSubblockH();
  matmulConfig.out_subblock_w = matmulConfigAttr.getOutSubblockW();
  matmulConfig.per_core_m = matmulConfigAttr.getPerCoreM();
  matmulConfig.per_core_n = matmulConfigAttr.getPerCoreN();
...
toNative(ttnn::UnaryWithParamAttr unaryWithParamAttr) {
  ::tt::target::ttnn::UnaryWithParamT unaryWithParam;

  unaryWithParam.op_type = toNative(unaryWithParamAttr.getOpType());
  for (const auto &param : unaryWithParamAttr.getParams()) {
    unaryWithParam.params.push_back(toNative(param));
  }
...
inline ::tt::target::ttnn::MatmulMultiCoreReuseMultiCastProgramConfigT toNative(
  ...
  ::tt::target::ttnn::MatmulMultiCoreReuseMultiCastProgramConfigT matmulConfig;
  ...
  matmulConfig.out_subblock_h = matmulConfigAttr.getOutSubblockH();
  matmulConfig.out_subblock_w = matmulConfigAttr.getOutSubblockW();
  matmulConfig.out_block_h = matmulConfigAttr.getOutBlockH();
  matmulConfig.out_block_w = matmulConfigAttr.getOutBlockW();
  matmulConfig.per_core_m = matmulConfigAttr.getPerCoreM();
  matmulConfig.per_core_n = matmulConfigAttr.getPerCoreN();
  matmulConfig.transpose_mcast = matmulConfigAttr.getTransposeMcast();
  if (matmulConfigAttr.getFusedActivation()) {
    matmulConfig.fused_activation =
        std::make_unique<::tt::target::ttnn::UnaryWithParamT>(
            toNative(matmulConfigAttr.getFusedActivation()));
  }
  matmulConfig.fuse_batch = matmulConfigAttr.getFuseBatch();
...
toNative(
  ...
  matmulConfig.mcast_in0 = matmulConfigAttr.getMcastIn0();
  matmulConfig.gather_in0 = matmulConfigAttr.getGatherIn0();
  matmulConfig.hop_cores = std::make_unique<::tt::target::ttnn::CoreRangeSetT>(
      toNative(matmulConfigAttr.getHopCores()));
  matmulConfig.num_global_cb_receivers =
      matmulConfigAttr.getNumGlobalCbReceivers();
  matmulConfig.untilize_out = matmulConfigAttr.getUntilizeOut();
...
toNative(ttnn::Conv2dConfigAttr config) {
  ::tt::target::ttnn::Conv2dConfigT conv2dConfig;

  conv2dConfig.weights_dtype = toNative(config.getWeightsDtype());
  if (config.getActivation()) {
    conv2dConfig.activation =
        std::make_unique<::tt::target::ttnn::UnaryWithParamT>(
            toNative(config.getActivation()));
  }
  conv2dConfig.deallocate_activation =
      toNative(config.getDeallocateActivation());
  conv2dConfig.reallocate_halo_output =
      toNative(config.getReallocateHaloOutput());
  conv2dConfig.act_block_h_override = toNative(config.getActBlockHOverride());
  conv2dConfig.act_block_w_div = toNative(config.getActBlockWDiv());
  conv2dConfig.reshard_if_not_optimal =
      toNative(config.getReshardIfNotOptimal());
  conv2dConfig.override_sharding_config =
      toNative(config.getOverrideShardingConfig());
  conv2dConfig.shard_layout = toNative(config.getShardLayout());
  if (config.getCoreGrid()) {
    conv2dConfig.core_grid =
        std::make_unique<::tt::target::ttnn::CoreRangeSetT>(
            toNative(config.getCoreGrid()));
  }
  conv2dConfig.transpose_shards = toNative(config.getTransposeShards());
  conv2dConfig.output_layout = toNative(config.getOutputLayout());
  conv2dConfig.enable_act_double_buffer =
...
toNative(ttnn::Conv2dSliceConfigAttr sliceConfigAttr) {
  ::tt::target::ttnn::Conv2dSliceConfigT sliceConfig;
  sliceConfig.slice_type = toNative(sliceConfigAttr.getSliceType());
  sliceConfig.num_slices = sliceConfigAttr.getNumSlices();
  return sliceConfig;
}
...
toFlatbuffer(FlatbufferObjectCache &cache,
  ...
  auto t = toNative(sliceConfigAttr);
  return ::tt::target::ttnn::Conv2dSliceConfig::Pack(*cache.fbb, &t);
}

inline ::tt::target::ttnn::Conv3dConfigT
toNative(ttnn::Conv3dConfigAttr config) {
  ::tt::target::ttnn::Conv3dConfigT conv3dConfig;
  if (config.getWeightsDtype()) {
    conv3dConfig.weights_dtype = toNative(*config.getWeightsDtype());
  }
  if (config.getTOutBlock()) {
    conv3dConfig.t_out_block = *config.getTOutBlock();
  }
  if (config.getWOutBlock()) {
    conv3dConfig.w_out_block = *config.getWOutBlock();
  }
  if (config.getHOutBlock()) {
    conv3dConfig.h_out_block = *config.getHOutBlock();
  }
  if (config.getCOutBlock()) {
    conv3dConfig.c_out_block = *config.getCOutBlock();
  }
  if (config.getCInBlock()) {
    conv3dConfig.c_in_block = *config.getCInBlock();
  }
...
toNative(ttnn::DeviceComputeKernelConfigAttr attr) {
  ::tt::target::ttnn::DeviceComputeKernelConfigT computeConfig;
  computeConfig.math_fidelity = toNative(attr.getMathFidelity());
  computeConfig.math_approx_mode = toNative(attr.getMathApproxMode());
  computeConfig.fp32_dest_acc_en = toNative(attr.getFp32DestAccEn());
  computeConfig.packer_l1_acc = toNative(attr.getPackerL1Acc());
  computeConfig.dst_full_sync_en = toNative(attr.getDstFullSyncEn());
...
toNative(::mlir::tt::ttnn::ShardSpecAttr shardSpecAttr) {
  ::tt::target::ttnn::ShardSpecT shardSpec;
  shardSpec.core_range_set =
      std::make_unique<::tt::target::ttnn::CoreRangeSetT>(
          toNative(shardSpecAttr.getCoreRangeSet()));
  llvm::ArrayRef<int64_t> shardShapeArr = shardSpecAttr.getShape().getShape();
  shardSpec.shape.reserve(shardShapeArr.size());
  std::transform(shardShapeArr.begin(), shardShapeArr.end(),
                 std::back_inserter(shardSpec.shape),
                 [](int64_t val) { return static_cast<int32_t>(val); });
  shardSpec.orientation =
      toNative(shardSpecAttr.getShardOrientation().getValue());
  return shardSpec;
}
...
toNative(::mlir::tt::ttnn::NDShardSpecAttr ndShardSpecAttr) {
  ::tt::target::ttnn::NDShardSpecT ndShardSpec;
  ndShardSpec.core_range_set =
      std::make_unique<::tt::target::ttnn::CoreRangeSetT>(
          toNative(ndShardSpecAttr.getCoreRangeSet()));
  llvm::ArrayRef<int64_t> shardShapeArr = ndShardSpecAttr.getShape().getShape();
  ndShardSpec.shape.reserve(shardShapeArr.size());
  std::transform(shardShapeArr.begin(), shardShapeArr.end(),
                 std::back_inserter(ndShardSpec.shape),
                 [](int64_t val) { return static_cast<int32_t>(val); });
  ndShardSpec.orientation =
      toNative(ndShardSpecAttr.getShardOrientation().getValue());
  ndShardSpec.distribution_strategy =
      toNative(ndShardSpecAttr.getShardDistributionStrategy().getValue());
  return ndShardSpec;
}
...
toNative(::mlir::tt::ttnn::MemoryConfigAttr memoryConfigAttr) {
  ...
  ::tt::target::ttnn::MemoryConfigT memoryConfig;

  memoryConfig.buffer_type =
  ...
  memoryConfig.tensor_memory_layout = toNative(tensorMemoryLayoutAttr);
  ...
  if (memoryConfigAttr.getShardSpec()) {
    ...
    memoryConfig.shard_spec = std::make_unique<::tt::target::ttnn::ShardSpecT>(
        toNative(*memoryConfigAttr.getShardSpec()));
  }
  if (memoryConfigAttr.getNdShardSpec()) {
    memoryConfig.nd_shard_spec =
...
toFlatbuffer(FlatbufferObjectCache &cache, mlir::MemRefType memref,
             ...
             ttnn::CoreRangeSetAttr coreRangeSetAttr) {
  auto t =
      toNative(memref, tensorMesh, bufferType, memLayoutAttr, coreRangeSetAttr);
...
inline flatbuffers::Offset<::tt::target::ttnn::MemoryDesc> toFlatbuffer(
    ...
    ttcore::GridAttr shardGrid, ttnn::ShardOrientationAttr shardOrientation,
    ttnn::ShardDistributionStrategyAttr shardDistributionStrategy) {
  ...
  ::mlir::MLIRContext *ctx = memref.getContext();
  auto bufferTypeAttr = ttnn::BufferTypeAttr::get(ctx, bufferType);
  mlir::tt::ttnn::NDShardSpecAttr ndShardSpecAttr;
  llvm::SmallVector<int64_t> shardShape(memref.getShape().begin(),
                                        memref.getShape().end());
  assert(shardShape.size() >= 2);
  shardShape[shardShape.size() - 2] *= tileShape.y();
  shardShape[shardShape.size() - 1] *= tileShape.x();
  ...
  ndShardSpecAttr = ttnn::NDShardSpecAttr::get(
      ctx, coreRangeSetAttr, ttnn::ShapeAttr::get(ctx, shardShape),
      shardOrientation, shardDistributionStrategy);

  auto memoryConfigAttr = ::mlir::tt::ttnn::MemoryConfigAttr::get(
      ctx, memLayoutAttr, bufferTypeAttr, std::nullopt, ndShardSpecAttr);
...
inline flatbuffers::Offset<::tt::target::ttnn::LayoutDesc>
ttnnNDLayoutAttrToFlatbuffer(FlatbufferObjectCache &cache,
                             ttnn::TTNNNDLayoutAttr layoutAttr) {
  return ::tt::target::ttnn::CreateLayoutDesc(
      *cache.fbb, toFlatbuffer(cache, ttcore::OOBVal::Undef),
      toFlatbuffer(cache, layoutAttr.getMemref(), layoutAttr.getBufferType(),
                   layoutAttr.getMemLayout(), layoutAttr.getGrid(),

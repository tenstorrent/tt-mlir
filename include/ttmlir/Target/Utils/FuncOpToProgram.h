  ...
  for (auto &input : entry.getBody().getArguments()) {
    ...
    if (argAttrDict) {
      ...
      if (runtimeTensorShardingAttr) {
        ...
        program.inputs.push_back(cache.getOrCreate(
            input, tensorValueToFlatbuffer, rtsAttr.getShardStatus().getValue(),
            rtsAttr.getLocalShape()));
      }
    } else {
      program.inputs.push_back(cache.getOrCreate(
          input, tensorValueToFlatbuffer, ttcore::ShardStatus::Unsharded,

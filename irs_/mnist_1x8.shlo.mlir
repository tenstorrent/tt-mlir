...
module @jit__lambda attributes {mhlo.num_partitions = 8 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func public @main(%arg0: tensor<1024xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{"X"}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_0']['bias'].value"), %arg1: tensor<784x1024xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{}, {"X"}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_0']['kernel'].value"), %arg2: tensor<512xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{"X"}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_1']['bias'].value"), %arg3: tensor<1024x512xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{}, {"X"}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_1']['kernel'].value"), %arg4: tensor<256xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{"X"}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_2']['bias'].value"), %arg5: tensor<512x256xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{}, {"X"}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_2']['kernel'].value"), %arg6: tensor<10xf32> {sdy.sharding = #sdy.sharding<@mesh, [{}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_3']['bias']"), %arg7: tensor<256x10xf32> {sdy.sharding = #sdy.sharding<@mesh, [{}, {}]>, ttcore.argument_type = #ttcore.argument_type<parameter>} loc("variables['params']['Dense_3']['kernel']"), %arg8: tensor<32x28x28x1xf32> {sdy.sharding = #sdy.sharding<@mesh, [{}, {}, {}, {}]>, ttcore.argument_type = #ttcore.argument_type<input>} loc("args[0]")) -> (tensor<32x10xf32> {jax.result_info = "result", sdy.sharding = #sdy.sharding<@mesh, [{}, {}]>}) {
    %0 = sdy.manual_computation(%arg0, %arg1, %arg2, %arg3, %arg4, %arg5, %arg6, %arg7, %arg8) in_shardings=[<@mesh, [{"X"}]>, <@mesh, [{}, {"X"}]>, <@mesh, [{"X"}]>, <@mesh, [{}, {"X"}]>, <@mesh, [{"X"}]>, <@mesh, [{}, {"X"}]>, <@mesh, [{}]>, <@mesh, [{}, {}]>, <@mesh, [{}, {}, {}, {}]>] out_shardings=[<@mesh, [{}, {}]>] manual_axes={"X"} (%arg9: tensor<128xbf16> loc("shard_map"), %arg10: tensor<784x128xbf16> loc("shard_map"), %arg11: tensor<64xbf16> loc("shard_map"), %arg12: tensor<1024x64xbf16> loc("shard_map"), %arg13: tensor<32xbf16> loc("shard_map"), %arg14: tensor<512x32xbf16> loc("shard_map"), %arg15: tensor<10xf32> loc("shard_map"), %arg16: tensor<256x10xf32> loc("shard_map"), %arg17: tensor<32x28x28x1xf32> loc("shard_map")) {
      ...
      %2 = stablehlo.convert %arg10 : (tensor<784x128xbf16>) -> tensor<784x128xf32> loc(#loc66)
      %3 = stablehlo.convert %arg9 : (tensor<128xbf16>) -> tensor<128xf32> loc(#loc66)
      %4 = stablehlo.dot_general %1, %2, contracting_dims = [1] x [0] : (tensor<32x784xf32>, tensor<784x128xf32>) -> tensor<32x128xf32> loc(#loc67)
      %5 = stablehlo.reshape %3 : (tensor<128xf32>) -> tensor<1x128xf32> loc(#loc68)
      %6 = stablehlo.broadcast_in_dim %5, dims = [0, 1] : (tensor<1x128xf32>) -> tensor<32x128xf32> loc(#loc69)
      %7 = stablehlo.add %4, %6 : tensor<32x128xf32> loc(#loc69)
      %8 = func.call @relu(%7) : (tensor<32x128xf32>) -> tensor<32x128xf32> loc(#loc70)
      %9 = "stablehlo.all_gather"(%8) <{all_gather_dim = 1 : i64, channel_handle = #stablehlo.channel_handle<handle = 1, type = 0>, replica_groups = dense<[[0, 1, 2, 3, 4, 5, 6, 7]]> : tensor<1x8xi64>, use_global_device_ids}> : (tensor<32x128xf32>) -> tensor<32x1024xf32> loc(#loc71)
      %10 = stablehlo.convert %arg12 : (tensor<1024x64xbf16>) -> tensor<1024x64xf32> loc(#loc72)
      %11 = stablehlo.convert %arg11 : (tensor<64xbf16>) -> tensor<64xf32> loc(#loc72)
      %12 = stablehlo.dot_general %9, %10, contracting_dims = [1] x [0] : (tensor<32x1024xf32>, tensor<1024x64xf32>) -> tensor<32x64xf32> loc(#loc73)
      %13 = stablehlo.reshape %11 : (tensor<64xf32>) -> tensor<1x64xf32> loc(#loc74)
      %14 = stablehlo.broadcast_in_dim %13, dims = [0, 1] : (tensor<1x64xf32>) -> tensor<32x64xf32> loc(#loc75)
      %15 = stablehlo.add %12, %14 : tensor<32x64xf32> loc(#loc75)
      %16 = func.call @relu_15(%15) : (tensor<32x64xf32>) -> tensor<32x64xf32> loc(#loc70)
      %17 = "stablehlo.all_gather"(%16) <{all_gather_dim = 1 : i64, channel_handle = #stablehlo.channel_handle<handle = 2, type = 0>, replica_groups = dense<[[0, 1, 2, 3, 4, 5, 6, 7]]> : tensor<1x8xi64>, use_global_device_ids}> : (tensor<32x64xf32>) -> tensor<32x512xf32> loc(#loc71)
      %18 = stablehlo.convert %arg14 : (tensor<512x32xbf16>) -> tensor<512x32xf32> loc(#loc76)
      %19 = stablehlo.convert %arg13 : (tensor<32xbf16>) -> tensor<32xf32> loc(#loc76)
      %20 = stablehlo.dot_general %17, %18, contracting_dims = [1] x [0] : (tensor<32x512xf32>, tensor<512x32xf32>) -> tensor<32x32xf32> loc(#loc77)
      %21 = stablehlo.reshape %19 : (tensor<32xf32>) -> tensor<1x32xf32> loc(#loc78)
      %22 = stablehlo.broadcast_in_dim %21, dims = [0, 1] : (tensor<1x32xf32>) -> tensor<32x32xf32> loc(#loc79)
      %23 = stablehlo.add %20, %22 : tensor<32x32xf32> loc(#loc79)
      %24 = func.call @relu_24(%23) : (tensor<32x32xf32>) -> tensor<32x32xf32> loc(#loc70)
      %25 = "stablehlo.all_gather"(%24) <{all_gather_dim = 1 : i64, channel_handle = #stablehlo.channel_handle<handle = 3, type = 0>, replica_groups = dense<[[0, 1, 2, 3, 4, 5, 6, 7]]> : tensor<1x8xi64>, use_global_device_ids}> : (tensor<32x32xf32>) -> tensor<32x256xf32> loc(#loc71)
  ...
  func.func private @relu(%arg0: tensor<32x128xf32> {ttcore.argument_type = #ttcore.argument_type<input>} loc(unknown)) -> tensor<32x128xf32> {
  ...
  func.func private @relu_15(%arg0: tensor<32x64xf32> {ttcore.argument_type = #ttcore.argument_type<input>} loc(unknown)) -> tensor<32x64xf32> {
    %cst = stablehlo.constant dense<0.000000e+00> : tensor<f32> loc(#loc91)
  ...
  func.func private @relu_24(%arg0: tensor<32x32xf32> {ttcore.argument_type = #ttcore.argument_type<input>} loc(unknown)) -> tensor<32x32xf32> {
    %cst = stablehlo.constant dense<0.000000e+00> : tensor<f32> loc(#loc91)
    %0 = stablehlo.broadcast_in_dim %cst, dims = [] : (tensor<f32>) -> tensor<32x32xf32> loc(#loc90)
    %1 = stablehlo.maximum %arg0, %0 : tensor<32x32xf32> loc(#loc90)
    return %1 : tensor<32x32xf32> loc(#loc91)
  } loc(#loc91)
} loc(#loc)
...
#loc16 = loc("/localdev/svuckovic/_workspace/repos/tt-xla/tests/jax/multi_chip/llmbox/8_devices/models/tensor_parallel/mnist_mlp/test_mnist_mlp.py":78:8 to :31)
...
#loc34 = loc("test_mnist_mlp_multichip_llmbox_1x8_inference_shardy"(#loc16))

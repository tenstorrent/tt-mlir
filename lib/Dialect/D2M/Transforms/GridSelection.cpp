...
static void insertStreamForTTNNTensor(Value operand,
  ...
  auto storageOp = builder.create<d2m::EmptyOp>(castOp.getLoc(), storageTensor, /*virtualGridMapping=*/nullptr);
  auto streamOp = builder.create<d2m::StreamLayoutOp>(
      castOp.getLoc(), streamOutputTensor, castOp.getResult(), AffineMapAttr::get(reblockMap), storageOp);
...
updateTTNNTensors(ArrayRef<TTNNTensorUpdateInfo> TTNNTensorsToUpdate,
  ...
  for (auto &info : TTNNTensorsToUpdate) {
    ...
    if (metalLayout.getMemorySpace() == ttcore::MemorySpace::DeviceDRAM) {
      insertStreamForTTNNTensor(info.operand, config, info.grid, builder);
    } else if (auto castOp =

...
///
/// Column cross-step: the outer column loop (allLoops[0]) has a different step
/// between producer and consumer. The absolute tile column index
...
getConsumerScratchAccessMap(ScratchLoopInfo &producer,
  ...
  // than the producer. Compute the absolute column tile index and decompose it
  // into the producer's scratch layout.
  //

...
public:
  ...
  LogicalResult matchAndRewrite(ToLayoutOp op,
    ...
    // 3. L1→DRAM (lowerDatamovementGeneric handles grid mismatch via views).
    if (currentInfo.hasLayout() && !currentInfo.isDRAM() &&
        targetInfo.hasLayout() && targetInfo.isDRAM()) {
    ...
    // 5. MASKING: Apply boundary masking after tilization if needed.
    ...
    // 6. MAPPING CHANGE: Grid/index_map/logical_shape/dim_alignments (after
    ...
    if (currentInfo.hasLayout() && targetInfo.hasLayout() &&
      ...
      if (needsMappingChange) {
        ...
        if (bothTilized && !isSimpleReblocking) {
          ...
          // 6a. Untilize to scalar space (preserve current layout properties).
          ...
          // 6b. Apply complex mapping change in scalar space.
          ...
          // 6c. Tilize back to match target format.
          ArrayRef<int64_t> tileShape =
    ...
    // 7. UNTILIZE: Before Device→System.
    bool needsUntilize =
        ttcore::isTiled(currentInfo.type) && !ttcore::isTiled(targetInfo.type);
    if (needsUntilize) {
      Type scalarType = getScalarType(currentInfo.type.getElementType());

  ...
  LogicalResult markSynchronizedOpBuffers(IRRewriter &rewriter,
    ...
    genericOp->getRegions().front().walk([&](Operation *op) {
      if (auto linalgGenericOp = dyn_cast<mlir::linalg::GenericOp>(op)) {
        ...
        rewriter.create<d2m::SynchronizedRegionOp>(
            linalgGenericOp->getLoc(), linalgGenericOp.getOutputs().getTypes(),
            linalgGenericOp.getInputs(), linalgGenericOp.getOutputs(),
            [&](mlir::OpBuilder &bbBuilder, mlir::Location bbLoc,
                mlir::ValueRange bbArgs) {
              auto newLinalgGenericOp =
                  bbBuilder.create<mlir::linalg::GenericOp>(
                      ...
                      linalgGenericOp.getIndexingMapsArray(),
                      linalgGenericOp.getIteratorTypesArray());
              IRMapping mapping;
              linalgGenericOp.getRegion().cloneInto(
                  &newLinalgGenericOp.getRegion(), mapping);
              bbBuilder.create<d2m::YieldOp>(bbLoc,
                                             newLinalgGenericOp.getOutputs());
            });

        rewriter.eraseOp(linalgGenericOp);
      } else if (auto tileUntilizeBlockOp =
                     dyn_cast<d2m::TileUntilizeBlockOp>(op)) {
        // wrap in synchronized region op
        rewriter.setInsertionPoint(op);
        auto newSynchronizedRegionOp =
            rewriter.create<d2m::SynchronizedRegionOp>(
                tileUntilizeBlockOp->getLoc(),
                tileUntilizeBlockOp->getResultTypes(),
                tileUntilizeBlockOp.getInput(), tileUntilizeBlockOp.getOutput(),
                [&](mlir::OpBuilder &bbBuilder, mlir::Location bbLoc,
                    mlir::ValueRange bbArgs) {
                  IRMapping mapping;
                  mapping.map(tileUntilizeBlockOp.getInput(), bbArgs[0]);
                  mapping.map(tileUntilizeBlockOp.getOutput(), bbArgs[1]);
                  auto newTileUntilizeBlockOp =
                      bbBuilder.clone(*tileUntilizeBlockOp, mapping);
                  bbBuilder.create<d2m::YieldOp>(
                      bbLoc, newTileUntilizeBlockOp->getResults());
                });
        rewriter.replaceOp(tileUntilizeBlockOp,
                           newSynchronizedRegionOp.getOperation());
      } else if (auto tileTilizeBlockOp =
                     dyn_cast<d2m::TileTilizeBlockOp>(op)) {
        // wrap in synchronized region op
        rewriter.setInsertionPoint(op);
        auto newSynchronizedRegionOp =
            rewriter.create<d2m::SynchronizedRegionOp>(
                tileTilizeBlockOp->getLoc(),
                tileTilizeBlockOp->getResultTypes(),
                tileTilizeBlockOp.getInput(), tileTilizeBlockOp.getOutput(),
                [&](mlir::OpBuilder &bbBuilder, mlir::Location bbLoc,
                    mlir::ValueRange bbArgs) {
                  IRMapping mapping;
                  mapping.map(tileTilizeBlockOp.getInput(), bbArgs[0]);
                  mapping.map(tileTilizeBlockOp.getOutput(), bbArgs[1]);
                  auto newTileTilizeBlockOp =
                      bbBuilder.clone(*tileTilizeBlockOp, mapping);
                  bbBuilder.create<d2m::YieldOp>(
                      bbLoc, newTileTilizeBlockOp->getResults());
                });
        rewriter.replaceOp(tileTilizeBlockOp,
                           newSynchronizedRegionOp.getOperation());
      }
    ...
    for (auto &[cb, usageInfo] : cbUsageInfo) {
      ...
      bool adjustResult =
          !mlir::isa<mlir::linalg::GenericOp>(usageInfo.producers[0]);
      if (failed(insertCBLayoutAttr(
              rewriter, mlir::cast<memref::AllocOp>(cb.getDefiningOp()),
              usageInfo.producers, adjustResult))) {
        return failure();
      }
    }

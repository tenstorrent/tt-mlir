...
//===----------------------------------------------------------------------===//
namespace {
class D2MAllocateStreams final : public OpRewritePattern<d2m::GenericOp> {
public:
  D2MAllocateStreams(MLIRContext *context, unsigned numStreamBuffers)
  ...
  static bool needsStream(Value operand, bool isOutput,
    ...
    if (mlir::isa_and_nonnull<d2m::StreamLayoutOp>(definingOp)) {
      return false;
    }
...
class D2MAllocate final : public impl::D2MAllocateBase<D2MAllocate> {
  ...
  LogicalResult runAllocateStreams(ModuleOp moduleOp) {
    ...
    patterns.add<D2MAllocateStreams>(&getContext(), numStreamBuffers);

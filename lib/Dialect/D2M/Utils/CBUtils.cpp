...
Value getCB(Operation* opUsingCB, Value cbGenericOperand,
  ...
  auto genericRegion = ttmlir::utils::getRegionWithParentOfType<GenericOp>(opUsingCB);
  assert(mlir::isa<ttcore::CBLayoutAttr>(mlir::cast<MemRefType>(cbGenericOperand.getType()).getLayout()) && "expected cb layout");
  ...
  auto getCBOp = rewriter.create<GetCBOp>(generic.getLoc(), cbType,
      *operandIndex);
  llvm::errs() << "getting cb for operand: " << *operandIndex << " in generic position: " << generic.getLoc() << " with result: " << getCBOp.getResult() << "\n";

...
computeDSTPackingForRegion(d2m::GenericOp generic,
  ...
  // DST execution classes (e.g. unary SFPU vs binary FPU on bf16), which
  // yields different per-linalg numTilesPerFlip and therefore different
  // phase-2 inner blocking in D2MGenericTileComputeLoops. Non-uniform
  // per-linalg blocking inside one generic produces scratch-slot shapes that
  // do not all line up: a producer's slot stored with one shape is then
  // read by a consumer that iterates with a different blocking, and the
  // index expressions generated by D2MInsertSpillAndScratch alias/skip
  // tiles, corrupting the result.
  //
  // Pre-fusion (origin/main), all linalgs in an aliased chain share the
  // same DPS output Value, and the per-result merge below would force the
  // most restrictive numTilesPerFlip. With the elementwise-fusion fix that
  // gives each producer its own intermediate tensor.empty, the shared-Value
  // assumption no longer holds, so we explicitly normalize across the whole
  // region: take the smallest numTilesPerFlip and recompute numDstFlips for
  // every pending result with the same shardSizeTiles.
  if (!pendingResults.empty()) {
    int64_t minNumTilesPerFlip = pendingResults.front().numTilesPerFlip;
    for (const PendingDSTPackingResult &pending : pendingResults) {
      minNumTilesPerFlip =
          std::min(minNumTilesPerFlip, pending.numTilesPerFlip);
    }
    numDstFlipsPerOp.clear();
    for (PendingDSTPackingResult &pending : pendingResults) {
      if (pending.numTilesPerFlip != minNumTilesPerFlip &&
          (pending.shardSizeTiles % minNumTilesPerFlip) == 0) {
        pending.numTilesPerFlip = minNumTilesPerFlip;
        pending.numDstFlips = pending.shardSizeTiles / minNumTilesPerFlip;
      }
      numDstFlipsPerOp.push_back(pending.numDstFlips);
    }
  }

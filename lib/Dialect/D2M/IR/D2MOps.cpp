...
LogicalResult d2m::CompositeViewOp::bufferize(
  ...
  for (Value input : getInputs()) {
    ...
    logicalSizes.push_back(
        mlir::cast<RankedTensorType>(input.getType()).getShape()[dim]);
  }
...
mlir::LogicalResult d2m::CompositeViewOp::verify() {
  ...
  const bool isTensorType = mlir::isa<RankedTensorType>(resultType);
  ...
  auto outShape = getVerificationShape(resultType);
  ...
  // Case 1. Logical "1 & 1 -> 2", device "32 & 32 -> 32".
  // Case 2. Logical "192 & 192 -> 384", device "192 & 192 -> 512".

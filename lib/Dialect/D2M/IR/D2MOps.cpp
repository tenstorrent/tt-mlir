...
static Value findAssocOperandForGetCB(d2m::GetCBOp getCbOp) {
  ...
  // TODO: this is wrong: what's wanted here is the cb corresponding to the
  // operand load/store
  ...
  std::function<void(Block &)> scanBlock = [&](Block &block) {
    for (Operation &op : block) {
      ...
      if (auto getCbOp = mlir::dyn_cast<d2m::GetCBOp>(&op)) {
        // TODO: this is wrong: what's wanted here is the cb corresponding to
        // the operand load/store

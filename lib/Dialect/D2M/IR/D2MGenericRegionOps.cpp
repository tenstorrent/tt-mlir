...
// bufferization API expectations (mirrors TTIR)
static mlir::Value wrapValueInTensorCompatibleType(mlir::RewriterBase &rewriter,
  ...
  // Use bufferization dialect utility to materialize cast-like tensor value.
  // In TTIR this used ToTensorOp; for now, just create an unrealized cast.
  auto cast =
      rewriter.create<mlir::UnrealizedConversionCastOp>(loc, tensorType, value);
...
::mlir::LogicalResult AwaitOp::verify() { return success(); }
...
mlir::LogicalResult YieldOp::verify() { return success(); }

...
::mlir::LogicalResult mlir::tt::ttir::NLPCreateQKVHeadsDecodeOp::verify() {
  ...
  auto check4D = [](ArrayRef<int64_t> shape) { return shape.size() == 4; };

  if (!check4D(inputShape)) {
    return emitOpError() << "input tensor must be a 4D tensor";
  }
  if (!check4D(queryShape)) {
    return emitOpError() << "query tensor must be a 4D tensor";
  }
  if (!check4D(keyShape)) {
    return emitOpError() << "key tensor must be a 4D tensor";
  }
  if (!check4D(valueShape)) {

...
static mlir::LogicalResult verifyAffineShapes(
    mlir::Operation *op, mlir::ArrayRef<mlir::AffineMap> indexingMaps,
    mlir::ArrayRef<mlir::SmallVector<int64_t>> shapes, char const *shapeName) {
  assert(indexingMaps.size() == shapes.size());

  auto compareCompatibleDims =
      +[](mlir::ArrayRef<int64_t> a,
          mlir::ArrayRef<int64_t> b) -> std::optional<int64_t> {
    int dim = 0;
  ...
  for (size_t operandA = 0; operandA < indexingMaps.size(); ++operandA) {
    for (size_t operandB = 0; operandB < indexingMaps.size(); ++operandB) {
      ...
      if (auto dim = compareCompatibleDims(shapeA, shapeB)) {
        return op->emitOpError()
               << shapeName << " shape mismatch between operand[" << operandA
               << "] " << shapeName << "_shape=[" << shapes[operandA]
               << "] and operand[" << operandB << "] " << shapeName
               << "_shape=[" << shapes[operandB] << "] at affine dim d" << *dim;
      }
    }
  }
...
::mlir::LogicalResult mlir::tt::ttir::GenericOp::verify() {
  ...
  if (hasGrid && !indexingMaps.empty()) {
    ...
    LogicalResult gridResult =
        verifyAffineShapes(getOperation(), indexingMaps, gridShapes, "grid");
...
mlir::tt::ttir::GenericOp::getIndexingMapsValue() {
  return llvm::to_vector<4>(llvm::map_range(getIndexingMaps(), [](Attribute a) {
...
::mlir::LogicalResult mlir::tt::ttir::YieldOp::verify() {
  return operandsInRegionArguments(
      getOperation(),
      getParentRegionOfType<GenericOp, ScatterOp>(getOperation()));
}

...
mlir::LogicalResult mlir::tt::ttir::ConvTranspose2dOp::verify() {
  ...
  auto checkBiggerThan = [&](llvm::SmallVector<int32_t, 2> &values, const char *name, int32_t minValue) -> mlir::LogicalResult {
    for (int32_t value : values) {
      if (value < minValue) {
        return emitOpError() << "Attribute '" << name << "' contains a value less than: " << value;
      }
    }
    return mlir::success();
  };

  llvm::SmallVector<int32_t, 2> stride = ttmlir::utils::parseAttrToTwoElementVector(getStrideAttr());
  if (stride.empty()) {
    return emitOpError("Expected int or pair of ints for stride");
  }
  if (failed(checkBiggerThan(stride, "stride", 1))) {
    return mlir::failure();
  }

  llvm::SmallVector<int32_t, 2> padding = ttmlir::utils::parseAttrToTwoElementVector(getPaddingAttr());
  if (padding.empty()) {
    return emitOpError("Expected int or pair of ints for padding");
  }
  if (failed(checkBiggerThan(padding, "padding", 0))) {
    return mlir::failure();
  }

  llvm::SmallVector<int32_t, 2> outputPadding = ttmlir::utils::parseAttrToTwoElementVector(getOutputPaddingAttr());
  if (outputPadding.empty()) {
    return emitOpError("Expected int or pair of ints for output padding");
  }
  if (failed(checkBiggerThan(outputPadding, "output padding", 0))) {
    return mlir::failure();
  }

  llvm::SmallVector<int32_t, 2> dilation = ttmlir::utils::parseAttrToTwoElementVector(getDilationAttr());
  if (dilation.empty()) {
    return emitOpError("Expected int or pair of ints for dilation");
  }
  ...
  if (inputChannels % groups != 0) {
    return emitOpError()
        << "Number of input channels from input tensor must be divisible by the number of groups. "
        << "Got " << inputChannels << " input channels and " << groups << " groups.";
  }

  if (outputChannels % groups != 0) {
    return emitOpError()
        << "Number of output channels from output tensor must be divisible by the number of groups. "
        << "Got " << outputChannels << " output channels and " << groups << " groups.";
  }

  if (inputChannels != kernelShape[0]) {
    return emitOpError()
        << "Number of input channels from input tensor must match the first dimension of the weight tensor. "
        << "Got " << inputChannels << " input channels and " << kernelShape[0] << " in the output tensor.";
  }

  if (outputChannels / groups != kernelShape[1]) {
    return emitOpError()
        << "Number of output channels per group from the output tensor must match the second dimension of the weight tensor. "
        << "Got " << (outputChannels / groups) << " output channels per group and " << kernelShape[1] << " expected.";
  }
  ...
  int32_t HOut = (Hin - 1) * stride[0] - 2 * padding[0] + dilation[0] * (kernelHeight - 1) + outputPadding[0] + 1;
  int32_t WOut = (Win - 1) * stride[1] - 2 * padding[1] + dilation[1] * (kernelWidth - 1) + outputPadding[1] + 1;
  if (HOut < 0 || WOut < 0) {
    return emitOpError()
        << "Given input size per channel: (" << Hin << " x " << Win << "). "
        << "Calculated output size per channel: (" << HOut << " x " << WOut << "). "
        << "Output size is too small";
  }

  int32_t expectedHOut = outputType.getDimSize(outputType.getRank() - 3);
  int32_t expectedWOut = outputType.getDimSize(outputType.getRank() - 2);
  if (HOut != expectedHOut || WOut != expectedWOut) {
    return emitOpError()
        << "Mismatch between calculated output size per channel and actual output tensor dimensions. "
        << "Calculated: (" << HOut << " x " << WOut << "), "
        << "Expected: (" << expectedHOut << " x " << expectedWOut << ").";
  }

  if (getBias().getImpl()) {
    mlir::RankedTensorType bias = getBias().getType();
    if (bias.getDimSize(bias.getRank() - 1) != outputChannels) {
      return emitOpError()
          << "Mismatch in bias tensor dimensions. "
          << "Bias tensor has " << bias.getDimSize(bias.getRank() - 1) << " channels, "
          << "but the output tensor expects " << outputChannels << " channels.";
    }
  }

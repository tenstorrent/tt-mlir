...
void mlir::tt::ttir::PoolingOp::getCanonicalizationPatterns(
    mlir::RewritePatternSet &patterns, mlir::MLIRContext *) {
  patterns.add(+[](PoolingOp op, mlir::PatternRewriter &rewriter) {
    ...
    for (size_t i = 0; i < op.getInputs().size(); i++) {
      FullOp constant =
          dyn_cast_or_null<FullOp>(op.getInputs()[i].getDefiningOp());
      if (!constant) {
        return failure();
      }
      FullOp newConstant;
      if (isa<IntegerAttr>(constant.getFillValue())) {
        ...
        newConstant = rewriter.create<FullOp>(

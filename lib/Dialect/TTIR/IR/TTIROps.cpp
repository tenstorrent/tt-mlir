...
static mlir::LogicalResult
verifyReduceOp(mlir::InFlightDiagnostic &&emitOpError,
  ...
  if (!reduceDims) {
    ...
    for (mlir::Attribute reduceDim : *reduceDims) {
      ...
      if (reduceDimInt < -inputTensorRank || reduceDimInt >= inputTensorRank) {
        return emitOpError << "Reduce dimensions are out of range";
      }
...
::mlir::LogicalResult mlir::tt::ttir::MaxOp::verify() {
  return verifyReduceOp(emitOpError(), getInput().getType(), getDimArg(),
                        getKeepDim(), getType().getShape());
}

...
//   SpatialMajor {0,3,5,1,2,4}: SpaceToDepth ordering  (c_out = rh*(r*C)+rw*C+c_in)
//   ChannelMajor {0,1,3,5,2,4}: pixel_unshuffle ordering (c_out = c_in*r²+rh*r+rw)
//
...
// [N, C, H/r, r, W/r, r].
//
// In practice the canonical TTIR lowering produces:
...
/// Returns true if composed == {0, 3, 5, 1, 2, 4} (SpatialMajor / SpaceToDepth permutation).
...
/// Returns true if composed == {0, 1, 3, 5, 2, 4} (ChannelMajor / pixel_unshuffle permutation).

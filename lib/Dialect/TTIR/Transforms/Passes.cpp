...
public:
  ...
  void runOnOperation() final {
    ...
    if (sequenceFound) {
      ...
      OpBuilder builder(maximumOp);

      mlir::tt::ttir::ReluOp reluOp = builder.create<mlir::tt::ttir::ReluOp>(
          maximumOp->getLoc(), maximumOp->getResult(0).getType(),
          maximumOp->getOperand(0),
          maximumOp->getOperand(2), // emptyOp
          builder.getArrayAttr(
              SmallVector<Attribute>(maximumOp->getNumOperands(),
                                     builder.getAttr<OperandConstraintAttr>(
                                         OperandConstraint::AnyDeviceTile))));

      reluOp->moveAfter(maximumOp);
      maximumOp->replaceAllUsesWith(reluOp);

...
private:
  bool isFusable(PermuteOp permuteOp) const {
    ...
    ArrayRef<int64_t> reshapeOutputShape =
        reshapeOp.getResult().getType().getShape();
    // Check that input shape is 4 dimensional, output shape is 3 dimensional.
    if (inputShape.size() != 4 || reshapeOutputShape.size() != 3) {
      return false;
    }
    ...
    if (inputShape[0] != reshapeOutputShape[0] ||
        inputShape[2] != reshapeOutputShape[1]) {
      return false;
    }

    if (inputShape[1] * inputShape[3] != reshapeOutputShape[2]) {
      return false;
    }
    return true;
  }

...
GqaExpansion detectGqaExpansion(Value v) {
  ...
  // Also match HF repeat_kv's unsqueeze/broadcast/reshape form. The existing
  // reshape/broadcast/reshape -> repeat_interleave canonicalization (whose
  // output the branch above peels) does not fire on it: HF inserts the GQA
  // group axis with an unsqueeze, not a reshape, so no repeat_interleave is
  // ever produced. Peel this form directly, else SDPA is handed an Hq-head copy
  // instead of the grouped Hkv-head cache (a large f32 GQA expansion per decode
  // step).

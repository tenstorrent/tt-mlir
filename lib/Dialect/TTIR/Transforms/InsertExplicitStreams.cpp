...
#define GEN_PASS_DEF_TTIRINSERTEXPLICITSTREAMS
#include "ttmlir/Dialect/TTIR/Transforms/Passes.h.inc"

namespace {
class TTIRInsertExplicitStreamsRewriter : public OpRewritePattern<ttir::GenericOp> {
public:
  ...
  LogicalResult matchAndRewrite(ttir::GenericOp op,
    ...
    SmallVector<bool> requestedStream =
        llvm::map_to_vector(*argAttrs, [](Attribute attr) -> bool {
          Attribute stream =
              llvm::cast<DictionaryAttr>(attr).get("ttir.stream");
    ...
    for (OpOperand *operand : op.getDpsInputOperands()) {
      ...
      const bool hasStream =
          mlir::isa<ttir::StreamLayoutOp>(operand->get().getDefiningOp());
      if (hasStream) {
        continue;
      }

      ttir::StreamLayoutOp stream =
          createStream(rewriter, op.getLoc(), operand);
  ...
  static ttir::StreamLayoutOp createStream(PatternRewriter &rewriter,
    ...
    return rewriter.create<ttir::StreamLayoutOp>(loc, streamMemref, operand,
                                                 storage);
  }
};
} // namespace
...
namespace {
class TTIRInsertExplicitStreams
    : public impl::TTIRInsertExplicitStreamsBase<
          TTIRInsertExplicitStreams> {
public:
  using impl::TTIRInsertExplicitStreamsBase<
      TTIRInsertExplicitStreams>::TTIRInsertExplicitStreamsBase;

  void runOnOperation() final {
    RewritePatternSet patterns(&getContext());
    patterns.add<TTIRInsertExplicitStreamsRewriter>(&getContext());

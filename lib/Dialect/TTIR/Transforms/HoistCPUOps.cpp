...
static void tagBufferizationAccess(mlir::func::FuncOp funcOp, unsigned argIdx,
  ...
  // Helper function to hoist an arbitrary op into a new function in
  // targetModule, generate a matching extern prototype in the sourceModule, and
  // replace the original op with a callOp to the extern function.
  static Value hoistOperationToFunction(mlir::Operation * opToHoist,
                                        mlir::ModuleOp sourceModule,
                                        mlir::ModuleOp targetModule) {

    const llvm::SmallVector<int64_t, 4> ranks =
        getOperandTensorRanks(opToHoist);
    ...
    const llvm::SmallString<16> functionName =
        generateHoistedFuncName(opToHoist);
    ...
    if (localFunc == nullptr) {
      ...
      for (auto arg : llvm::enumerate(hoistedFunc.getArguments())) {
        ...
        localFunc = func::FuncOp::create(
            opToHoist->getLoc(), localFunctionName.str(), localFuncType);
        ...
        // An analysis class which currently relies on manually tagging ops with
        // a `should_hoist` attribute, but in the future will also tag fall-back
        // ops, etc.
        namespace {
        class TTIRHoistAnalyze {
        public:
          using HoistOpSet =
              llvm::SmallVector<llvm::SmallSet<mlir::Operation *, 4>>;
        ...
        namespace {
        // Transform pass to hoist specific ops (based on configured analysis
        // pass) into a cpu submodule for later independent lowering.
        ...
        public:
          ...
          void runOnOperation() final {
            ...
            for (Operation &op : rootModule.getBodyRegion().front()) {
              if (auto maybeDeviceModule =
                      dyn_cast<ttcore::DeviceModuleOp>(op)) {
            ...
            const TTIRHoistAnalyze::HoistOpSet &hoistOpSets =
                analysisPass.getResults();

            // We don't want to create a CPUModuleOp etc. if we aren't hoisting
            // any ops.
            ...
            for (auto &op : rootModule.getBody()->getOperations()) {
              if (auto module = llvm::dyn_cast<ttcore::CPUModuleOp>(op)) {
                ...
                assert(cpuInnerModule &&
                       "CPUModuleOp must contain 1 ModuleOp!");
                break;
              }
            }
            ...
            if (!cpuModule) {
              ...
              rewriter.setInsertionPointToStart(
                  &cpuModule.getBodyRegion().front());
              cpuInnerModule = rewriter.create<mlir::ModuleOp>(loc);
            }

            for (const auto &opSet : hoistOpSets) {
              assert(opSet.size() == 1 && "currently don't support hoisting "

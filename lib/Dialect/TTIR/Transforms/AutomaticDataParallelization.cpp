        ...
        for (auto &op : entryBlock) {
          if (func::ReturnOp returnOp = llvm::dyn_cast<func::ReturnOp>(&op)) {
            ...
            for (auto tensor : returnTensors) {
              ...
              // If output type rank < 4, we assume it's been replicated on all devices and grab it as is from a single device. Else, we want to grab the sharded tensor
              llvm::SmallVector<int64_t> shardShape = {meshShapeRef[1], 1, 1, 1};
              mlir::tt::MeshShardType meshShardType = mlir::tt::MeshShardType::Devices;

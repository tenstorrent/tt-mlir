...
Conv2dConfigParams::Conv2dConfigParams(const Conv2dConfigAttr &attr,
  ...
  enableSubblockPadding = getOrDefaultBool(attr.getEnableSubblockPadding());
...
Conv2dConfigParams::buildConv2dConfigAttr(::mlir::MLIRContext *ctx) const {
  ...
  return Conv2dConfigAttr::get(
      ...
      toBoolAttr(enableSplitReader), toBoolAttr(enableSubblockPadding),
      toBoolAttr(inPlace));
}
...
void Conv2dConfigParams::applyOverrides(const Conv2dConfigParams &overrides) {
  ...
  if (overrides.enableSubblockPadding.has_value()) {
    enableSubblockPadding = overrides.enableSubblockPadding;
  }

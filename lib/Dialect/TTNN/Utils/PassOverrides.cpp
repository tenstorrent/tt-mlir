...
bool Conv2dConfigOverrideParser::parse(
  ...
  for (StringRef opOverrides : opOverrideList) {
    ...
    for (StringRef param : conv2dConfigParams) {
      ...
      if (paramName == "dtype") {
        auto dtype = mlir::ttcore::DataTypeStringToEnum(paramValue);
        if (!dtype) {
        ...
        auto weightsDtype = mlir::ttcore::DataTypeStringToEnum(paramValue);
        if (!weightsDtype) {
...
bool OutputLayoutOverrideParser::parse(
  ...
  for (StringRef opOverrides : opOverrideList) {
    ...
    for (StringRef param : layoutParamParts) {
      ...
      } else if (auto dataType = mlir::ttcore::DataTypeStringToEnum(param)) {
...
std::string OutputLayoutOverrideParser::toString(
  ...
  for (const auto &entry : value) {
    ...
    if (params.dataType.has_value()) {
      parts.push_back(std::string(
          mlir::ttcore::DataTypeEnumToString(params.dataType.value())));
    }

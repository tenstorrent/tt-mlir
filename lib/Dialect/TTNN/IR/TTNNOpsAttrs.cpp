...
struct Conv2dConfigAttrParams {
  std::optional<mlir::tt::DataType> dtype;
  std::optional<mlir::tt::DataType> weightsDtype;
...
public:
  ...
  Conv2dConfigAttrParams(Conv2dConfigAttr attr, bool partial = true) {
    ...
    dtype =
        getOrDefaultOpt(attr.getDtype(), mlir::tt::DataType::BFloat16, partial);
    weightsDtype = getOrDefaultOpt(attr.getWeightsDtype(),
                                   mlir::tt::DataType::BFloat16, partial);
    activation = attr.getActivation()
                     ? attr.getActivation()
                     : (partial ? nullptr : mlir::StringAttr::get(ctx, ""));
    deallocateActivation =
        getOrDefaultBool(attr.getDeallocateActivation(), ctx);
...
Conv2dConfigAttr Conv2dConfigAttr::getEmpty(::mlir::MLIRContext *context) {
  return Conv2dConfigAttr::get(context, /*dtype=*/std::nullopt,
                               ...
                               /*preprocessWeightsOnDevice=*/nullptr,
                               /*alwaysPreprocessWeights=*/nullptr,
...
bool Conv2dConfigAttr::hasPreprocessWeightsOnDevice() const {
  return getPreprocessWeightsOnDevice() != nullptr;
}

bool Conv2dConfigAttr::hasAlwaysPreprocessWeights() const {
  return getAlwaysPreprocessWeights() != nullptr;
}

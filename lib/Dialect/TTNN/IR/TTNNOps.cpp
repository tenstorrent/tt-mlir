...
mlir::tt::ttnn::SplitQueryKeyValueAndSplitHeadsOp::verify() {
  ...
  auto inputShape = inputType.getShape();
  int64_t batchSize = inputShape[BATCH_DIM];
  int64_t sequenceLength = inputShape[SEQUENCE_LENGTH_DIM];
  int64_t numHeads = getNumHeads();
  int64_t numKVHeads = getNumKvHeads() ? *getNumKvHeads() : numHeads;
  int64_t headSize = inputShape[HIDDEN_DIMENSION] / (2 * numKVHeads + numHeads);

  llvm::SmallVector<int64_t, 4> expectedQueryShape = {batchSize, numHeads,
  ...
  llvm::SmallVector<int64_t, 4> expectedKeyShape = {batchSize, numKVHeads,
                                                    sequenceLength, headSize};
  ...
  llvm::SmallVector<int64_t, 4> expectedValueShape = {batchSize, numKVHeads,
                                                      sequenceLength, headSize};

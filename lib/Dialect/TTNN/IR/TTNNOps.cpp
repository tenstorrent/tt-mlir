...
::mlir::LogicalResult mlir::tt::ttnn::Conv2dOp::verify() {
  ...
  if (!getWeight().getDefiningOp<mlir::tt::ttnn::PrepareConv2dWeightsOp>() &&
      !getWeight().getDefiningOp<mlir::ttcore::LoadCachedOp>()) {
...
::mlir::LogicalResult MeshShardOp::verify() {
  ...
  ::mlir::ttcore::MeshShardType shardType = getShardType();
...
::mlir::LogicalResult UpdateCacheOp::verify() {
  ...
  const ::mlir::ttcore::DataType cacheDataType =
      ::mlir::ttcore::elementTypeToDataType(cacheType.getElementType());
  const ::mlir::ttcore::DataType inputDataType =
      ::mlir::ttcore::elementTypeToDataType(inputType.getElementType());
  ...
  ::mlir::WalkResult walkResult =
      funcOp.walk(
          [&](Operation *op) -> ::mlir::WalkResult {
            ...
            if (::mlir::isa<::mlir::ttcore::LoadCachedOp>(op)) {
              emitOpError()

...
void OpConfigAnalysis::analysisImplementation() {
  ...
  for (auto opConfigs : analysisInput.legalConfigs) {
    TTMLIR_DEBUG(ttmlir::LogComponent::Optimizer, "Picking op config for op {}",
                 opConfigs.first->getName());
    for (auto config : opConfigs.second) {
      TTMLIR_DEBUG(ttmlir::LogComponent::Optimizer, "  Candidate config {}",
                   config.outputLayout);
    }
    // Prefer tiled layouts if available.
    ...
    for (auto config : opConfigs.second) {
      if (config.outputLayout.isTiled() == existingLayout.isTiled()) {
        TTMLIR_DEBUG(ttmlir::LogComponent::Optimizer, "  Picking {} layout {}",

...
static llvm::SmallVector<OpConfig> getUniqueTestConfigsForMatmulLinear(
...
static llvm::SmallVector<OpConfig>
  ...
  for (const auto &shardSpec : *shardSpecs) {
    ...
    for (const Edge &edge : edges->second) {
      ...
      if (reshardOnEdge) {
        ...
        llvm::SmallVector<OpConfig> testConfigs = getUniqueTestConfigs(
            consumerConfigs,
...
bool ShardSolver::insertReshard(const Edge &edge) {
  ...
  *consumerBitset = kBitsetNone;
  ...
  llvm::SmallVector<OpConfig> testConfigs = getUniqueTestConfigs(
      consumerConfigs, mlir::isa<ttnn::MatmulOp, ttnn::LinearOp>(consumerOp));

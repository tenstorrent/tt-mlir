...
bool ShardSolver::resolveStep() {
  ...
  for (const auto shardSpec : *shardSpecs) {
    ...
    for (const Edge &edge : edges->second) {
      ...
      // reshardOnEdge can only happen if an override exists for the edge.
      if (reshardOnEdge) {
      ...
      if (paths.empty() || ((*producerBitset & edgeProducerBitset) == 0) ||
        ...
        // Try resharding, if it fails return false as we cannot resolve.
...
bool ShardSolver::insertReshard(const Edge &edge) {
  ...
  *consumerBitset = kBitsetNone;
  ...
  for (const TTNNLayoutAttr &producerLayout : layouts) {
    llvm::Expected<TTNNLayoutAttr> shardCompatible = checkShardCompatible(
        consumerOp->getOperand(edge.operandIndex), producerLayout, consumerOp,
    ...
    if (shardCompatible) {
      ...
      TTMLIR_TRACE(ttmlir::LogComponent::Optimizer,
                   ...
                   "layout: {}, consumer idx: {}",
                   edge, producerLayout, consumerConfigIdx);
    ...
    if (llvm::DebugFlag) {
      ...
      errorCount[errorMsg].push_back(producerLayout);
    } else {
      llvm::consumeError(shardCompatible.takeError());
    }
  }
  ...
  if (!validConfigExists) {
    ...
    for ([[maybe_unused]] auto &layout : layouts) {
...
llvm::Expected<TTNNLayoutAttr> ShardSolver::checkShardCompatible(
  ...
  if (consumerConfig.outputLayout &&
    ...
    return llvm::createStringError(
        "[TTNN Optimizer] Output layout mismatch: backend returned layout "

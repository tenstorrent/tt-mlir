...
void LegalOpConfigAnalysis::fillOpSpecificAttrs() {
  if (auto conv2dOp = mlir::dyn_cast<ttnn::Conv2dOp>(op)) {
    ...
    Conv2dConfigAttr conv2dConfigAttrBase =
        analysisResult.begin()->opSpecificAttr
            ? mlir::cast<Conv2dConfigAttr>(
                  analysisResult.begin()->opSpecificAttr)
        : conv2dOp.getConv2dConfigAttr()
            ? conv2dOp.getConv2dConfigAttr()
            : Conv2dConfigAttr::getEmpty(op->getContext());

    // TODO(vkovacevic): This is needed to get through a tt-metal assert in
    // prepare_conv2d_weights.cpp where `weight_tensor_.dtype() ==
    // weights_bias_dtype`.
    //
    DataType dtype = elementTypeToDataType(
        mlir::cast<RankedTensorType>(conv2dOp.getOperand(0).getType())
            .getElementType());
    DataType weightsDtype = elementTypeToDataType(
        mlir::cast<RankedTensorType>(conv2dOp.getOperand(1).getType())
            .getElementType());
    conv2dConfigAttrBase = conv2dConfigAttrBase.withDtype(dtype);
    conv2dConfigAttrBase = conv2dConfigAttrBase.withWeightsDtype(weightsDtype);
    ...
    auto addConfigs = [&](const Conv2dConfigAttr &configAttr) {
      for (const OpConfig &existingOpConfig : analysisResult) {
        newLegalConfigs.emplace_back(existingOpConfig.outputLayout, configAttr);
      }
    };

    ...
    if (!beamState[op].empty()) {
      ...
      if (chosen.configHint.outputLayout) {
        TTMLIR_DEBUG(
            ...
            chosen.score.coreCount, chosen.score.isSharded, chosen.score.isL1,
            chosen.score.requiresReshard, chosen.configHint.outputLayout);
      } else {
        TTMLIR_DEBUG(ttmlir::LogComponent::GreedyOptimizer,
                     "[op {0}] -> chosen: <no output layout> "
  ...
  for (size_t ci = 0; ci < candidates.size(); ++ci) {
    ...
    if (c.configHint.outputLayout) {
      TTMLIR_TRACE(ttmlir::LogComponent::GreedyOptimizer,
                   ...
                   ci, op->getName(),
                   c.configHint.outputLayout.getBufferType(),
      ...
      TTMLIR_TRACE(ttmlir::LogComponent::GreedyOptimizer,
                   "  BEAM[{0}] {1}: <no output> "
                   "score(reshard={2}) inputs=[{3}]",
                   ci, op->getName(), c.score.requiresReshard,

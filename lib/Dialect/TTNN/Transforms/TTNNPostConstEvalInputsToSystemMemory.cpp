...
static int64_t getPaddingWasteBytes(RankedTensorType type) {
  ...
  return getTiledVolume(type.getShape()) - ttmlir::utils::volume(type.getShape());
}
...
public:
  ...
  LogicalResult matchAndRewrite(ttnn::ToLayoutOp op,
    ...
    auto tiledInputType = mlir::cast<RankedTensorType>(op.getResult().getType());
    if (!isPureTileLayoutChange(inputType, tiledInputType) || !op->hasOneUse()) {
      return failure();
    }

...
LogicalResult SparseMatmulTileDimsRewritePattern::matchAndRewrite(
  ...
  if (!isASparse && isBSparse) {
    ...
    // Down: A=[BD*S, E, 1, inter] (standard) or [BD, S, E, inter] (MoE A2a)
    // For [BD, S, E, inter] layout, reshape to standard [BD*S, E, 1, inter]
    // so the existing tiling logic and runtime kernel work correctly.
    auto bShape = inputBType.getShape();
    int64_t bExperts = bShape[1];
    if (inputAShape[1] != bExperts && inputAShape.size() == 4 &&
        inputAShape[2] == bExperts) {
      int64_t BD = inputAShape[0];
      int64_t S = inputAShape[1];
      int64_t Elocal = inputAShape[2];
      int64_t inter = inputAShape[3];
      int64_t H = bShape[3];

      // Reshape input_a: [BD, S, E, inter] -> [BD*S, E, 1, inter]
      Value flatA = createReshape(rewriter, loc, inputA,
                                  {BD * S, Elocal, 1, inter}, "_down_flatten");

      // Create new sparse_matmul with standard layout
      auto flatAType = cast<RankedTensorType>(flatA.getType());
      llvm::SmallVector<int64_t> flatOutShape = {BD * S, Elocal, 1, H};
      auto flatOutType =
          ttnn::utils::RankedTensorTypeFactory::create(flatAType, flatOutShape);
      ...
      auto deviceAttr = ttcore::lookupDevice(srcOp);
      auto programConfigAttr =
          createProgramConfig(rewriter.getContext(), flatAType, inputBType,
                              deviceAttr, tileHeight, tileWidth);

      auto flatSparse = rewriter.create<ttnn::SparseMatmulOp>(
          loc, flatOutType, cast<TypedValue<RankedTensorType>>(flatA), inputB,
          sparsityMask, srcOp.getIsInputASparse(), srcOp.getIsInputBSparse(),
          nnzAttr, programConfigAttr, /*memory_config=*/nullptr,
          /*dtype=*/nullptr,
          /*compute_config=*/nullptr);

      // Reshape output: [BD*S, E, 1, H] -> [BD, S, E, H]
      auto restored = createReshape(rewriter, loc, flatSparse.getResult(),
                                    {BD, S, Elocal, H}, "_down_restore");

      rewriter.replaceOp(srcOp, restored);
      return success();
    }

...
LogicalResult ComplexReshapePattern::matchAndRewrite(ttnn::ReshapeOp srcOp,
  ...
  if ((Bi == Bo && Ci * Hi == Co) && (Ho * Wo == Wi) && (Ho > 1) && (Hi > 1)) {
    return decomposeReshapeType1(srcOp, rewriter, inputType, outputType,
      inputShape, outputShape);
  } else if ((Bi == Bo && Co * Ho == Ci) && (Hi * Wi == Wo) && (Ho > 1) && (Wi > 1)) {
    return decomposeReshapeType2(srcOp, rewriter, inputType, outputType,
      inputShape, outputShape);
  } else {
    return failure();
  }
...
LogicalResult ComplexReshapePattern::decomposeReshapeType1(
    ttnn::ReshapeOp srcOp, PatternRewriter &rewriter,
  ...
  RankedTensorType intermediateShapeType =
      ttnn::utils::RankedTensorTypeFactory::create(inputType, intermediateShape);
  ...
  auto firstNewReshape = rewriter.create<ttnn::ReshapeOp>(
      ttmlir::utils::appendLocationSuffix(srcOp.getLoc(), "_intermediate_reshape"),
      intermediateShapeType, srcOp.getInput(),
      rewriter.getI32ArrayAttr(intermediateShape32),
...
LogicalResult ComplexReshapePattern::decomposeReshapeType2(
  ttnn::ReshapeOp srcOp, PatternRewriter &rewriter,
  mlir::RankedTensorType inputType, mlir::RankedTensorType outputType,
  llvm::ArrayRef<int64_t> inputShape,
  llvm::ArrayRef<int64_t> outputShape) const {
...
llvm::SmallVector<int64_t, 4> intermediateShape = {Bi, Ci, 1, Hi *Wi};
RankedTensorType intermediateShapeType =
    ttnn::utils::RankedTensorTypeFactory::create(inputType, intermediateShape);

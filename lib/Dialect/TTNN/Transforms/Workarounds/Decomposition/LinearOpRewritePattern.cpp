...
static bool isEffectively1DBias(TypedValue<RankedTensorType> bias) {
  ...
  return llvm::count_if(biasShape, [](int64_t dim) { return dim > 1; }) <= 1;
}
...
LinearOpRewritePattern::matchAndRewrite(ttnn::LinearOp srcOp,
  ...
  if (srcOp.getActivation()) {
    ...
    if (activationStr == "relu") {
      activationResult = rewriter.create<ttnn::ReluOp>(
          ttmlir::utils::appendLocationSuffix(srcOp.getLoc(), "_decomp_relu"),
          outputType, addOp.getResult());
    } else if (activationStr == "sigmoid") {
      activationResult = rewriter.create<ttnn::SigmoidOp>(
          ttmlir::utils::appendLocationSuffix(srcOp.getLoc(),
                                              "_decomp_sigmoid"),
          outputType, addOp.getResult());
    } else if (activationStr == "gelu") {
      activationResult = rewriter.create<ttnn::GeluOp>(
          ttmlir::utils::appendLocationSuffix(srcOp.getLoc(), "_decomp_gelu"),
          outputType, addOp.getResult());
    } else if (activationStr == "tanh") {
      activationResult = rewriter.create<ttnn::TanhOp>(
          ttmlir::utils::appendLocationSuffix(srcOp.getLoc(), "_decomp_tanh"),
          outputType, addOp.getResult());
    } else {
      matmulOp.emitError()
    ...
    rewriter.replaceOp(srcOp, activationResult);
  } else {

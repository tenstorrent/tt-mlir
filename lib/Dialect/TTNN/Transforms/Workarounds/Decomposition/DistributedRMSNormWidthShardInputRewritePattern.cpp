...
LogicalResult DistributedRMSNormWidthShardInputRewritePattern::matchAndRewrite(
  ...
  // grid width. On Wormhole (8x8) 64 shards fill an 8x8 rectangle exactly, so
  // it happened to work. On Blackhole (11x6) 64 shards wrap to 5 full rows
  // (55 cores) + 9 cores, leaving a 66-core bounding box with 2 phantom cores
  // at (9,5) and (10,5) -> uninitialized read.
  //

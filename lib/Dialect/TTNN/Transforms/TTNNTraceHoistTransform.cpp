...
private:
  bool shouldHoistOp(Operation *op) {
    ...
    shouldHoist &= !::mlir::isa<mlir::tt::ttnn::ConstantOp>(op);
  ...
  void collectFunctionBoundary(llvm::ArrayRef<Operation *> opsToHoist,
    ...
    std::sort(inputs.begin(), inputs.end(), [](mlir::Value a, mlir::Value b) {
  ...
  createTraceFunctionAndOp(func::FuncOp funcOp,
    ...
    llvm::SmallVector<mlir::Value, 8> returnValues;
    for (auto output : outputs) {
  ...
  mlir::LogicalResult processFuncOp(func::FuncOp funcOp) {
    ...
    llvm::SmallVector<Operation *, 8> opsToHoist;

...
private:
  void process(ttnn::ToLayoutOp toTileOp) {
    ...
    //
    // In some configurations, the compiler inserts a to_memory_config(DRAM TILE)
    // after r_final (mc_dram_final) which converts ROW_MAJOR → TILE before the
    // downstream depthwise conv. In other configurations (e.g. larger models or
    // multiple camera inputs) this op may be absent. Without it, conv2d_DRAM
    // internally allocates an L1 tilize buffer at L1_UNRESERVED_BASE
    // (0x98000 = 622592), which clashes with the DramHeight kernel's own static
    // CBs → TT_THROW at runtime.
    //

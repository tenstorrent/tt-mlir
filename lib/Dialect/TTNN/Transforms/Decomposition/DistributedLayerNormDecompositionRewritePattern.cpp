...
// The partial statistics output of layer_norm_pre_all_gather has last dim equal
// to 2 * TILE_WIDTH. Each tile holds either sum(x^2) or sum(x) for the local
// shard, packed as two consecutive tiles along the width dimension.
static constexpr int64_t kLayerNormStatsWidth = 64; // 2 * TILE_WIDTH (32)

LogicalResult
DistributedLayerNormDecompositionRewritePattern::matchAndRewrite(
  ...
  // Computes local partial statistics (Welford: sum(x) and sum(x^2)) on the
  // local shard of norm_input. Output shape has last dim = kLayerNormStatsWidth
  // (= 64 = 2 * TILE_WIDTH).

  SmallVector<int64_t> statsShape(inputShape.begin(), inputShape.end());
  statsShape.back() = kLayerNormStatsWidth;
  ...
  // The gathered stats tensor has last dim = kLayerNormStatsWidth * numDevices.

  SmallVector<int64_t> gatheredShape(statsShape.begin(), statsShape.end());
  gatheredShape.back() = kLayerNormStatsWidth * numDevices;
  ...
  auto postAllGatherOp = rewriter.create<ttnn::LayerNormPostAllGatherOp>(
      ...
      normInput, allGatherOp.getResult(),
      op.getWeight(), op.getBias(),
      op.getEpsilonAttr(),
      /*dtype=*/dtypeAttr,
      /*memory_config=*/nullptr,
      /*compute_config=*/nullptr,

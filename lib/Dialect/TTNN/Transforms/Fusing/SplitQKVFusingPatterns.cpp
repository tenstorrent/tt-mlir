...
//
// Recognized purely from op kinds + shapes (no model-specific constants).
...
std::optional<DecodeChain> matchDecodeChain(Value start) {
  ...
  dc.half = half;
  dc.rotatedIsFirstConcatOperand = (concat.getInputs()[0] == rope.getResult());
  return dc;
}

// Rebuilds the matched RMSNorm + partial-RoPE chain on `decodeOut` (shape
// [1, B, H, D]) and returns the value that should replace `dc.layoutChange`.
// For Direct chains there is nothing to rebuild -> returns `decodeOut`.
...
mlir::LogicalResult QKVDecodeNormRopeReorderFusing::matchAndRewrite(
  ...
  for (Value out : outputs) {
    ...
    if (!dc || dc->kind != DecodeChainKind::NormRope) {
      continue;
    }

...
// ops (to_device / from_device / typecast / to_layout for tilize-untilize /
// to_memory_config). The strategy is intentionally simple and split into two
// steps:
//
...
//     Step 1 runs in the preferred (higher-ranked) of the input/output
//     memories. The typecast runs on device for any dtype, so as long as one
...
//
// Memory preference ranking: L1-sharded > L1-interleaved > DRAM > host.
...
private:
  ...
  bool layoutChangeRunsBeforeMemoryMove(const LayoutInfo &input,
    ...
    if (needLayout) {
      ...
      // produce a non-tile-aligned physical shard (e.g. 412x64) that
      // ttnn::tilize rejects; tilizing in the (interleaved) input memory and
      // resharding the already-tiled result avoids that.
      if (output.isTilized()) {
        return true;
      }
  ...
  // type (a fused tilize/untilize + cast). The result encoding carries both the
  // layout and the dtype; the flatbuffer/runtime read the dtype from there.
  mlir::Value
  ...
  // interleaved buffer instead. The documented-bad transitions are:
  //   HS<->WS, BS->HS (any HeightSharded side with an orientation change) and
  //   WS->WS. (BS->WS and same-orientation reshards are fine.)
  ...
  // (optionally routed through an interleaved buffer for the unsafe reshards).

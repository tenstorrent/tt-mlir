...
public:
  ...
  void runOnOperation() final {
    ...
    legalTensorLayoutAnalysis.init(
        mlir::tt::ttnn::LegalTensorLayoutAnalysisInput(maxGrid, &scalarTypes,
                                                       rowMajorEnabled));
    ...
    moduleOp->walk([&](func::FuncOp func) {
      ...
      processSpillOps(spillToDramOps, insertedMemoryReconfigOps);
...
private:
  ...
  static void processSpillOps(const std::vector<Operation *> &spillToDramOps,
    ...
    for (Operation *spilledOp : spillToDramOps) {
      ...
      MemoryConfigAttr memConfigAttr = MemoryConfigAttr::get(
          spilledOp->getContext(),
          BufferTypeAttr::get(spilledOp->getContext(), BufferType::DRAM),
          ShardSpecAttr::get(spilledOp->getContext(),
                             ShapeAttr::get(spilledOp->getContext(),
                                            dramLayout.getShardShape())),

...
private:
  ...
  void processFunc(mlir::func::FuncOp funcOp, ttcore::DataType dtype,
    ...
    funcOp.walk([&](Operation *op) {
      ...
      } else if (auto updateOp = mlir::dyn_cast<UpdateCacheOp>(op)) {
        castCacheOpInput(updateOp, dtype, builder);
      } else if (auto pagedFillOp = mlir::dyn_cast<PagedFillCacheOp>(op)) {
        castCacheOpInput(pagedFillOp, dtype, builder);
      } else if (auto pagedUpdateOp = mlir::dyn_cast<PagedUpdateCacheOp>(op)) {
        castCacheOpInput(pagedUpdateOp, dtype, builder);
      }
    });
  }
};

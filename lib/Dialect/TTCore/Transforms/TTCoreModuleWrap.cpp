...
public:
  ...
  LogicalResult matchAndRewrite(func::FuncOp stubFuncOp,
    ...
    // is private)
    if (!stubFuncOp->hasAttr(ttir::CPUHoistedFuncAttr::name) ||
    ...
    auto innerModule =
        dyn_cast_if_present<mlir::ModuleOp>(deviceModule.getBody()->front());
    if (!innerModule) {
      return failure();
    }
    ...
    for (auto callOp : callOps) {
      rewriter.modifyOpInPlace(callOp, [&]() {
        ...
        callOp->removeAttr(ttir::CPUHoistedCallAttr::name);
      });
    }

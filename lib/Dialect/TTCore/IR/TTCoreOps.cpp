...
LogicalResult CompositeOp::verify() {
  ModuleOp parentModule = getOperation()->getParentOfType<ModuleOp>();
  if (!parentModule) {
    return emitOpError("could not find parent module");
  }

  auto decompFuncName = getDecomposition();
  auto *symbolOp = SymbolTable::lookupSymbolIn(parentModule, decompFuncName);
  if (!symbolOp) {
    return emitOpError("decomposition function '")
           << decompFuncName << "' not found in parent module";
  }

  auto funcOp = dyn_cast<func::FuncOp>(symbolOp);
  if (!funcOp) {
    return emitOpError("decomposition symbol '")
           << decompFuncName << "' is not a func.func";
  }

  if (funcOp.getNumArguments() != getInputs().size()) {
    return emitOpError("decomposition function expects ")
           << funcOp.getNumArguments() << " arguments, but composite has "
           << getInputs().size() << " inputs";
  }

  for (auto [idx, pair] : llvm::enumerate(
           llvm::zip(getInputs().getTypes(), funcOp.getArgumentTypes()))) {
    auto [inputType, funcArgType] = pair;
    if (inputType != funcArgType) {
      return emitOpError("input type mismatch at index ")
             << idx << ": composite has " << inputType
             << ", decomposition expects " << funcArgType;
    }
  }

  if (funcOp.getNumResults() != getNumResults()) {
    return emitOpError("decomposition function returns ")
           << funcOp.getNumResults() << " results, but composite produces "
           << getNumResults() << " results";
  }

  for (auto [idx, pair] : llvm::enumerate(
           llvm::zip(getResults().getTypes(), funcOp.getResultTypes()))) {
    auto [resultType, funcResultType] = pair;
    if (resultType != funcResultType) {
      return emitOpError("result type mismatch at index ")
             << idx << ": composite produces " << resultType
             << ", decomposition returns " << funcResultType;
    }
  }

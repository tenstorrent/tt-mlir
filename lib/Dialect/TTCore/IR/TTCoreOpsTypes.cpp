...
unsigned ChipDescAttr::getDstLogicalSizeTiles(
    Type type, bool fullSyncEn, std::optional<unsigned> overrideSize) const {
  auto nDstTiles = getDstPhysicalSizeTiles();
  if (overrideSize.value_or(0) > 0) {
    nDstTiles = overrideSize.value();
  }
  if (!fullSyncEn) {
    nDstTiles /= 2;
  }

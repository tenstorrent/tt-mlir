...
bool MetalLayoutAttr::isTiled() const {
  return ::mlir::isa<TileType>(getElementType());
}

uint64_t MetalLayoutAttr::getElementSizeBytes() const {
  return mlir::ttcore::getElementSizeBytes(getElementType());
}
...
MemorySpace MetalLayoutAttr::getMemorySpace() const {
  return mlir::cast<MemorySpaceAttr>(getMemref().getMemorySpace()).getValue();
}

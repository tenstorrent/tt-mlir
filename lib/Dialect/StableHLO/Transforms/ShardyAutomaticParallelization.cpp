...
public:
  ...
  void runOnOperation() final {
    ...
    if (failed(pm.run(rootModule))) {
      rootModule.emitWarning("Failed to run inliner pass.");
      signalPassFailure();
      return;
    }
    pm.clear();
      ...
      for (auto arg : funcOp.getArguments()) {
        ...
        if (auto argAttrDict = funcOp.getArgAttrDict(arg.getArgNumber())) {
          ...
          if (!shardingAttr) {
            ...
            shardingAttr = sdy_utils::getDefaultTensorShardingAttr(
            ...
            mlir::DictionaryAttr newDictAttr =
                sdy_utils::addDictionaryAttrShardingAnnotation(
          ...
          // Set argument with it's default sdy.sharding attribute
          mlir::DictionaryAttr newDictAttr =
              sdy_utils::addDictionaryAttrShardingAnnotation(context,
                                                             shardingAttr);
          funcOp.setArgAttrs(arg.getArgNumber(), newDictAttr);
        }
    ...
    for (auto &op : rootModule.getBody()->getOperations()) {
      ...
      for (auto arg : funcOp.getArguments()) {
        if (auto argAttrDict = funcOp.getArgAttrDict(arg.getArgNumber())) {
          funcOp.setArgAttrs(arg.getArgNumber(),
                             sdy_utils::removeDictionaryAttrShardingAnnotation(
                                 context, argAttrDict));
        }
      }
    ...
    for (auto &op : rootModule.getBody()->getOperations()) {
      ...
      funcOp.walk([&](Operation *currOp) {
        if (isa<func::FuncOp>(currOp) || isa<func::ReturnOp>(currOp) ||
            ...
            isa<mlir::stablehlo::ReturnOp>(currOp)) {
          return;
        }
    ...
    if (failed(pm.run(rootModule))) {
      rootModule.emitError("Failed to run close shardings pass from sdy.\n");
      signalPassFailure();
      return;
    }
    pm.clear();

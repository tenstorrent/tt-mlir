...
// internally. All dimensions use kNeedReplication to prevent Shardy
// from adding any communication around the op.
//
...
getAllToAllDispatchShardingRule(mlir::stablehlo::CustomCallOp op) {
  ...
  if (op.getNumResults() == 2) {
    ...
    auto tupleType =
        llvm::dyn_cast<mlir::TupleType>(op.getResult(0).getType());
  ...
  builder.addFactor(
      {mlir::sdy::kNullDim, mlir::sdy::kNullDim, 2},
...
// without adding communication. All other dims use kNeedReplication.
//
...
getAllToAllCombineShardingRule(mlir::stablehlo::CustomCallOp op) {
  if (op.getNumOperands() != 3) {
    op.getOperation()->emitWarning()
        << "all_to_all_combine expects 3 operands, got "
        << op.getNumOperands();
    return mlir::sdy::OpShardingRuleAttr();
  }
  ...
  if (!resultType || resultType.getRank() != 4) {
    op.getOperation()->emitWarning()
        << "all_to_all_combine: result must be 4D";
    return mlir::sdy::OpShardingRuleAttr();
  }
  ...
  // E factor: expertOut[0]=dim0, metadata=kNull, mapping[2]=dim2,
  //           result=kNull (E is consumed by combine)
  // kPassThrough: allows EP sharding without communication
  builder.addFactor({0, mlir::sdy::kNullDim, 2},

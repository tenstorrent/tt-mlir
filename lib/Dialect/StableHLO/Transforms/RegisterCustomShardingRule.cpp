...
static constexpr llvm::StringLiteral rawKernelTargetName = "tt.raw_kernel";
...
// Sharding rule for the streaming selective-experts matmul (tt-xla low-batch
// MoE), emitted as `stablehlo.custom_call @tt.raw_kernel`. Operand layout:
...
// One rule covers both: K is kReduction, N is kPassThrough, and Shardy picks
// the sharded factor from the actual param sharding. Experts (E) stay
// replicated (streaming selects the global top-k on every device).
static mlir::sdy::OpShardingRuleAttr
getRawKernelShardingRule(mlir::stablehlo::CustomCallOp op) {
...
private:
  ...
  llvm::DenseMap<llvm::StringRef, std::function<mlir::sdy::OpShardingRuleAttr(
      ...
      customCallShardingRules = {
          ...
          {rawKernelTargetName, getRawKernelShardingRule},
      };
};

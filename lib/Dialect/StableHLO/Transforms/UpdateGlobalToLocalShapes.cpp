...
static mlir::LogicalResult updateManualAxes(MLIRContext *context,
  ...
  mlir::WalkResult result = funcOp.getBody().walk([&](mlir::Operation *op) {
    ...
    for (uint32_t i = 0; i < initialArgSize; i++) {
      ...
      if (failed(newType)) {
        manualComputationOp.emitError("11Could not apply propagated tensor "
...
static mlir::LogicalResult updateShapes(MLIRContext *context,
  ...
  mlir::WalkResult result = funcOp.getBody().walk([&](mlir::Operation *op) {
    for (auto namedAttr : op->getAttrs()) {
      ...
      if (failed(newTypes)) {
        auto debugTensorShardings = [](llvm::ArrayRef<mlir::sdy::TensorShardingAttr> shardings) {
          std::string result;
          llvm::raw_string_ostream os(result);
      
          os << "[";
          for (size_t i = 0; i < shardings.size(); ++i) {
              auto attr = shardings[i];
              if (attr)
                  os << attr;          // ✅ 공용 경로로 출력
              else
                  os << "null";
              if (i + 1 != shardings.size())
                  os << ", ";
          }
          os << "]";
          return os.str();
      };
        // Create debug string for globalMeshOp and tensorShardings
        std::string debugMsg = "Could not apply propagated tensor shardings to tensor dimensions. ";
        debugMsg += "globalMeshOp.getMesh(): ";
        auto meshAxes = globalMeshOp.getMesh().getAxes();
        debugMsg += std::to_string(meshAxes.size()) + " axes [";
        for (size_t i = 0; i < meshAxes.size(); ++i) {
          debugMsg += std::to_string(meshAxes[i].getSize());
          if (i + 1 < meshAxes.size()) debugMsg += ", ";
        }
          debugMsg += "], tensorShardings: " + debugTensorShardings(tensorShardings);
        debugMsg += "]";
        op->emitError(debugMsg);
        return WalkResult::interrupt();
      }

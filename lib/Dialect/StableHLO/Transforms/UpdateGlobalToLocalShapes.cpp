...
// Check if the integer DenseElementsAttr values form an arithmetic sequence
// v[i] = firstValue + i * commonDiff. Returns true and sets firstValue and
// commonDiff on success.
static bool isArithmeticSequence(mlir::DenseElementsAttr attr,
                                  int64_t &firstValue, int64_t &commonDiff) {
  if (!mlir::isa<mlir::IntegerType>(attr.getElementType())) {
    return false;
  }

  auto values = attr.getValues<mlir::APInt>();
  ...
  firstValue = (*it).getSExtValue();
  ++it;

  if (it == end) {
    commonDiff = 0;
    return true; // Single element: trivially arithmetic.
  }

  commonDiff = (*it).getSExtValue() - firstValue;
  ++it;

  for (int64_t i = 2; it != end; ++it, ++i) {
    if ((*it).getSExtValue() != firstValue + i * commonDiff) {
      return false;
    }
  }
...
// Attempt to replace an arithmetic sequence constant (e.g., [0,1,...,n-1])
// with a stablehlo.partition_id-based runtime computation. For an 8-device
// mesh sharding the constant across all 8 devices, device k gets value
// (a + k*d). Returns true if the replacement was successfully emitted and the
// caller should erase the original op.
static bool tryEmitArithmeticSequenceReplacement(
    mlir::OpBuilder &builder, mlir::stablehlo::ConstantOp constantOp,
    mlir::RankedTensorType newType, mlir::sdy::TensorShardingAttr sharding,
    mlir::sdy::MeshOp globalMeshOp) {

  // Only handle integer element types.
  auto elemType = mlir::dyn_cast<mlir::IntegerType>(newType.getElementType());
  if (!elemType) {
    return false;
  }

  // Only handle cases where the local shard is exactly 1 element.
  if (newType.getNumElements() != 1) {
    return false;
  }

  // Check that global values form an arithmetic sequence.
  auto globalAttr =
      mlir::cast<mlir::DenseElementsAttr>(constantOp.getValue());
  int64_t a, d;
  if (!isArithmeticSequence(globalAttr, a, d)) {
    return false;
  }

  // Find the name of the axis used for sharding.
  llvm::StringRef shardAxisName;
  for (auto dimSharding : sharding.getDimShardings()) {
    auto axisRefs = dimSharding.getAxes();
    if (!axisRefs.empty()) {
      shardAxisName = axisRefs[0].getName();
      break;
    }
  }
  if (shardAxisName.empty()) {
    return false;
  }

  // Walk the mesh axes to find axisSize and the stride (product of sizes of
  // all mesh axes that come after the sharding axis in the mesh ordering).
  int64_t axisSize = 0;
  int64_t stride = 1;
  bool foundAxis = false;
  for (auto meshAxis : globalMeshOp.getMesh().getAxes()) {
    if (meshAxis.getName() == shardAxisName) {
      axisSize = meshAxis.getSize();
      foundAxis = true;
    } else if (foundAxis) {
      stride *= meshAxis.getSize();
    }
  }

  if (!foundAxis || axisSize <= 1) {
    return false;
  }

  // Emit the replacement ops before the original constant.
  builder.setInsertionPoint(constantOp);
  auto loc = constantOp.getLoc();
  MLIRContext *ctx = constantOp.getContext();

  auto ui32ElemType =
      mlir::IntegerType::get(ctx, 32, mlir::IntegerType::Unsigned);
  auto ui32ScalarType = mlir::RankedTensorType::get({}, ui32ElemType);

  // Helper lambda: create a scalar integer constant of a given shaped type.
  // Uses the splat DenseElementsAttr::get(type, attribute) overload which
  // avoids raw-byte size mismatches between host int64_t and the element type.
  auto makeScalarConst = [&](mlir::RankedTensorType scalarType,
                             int64_t value) -> mlir::Value {
    auto intType = mlir::cast<mlir::IntegerType>(scalarType.getElementType());
    mlir::Attribute valAttr = mlir::IntegerAttr::get(intType, value);
    return builder
        .create<mlir::stablehlo::ConstantOp>(
            loc, mlir::DenseElementsAttr::get(scalarType, valAttr))
        .getResult();
  };

  // %pid = stablehlo.partition_id : tensor<ui32>
  auto pid = builder.create<mlir::stablehlo::PartitionIdOp>(loc);
  mlir::Value k_u32 = pid.getResult();

  // k_u32 = pid / stride  (integer division when stride > 1)
  if (stride > 1) {
    mlir::Value strideCst = makeScalarConst(ui32ScalarType, stride);
    k_u32 = builder
                .create<mlir::stablehlo::DivOp>(loc, ui32ScalarType, k_u32,
                                                strideCst)
                .getResult();
  }

  // k_u32 = k_u32 % axisSize
  {
    mlir::Value sizeCst = makeScalarConst(ui32ScalarType, axisSize);
    k_u32 = builder
                .create<mlir::stablehlo::RemOp>(loc, ui32ScalarType, k_u32,
                                                sizeCst)
                .getResult();
  }

  // Convert k from ui32 to the constant's element type.
  auto scalarElemType = mlir::RankedTensorType::get({}, elemType);
  mlir::Value k =
      builder.create<mlir::stablehlo::ConvertOp>(loc, scalarElemType, k_u32)
          .getResult();

  // Compute result = a + k * d.
  mlir::Value result;
  if (d == 0) {
    // Degenerate constant (would be splat; handle gracefully).
    result = makeScalarConst(scalarElemType, a);
  } else if (a == 0 && d == 1) {
    // Common iota case: result = k.
    result = k;
  } else {
    // General: result = a + k * d.
    mlir::Value dCst = makeScalarConst(scalarElemType, d);
    mlir::Value kd =
        builder
            .create<mlir::stablehlo::MulOp>(loc, scalarElemType, k, dCst)
            .getResult();
    if (a == 0) {
      result = kd;
    } else {
      mlir::Value aCst = makeScalarConst(scalarElemType, a);
      result = builder
                   .create<mlir::stablehlo::AddOp>(loc, scalarElemType, aCst,
                                                   kd)
                   .getResult();
    }
  }

  // Reshape to newType when the local shard shape is not a scalar
  // (e.g., tensor<i32> -> tensor<1xi32>).
  if (result.getType() != newType) {
    result = builder.create<mlir::stablehlo::ReshapeOp>(loc, newType, result)
                 .getResult();
  }

  // Replace all uses of the original constant op with the computed value.
  constantOp.getResult().replaceAllUsesWith(result);
  return true;
}
...
static mlir::LogicalResult updateShapes(MLIRContext *context,
  ...
  mlir::WalkResult result = funcOp.getBody().walk([&](mlir::Operation *op) {
    for (auto namedAttr : op->getAttrs()) {
      ...
      // For non-splat ConstantOps, attempt partition_id-based replacement for
      // arithmetic sequence constants (e.g. [0,1,...,n-1]) before falling
      // through to the generic createNewOperationState path.
      if (auto constantOp =
              mlir::dyn_cast<mlir::stablehlo::ConstantOp>(op)) {
        auto denseAttr =
            mlir::cast<mlir::DenseElementsAttr>(constantOp.getValue());
        if (!denseAttr.isSplat() &&
            tryEmitArithmeticSequenceReplacement(builder, constantOp,
                                                 (*newTypes)[0],
                                                 tensorShardings[0],
                                                 globalMeshOp)) {
          op->erase();
          break; // Op replaced; exit the attribute loop.
        }
      }

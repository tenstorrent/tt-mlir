...
void removeSdyTensorShardings(MLIRContext *context, func::FuncOp &funcOp) {
  ...
  for (auto arg : funcOp.getArguments()) {
    if (auto argAttrDict = funcOp.getArgAttrDict(arg.getArgNumber())) {
      ...
      funcOp.setArgAttrs(
          arg.getArgNumber(),
          mlir::tt::shardy_utils::removeDictionaryAttrSdyShardingAnnotations(
              context, argAttrDict));
    }
  }
  ...
  for (uint32_t i = 0; i < funcType.getNumResults(); i++) {
    if (auto resultAttrDict =
      ...
      funcOp.setResultAttrs(
          i, mlir::tt::shardy_utils::removeDictionaryAttrSdyShardingAnnotations(
  ...
  for (auto arg : funcOp.getArguments()) {
    ...
    if (auto argAttrDict = funcOp.getArgAttrDict(arg.getArgNumber())) {
      ...
      if (!shardingAttr) {
        ...
        shardingAttr = mlir::tt::shardy_utils::getDefaultTensorSdyShardingAttr(
        ...
        mlir::DictionaryAttr newDictAttr =
            mlir::tt::shardy_utils::addDictionaryAttrSdyShardingAnnotation(

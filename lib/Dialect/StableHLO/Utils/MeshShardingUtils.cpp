...
llvm::Expected<bool> MeshSharding::parseGSPMDDevicesStr(StringRef devicesStr, llvm::ArrayRef<int64_t> shardShape, llvm::ArrayRef<int64_t> meshShape, llvm::ArrayRef<int64_t> deviceIds) {
...
llvm::Expected<bool> MeshSharding::determineGSPMDShardingDims(llvm::ArrayRef<int64_t> shardShape, llvm::ArrayRef<int64_t> shardDims, llvm::ArrayRef<int64_t> meshShape, llvm::ArrayRef<int64_t> deviceIds) {
...
llvm::Expected<GSPMDMeshSharding> GSPMDMeshSharding::generate(llvm::StringRef opShardingStr, llvm::StringRef operandShardingStr, mlir::tt::ttcore::ShardStatus shardStatus) {
  // Need to parse GSPMD sharding string and fill out MeshSharding info.
  mlir::tt::ttcore::MeshShardDirection shardDirection = mlir::tt::ttcore::MeshShardDirection::ShardToFull;
  mlir::tt::ttcore::MeshShardType shardType = mlir::tt::ttcore::MeshShardType::Identity;
  ...
  if (!operandShardingStr.consume_front("{") || !operandShardingStr.consume_back("}")) {
    return llvm::createStringError(std::errc::invalid_argument,
  ...
  // Our goal is to map opShardingStr and operandShardingStr to a mesh shard op which is represented by MeshSharding object.
  // If any argument is pre-sharded, we will create an identity mesh shard op by default. This is to ensure the shapes are consistent
  // in the graph but runtime will not attempt to shard the data again.
  // opShardingStr and operandShardingStr may or may not have last_tile_dim_replicate annotated.

  // opShardingStr and operandShardingStr will have one of the following combinations. All others are not legal.
  ...
  auto containsKeyword = [](const llvm::SmallVector<llvm::StringRef> &tokens, std::string keyword) -> bool {
    return llvm::any_of(tokens, [](llvm::StringRef str) { return str.contains(keyword); });
  };

  // Any combination which does not have manual is not supported.
  if (!containsKeyword(opShardingStrTokens, "manual") && !containsKeyword(operandShardingStrTokens, "manual")) {
    return llvm::createStringError(std::errc::invalid_argument, "Fail to parse GSPMD sharding without manual.");
  }

  // Maximal is not supported with GSPMD sharding.
  if (containsKeyword(opShardingStrTokens, "maximal") || containsKeyword(operandShardingStrTokens, "maximal")) {
    return llvm::createStringError(std::errc::invalid_argument, "Fail to parse GSPMD sharding with maximal.");
  }

  // Check for replicated sharding.
  if (containsKeyword(opShardingStrTokens, "replicated") || containsKeyword(operandShardingStrTokens, "replicated")) {
  ...
  if (containsKeyword(opShardingStrTokens, "last_tile_dim_replicate") || containsKeyword(operandShardingStrTokens, "last_tile_dim_replicate")) {
    containsLastTileDimReplicate = true;
  }

  // Check for devices sharding.
  if (containsKeyword(opShardingStrTokens, "devices=") || containsKeyword(operandShardingStrTokens, "devices=")) {
    shardType = ttcore::MeshShardType::Devices;

    // Extract device ids from the devices string.
    llvm::SmallVector<llvm::StringRef> shardingStrTokens = containsKeyword(opShardingStrTokens, "devices=") ? opShardingStrTokens : operandShardingStrTokens;
    
    for (auto str : shardingStrTokens) {
      if (str.consume_front("devices=")) {
        // Parse the devices string and fill out shardShape, meshShape and deviceIds.
        auto error = mlir::tt::sharding_utils::parseGSPMDDevicesStr(str, shardShape, meshShape, deviceIds);
    ...
    auto error = mlir::tt::sharding_utils::determineGSPMDShardingDims(shardShape, shardDims, meshShape, deviceIds);
    if (auto e = error.takeError()) {
      return e;
    }
  }

  // Check if the input is already pre-sharded. If it is, override shardType to Identity.
  shardType = shardType == ttcore::MeshShardType::Identity ? ttcore::MeshShardType::Identity : shardType;

  return GSPMDMeshSharding{
      shardDirection, shardType, shardShape, shardDims, meshShape, deviceIds,
      meshName, shardStatus, opShardingStr.str(), operandShardingStr.str(), lastTileDimReplicate};
}

// Parse Shardy sharding attribute.
llvm::Expected<bool>
MeshSharding::parseSdySharding(mlir::sdy::TensorShardingAttr sdySharding, mlir::sdy::MeshAttr meshAttr, llvm::ArrayRef<int64_t> shardShape, llvm::ArrayRef<int64_t> shardDims, llvm::SmallVector<int64_t> meshShape) {
...
llvm::Expected<ShardyMeshSharding> generate(sdy::MeshAttr meshAttr, sdy::TensorShardingAttr sdySharding, ttcore::MeshShardDirection shardDirection) {
  ...
  llvm::SmallVector<int64_t> meshShape = {-1};
  llvm::SmallVector<int64_t> deviceIds = {-1};
  mlir::tt::ttcore::ShardStatus shardStatus = mlir::tt::ttcore::ShardStatus::Unsharded;

  // Lambda function to set non-device shard types.
  auto setNonDevicesShardType = [&](ttcore::MeshShardType targetShardType) -> void {
  ...
  return ShardyMeshSharding{
      shardDirection, shardType, shardShape, shardDims, meshShape, deviceIds, shardStatus,
      meshAttr, sdySharding};
}

...
public:
  ...
  void runOnOperation() final {
    ...
    auto isConstevalOp = [](Operation *op) {
      ...
      // Check if the operation is a marked as part of the consteval subgraph.
      return op->hasAttr("emitpy.consteval");
    };

    // Copy all import ops to both files.
    for (auto importOp :
    ...
    // Move all operations to the appropriate file operation.

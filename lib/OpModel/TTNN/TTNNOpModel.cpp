...
llvm::Expected<OpConstraints> Conv2dOpInterface::getOpConstraints(
  ...
  auto conv2dOpQuery = [=]() {
    ...
    //        == "Tensor(
    // storage=DeviceStorage(memory_config=MemoryConfig(memory_layout=TensorMemoryLayout::INTERLEAVED,buffer_type=BufferType::L1,shard_spec=std::nullopt)),
    // tensor_spec=
    // TensorSpec(logical_shape=Shape([1, 2048, 4,
    // 128]),tensor_layout=TensorLayout(dtype=DataType::BFLOAT16,page_config=PageConfig(config=RowMajorPageConfig(tile=Tile(tile_shape={32,
    // 32},face_shape={16, 16},num_faces=4))),
    // memory_config=MemoryConfig(memory_layout=TensorMemoryLayout::INTERLEAVED,buffer_type=BufferType::L1,shard_spec=std::nullopt),alignment=Alignment([1]))))"
    ...
    string biasSpecStr =
        biasTensor ? tensorSpecToString(biasTensor->tensor_spec()) : "std::nullopt";
    ...
    std::cout
        ...
        << "kernel_size=" << stdArrayToString(
               conversion::convertLLVMArrayRefToStdArray<uint32_t, 2>(
                   kernel_size))
        << ", "
        << "stride=" << stdArrayToString(
               conversion::convertLLVMArrayRefToStdArray<uint32_t, 2>(stride))
        << ", "
        << "padding=" << stdArrayToString4(get<std::array<uint32_t, 4>>(
               conversion::convertLLVMArrayRefToMultiSizeStdArray<uint32_t, 2,
                                                                  4>(padding)))
        << ", "
        << "dilation=" << stdArrayToString(
               conversion::convertLLVMArrayRefToStdArray<uint32_t, 2>(dilation))
        << ", " << groups << ", /*bias*/" << biasSpecStr << ", "
        << conv2dCfgToStr(localConfig) << ", " << outputMemoryConfigStr << ")"
        << std::endl;

...
llvm::Expected<OpConstraints> OpModel<DivOp>::getOpConstraints(
    ttcore::GridAttr deviceGrid, llvm::ArrayRef<int64_t> inputShapeA,
    TTNNLayoutAttr inputLayoutA, llvm::ArrayRef<int64_t> inputShapeB,
    TTNNLayoutAttr inputLayoutB, bool accurateMode,
    std::optional<std::string> roundMode, TTNNLayoutAttr outputLayout) {
#ifdef TTMLIR_ENABLE_OPMODEL
  ::tt::tt_metal::distributed::MeshDevice *device =
      SingletonDeviceContext::getInstance().getDevice();
  ...
  auto query = [=]() {
    return ::ttnn::graph::query_op_constraints(
        ::ttnn::div, device, inputSpecA, inputSpecB, accurateMode, roundMode,
        /*output_dtype=*/std::nullopt, outputMemoryConfig);
  };
...
llvm::Expected<size_t> OpModel<DivOp>::getOpRuntime(
    llvm::ArrayRef<int64_t> inputShapeA, TTNNLayoutAttr inputLayoutA,
    llvm::ArrayRef<int64_t> inputShapeB, TTNNLayoutAttr inputLayoutB,
    bool accurateMode, std::optional<std::string> roundMode,
    TTNNLayoutAttr outputLayout) {
#ifdef TTMLIR_ENABLE_OPMODEL
  ::tt::tt_metal::distributed::MeshDevice *device =
      SingletonDeviceContext::getInstance().getDevice();
...
#else
  return llvm::createStringError(llvm::inconvertibleErrorCode(),
                                 "OpRuntime not yet implemented for div");
#endif // TTMLIR_ENABLE_OPMODEL
}

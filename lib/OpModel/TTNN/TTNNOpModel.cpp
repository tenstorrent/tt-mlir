...
llvm::Expected<OpConstraints> OpModel<Conv2dOp>::getOpConstraints(
  ...
  auto conv2dOpQuery = [=]() {
    unifiedOpLib::Conv2dOpResult result = unifiedOpLib::callConv2d(
        unifiedOpLib::CallType::QUERY_OP_CONSTRAINTS, conv2dOpT, inputSpec,
        weightSpec,
...
llvm::Expected<size_t> OpModel<Conv2dOp>::getOpRuntime(
  ...
  auto conv2dOpQuery = [=]() {
    unifiedOpLib::Conv2dOpResult result = unifiedOpLib::callConv2d(
        unifiedOpLib::CallType::QUERY_OP_RUNTIME, conv2dOpT, inputSpec,
        weightSpec,
        biasSpec.has_value() ? std::optional<unifiedOpLib::TensorArg>(*biasSpec)
                             : std::nullopt,
        *device);
...
#ifdef TTMLIR_ENABLE_OPMODEL
static ::tt::target::ttnn::Conv3dOpT buildConv3dOpTFromMLIR(
    uint32_t in_channels, uint32_t out_channels, uint32_t batch_size,
  ...
  opT.compute_config =
      (deviceComputeKernelConfig.has_value() && *deviceComputeKernelConfig)
          ? std::make_unique<::tt::target::ttnn::DeviceComputeKernelConfigT>(
                toNative(*deviceComputeKernelConfig))
          : nullptr;
  auto memConfigT = detail::getNullableMemoryConfigT(outputLayout);
  if (memConfigT.has_value()) {
    opT.memory_config =
        std::make_unique<::tt::target::ttnn::MemoryConfigT>(*memConfigT);
  }
  return opT;
}
#endif // TTMLIR_ENABLE_OPMODEL
...
llvm::Expected<OpConstraints> OpModel<Conv3dOp>::getOpConstraints(
  ...
  auto conv3dOpQuery = [=]() {
    unifiedOpLib::Conv3dOpResult result = unifiedOpLib::callConv3d(
        unifiedOpLib::CallType::QUERY_OP_CONSTRAINTS, conv3dOpT, inputSpec,
        weightSpec,
        biasSpec.has_value() ? std::optional<unifiedOpLib::TensorArg>(*biasSpec)
                             : std::nullopt,
        *device);
...
llvm::Expected<size_t> OpModel<Conv3dOp>::getOpRuntime(
  ...
  auto conv3dOpRuntime = [=]() {
    unifiedOpLib::Conv3dOpResult result = unifiedOpLib::callConv3d(
        unifiedOpLib::CallType::QUERY_OP_RUNTIME, conv3dOpT, inputSpec,
        weightSpec,
        biasSpec.has_value() ? std::optional<unifiedOpLib::TensorArg>(*biasSpec)
                             : std::nullopt,
        *device);
...
#ifdef TTMLIR_ENABLE_OPMODEL
static ::tt::target::ttnn::ConvTranspose2dOpT buildConvTranspose2dOpTFromMLIR(
    uint32_t in_channels, uint32_t out_channels, uint32_t batch_size,
    uint32_t input_height, uint32_t input_width,
    llvm::ArrayRef<int32_t> kernel_size, llvm::ArrayRef<int32_t> stride,
...
llvm::Expected<OpConstraints> OpModel<ConvTranspose2dOp>::getOpConstraints(
  ...
  ::tt::target::ttnn::ConvTranspose2dOpT opT = buildConvTranspose2dOpTFromMLIR(
      in_channels, out_channels, batch_size, input_height, input_width,
  ...
  auto convTranspose2dOpQuery = [=]() {
    unifiedOpLib::ConvTranspose2dOpResult result =
        unifiedOpLib::callConvTranspose2d(
            unifiedOpLib::CallType::QUERY_OP_CONSTRAINTS, opT, inputSpec,
            weightSpec,
            biasSpec.has_value()
                ? std::optional<unifiedOpLib::TensorArg>(*biasSpec)
                : std::nullopt,
            *device);
    assert(std::holds_alternative<::ttnn::graph::ConstraintQueryResponse>(
               result) &&
...
llvm::Expected<size_t> OpModel<ConvTranspose2dOp>::getOpRuntime(
  ...
  auto convTranspose2dOpQuery = [=]() {
    unifiedOpLib::ConvTranspose2dOpResult result =
        unifiedOpLib::callConvTranspose2d(
            unifiedOpLib::CallType::QUERY_OP_RUNTIME, opT, inputSpec,
            weightSpec,
            biasSpec.has_value()
                ? std::optional<unifiedOpLib::TensorArg>(*biasSpec)
                : std::nullopt,
...
#ifdef TTMLIR_ENABLE_OPMODEL
static ::tt::target::ttnn::PrepareConv2dWeightsOpT
...
llvm::Expected<OpConstraints> OpModel<PrepareConv2dWeightsOp>::getOpConstraints(
  ...
  auto prepareConv2dWeightsQuery = [=]() {
    unifiedOpLib::PrepareConv2dWeightsOpResult result =
        unifiedOpLib::callPrepareConv2dWeights(
            unifiedOpLib::CallType::QUERY_OP_CONSTRAINTS, opT, &weightTensor,
            *device);
    assert(std::holds_alternative<::ttnn::graph::ConstraintQueryResponse>(
               result) &&
...
#ifdef TTMLIR_ENABLE_OPMODEL
static ::tt::target::ttnn::PrepareConvTranspose2dWeightsOpT
...
OpModel<PrepareConvTranspose2dWeightsOp>::getOpConstraints(
  ...
  auto prepareConvTranspose2dWeightsQuery = [=]() {
    ...
    assert(
        std::holds_alternative<::ttnn::graph::ConstraintQueryResponse>(
            result) &&
        "Expected PrepareConvTranspose2dWeightsOp constraints query to return "
        "ConstraintQueryResponse");
    return std::get<::ttnn::graph::ConstraintQueryResponse>(result);
  };
...
#ifdef TTMLIR_ENABLE_OPMODEL
static ::tt::target::ttnn::PrepareConvTranspose2dBiasOpT

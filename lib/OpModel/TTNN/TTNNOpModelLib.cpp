...
bool isTensorLayoutLegalForOp(Operation *op,
  ...
  // we return false.
  try {
    conversion::getTensorSpec(outputType.getShape(), layout);
  } catch (const std::exception &e) {
    return false;
  }
  return true;
...
getEltwiseBinaryOpConstraints(std::string_view opName, OpSymbol opSymbol,
  ...
  const auto [inputSpecA, inputSpecB, outputSpec] = detail::convertToTensorSpec(
...
Conv2dOpInterface::getOpConstraints(
  ...
  // Prepare io specs
  const auto [inputSpec, weightSpec, outputSpec] = detail::convertToTensorSpec(
      device, std::make_tuple(inputShape, inputLayout),

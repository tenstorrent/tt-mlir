...
private:
  void getConstsAndParams() {
    ...
    constParams = mlir::ttcore::getConstsAndParams(*funcOp);
  }
  ...
  bool isCreationOp(mlir::Operation *op) {
    assert(op != nullptr);
    return op->hasTrait<mlir::ttcore::Trait::TTCoreCreationOpTrait>();
  }

  bool isSharedOp(mlir::Operation *op) {
    assert(op != nullptr);
    return op->hasTrait<mlir::ttcore::Trait::TTCoreDuplicateConstEvalTrait>();
  }
...
static void deduplicateSharedOps(func::FuncOp funcOp) {
  ...
  funcOp.walk([&](Operation *op) {
    if (op->hasTrait<mlir::ttcore::Trait::TTCoreDuplicateConstEvalTrait>()) {
...
static void inlineConstEvalFunction(mlir::func::FuncOp funcOp,
                                    mlir::ttcore::LoadCachedOp callOp,
                                    OpBuilder &builder) {
...
static void undoConstEvalImpl(mlir::ModuleOp module,
  ...
  llvm::DenseMap<mlir::func::FuncOp, mlir::ttcore::LoadCachedOp> funcToCall;
  ...
  module.walk([&](mlir::ttcore::LoadCachedOp loadOp) {
  ...
  for (auto funcOp : constEvalFuncs) {
    ...
    mlir::ttcore::LoadCachedOp &callOp = callIt->second;

...
createOp(FlatbufferObjectCache &cache, MeshShardOp op) {
  ...
  auto output = cache.getOrCreate(op.getResult(), tensorValueToFlatbuffer,
                                  kHostAllocatedSize);
  auto device = getOperandThroughDPSOps(op.getDevice());
  const mlir::ttcore::MeshShardDirection shardDirection =
      op.getShardDirection();
  const mlir::ttcore::MeshShardType shardType = op.getShardType();
  ...
  if (shardDirection == mlir::ttcore::MeshShardDirection::FullToShard) {
    meshShardDirection =
        ::tt::target::ttnn::MeshShardDirection::FullToShardShape;
  } else if (shardDirection == mlir::ttcore::MeshShardDirection::ShardToFull) {
    meshShardDirection =
  ...
  if (shardType == mlir::ttcore::MeshShardType::Replicate) {
    meshShardType = ::tt::target::ttnn::MeshShardType::Replicate;
  } else if (shardType == mlir::ttcore::MeshShardType::Devices) {
    meshShardType = ::tt::target::ttnn::MeshShardType::Devices;
  } else if (shardType == mlir::ttcore::MeshShardType::Identity) {

...
createOp(FlatbufferObjectCache &cache, UpsampleOp op) {
  ...
  flatbuffers::Offset<::tt::target::ttnn::TensorRef> output = cache.getOrCreate(
      op.getResult(), tensorValueToFlatbuffer,
      mlir::tt::ttcore::ShardStatus::Unsharded, /*local_shape*/ std::nullopt);
...
createSortOp(FlatbufferObjectCache &cache, SortOp op) {
  ...
  for (auto result : op.getResults()) {
    outputs.push_back(
        cache.getOrCreate(result, tensorValueToFlatbuffer,
                          mlir::tt::ttcore::ShardStatus::Unsharded,
                          /*local_shape*/ std::nullopt));
  }
...
createMaxPool2dWithIndicesOp(FlatbufferObjectCache &cache,
  ...
  auto out = cache.getOrCreate(op.getResult(), tensorValueToFlatbuffer,
                               mlir::tt::ttcore::ShardStatus::Unsharded,
                               /*local_shape*/ std::nullopt);
  auto outIndices = cache.getOrCreate(
      op.getResultIndices(), tensorValueToFlatbuffer,
      mlir::tt::ttcore::ShardStatus::Unsharded, /*local_shape*/ std::nullopt);
...
createOp(FlatbufferObjectCache &cache, BeginTraceCaptureOp op) {
  ...
  auto traceIdTensor = cache.getOrCreate(
      op.getTraceId(), tensorValueToFlatbuffer,
      mlir::tt::ttcore::ShardStatus::Unsharded, /*local_shape*/ std::nullopt);
  uint32_t cqId = op.getCqId();

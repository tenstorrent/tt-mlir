...
// carries an opaque `kernel_artifact` attribute populated by the tt-xla
// plugin's tt-lang Python bridge (see
// pjrt_implementation/src/api/module_builder/tt_lang_bridge.cc). Here we
// translate that JSON payload into the existing `GenericOp` flatbuffer
// schema so the runtime can execute it through the same generic-kernel
// path it already supports for hand-written kernels.
::flatbuffers::Offset<::tt::target::ttnn::GenericOp>
createOp(FlatbufferObjectCache &cache, TtLangOp op) {
  ...
  if (!artifactAttr) {
    op.emitError("ttnn.tt_lang_op is missing the `kernel_artifact` "
                 "attribute; the tt-xla tt-lang bridge must populate it "
                 "before flatbuffer emission.");
    return 0;
  }

  llvm::ArrayRef<int8_t> bytes = artifactAttr.asArrayRef();
  llvm::StringRef artifactJson(reinterpret_cast<const char *>(bytes.data()),
                               bytes.size());
  ...
  //     pool".
  //
  // `ttnn.tt_lang_op` does not (yet) implement
  // `DestinationStyleOpInterface`; instead, the user's @tt_torch.kernel
  // decorator tags each operand "in" or "out" via the `arg_roles`
  // string attribute. We honour that attribute here:
  //
  //   (a) "in"-roled operands populate `ios` first (in operand
  //       declaration order);
  //   (b) "out"-roled operands populate `ios` last (in operand
  //       declaration order), guaranteeing that `io_tensors[n - 1]`
  //       names an "out" buffer;
  //   (c) for each op result we alias the cache entry to the
  //       corresponding "out" operand's TensorRef so downstream
  //       IR (`return %0`, follow-on ops) resolves the result to the
  //       same global_id the runtime updated.
  //
  // When tt-lang grows DestinationStyleOpInterface this whole block
  // collapses to the same `getInputsAndOutputs()` walk the standard
  // GenericOp emitter uses; until then this aliasing logic is what
  // keeps tt-lang ops runnable on hardware.
  llvm::StringRef rolesStr = op.getArgRoles();
  llvm::SmallVector<llvm::StringRef> roles;
  rolesStr.split(roles, ',', /*MaxSplit=*/-1, /*KeepEmpty=*/true);
  for (auto &r : roles) {
    r = r.trim();
    if (r.empty()) {
      op.emitError(
  ...
  if (roles.size() != numOperands) {
    op.emitError("ttnn.tt_lang_op: arg_roles has ")
        << roles.size() << " entries but op has " << numOperands
        << " operands.";
    return 0;
  }

  llvm::SmallVector<unsigned> inIndices;
  llvm::SmallVector<unsigned> outIndices;
  ...
  if (outIndices.size() != op.getResults().size()) {
    op.emitError("ttnn.tt_lang_op: arg_roles declares ")
        << outIndices.size() << " 'out' operand(s) but the op has "
        << op.getResults().size() << " result(s).";
    return 0;
  }
  if (outIndices.empty()) {
    op.emitError(
        "ttnn.tt_lang_op: at least one operand must be tagged 'out' so "
        "the runtime knows which io_tensors entry to write into.");
    return 0;
  }
  ...
  // Map operand declaration index -> position in `ios` (== position in
  // the GenericOp's flatbuffer `io_tensors` array). This is what the
  // runtime sees when it expands ``KernelArgTensorAccessorArgs`` markers
  // back into per-buffer compile-time args, so it must agree with the
  // order in which we push operands below.
  llvm::SmallVector<uint32_t> declToIoIndex(numOperands);
  auto pushOperand = [&](unsigned operandIdx) {
    Value operand = op.getInputs()[operandIdx];
    auto offset = cache.at<::tt::target::ttnn::TensorRef>(
        getOperandThroughDPSOps(operand));
    operandOffsets[operandIdx] = offset;
    declToIoIndex[operandIdx] = static_cast<uint32_t>(ios.size());
    ios.push_back(offset);
  };
  for (unsigned idx : inIndices) {
    pushOperand(idx);
  }
  ...
  for (unsigned resultIdx = 0; resultIdx < op.getResults().size();
       ++resultIdx) {
    Value result = op.getResult(resultIdx);
    cache.insert(result, operandOffsets[outIndices[resultIdx]]);
  }

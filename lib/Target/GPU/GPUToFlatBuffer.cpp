...
#include "ttmlir/Target/GPU/GPUToFlatBuffer.h"
...
rootModule->getAttr(ttcore::SystemDescAttr::name)));
(void)systemDesc; // TODO: Use systemDesc in final program creation

flatbuffers::Offset<::tt::target::DebugInfo> debugInfo =
debugInfoToFlatbuffer(fbb, "gpu", rootModule, goldenMap, moduleCache);
(void)debugInfo; // TODO: Use debugInfo in final program creation
*/
  ::flatbuffers::Offset<gpu::Program> program;
  std::vector<::flatbuffers::Offset<::gpu::Kernel>> kernels;
  std::vector<::flatbuffers::Offset<::gpu::MemRefDesc>> memRefDescs;
  llvm::StringMap<MemRefDesc> memRefDescMap;
  llvm::StringMap<Kernel> kernelMap;
  ...
  rootModule.walk([&](mlir::gpu::LaunchFuncOp launchFuncOp) {
    ...
    auto kernelOffset = ::gpu::CreateKernel(
  ...
  for (const auto &pair : memRefDescMap) {
    ...
    const gpu::MemRefDesc &memRefDesc = pair.second;
    ...
    auto memrefOffset =
        ::gpu::CreateMemRefDesc(fbb, nameOffset, typeOffset, valueOffset);
  ...
  auto finalProgram = ::gpu::CreateProgram(fbb, kernelsVector, memrefsVector,
...
LogicalResult translateGPUToFlatbuffer(Operation *op, llvm::raw_ostream &os) {
  std::shared_ptr<void> data = gpuToFlatbuffer(op);

...
static int32_t getNocElementAlignmentL1(ConcreteOp op,
  ...
  return nocAlignmentL1 / elementBytes;
}
...
public:
  D2MConcatRewriter(const TypeConverter &typeConverter, mlir::MLIRContext *ctx,
                    ...
                    bool /*ttnnMode*/, bool /*collapseTensors*/,
                    bool enableMulticastInference)
      : OpConversionPattern<ttir::ConcatOp>(typeConverter, ctx, /*benefit=*/10),
        D2MNamedRewriterCommon(defaultInputMemSpace, defaultOutputMemSpace,
  ...
  matchAndRewrite(ttir::ConcatOp op, ttir::ConcatOp::Adaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    auto tensorType = mlir::cast<RankedTensorType>(op.getOperand(0).getType());
    ...
    const int32_t alignToElements = getNocElementAlignmentL1(op, tensorType);

    if (dim >= rank - 2) {
      const int nInputs = static_cast<int>(op.getInputs().size());
      for (int i = 0; i < nInputs; i++) {
        auto inputType =
            mlir::cast<RankedTensorType>(op.getInputs()[i].getType());
        const int64_t dimSize = inputType.getShape()[dim];
        ...
        // If we have decided to do a row-major width concat, but at least one
        // (including the last) row size violates the NoC constraints, use the
    ...
    auto compositeView = rewriter.create<d2m::CompositeViewOp>(
        loc, outputs[0].getType(), inputs, dim, /*logicalSizes=*/nullptr);

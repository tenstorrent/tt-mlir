...
public:
  ...
  permuteInnerDims(ttir::PermuteOp op, typename ConcreteOp::Adaptor adaptor,
    ...
    auto stream = d2m::StreamLayoutOp::create(
        rewriter, loc, viewType, inputs[0], permuted.transposeMap, storage);
  ...
  matchAndRewrite(TensorManipulationOp op,
    ...
    auto storage =
        d2m::EmptyOp::create(rewriter, op.getLoc(), outputs[0].getType(),
                             ...
                             /*virtualGridForwardMapping=*/nullptr);
    auto view =
        d2m::StreamLayoutOp::create(rewriter, op.getLoc(), newOutTy, inputs[0],
                                    deviceMap, storage.getResult());

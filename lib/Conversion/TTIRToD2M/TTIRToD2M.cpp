...
"func.func"() <{function_type = (tensor<1x3x320x320xf32>, tensor<1x1xi64>, tensor<1x3x32x32xf32>) -> tensor<1x3x320x320xf32>, sym_name = "test_scatter", sym_visibility = "public"}> ({
^bb0(%arg0: tensor<1x3x320x320xf32>, %arg1: tensor<1x1xi64>, %arg2: tensor<1x3x32x32xf32>):
  %0 = "d2m.empty"() : () -> tensor<1x3x320x320xf32>
  %1 = "d2m.empty"() : () -> tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %2 = "d2m.to_layout"(%arg0, %1) : (tensor<1x3x320x320xf32>, tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>) -> tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %3 = "d2m.empty"() : () -> tensor<1x1x1x1xi64, #ttcore.metal_layout<logical_shape = 1x1, dim_alignments = 1x1, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, dram>>
  %4 = "d2m.to_layout"(%arg1, %3) : (tensor<1x1xi64>, tensor<1x1x1x1xi64, #ttcore.metal_layout<logical_shape = 1x1, dim_alignments = 1x1, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, dram>>) -> tensor<1x1x1x1xi64, #ttcore.metal_layout<logical_shape = 1x1, dim_alignments = 1x1, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, dram>>
  %5 = "d2m.empty"() : () -> tensor<1x1x96x32xf32, #ttcore.metal_layout<logical_shape = 1x3x32x32, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %6 = "d2m.to_layout"(%arg2, %5) : (tensor<1x3x32x32xf32>, tensor<1x1x96x32xf32, #ttcore.metal_layout<logical_shape = 1x3x32x32, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>) -> tensor<1x1x96x32xf32, #ttcore.metal_layout<logical_shape = 1x3x32x32, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %7 = "d2m.empty"() : () -> tensor<1x3x320x320xf32>
  %8 = "d2m.to_layout"(%arg0, %7) : (tensor<1x3x320x320xf32>, tensor<1x3x320x320xf32>) -> tensor<1x3x320x320xf32>
  %9 = "d2m.empty"() : () -> tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %10 = "d2m.to_layout"(%8, %9) : (tensor<1x3x320x320xf32>, tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>) -> tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %11 = "d2m.generic"(%2, %4, %6, %10) <{block_factors = [1, 1], grid = #ttcore.grid<1x1>, indexing_maps = [affine_map<(d0, d1) -> (0, 0)>, affine_map<(d0, d1) -> (0, 0)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (0, 0)>], iterator_types = [#ttcore.iterator_type<parallel>, #ttcore.iterator_type<parallel>], operandSegmentSizes = array<i32: 3, 1>, threads = [#d2m.thread<datamovement>]}> ({
  ^bb0(%arg3: !d2m.cb<tensor<960x320xf32>, riscv_l1>, %arg4: !d2m.cb<tensor<1x1xi64>, riscv_l1>, %arg5: !d2m.cb<tensor<96x32xf32>, riscv_l1>, %arg6: !d2m.cb<tensor<960x320xf32>, riscv_l1>):
    %14 = "d2m.wait"(%arg3) : (!d2m.cb<tensor<960x320xf32>, riscv_l1>) -> tensor<960x320xf32>
    %15 = "d2m.wait"(%arg4) : (!d2m.cb<tensor<1x1xi64>, riscv_l1>) -> tensor<1x1xi64>
    %16 = "d2m.wait"(%arg5) : (!d2m.cb<tensor<96x32xf32>, riscv_l1>) -> tensor<96x32xf32>
    %17 = "d2m.reserve"(%arg6) : (!d2m.cb<tensor<960x320xf32>, riscv_l1>) -> tensor<960x320xf32>
      ...
      ^bb0(%arg8: index):
        ...
        ^bb0(%arg9: index):
          ...
          ^bb0(%arg10: index):
            ...
            %19 = "tensor.dim"(%15, %18) : (tensor<1x1xi64>, index) -> index
            %20 = "tensor.extract_slice"(%15, %arg7, %19) <{operandSegmentSizes = array<i32: 1, 1, 1, 0>, static_offsets = array<i64: -9223372036854775808, 0>, static_sizes = array<i64: 1, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (tensor<1x1xi64>, index, index) -> tensor<1x?xi64>
            ...
            %26 = "tensor.extract"(%20, %25) : (tensor<1x?xi64>, index) -> i64
            ...
            %36 = "arith.addi"(%26, %31) <{overflowFlags = #arith.overflow<none>}> : (i64, index) -> index
            %37 = "arith.addi"(%36, %35) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %38 = "arith.addi"(%22, %32) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %39 = "arith.addi"(%38, %arg8) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %40 = "arith.addi"(%23, %33) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %41 = "arith.addi"(%40, %arg9) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %42 = "arith.addi"(%24, %34) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %43 = "arith.addi"(%42, %arg10) <{overflowFlags = #arith.overflow<none>}> : (index, index) -> index
            %44 = "d2m.dma"(%16, %arg7, %arg8, %arg9, %arg10, %17, %37, %39, %41, %43) <{operandSegmentSizes = array<i32: 1, 4, 1, 4, 0, 0>}> : (tensor<96x32xf32>, index, index, index, index, tensor<960x320xf32>, index, index, index, index) -> !d2m.mem_tx
            "d2m.dma_wait"(%44) : (!d2m.mem_tx) -> ()
            "affine.yield"() : () -> ()
          }) : () -> ()
          "affine.yield"() : () -> ()
        }) : () -> ()
  ...
  }) : (tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>, tensor<1x1x1x1xi64, #ttcore.metal_layout<logical_shape = 1x1, dim_alignments = 1x1, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, dram>>, tensor<1x1x96x32xf32, #ttcore.metal_layout<logical_shape = 1x3x32x32, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>, tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>) -> tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>
  %12 = "d2m.empty"() : () -> tensor<1x3x320x320xf32>
  %13 = "d2m.to_layout"(%11, %12) : (tensor<1x1x960x320xf32, #ttcore.metal_layout<logical_shape = 1x3x320x320, dim_alignments = 1x1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, dram>>, tensor<1x3x320x320xf32>) -> tensor<1x3x320x320xf32>
  "func.return"(%13) : (tensor<1x3x320x320xf32>) -> ()
*/
...
We first insert necessary layout conversions for the input operand, scatter_indices, updates and output tensor.
All these will be collapsed into 2D tensors and live in dram memory space.

Then we create the d2m.generic op
This will be 1x1 grid
Operand, scatter_indices, updates and the output tensor will all be inputs into the d2m.generic op.
However, the only input that participates in the iteration space is the update tensor.
Thus, we will "broadcast" the operand, scatter_indices and output across the update tensor iteration space.

Next, we want to fill the body of the d2m.generic op.
We will use the algorithm found on stablehlo operations page: https://openxla.org/stablehlo/spec#scatter
The general idea is to iterate over the update tensor space, and for each index in the update tensor, we map
the update index to some output index.
Then we copy the value found at that update index to the output index.
...
class D2MScatterOpRewriter : public OpConversionPattern<ttir::ScatterOp> {
  ...
  matchAndRewrite(ttir::ScatterOp scatterOp, OpAdaptor adaptor,
    ...
    ArrayRef<int32_t> scatterIndicesBatchingDims = scatterOp.getScatterIndicesBatchingDims();
    ArrayRef<int32_t> scatterDimsToOperandDims = scatterOp.getScatterDimsToOperandDims();
    ...
    auto inputLayoutOp = builder.create<d2m::ToLayoutOp>(loc, input, inputEmpty, nullptr);
    ...
    auto scatterIndicesLayoutOp = builder.create<d2m::ToLayoutOp>(loc, scatterIndices, scatterIndicesEmpty, nullptr);
    ...
    auto updatesLayoutOp = builder.create<d2m::ToLayoutOp>(loc, updates, updatesEmpty, nullptr);
    ...
    auto copiedInputLayoutOp = builder.create<d2m::ToLayoutOp>(loc, input, copiedInputEmpty, nullptr);
    ...
    auto outputLayoutOp = builder.create<d2m::ToLayoutOp>(loc, copiedInputLayoutOp.getResult(0), outputEmpty, nullptr);
    ...
    // Operand does not participate in iteration space of generic op, so we "broadcast" it
    ...
    // Scatter indices does not participate in iteration space of generic op, so we "broadcast" it
    // affine_map<(d0, d1) -> (0, 0)>
    ...
    // Update participates in iteration space of generic op, and its the only one, so it receives all the iterators
    ...
    // Output does not participate in iteration space of generic op, so we "broadcast" it
    // affine_map<(d0, d1) -> (0, 0)>
    ...
    auto finalOutputToLayoutOp = builder.create<d2m::ToLayoutOp>(loc, genericOp.getResult(0), finalOutput, nullptr);

    // 4) Fill in the body of d2m.generic op
    // During this stage, we will develop the affine loops to iterate across the update tensor
    // For each element in the update tensor, we will calculate the corresponding index in the output tensor
    // We will then update the output tensor at that index with the value from the update tensor
    Region &region = genericOp.getRegion(0);
    Block &block = region.front();
    OpBuilder::InsertionGuard guard(rewriter); 
    builder.setInsertionPointToEnd(&block);
    ...
    auto scatterIndicesWaitOp = builder.create<d2m::WaitOp>(loc, scatterIndicesBlockArgument);
    auto updateWaitOp = builder.create<d2m::WaitOp>(loc, updateBlockArgument);
    auto outputReserveOp = builder.create<d2m::ReserveOp>(loc, outputBlockArgument);
    ...
    */
    ...
    /*
    We need to check the bounds of result_index before performing the scatter update, this is currently todo
    if is_index_in_bounds(output, result_index):
        output[tuple(result_index)] = updates[i, j, k, a, b] + output[tuple(result_index)]
    */

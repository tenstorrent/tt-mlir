...
//  write out chunk to next device in the correct place in the output directly
//  and increment semaphore

// concerns: local input forwarding could intheory be parallelized with
// forwarding but that requires actually having addition connections/BW to
// benefit actually it's not just local input that would benefit from
// parallelized forwarding each chunk in above approach would have to be
// completely received wherease ideally we would like to the following in
// parallel

// thread 1: start sending local input to next device
// thread 2: take what we previously received and forward it to next device
//  aside form above conversation, this also requires a CB for forwrading to
//  happen correctly in parallel
//    a) or at least a semaphore incrementing to tell us we need to forward
//    another bit of data and in that case we would be forwarding directly to
//    the output b) or we could have a CB for forwarding and then we could
//    forward to and do correct remote cb ops for streaming remotely but then we
//    also have to locally store (with a noc remote store) the chunk we recive
//    before we forward
// thread 3 (if we do 2b): take incoming data that came from previous device and
// store in output tensor

// 2b is the best option imo since we can forward large chunks regardless of how
// we have sharded our output and noc remote store can just handle the coalesced
// write and we also get a lot of parallelization so each chunk of data will
// only take as much time as needed to complexity is we need 3 threads, two
// fabric ones, one local one

// getting two fabric threads is not possible right now, bt we should still have
// the remote streaming mechanism and also have a separate thread for doing
// local store? so we can move large chunks of data through fabric?

// method 2b also wold have a very similar to ideal RS implementation perhaps,
// except we have even one more thread that is loading locally the device local
// input chunk to add with and then compute wait on both chunks, adds them, and
// writes out data to next device (remote forwrad streaming)
...
//
// wait for semaphore inc
// load correct chunk in local input and add with chunk in scratch buffer and
// store output in a a cb or directly in output store result back in scratch or
// in output, then send it to next device and increment semaphore
...
class D2MReduceScatterRingForwardingAlgorithmRewriter
...
public:
  ...
  Value createEmptyOp(mlir::ConversionPatternRewriter &rewriter,
                      mlir::Location loc, Type originalInputType,
                      llvm::SmallVector<int64_t> workerGridShape) const {
    auto tensorType = mlir::cast<mlir::RankedTensorType>(originalInputType);
    ArrayRef<int64_t> logicalShape = tensorType.getShape();
    ...
    auto emptyOp = rewriter.create<d2m::EmptyOp>(loc, deviceShape,
                                                 tiledElementType, layout);
  ...
  // rewriter, inputRank, op.getScatterDim(),
  // inputPhysicalShape[op.getScatterDim()] / num_devices get affine map to
  // translate unit grid to unitgrid+grid dim for device/shard idx need a view
  // where first dim is device idx, and later dims are normal worker core grid
  // have a normal operand grid
  // fix!!!: not needed??? is below function createDeviceShardedViewLayoutOp
  // enough???
  Value extendedRankView(mlir::ConversionPatternRewriter &rewriter,
                         mlir::Location loc, Value tensor) const {
    ...
    auto newTensorShape =
        layout.getDeviceShape(newGrid, ttcore::TileType::getDefaultShape());

    auto newTensorType = RankedTensorType::get(
        newTensorShape, tensorType.getElementType(), tensorType.getEncoding());

    auto unitReblockingView =
        rewriter.create<d2m::ViewLayoutOp>(loc, newTensorType, tensor, mapping,
                                           /*reinterpretLayout=*/false);

    return unitReblockingView.getResult();
  }

  // TODO: see if this can be simplified/commonized with
  // projectLogicalMapToUnitDeviceSpace
  Value createDeviceShardedViewLayoutOp(
      mlir::ConversionPatternRewriter &rewriter, mlir::Location loc,
      Value tensor, uint32_t dimToShard, uint32_t num_devices) const {
    auto tensorType = mlir::cast<mlir::RankedTensorType>(tensor.getType());
    auto layout = mlir::cast<ttcore::MetalLayoutAttr>(tensorType.getEncoding());
    llvm::SmallVector<int64_t> physicalShape =
        layout.getPhysicalShape(ttcore::TileType::getDefaultShape());
    uint32_t logicalRank = physicalShape.size();
    uint32_t extendedRank = logicalRank + 1;
    auto deviceShardOffset = physicalShape[dimToShard] / num_devices;

    // Create affine map to translate input indices to output indices in the
    // following manner: (num_devices, 1, 1, 1, y, x) -> (1, 1, Y, X)
    ...
    for (uint32_t i = 0; i < logicalRank; i++) {
      auto currentIndex =
          mlir::getAffineDimExpr(extendedRank + (i + 1), rewriter.getContext());
      if (i == dimToShard) {
    ...
    auto deviceShardIndexingMap = mlir::AffineMap::get(
        2 * extendedRank, 0, results, rewriter.getContext());

    // create a map transform indices on unit grid to indices on full tensor
    // grid
    llvm::SmallVector<int64_t> unitGrid(logicalRank, 1);
    auto [_, unitGridReblockMap] = ttmlir::utils::calculateReblockMapForGrid(
        tensorType.getShape(), unitGrid, tensorType.getContext());

    // compose the two maps to get the full map
    // input -> deviceShardIndexingMap -> reblockMap -> output
    auto reblockMap = unitGridReblockMap.compose(deviceShardIndexingMap);

    // new device shape is same as old shape with an extra dim for both grid and
    // shard grid shape[0] = num_devices shard shape[0] = 1 shard
    // shape[dimToShard] = original shard shape[dimToShard] / num_devices get
    // device shape on unit grid
    auto newTensorShape =
        layout.getDeviceShape(llvm::SmallVector<int64_t>(logicalRank, 1),
                              ttcore::TileType::getDefaultShape());
    // then adjust shard idx[dimToShard] to be divided by num_devices
    newTensorShape[logicalRank + dimToShard] /= num_devices;
    // then add a new dim for the device idx
    newTensorShape.insert(newTensorShape.begin() + logicalRank, 1);
  ...
  // TODO: it would be good to always add a reverse view on the output actually
  // (i.e. port this logic to grid selection if it doesn't already exist)
  Value createInverseDeviceShardedViewLayoutOp(
      mlir::ConversionPatternRewriter &rewriter, mlir::Location loc,
      Value tensor, uint32_t dimToShard, uint32_t num_devices,
      ArrayRef<int64_t> targetGrid) const {
    auto tensorType = mlir::cast<mlir::RankedTensorType>(tensor.getType());
    auto layout = mlir::cast<ttcore::MetalLayoutAttr>(tensorType.getEncoding());
    llvm::SmallVector<int64_t> physicalShape =
        layout.getPhysicalShape(ttcore::TileType::getDefaultShape());
    ...
    // create a view that projects from final grid onto unit grid, then expands
    // to higher rank to include device idx
    llvm::SmallVector<int64_t> unitGrid(logicalRank, 1);
    auto [_, gridReblockMap] = ttmlir::utils::calculateReblockMapForGrid(
        layout.getDeviceShape(unitGrid, ttcore::TileType::getDefaultShape()),
        targetGrid, tensorType.getContext());

    // Create affine map to translate input indices to output indices in the
    // following manner: (1, 1, Y, X) -> (num_devices, 1, 1, 1, y, x)
    auto deviceShardOffset = physicalShape[dimToShard] / num_devices;
    mlir::SmallVector<mlir::AffineExpr> results;
    // device index * device shard size + shard index
    auto deviceShardOffsetExpr =
        mlir::getAffineConstantExpr(deviceShardOffset, rewriter.getContext());
    auto deviceIndex =
        mlir::getAffineDimExpr(logicalRank + dimToShard, rewriter.getContext())
            .floorDiv(deviceShardOffsetExpr);
    ...
    auto inverseDeviceShardIndexingMap = mlir::AffineMap::get(
        2 * logicalRank, 0, results, rewriter.getContext());

    // input indices -> reblockMap -> inverseDeviceShardIndexingMap -> output
    auto reblockMap = inverseDeviceShardIndexingMap.compose(gridReblockMap);

    auto targetDeviceShape =
        layout.getDeviceShape(targetGrid, ttcore::TileType::getDefaultShape());
    auto newTensorType =
        RankedTensorType::get(targetDeviceShape, tensorType.getElementType(),
                              tensorType.getEncoding());
    auto unitReblockingView = rewriter.create<d2m::ViewLayoutOp>(
        loc, newTensorType, tensor, reblockMap,
        /*reinterpretLayout=*/false);
  ...
  uint32_t getWorkerCoreSplitDim(Value input, uint32_t num_cores,
                                 uint32_t excludeDim = 0) const {
    auto inputTensorType = mlir::cast<mlir::RankedTensorType>(input.getType());
    auto inputLayout =
        mlir::cast<ttcore::MetalLayoutAttr>(inputTensorType.getEncoding());
    auto inputPhysicalShape =
        inputLayout.getPhysicalShape(ttcore::TileType::getDefaultShape());
  ...
  LogicalResult
  matchAndRewrite(ttir::ReduceScatterOp op,
                  ttir::ReduceScatterOp::Adaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ...
    int num_cores = 1; // fix once added logic for ring direction
                       // topology == ttcore::Topology::Ring ? num_links * 2 :
                       // num_links * 1;
    ...
    auto input = rewriter
                     .create<d2m::ToLayoutOp>(
                         origInputs[0].getLoc(), origInputs[0],
                         createEmptyOp(rewriter, loc, origInputs[0].getType(),
                                       workerGridShape))
                     ->getResult(0);
    auto scratchInput =
        createEmptyOp(rewriter, loc, origInputs[0].getType(), workerGridShape);
    auto output = rewriter
                      .create<d2m::ToLayoutOp>(
                          origOutputs[0].getLoc(), origOutputs[0],
                          createEmptyOp(rewriter, loc, origOutputs[0].getType(),
                                        workerGridShape))
                      ->getResult(0);
    auto inputDeviceShardedView = createDeviceShardedViewLayoutOp(
        rewriter, loc, input, op.getScatterDim(), num_devices);
    auto scratchInputDeviceShardedView = createDeviceShardedViewLayoutOp(
        rewriter, loc, scratchInput, op.getScatterDim(), num_devices);
    auto outputDeviceShardedView = createDeviceShardedViewLayoutOp(
        rewriter, loc, output, op.getScatterDim(), 1);
    ...
    uint32_t workerCoreSplitDim =
        getWorkerCoreSplitDim(inputDeviceShardedView, num_cores, 0);
    // inputRank is the logical rank of the original tensor
    uint32_t inputRank =
        mlir::cast<mlir::RankedTensorType>(origInputs[0].getType())
            .getShape()
            .size();
    // gridRank is the grid rank from the device-sharded view (half of the
    // device shape rank)
    auto inputDeviceShardedViewType =
        mlir::cast<mlir::RankedTensorType>(inputDeviceShardedView.getType());
    ...
    Value inputStreamResult =
        rewriter
            .create<d2m::ViewLayoutOp>(op.getLoc(), inputStreamTensorType,
                                       inputDeviceShardedView,
                                       inputStreamReblockMap)
            ->getResult(0);
    Value scratchInputStreamResult =
        rewriter
            .create<d2m::ViewLayoutOp>(op.getLoc(), inputStreamTensorType,
                                       scratchInputDeviceShardedView,
                                       inputStreamReblockMap)
            ->getResult(0);

    auto outputViewTensorType =
        mlir::cast<mlir::RankedTensorType>(outputDeviceShardedView.getType());
    auto outputStreamTensorType =
        d2m::utils::reblockShapedType(outputViewTensorType, outputViewGrid);
    auto outputStreamReblockMap = ttmlir::utils::calculateReblockMap(
        outputViewTensorType.getShape(), outputStreamTensorType.getShape(),
        rewriter.getContext());
    Value outputStreamResult =
        rewriter
            .create<d2m::ViewLayoutOp>(op.getLoc(), outputStreamTensorType,
                                       outputDeviceShardedView,
                                       outputStreamReblockMap)
            ->getResult(0);
    ...
    auto generic = rewriter.create<d2m::GenericOp>(
        loc, TypeRange(outputStreamResult),
        ValueRange({inputStreamResult, scratchInputStreamResult}),
        ValueRange({outputStreamResult}),
        ValueRange({startSemaphore, iterationSemaphore}), genericGrid,
      ...
      // Populate 'block'.
      {
        ...
        for (uint32_t dim = 0; dim < meshShape.size(); dim++) {
          if (dim == clusterAxis) {
            ...
            nextDevice.push_back(rewriter.create<arith::RemSIOp>(
                loc,
                rewriter.create<arith::AddIOp>(
                    loc, rewriter.create<d2m::MeshPositionOp>(loc, dim),
                    rewriter.create<arith::ConstantIndexOp>(loc, 1)),
                rewriter.create<arith::ConstantIndexOp>(loc, num_devices)));
            // subtract 1 from mesh position for previous device and mod by
            // num_devices
            previousDevice.push_back(rewriter.create<arith::RemSIOp>(
                loc,
        ...
        // Synchronize with previous device since that is our only sender in
        // ring forwarding algorithm
        rewriter.create<d2m::DeviceSynchronizeOp>(
            loc, startSemaphore, previousDevice, ValueRange(), 1, coreIndices);

        auto meshPosition =
            rewriter.create<d2m::MeshPositionOp>(loc, clusterAxis);
        SmallVector<Value> beginIterationIndices = coreIndices;
        beginIterationIndices.insert(beginIterationIndices.begin(),
                                     meshPosition);
        ...
        // fix!!!
        {
          auto blockArgsVec = createBlockArguments(
              ...
              TypeRange({}), generic, {beginIterationIndices}, mcastGridDims);
          Value inputLocalBuffer = blockArgsVec[0];

          // initial load/store from input to scratch tensor
          auto inputLocalBufferLoadResult =
              rewriter
                  .create<d2m::RemoteLoadOp>(
                      loc, inputLocalBuffer.getType(), inputLocalBuffer,
          ...
          rewriter.create<d2m::RemoteStoreOp>(
              ...
              beginIterationIndices, inputLocalBufferLoadResult, nextDevice,
              ValueRange(), iterationSemaphore, coreIndices);
        }
        ...
        //   remote_store from buf on next device into scratch input with sem
        //   inc remote_load local_buf input[indices] (order probably doesn’t
        //   matter here if scheduled on separate threads) wait for semaphore
        //   remote_load local_buf scratch_input[indices]
        //   add tensor 0, tensor 1 to get tensor 2 and store in buf?
        // endfor
        auto forLoop =
            rewriter.create<mlir::affine::AffineForOp>(loc, 1, num_devices, 1);
        {
          OpBuilder::InsertionGuard guard(rewriter);
          rewriter.setInsertionPointToStart(forLoop.getBody());

          // iterate over the shards starting from the one corresposing to our
          // mesh position and going backwards (partially reducing our local
          // shard with the shard coming from previous device in each iteration)
          // until we have iterated over all of them
          SmallVector<Value> iterationIndices = coreIndices;
          Value iterationIndex = rewriter.create<arith::RemSIOp>(
              loc,
              rewriter.create<arith::SubIOp>(
                  loc,
                  rewriter.create<arith::AddIOp>(
          ...
          auto blockArgsVec = createBlockArguments(
              rewriter, block, loc,
              TypeRange({inputStreamResult, scratchInputStreamResult}),
              TypeRange({outputStreamResult}), generic,
              {beginIterationIndices, beginIterationIndices}, mcastGridDims);
          Value inputLocalBuffer = blockArgsVec[0];
          ...
          auto inputLocalBufferLoadResult =
              rewriter
                  ...
                  .getResult();
          rewriter.create<d2m::SemaphoreWaitOp>(loc, iterationSemaphore,
                                                iteration_count);
          auto scratchLocalBufferLoadResult =
              rewriter
                  .create<d2m::RemoteLoadOp>(
                      loc, scratchLocalBuffer.getType(), scratchLocalBuffer,
          ...
          SmallVector<mlir::AffineMap> linalgIndexingMaps =
              getIdentityAffineMapsArray(rewriter, numInputs + numOutputs,
                                         gridRank);
          SmallVector<mlir::utils::IteratorType> linalgIteratorTypes(
              gridRank, mlir::utils::IteratorType::parallel);
          auto linalgGeneric = rewriter.create<mlir::linalg::GenericOp>(
              loc,
              /* result tensor types */
              outputLocalBuffer.getType(),
              /* inputs */
              ValueRange{inputLocalBufferLoadResult,
                         scratchLocalBufferLoadResult},
          ...
          rewriter.create<d2m::RemoteStoreOp>(
              loc, scratchInputStreamResult.getType(), scratchInputStreamResult,
              iterationIndices, linalgGeneric->getResult(0), nextDevice,
              ValueRange(), iterationSemaphore, coreIndices);
        } // end for loop
        ...
        outputIndices.insert(outputIndices.begin(),
    ...
    auto outputLayout =
        mlir::cast<ttcore::MetalLayoutAttr>(outputTensorType.getEncoding());
    // TODO: make getGridShape a statuc function in DeviceLayoutInterface so we
    // don't have to create a layout unnecessarily
    auto targetGrid = outputLayout.getGridShape(outputTensorType);

    auto inverseDeviceShardView = createInverseDeviceShardedViewLayoutOp(
        rewriter, loc, generic->getResult(0), op.getScatterDim(), num_devices,
        targetGrid);

...
mlir::Value decomposeGroupNorm(mlir::TypedValue<mlir::RankedTensorType> input,
  ...
  // Reshape [N, 1, S, C] -> [N, S, G, Cpg] so per-group reductions run over the
  // contiguous (S, Cpg) sub-tensor (dims 1 and 3).
  SmallVector<int64_t> groupedShape = {N, S, G, Cpg};
  mlir::Value grouped = generateReshape(input, groupedShape, rewriter,
  ...
  mlir::Value result =
      generateReshape(mlir::cast<mlir::TypedValue<RankedTensorType>>(

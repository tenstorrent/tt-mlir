...
createSparseMatmulProgramConfigAttr(MLIRContext *ctx, RankedTensorType aType,
  ...
  int64_t mTiles = std::max(static_cast<int64_t>(1), (M + tileSize - 1) / tileSize);
  int64_t kTiles = std::max(static_cast<int64_t>(1), (K + tileSize - 1) / tileSize);
  int64_t nTiles = std::max(static_cast<int64_t>(1), (N + tileSize - 1) / tileSize);

  // ---- Core grid selection ------------------------------------------------
  // For 1D multicast the total core count is coreX * coreY and N tiles are
  // spread evenly across all cores.  We enumerate (x,y) pairs that fit the
  // device and pick the one whose product is the largest factor of nTiles.
  int64_t bestCores = 1;
  int64_t bestX = 1;
  int64_t bestY = 1;
  ...
  int64_t in0BlockW = largestFactorAtMost(kTiles, std::min(kTiles, static_cast<int64_t>(32)));

  // The sparse matmul kernel in tt-metal expects out_block and out_subblock
  // sizes of 1.  Matching the hand-tuned tt-metal GPT-OSS expert configs
  // (experts/config.py) which also use 1 for all four parameters.

      ...
      if (adaptor.getShardDirection() ==
        ...
        if (auto blockArg = mlir::dyn_cast<BlockArgument>(input)) {
          ...
          if (auto funcOp = mlir::dyn_cast<func::FuncOp>(parentOp)) {
            ...
            // Get the global type from the input (before conversion)
            Type globalType = input.getType();

            // Get the output type (converted)
            Type outputType =
                this->getTypeConverter()->convertType(op.getType());
            if (outputType && globalType) {
              ...
              rewriter.modifyOpInPlace(
                  funcOp, [&funcOp, &modifiedFuncType, argIndex, &outputType,
                           &globalType, &addMeshShardGlobalShapeAttr]() {
                    ...
                    // Add global type attribute to mark this argument as
                    // modified by mesh_shard
                    addMeshShardGlobalShapeAttr(funcOp, argIndex, globalType);
                  });
            }
          }
        }
          ...
          if (auto funcOp = mlir::dyn_cast<func::FuncOp>(parentOp)) {
            ...
            if (inputType && globalType) {
              ...
              rewriter.modifyOpInPlace(funcOp, [&funcOp, &modifiedFuncType,
                ...
                funcOp.setType(modifiedFuncType);
                // Store global type as result attribute

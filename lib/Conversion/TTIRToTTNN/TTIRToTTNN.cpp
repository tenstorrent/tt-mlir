...
public:
  ...
  matchAndRewrite(ttir::SelectiveReduceCombineOp op, OpAdaptor adaptor,
    ...
    // must be re-materialized fresh every forward / decode step: a buffer reused
    // across steps keeps the previous step's writes in its unwritten slots and
    // decode output diverges per batch. Mark it "ttnn.non_cacheable" so the
    // const-eval pass does NOT hoist+cache it (a cached buffer is persistent and
    // shared across steps) — instead it stays inline in the forward func and the
    // runtime allocates it dynamically per execution, with tight
    // allocate->write->free liveness (no static-plan fragmentation; only the
    // combine buffers are inlined, all other ttnn.full constants stay cached).
    // The unique id additionally keeps the per-layer inline full ops distinct so
    // CSE cannot merge them back into a single shared buffer.

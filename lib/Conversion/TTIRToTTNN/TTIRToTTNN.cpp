...
public:
  ...
  matchAndRewrite(ttir::RMSNormOp op, OpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<ttnn::RMSNormOp>(
        ...
        adaptor.getResidual(), adaptor.getEpsilon(), /*memoryConfig*/ nullptr,
...
namespace {
class GatedActivationOpConversionPattern
    : public OpConversionPattern<ttir::GatedActivationOp> {
public:
  using OpConversionPattern<ttir::GatedActivationOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ttir::GatedActivationOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<ttnn::GatedActivationOp>(
        op, this->getTypeConverter()->convertType(op.getType()),
        adaptor.getInput(), adaptor.getActivationAttr(), adaptor.getDimAttr(),
        /*memory_config=*/nullptr);
    return success();
  }
};
...
namespace {
class SliceReshapeOpConversionPattern
    : public OpConversionPattern<ttir::SliceReshapeOp> {
public:
  using OpConversionPattern<ttir::SliceReshapeOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(ttir::SliceReshapeOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<ttnn::SliceReshapeOp>(
        op, this->getTypeConverter()->convertType(op.getType()),
        adaptor.getInput(), adaptor.getBeginsAttr(), adaptor.getEndsAttr(),
        adaptor.getStepAttr(), adaptor.getShapeAttr(),
        /*memory_config=*/nullptr);
    return success();
  }
};
...
void populateTTIRToTTNNPatterns(MLIRContext *ctx, RewritePatternSet &patterns,
  ...
  patterns
      .add<TensorEmptyConversionPattern,
           ...
           GatedActivationOpConversionPattern,
           SliceReshapeOpConversionPattern,

...
static void setInsertionPointAfterOperands(OpBuilder &rewriter,
  ...
  Block *targetBlock = rewriter.getInsertionBlock();
  ...
  for (Value operand : operands) {
    ...
    // Only consider operations in the same block as the current insertion
    // point. Operations in different blocks (e.g., from scf.if regions) cannot
    // be compared with isBeforeInBlock.
    if (definingOp->getBlock() != targetBlock) {
      continue;
    }
    if (!latestDefOp || (latestDefOp->getBlock() == definingOp->getBlock() &&
  ...
  if (!latestDefOp) {
    // No defining ops in the current block; keep current insertion point.
    return;
  }

...
llvm::Expected<bool> MeshSharding::parseGSPMDDevicesStr(StringRef devicesStr) {
  ...
  // devciesStr can be either (1) '<=[` IotaReshapeDimensions `]` [`T`
  // IotaTransposeDimensions]) or (2) TileAssignmentDevices
  ...
  auto firstClosingBracket = devicesStr.find(']');
  auto axesStr = devicesStr.substr(0, firstClosingBracket + 1);
  ...
  if (reshapeDevicesStrParsing) {
    ...
    auto [unused, restStr] = devicesStr.split("]");
    // Parse devices string after "]" e.g., 0,1,2,3,4,5,6,7.
    if (!parseDimsFromDimensionStr(restStr, deviceIds, false)) {

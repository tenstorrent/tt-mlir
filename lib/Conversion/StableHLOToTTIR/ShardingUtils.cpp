...
bool MeshSharding::determineMeshShardOpCreationAndShardType(
    bool foundSharding) {
  assert(shardType == mlir::ttcore::MeshShardType::Replicate ||
         shardType == mlir::ttcore::MeshShardType::Devices);
  ...
  // identical.
  if (foundSharding && shardType == mlir::ttcore::MeshShardType::Replicate) {
    return false;
  }
...
llvm::Expected<bool> MeshSharding::convertSdyShardingToMeshSharding(
  ...
  if (meshAttr.getAxes().empty()) {
    if (meshAttr.getDeviceIds().empty()) {
      // replicated
      setNonDevicesShardType(mlir::ttcore::MeshShardType::Replicate);
    } else {
      // maximal
      setNonDevicesShardType(mlir::ttcore::MeshShardType::Maximal);
...
MeshSharding::getTensorMeshShardingAttr(mlir::PatternRewriter &rewriter) {
  ...
  llvm::SmallVector<mlir::ttcore::TensorMeshShardingAxisAttr>
      tensorMeshShardingAxisAttr;

  // Only devices has sharding info, so rest shard_types return empty
  // TensorMeshShardingAxis.
  if (shardType != mlir::ttcore::MeshShardType::Devices) {
    return mlir::ttcore::TensorMeshShardingAttr::get(
  ...
  for (auto [shape, axes] : llvm::zip_equal(shardShape, tensorAxes)) {
    tensorMeshShardingAxisAttr.push_back(
        mlir::ttcore::TensorMeshShardingAxisAttr::get(context, shape, axes));
  }

  return mlir::ttcore::TensorMeshShardingAttr::get(context, meshNameStrAttr,
                                                   tensorMeshShardingAxisAttr);
}
...
MeshSharding::fillStrategyMapFromSharding(
  ...
  mlir::ttcore::MeshShardType meshType = meshSharding.getShardType();
  if (meshType == mlir::ttcore::MeshShardType::Replicate) {
  ...
  } else if (meshType == mlir::ttcore::MeshShardType::Devices) {
  ...
  } else if (meshType == mlir::ttcore::MeshShardType::Identity) {
    strategy["strategy"] = "identity";
  } else {
    return mlir::failure();
  }

...
llvm::Expected<bool>
MeshSharding::parseGSPMDDevicesStr(const StringRef devicesStr) {
  ...
  if (!parseDimsFromDimensionStr(reshapeStr[0], meshShape)) {
    return llvm::createStringError(
        "Fail to parse GSPMD devices reshape string: " + reshapeStr[0].str());
  }
  return true;
}

// Based on current MeshSharding info, finalize sharding dimensions.
llvm::Expected<bool> MeshSharding::determineGSPMDShardingDims(void) {
  ...
  if (!reverseOrder) {
    if (isLineDeviceConfig) {
      ...
      SmallVector<int64_t> extractedMeshShape(orgShardShape.size(), 0);
      auto *it = llvm::copy_if(orgShardShape, extractedMeshShape.begin(),
                               [](int64_t s) { return !(s == int64_t{1}); });
      extractedMeshShape.resize(std::distance(extractedMeshShape.begin(), it));
      meshShape = extractedMeshShape;
    }
  }
  ...
  for (uint64_t i = 0; i < shardShape.size(); ++i) {
    ...
    if (shardShape[i] != 1) {
      ...
      // non-negative shardShape[i] and meshShape[shardDimIdx] is supposed to be
      // identical.
      if (shardShape[i] > 0 && shardShape[i] != meshShape[shardDimIdx]) {
        return llvm::createStringError(
            "Fail to determine shard_dims. shardShape[%d] (%d) != "
            "meshShape[%d] (%d)",
...
MeshSharding::convertGSPMDShardingToMeshSharding(StringRef shardingStr) {
  ...
  for (auto str : shardingStrTokens) {
    if (str.contains("manual")) {
      ...
      return llvm::createStringError("Unknown GSPMD sharding: " + str.str());
    }
  }
...
llvm::Expected<bool> MeshSharding::convertSdyShardingToMeshSharding(
  ...
  if (meshAttr.getAxes().empty()) {
    if (meshAttr.getDeviceIds().empty()) {
      ...
      deviceIds = llvm::SmallVector<int64_t>(meshAttr.getDeviceIds().begin(),
                                             meshAttr.getDeviceIds().end());
    }
    return true;
  }
  ...
  for (auto [dimIdx, dimSharding] :
    ...
    for (auto [axisIdx, axis] : llvm::enumerate(dimSharding.getAxes())) {
      if (axisIdx >= 1) {
        return llvm::createStringError(
            "Sharding one dimension with multiple hardware mesh dimensions is "
            "not supported.");
      }
      shardShape[dimIdx] *= axis.getSize(meshAttr);
      // Sharding makes sense when it is higher than 1.
      if (axis.getSize(meshAttr) > 1) {
        shardDims[axisPosition[axis.getName()]] = dimIdx;
      }
    }
  }

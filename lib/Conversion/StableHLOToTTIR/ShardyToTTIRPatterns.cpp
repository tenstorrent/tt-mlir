...
public:
  ...
  matchAndRewrite(mlir::sdy::ManualComputationOp srcOp,
    ...
    for (auto [globalOperand, argSharding, localArgType] : llvm::zip_equal(
      ...
      mlir::tensor::EmptyOp outputTensor =
          rewriter.create<mlir::tensor::EmptyOp>(loc, outputType.getShape(),
      ...
      auto meshShardOp = rewriter.create<mlir::tt::ttir::MeshShardOp>(
          loc, outputType, globalOperand, outputTensor,
    ...
    for (auto [returnOperand, outSharding, opResult] : llvm::zip_equal(
      ...
      auto meshShardOp = rewriter.create<mlir::tt::ttir::MeshShardOp>(
          loc, outputType, returnOperand.get(), outputTensor,
          meshSharding.getShardType(), meshSharding.getShardDirection(),
          meshSharding.getShardShape(), meshSharding.getShardDims());

      opResult.replaceAllUsesWith(meshShardOp.getResult());
    }
...
public:
  ...
  matchAndRewrite(mlir::sdy::MeshOp srcOp, mlir::sdy::MeshOp::Adaptor adaptor,
    ...
    mlir::tt::utils::addMeshToModuleAttribute(getContext(), module, meshName,
                                              meshShape);
    rewriter.eraseOp(srcOp);
    return llvm::success();
  }

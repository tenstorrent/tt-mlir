      ...
      for (auto arg : srcOp->getOperands()) {
        ...
        if (auto argTensorMeshShardingAttr =
          ...
          if (argTensorMeshShardingAttr != tensorMeshShardingAttr) {
            llvm_unreachable("Inconsistency found in TensorMeshShardingAttr : "
                             "expected and current are different.");
          }
          continue;
        }
...
public:
  ...
  void runOnOperation() final {
    ...
    for (auto funcOp : moduleOp.getOps<mlir::func::FuncOp>()) {
      ...
      funcOp->walk([&](mlir::Operation *op) {
        // ReturnOp is the root of this analysis and thus shouldn't be touched.
        // MeshShardOps are ones that convert single to multi device tensor or
        // vice versa, so do not need to be analyzed.
        if (mlir::isa<mlir::func::ReturnOp, mlir::tt::ttir::MeshShardOp>(op)) {
          return;
        }
  ...
  llvm::StringRef getDescription() const override {
    return "Clean up the tensor annotations in the module."
           "1. Single device tensor op has multi device tensor argument in a "
           "single op. In this case, we need to insert MeshShardOp before the "
           "op and change the tensor to single device tensor."
           "2. Multi device tensor ops have missing multi device tensor "
           "annotation in case of frontend automatic parallelism with "
           "pre-sharded inputs and outputs. In this case, we need to propagte "
           "the missing TensorMeshShardingAttr.";
  }

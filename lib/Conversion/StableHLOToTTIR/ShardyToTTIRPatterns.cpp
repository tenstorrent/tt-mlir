  ...
  matchAndRewrite(mlir::sdy::ManualComputationOp srcOp,
    ...
    for (auto [retIdx, args] : llvm::enumerate(llvm::zip_equal(
      ...
      auto meshShardOp = rewriter.create<mlir::tt::ttir::MeshShardOp>(
          ...
          // Explicitly set the shard type to Identity for outputs to prevent each device
          // from having the same redundant full copy of the tensor. This mesh_shard op
          // will eventually fully be removed in the future.

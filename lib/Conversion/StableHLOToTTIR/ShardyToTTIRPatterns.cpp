...
static bool checkTypeIfTensorMeshShardingAttrExist(
    mlir::Type valueType,
    mlir::ttcore::TensorMeshShardingAttr incomingTensorMeshShardingAttr) {
  ...
  if (auto existingTensorMeshShardingAttr =
          dyn_cast_if_present<mlir::ttcore::TensorMeshShardingAttr>(encoding)) {
...
void propagateTensorMeshSharding(
    OpTy srcOp, mlir::ttcore::TensorMeshShardingAttr tensorMeshShardingAttr) {
...
public:
  ...
  matchAndRewrite(mlir::sdy::ManualComputationOp srcOp,
    ...
    auto tensorMeshShardingAttr =
        mlir::ttcore::TensorMeshShardingAttr::get(context, meshNameStrAttr);
...
public:
  ...
  matchAndRewrite(mlir::sdy::MeshOp srcOp, mlir::sdy::MeshOp::Adaptor adaptor,
    ...
    if (!sdyMesh.empty()) {
      ...
      mlir::ttcore::utils::addMeshToModuleAttribute(rewriter, module, meshName,
                                                    meshShape);
    }
...
// issue.
llvm::LogicalResult
analyzeSingleOpAndFixWrongTensorAnnotation(mlir::ttcore::MeshesAttr meshes,
  ...
  if (auto tensorMeshShardingAttr =
          mlir::dyn_cast_if_present<mlir::ttcore::TensorMeshShardingAttr>(
...
public:
  ...
  void runOnOperation() final {
    ...
    auto meshes = moduleOp->getAttrOfType<mlir::ttcore::MeshesAttr>(
        mlir::ttcore::MeshesAttr::name);

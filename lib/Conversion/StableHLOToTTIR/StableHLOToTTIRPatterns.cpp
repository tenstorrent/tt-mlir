...
public:
  ...
  matchAndRewrite(mlir::stablehlo::CustomCallOp srcOp,
    ...
    // All @SPMD* calls have @Sharding as their first operand. Therefore, we skip @Sharding
    // and handle it in the other two cases together.
    if (callTargetName != mlir::tt::sharding_utils::kSPMDFullToShardShapeCallTargetName &&
    callTargetName != mlir::tt::sharding_utils::kSPMDShardToFullShapeCallTargetName) {
      return failure();
    }
    ...
    // We also want to extract the mhlo.sharding attribute from this op's @Sharding operand.
    auto shardingOperand = srcOp->getOperand(0);
    auto definingOp = shardingOperand.getDefiningOp<stablehlo::CustomCallOp>();
    auto operandShardingAttr = definingOp->getAttrOfType<mlir::StringAttr>(mlir::tt::sharding_utils::kXlaShardingAttr);

    if (!operandShardingAttr) {
      return rewriter.notifyMatchFailure(
          srcOp, "Requires operand to have mhlo.sharding attribute.");
    }

    // Once extracted, we can generate the GSPMDMeshSharding object.
    llvm::Expected<GSPMDMeshSharding> gspmdMeshSharding = generate(opShardingAttr.getValue(), operandShardingAttr.getValue(), mlir::tt::ttcore::ShardStatus::Unsharded);
    if (auto err = gspmdMeshSharding.takeError()) {
      return rewriter.notifyMatchFailure(srcOp, "Error trying to parse GSPMD annotation.");
    }

    // Insert the new MeshShardOp with the generated GSPMDMeshSharding.
    auto outputType = mlir::cast<RankedTensorType>(getTypeConverter()->convertType(srcOp->getResult(0).getType()));

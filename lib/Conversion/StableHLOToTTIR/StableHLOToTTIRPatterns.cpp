...
public:
  ...
  //
  // `flattenedIndexedDims` (info.startIndexMap) are the operand dims actually
  // flattened into embedding rows: `startIndexMap` minus the full-slice dims.
  ...
  flattenStartIndices(ConversionPatternRewriter &rewriter,
    ...
    // permuteInput put the flattened dims at the front of inputShape, so
    // computeStrides of that prefix gives their strides. Dropped dims still
    // occupy an index component, so key by original operand dim, not position.

...
public:
  ...
  matchAndRewrite(mlir::stablehlo::GatherOp srcOp,
    ...
    if (numIndexingDims > 1) {
      auto startIndicesElemType = startIndices.getType().getElementType();
      ...
      // flattenStartIndices returns f32; cast back to the original integer type
      // since ttir.embedding requires integer indices.
      auto flattenedResult = flattenedIndices.getResult();
      auto castBackType =
          mlir::cast<RankedTensorType>(flattenedResult.getType())
              .clone(startIndicesElemType);
      auto castBackOp = rewriter.create<ttir::TypecastOp>(
          ttmlir::utils::appendLocationSuffix(srcOp->getLoc(),
                                              "_flattenCastBack"),
          castBackType, flattenedResult);
      startIndices = mlir::cast<mlir::TypedValue<mlir::RankedTensorType>>(
          castBackOp.getResult());

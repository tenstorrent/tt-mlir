...
public:
  ...
  matchAndRewrite(mlir::stablehlo::CustomCallOp srcOp,
    ...
    // For 2D mesh with compound-sharded experts (E_total > dispatch_devices),
    // the combine only handles communication along the dispatch axis. Insert
    // all_reduce(sum) on the OTHER axis to aggregate partial expert results
    // from device columns that hold different expert subsets.
    // Use the mapping tensor's E dimension (shape[2]) since it is replicated
    // and retains the global expert count even after local shape conversion.
    auto mappingType = cast<RankedTensorType>(expertMapping.getType());
    int64_t eTotal = mappingType.getShape()[2];
    if (eTotal > numDevices && numDevices > 1) {

...
LogicalResult determineClusterAxis(::mlir::DenseIntElementsAttr replicaGroups,
...
public:
  ...
  matchAndRewrite(mlir::stablehlo::AllReduceOp srcOp,
    ...
    if (failed(
            getReduceType<mlir::stablehlo::AllReduceOp>(srcOp, reduceType))) {
      return rewriter.notifyMatchFailure(
    ...
    auto inputShape =
        mlir::cast<RankedTensorType>(adaptor.getOperands()[0].getType())
            .getShape();
    int32_t dimension = -1; // gather_dim == scatter_dim
    ...
    for (const auto &tensorDimValue : inputShape) {
      dimension++;
      if (tensorDimValue % sizeOfDevices == 0) {
        break;
      }
    }
    // No tensor dimension was found that was divisible cleanly by the number of
    // devices. Therefore, we cannot perform the reduce_scatter or all_gather.
    if (dimension == -1) {
      return failure();
    }
...
public:
  ...
  matchAndRewrite(mlir::stablehlo::ReduceScatterOp srcOp,
    ...
    // Convert reduceType shlo attribute into ttir attribute
    ReduceType reduceType;
    if (failed(getReduceType<mlir::stablehlo::ReduceScatterOp>(srcOp,
                                                               reduceType))) {
      return rewriter.notifyMatchFailure(

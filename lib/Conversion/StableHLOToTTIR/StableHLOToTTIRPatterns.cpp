...
private:
  ...
  matchAndRewriteInternalArgMax(mlir::stablehlo::ReduceOp &srcOp,
    ...
    // stablehlo.reduce op generates two results (only one is consumed) whereas
    // ttir.argmax generates one result only. So 'rewriter.replaceOpWithNewOp'
    // will not work due to difference in number of outputs.
  ...
  bool isArgMax(mlir::stablehlo::ReduceOp &srcOp,
    ...
    if (!hasValidOutputs(srcOp.getResults())) {
      return false;
    }

    if (!hasValidInitValues(srcOp.getInitValues())) {
      return false;
    }

    if (!hasValidReducerBody(srcOp.getBody())) {
      return false;
    }
  ...
  bool hasValidReducerBody(mlir::Region &body) const {
    ...
    if (operations.size() == 7) {
      if (verifyTorchOpPattern(operations.front())) {
        return true;
      }
    } else if (operations.size() == 10) {
      if (verifyJaxOpPattern(operations.front())) {
        return true;
      }
    }

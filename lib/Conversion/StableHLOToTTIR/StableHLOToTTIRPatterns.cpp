...
public:
  ...
  matchAndRewrite(mlir::stablehlo::ConvolutionOp srcOp,
    ...
    ttir::ConvolutionOp convOp = ttir::ConvolutionOp::create(
  ...
  matchAndRewrite(mlir::stablehlo::ReduceWindowOp srcOp,
    ...
    for (size_t i = 0; i < srcOp.getInputs().size(); ++i) {
      ...
      if (needsReshape && inputType.getRank() == 2) {
        ...
        input = ttir::ReshapeOp::create(rewriter, srcOp.getLoc(), inputType4D,
                                        input,
      ...
      if (isMaxPool(srcOp, initVal, frontOp)) {
        ...
        if (divOp && i == 0) {
          ...
          ttir::PoolingOp poolingOp = ttir::PoolingOp::create(
              rewriter, srcOp.getLoc(), resultType, input, method,
...
public:
  ...
  matchAndRewrite(mlir::stablehlo::DynamicSliceOp srcOp,
    ...
    rewriter.replaceOp(srcOp, mlir::tt::ttir::SliceDynamicOp::create(
                                  rewriter, srcOp.getLoc(), outputType,
                                  adaptor.getOperand(), startIndicesTensor,
  ...
  matchAndRewrite(mlir::stablehlo::ScatterOp srcOp,
    ...
    if (scatterDimsToOperandDims.size() > 1) {
      ...
      Value scatterResult = ttir::ScatterOp::create(
          ...
          finalIndexTensor, flattenedUpdate, dimAttr, reduceTypeAttr);
...
private:
  ...
  Value extractMultiDimensionalScatterIndices(mlir::stablehlo::ScatterOp op,
    ...
    for (int64_t dim = 0; dim < numIndexDims; ++dim) {
      ...
      Value dimensionIndices = ttir::SliceStaticOp::create(
          rewriter,
          ttmlir::utils::appendLocationSuffix(
      ...
      if (strides[dim] > 1) {
        ...
        Value strideTensor = ttir::FullOp::create(
            rewriter,
            ttmlir::utils::appendLocationSuffix(
        ...
        dimensionIndices = ttir::MultiplyOp::create(
            rewriter,
            ttmlir::utils::appendLocationSuffix(
...
public:
  ...
  matchAndRewrite(mlir::stablehlo::SortOp srcOp,
    ...
    auto reshape =
        ttir::ReshapeOp::create(rewriter, loc, expandedType, indices,
    ...
    for (int64_t idx = 0; idx < rank; ++idx) {
      ...
      Value arangeOp = ttir::ArangeOp::create(
          rewriter, loc, expandedType, /*start=*/0,
    ...
    Value concatIndices =
        ttir::ConcatOp::create(rewriter, loc, concatType, toConcat, rank);
    ...
    for (size_t i = 1; i < srcOp.getInputs().size(); ++i) {
      ...
      auto gathered = ttir::GatherOp::create(

...
// index to UInt32 when needed and applying the Rank-1 workaround. Used by both
// the composite path (TenstorrentGatherConversionPattern) and the custom_call
// path (CustomCallGatherConversionPattern) since they only differ in how the
// `dim` attribute is sourced.

...
struct ConvertLinalgToLLVMPass
  ...
  void runOnOperation() final {
    // Creating a MLIRContext w/o multithreading is easier than pre-registering
    // all the dialects we need.
    ...
    mlir::ModuleOp tempModule =
        builder.create<mlir::ModuleOp>(cpuModule.getLoc());
    ...
    // Clone operations from `cpuModule` into `tempModule`, and remove
    // originals.
    ...
    bufferizationOptions.copyBeforeWrite = false;
    bufferizationOptions.enforceAliasingInvariants = false;

    tempPM.addPass(
        mlir::bufferization::createOneShotBufferizePass(bufferizationOptions));

    // tempPM.addPass(mlir::bufferization::createBufferHoistingPass());
    // tempPM.addPass(mlir::bufferization::createBufferLoopHoistingPass());
    tempPM.addPass(mlir::memref::createFoldMemRefAliasOpsPass());
    ...
    tempPM.addPass(mlir::createCanonicalizerPass());
    tempPM.addPass(mlir::createSCCPPass());
    tempPM.addPass(mlir::createCSEPass());
...
std::unique_ptr<OperationPass<tt::CPUModuleOp>>

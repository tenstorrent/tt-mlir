...
public:
  using TTNNToEmitCBaseOpConversionPattern<
      mlir::tt::ttnn::CumSumOp>::TTNNToEmitCBaseOpConversionPattern;
  ...
  matchAndRewrite(mlir::tt::ttnn::CumSumOp srcOp,
    ...
    ttnn_to_emitc::EmitCTTNNEmitter<mlir::tt::ttnn::CumSumOp> emitter(
        srcOp, adaptor, rewriter);
...
public:
  ...
  matchAndRewrite(mlir::tt::ttnn::ToLayoutOp srcOp, OpAdaptor adaptor,
    ...
    mlir::Attribute dtypeArg = srcOp.hasDtypeChange()
                                   ? emitter.emit(srcOp.getDtypeAttr())
                                   : emitter.emit(std::nullopt);
    llvm::SmallVector<mlir::Attribute> args{
        emitter.emit(srcOp.getInput()),
...
private:
  std::string getPrefixSearchPattern() const override {
    return "ttnn.rmsnorm_bw";
  }
  std::string getPrefixSwapPattern() const override {
    return "ttml::metal::rmsnorm_bw";
  }

public:
  using TTNNToEmitCBaseOpConversionPattern<
      mlir::tt::ttnn::RMSNormBackwardOp>::TTNNToEmitCBaseOpConversionPattern;
  using Adaptor = mlir::tt::ttnn::RMSNormBackwardOp::Adaptor;

  LogicalResult
  matchAndRewrite(mlir::tt::ttnn::RMSNormBackwardOp srcOp, Adaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ttnn_to_emitc::EmitCTTNNEmitter<mlir::tt::ttnn::RMSNormBackwardOp> emitter(
        srcOp, adaptor, rewriter);

    // Arg order matches ttml::metal::rmsnorm_bw(input, gamma, rms, dL_dout).
    llvm::SmallVector<mlir::Attribute> args{
        emitter.emit(srcOp.getInput()),
        emitter.emit(srcOp.getGamma()),
        emitter.emit(srcOp.getRms()),
        emitter.emit(srcOp.getGradOutput()),
    };

    using ReturnTy = std::vector<std::optional<::ttnn::Tensor>>;
    auto rmsNormBackwardOp = rewriter.create<emitc::CallOpaqueOp>(
        srcOp.getLoc(),
        rewriter.getType<emitc::OpaqueType>(ttnn_to_emitc::TypeNameV<ReturnTy>),
        convertOpName(srcOp), rewriter.getArrayAttr(args),
        /*template_args=*/nullptr, adaptor.getOperands());
    ...
    for (unsigned i = 0; i < srcOp.getNumResults(); ++i) {
      auto indexOp = rewriter.create<emitc::LiteralOp>(
          srcOp.getLoc(), rewriter.getIndexType(), std::to_string(i));
      auto subscriptOp = rewriter.create<emitc::SubscriptOp>(
          srcOp.getLoc(), optionalLValueType, rmsNormBackwardOp.getResult(0),
          indexOp.getResult());
      auto loadOp = rewriter.create<emitc::LoadOp>(srcOp.getLoc(), optionalType,
                                                   subscriptOp.getResult());
      auto valueOp = rewriter.create<emitc::CallOpaqueOp>(
...
private:
  std::string getPrefixSearchPattern() const override {
    return "ttnn.layernorm_fw";
  }
  std::string getPrefixSwapPattern() const override {
    return "ttml::metal::layernorm_fw";
  }

public:
  using TTNNToEmitCBaseOpConversionPattern<
      mlir::tt::ttnn::LayerNormForwardOp>::TTNNToEmitCBaseOpConversionPattern;
  using Adaptor = mlir::tt::ttnn::LayerNormForwardOp::Adaptor;

  LogicalResult
  matchAndRewrite(mlir::tt::ttnn::LayerNormForwardOp srcOp, Adaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ttnn_to_emitc::EmitCTTNNEmitter<mlir::tt::ttnn::LayerNormForwardOp> emitter(
        srcOp, adaptor, rewriter);

    // Arg order matches ttml::metal::layernorm_fw(input, gamma, beta, epsilon,
    // return_mean_rstd).
    llvm::SmallVector<mlir::Attribute> args{
        emitter.emit(srcOp.getInput()),
        emitter.emit(srcOp.getWeight()),
        emitter.emit(srcOp.getBias()),
        emitter.emit(srcOp.getEpsilon()),
        emitter.emit(srcOp.getReturnMeanRstd()),
    };

    using ReturnTy = std::vector<std::optional<::ttnn::Tensor>>;
    auto layerNormForwardOp = rewriter.create<emitc::CallOpaqueOp>(
        srcOp.getLoc(),
        rewriter.getType<emitc::OpaqueType>(ttnn_to_emitc::TypeNameV<ReturnTy>),
        convertOpName(srcOp), rewriter.getArrayAttr(args),
        /*template_args=*/nullptr, adaptor.getOperands());
    ...
    for (unsigned i = 0; i < srcOp.getNumResults(); ++i) {
      auto indexOp = rewriter.create<emitc::LiteralOp>(
          srcOp.getLoc(), rewriter.getIndexType(), std::to_string(i));
      auto subscriptOp = rewriter.create<emitc::SubscriptOp>(
          srcOp.getLoc(), optionalLValueType, layerNormForwardOp.getResult(0),
          indexOp.getResult());
      auto loadOp = rewriter.create<emitc::LoadOp>(srcOp.getLoc(), optionalType,
                                                   subscriptOp.getResult());
      auto valueOp = rewriter.create<emitc::CallOpaqueOp>(
...
public:
  ...
  matchAndRewrite(mlir::tt::ttnn::RMSNormPreAllGatherOp srcOp,
    ...
    ttnn_to_emitc::EmitCTTNNEmitter<mlir::tt::ttnn::RMSNormPreAllGatherOp>
        emitter(srcOp, adaptor, rewriter);
...
public:
  ...
  matchAndRewrite(mlir::tt::ttnn::GroupNormOp srcOp, OpAdaptor adaptor,
    ...
    llvm::SmallVector<mlir::Attribute> args{
        ...
        emitter.emit(srcOp.getMemoryConfigAttr()),
        emitter.emit(/* dtype= */ std::nullopt),
        emitter.emit(/* core_grid= */ std::nullopt),
        emitter.emit(/* inplace= */ std::nullopt),
        emitter.emit(/* output_layout= */ std::nullopt),
        emitter.emit(/* num_out_blocks= */ std::nullopt),
    };
...
public:
  using TTNNToEmitCBaseOpConversionPattern<
      mlir::tt::ttnn::RotaryEmbeddingOp>::TTNNToEmitCBaseOpConversionPattern;

  LogicalResult
  matchAndRewrite(mlir::tt::ttnn::RotaryEmbeddingOp srcOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {

    ttnn_to_emitc::EmitCTTNNEmitter<mlir::tt::ttnn::RotaryEmbeddingOp> emitter(
        srcOp, adaptor, rewriter);
...
public:
  using TTNNToEmitCBaseOpConversionPattern<
      mlir::tt::ttnn::TopKRouterGptOp>::TTNNToEmitCBaseOpConversionPattern;

  LogicalResult
  matchAndRewrite(mlir::tt::ttnn::TopKRouterGptOp srcOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ttnn_to_emitc::EmitCTTNNEmitter<mlir::tt::ttnn::TopKRouterGptOp> emitter(
        srcOp, adaptor, rewriter);

    llvm::SmallVector<mlir::Attribute> args{
        emitter.emit(srcOp.getInput()),      emitter.emit(srcOp.getWeight()),
        emitter.emit(srcOp.getBias()),       emitter.emit(srcOp.getK()),
        emitter.emit(srcOp.getNumExperts()),
    };

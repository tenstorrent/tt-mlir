...
public:
  ...
  matchAndRewrite(ttir::GetGlobalOperandOp op,
    ...
    auto arg =
        rewriter.getAttr<ArgAttr>(op.getOperandIndex(), ArgType::BufferAddress);
    size_t argIndex;
...
namespace {
class TTIRNullTxRewriter
    : public OpConversionPattern<ttir::NullTxOp> {
public:
    ...
    for (auto arg : blockArgs) {
      ...
      if (mlir::isa<CBType>(argType)) {
        ...
        ctArgSpecVector.push_back(
            rewriter.getAttr<ArgAttr>(arg.getArgNumber(), ArgType::CBPort));
        ...
        rtArgSpecVector.push_back(rewriter.getAttr<ArgAttr>(
            currentSemaphoreIndex++, ArgType::Semaphore));
      } else {

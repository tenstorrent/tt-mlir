...
public:
  ...
  static Value buildL1Address(OpBuilder &rewriter, Location loc, Value cb,
    ...
    Value baseAddr = rewriter.create<ttkernel::GetReadPtrOp>(loc, cb);
  ...
  matchAndRewrite(ttir::DMAOp op, ttir::DMAOpAdaptor adaptor,
    ...
    if (op.isSrcLocal() && op.isDstLocal()) {
      ...
      auto srcL1Addr = buildL1Address(rewriter, op.getLoc(), adaptor.getSrc(),
                                      op.getSrcIndices());
      ...
      auto dstL1Addr = buildL1Address(rewriter, op.getLoc(), adaptor.getDst(),
                                      op.getDstIndices());
      auto size = i32(rewriter, op->getLoc(), op.getSizeBytes());

...
protected:
  ...
  static Operation *unLayoutResult(mlir::ConversionPatternRewriter &rewriter,
    ...
    auto rt = mlir::cast<RankedTensorType>(toResultType);
    auto output =
        rewriter.create<d2m::EmptyOp>(fromValue.getLoc(), rt.getShape(),
                                      rt.getElementType(), rt.getEncoding());
...
struct ToLayoutOpLowering : public OpConversionPattern<ttir::ToLayoutOp> {
...
struct TTIREmptyToD2MEmptyRewriter : public OpConversionPattern<ttir::EmptyOp> {
...
void populateTTIRToD2MGenericPatterns(
  ...
  // ToLayout conversion - add separately since it doesn't need the extra parameters
  patterns.add<ToLayoutOpLowering>(typeConverter, ctx);

  // Creation ops - add separately since they don't need the extra parameters
...
#define GEN_PASS_DEF_TTIRTOD2MGENERIC
#include "ttmlir/Conversion/Passes.h.inc"

namespace {
class TTIRToD2MGenericPass final
    : public mlir::tt::impl::TTIRToD2MGenericBase<TTIRToD2MGenericPass> {
public:
  using Base = mlir::tt::impl::TTIRToD2MGenericBase<TTIRToD2MGenericPass>;
...
std::unique_ptr<OperationPass<ModuleOp>> createTTIRToD2MGenericPass() {
  return std::make_unique<TTIRToD2MGenericPass>();
}

std::unique_ptr<OperationPass<ModuleOp>>
createTTIRToD2MGenericPass(const TTIRToD2MGenericOptions &options) {
  return std::make_unique<TTIRToD2MGenericPass>(options);
}

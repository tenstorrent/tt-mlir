...
struct ConvertD2MToTTMetal
  ...
  void runOnOperation() final {
    ...
    // Explicit datamovement processor index (set by D2MScheduleDMA when
    // num-datamovement-processors > 2) is allowed through. Downstream
    // patterns read the processor index off ThreadAttr; the TTMetal
    // kernel descriptor carries it for runtime dispatch.

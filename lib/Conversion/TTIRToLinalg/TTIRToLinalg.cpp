...
public:
  ...
  matchAndRewrite(ttir::AvgPool2dOp op, OpAdaptor adaptor,
    ...
    // TOSA avg_pool2d requires padding < kernel size. If calculated padding
    // exceeds this limit, we need to explicitly pad the input tensor first.
    int64_t explicitPadTop = 0, explicitPadBottom = 0;
    int64_t explicitPadLeft = 0, explicitPadRight = 0;

    if (paddingTop >= kernelHeight) {
      explicitPadTop = paddingTop - kernelHeight + 1;
      paddingTop = kernelHeight - 1;
    }
    if (paddingBottom >= kernelHeight) {
      explicitPadBottom = paddingBottom - kernelHeight + 1;
      paddingBottom = kernelHeight - 1;
    }
    if (paddingLeft >= kernelWidth) {
      explicitPadLeft = paddingLeft - kernelWidth + 1;
      paddingLeft = kernelWidth - 1;
    }
    if (paddingRight >= kernelWidth) {
      explicitPadRight = paddingRight - kernelWidth + 1;
      paddingRight = kernelWidth - 1;
    }

    // Apply explicit padding if needed.
    if (explicitPadTop > 0 || explicitPadBottom > 0 || explicitPadLeft > 0 ||
        explicitPadRight > 0) {
      // Create padding shape for tosa.pad: 1D shape with [batch_before,
      // batch_after, height_before, height_after, width_before, width_after,
      // channel_before, channel_after].
      SmallVector<int64_t> paddingValues = {
          0,                 // batch before
          0,                 // batch after
          explicitPadTop,    // height before
          explicitPadBottom, // height after
          explicitPadLeft,   // width before
          explicitPadRight,  // width after
          0,                 // channel before
          0                  // channel after
      };
      auto paddingShapeType = tosa::shapeType::get(rewriter.getContext(), 8);
      auto paddingAttr = rewriter.getIndexTensorAttr(paddingValues);
      Value paddingTensor = rewriter.create<tosa::ConstShapeOp>(
          op.getLoc(), paddingShapeType, paddingAttr);

      // Update input dimensions after explicit padding.
      int64_t newHeight = inputHeight + explicitPadTop + explicitPadBottom;
      int64_t newWidth = inputWidth + explicitPadLeft + explicitPadRight;
      auto paddedInputType =
          RankedTensorType::get({inputType.getShape()[0], newHeight, newWidth,
                                 inputType.getShape()[3]},
                                inputType.getElementType());

      input = rewriter.create<tosa::PadOp>(op.getLoc(), paddedInputType, input,
                                           paddingTensor);
...
void populateTTIRToTosaPatterns(MLIRContext *ctx, RewritePatternSet &patterns,
  ...
  patterns.add<BroadcastOpConversionPattern, SinOpConversionPattern,
               ...
               Upsample2dOpConversionPattern, Conv2dOpConversionPattern,

...
auto createPrepareConv2dBiasTuple(
    Tag tag, const ::tt::target::ttnn::PrepareConv2dBiasOpT &opT,
    TensorArg biasTensor, ::ttnn::MeshDevice &targetDevice,
  ...
  return std::make_tuple(
      *std::get<const ::ttnn::Tensor *>(biasTensor), params.inputMemoryConfig,
      ...
      params.padding, params.dilation, opT.groups, &targetDevice,
...
callPrepareConv2dBias(CallType callType,
  ...
  PrepareConv2dBiasResolvedParams params =
      resolvePrepareConv2dBiasParams(opT, callType);

  auto makeTuple = [&](auto tag) {
    return createPrepareConv2dBiasTuple(tag, opT, biasTensor, targetDevice,
                                        params);
  };
    ...
    return std::apply(
        [&](auto &&...args) {
          return QUERY_OP_CONSTRAINTS(
              ::ttnn::operations::conv::conv2d::prepare_conv_bias,
  ...
  case CallType::QUERY_OP_RUNTIME: {
    ...
    response.error_message =
        "Runtime query not implemented for PrepareConv2dBiasOp yet";
    return response;
  }

...
resolveConv3dParams(const ::tt::target::ttnn::Conv3dOpT &conv3dOpT,
  ...
  if (conv3dOpT.out) {
    params.outputMemoryConfig = operations::utils::createMemoryConfigIfNeeded(
        operations::utils::getTensorRefMemoryConfig(*conv3dOpT.out), callType);
...
Conv3dOpResult callConv3d(CallType callType,
  ...
  auto makeTuple = [&](auto tag) {
    return createConv3dTuple(tag, conv3dOpT, input, weight, bias, targetDevice,
                             params);
  };
  ...
  case CallType::QUERY_OP_CONSTRAINTS:
    return std::apply(
        [&](auto &&...args) {
          return QUERY_OP_CONSTRAINTS(::ttnn::experimental::conv3d,
                                      &targetDevice,

...
auto createConvTranspose2dTuple(
  ...
  return std::make_tuple(
      ...
      &targetDevice, convTranspose2dOpT.in_channels,
...
ConvTranspose2dOpResult callConvTranspose2d(
  ...
  ConvTranspose2dResolvedParams params =
      resolveConvTranspose2dParams(convTranspose2dOpT, callType);

  auto makeTuple = [&](auto tag) {
    return createConvTranspose2dTuple(tag, convTranspose2dOpT, input, weight,
                                      bias, targetDevice, params);
  };
  ...
  case CallType::QUERY_OP_CONSTRAINTS:
    return std::apply(
        [&](auto &&...args) {
          return QUERY_OP_CONSTRAINTS(::ttnn::conv_transpose2d, &targetDevice,
  ...
  case CallType::QUERY_OP_RUNTIME:
    return std::apply(
        [&](auto &&...args) {
          return QUERY_OP_RUNTIME(::ttnn::conv_transpose2d, &targetDevice,
                                  std::forward<decltype(args)>(args)...);
        },
        makeTuple(QueryTag{}));

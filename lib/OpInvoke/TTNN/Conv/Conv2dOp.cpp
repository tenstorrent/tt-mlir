...
resolveConv2dParams(const ::tt::target::ttnn::Conv2dOpT &conv2dOp) {
  LOG_ASSERT(conv2dOp.kernel_size.size() == 2,
             "Kernel size expected to have 2 elements");
  LOG_ASSERT(conv2dOp.stride.size() == 2, "Stride expected to have 2 elements");
  LOG_ASSERT(conv2dOp.padding.size() == 2 || conv2dOp.padding.size() == 4,
             "Padding expected to have 2 or 4 elements");
  LOG_ASSERT(conv2dOp.dilation.size() == 2,
  ...
  if (conv2dOp.out) {
    ...
    LOG_ASSERT(operations::utils::inSystemMemory(*conv2dOp.out) ||

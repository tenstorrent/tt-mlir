...
resolveMatmulParams(const ::tt::target::ttnn::MatmulOpT &matmulOp) {
  ...
  if (matmulOp.out) {
    ...
    LOG_ASSERT(operations::utils::inSystemMemory(*matmulOp.out) ||
...
resolveLinearParams(const ::tt::target::ttnn::LinearOpT &linearOp) {
  ...
  if (linearOp.out) {
    ...
    LOG_ASSERT(operations::utils::inSystemMemory(*linearOp.out) ||
                   params.outputMemoryConfig.has_value(),
               "Memory config must exist for device tensors");
  }
...
SparseMatmulResolvedParams resolveSparseMatmulParams(
  ...
  if (sparseMatmulOp.out) {
    ...
    LOG_ASSERT(operations::utils::inSystemMemory(*sparseMatmulOp.out) ||
                   params.outputMemoryConfig.has_value(),
               "Memory config must exist for device tensors");
  }

...
ttrt (Tenstorrent Runtime) is a tool included in the tt-mlir toolchain designed to inspect and execute flatbuffer files produced by the Tenstorrent compiler stack--without a front-end runtime. The compiler stack converts models through multiple intermediate representations (IRs) before producing flatbuffers that can run on Tenstorrent hardware. TTRT acts as a "Swiss-army knife" utility for flatbuffers, allowing you to: 
* Run them
* Debug them
...
* **Compile models with tt-mlir** – transforms high-level models into MLIR, applies optimization passes, and outputs flatbuffers.

* **Use ttrt** – runs the flatbuffers, debugs them, and generates artifacts such as system descriptors, logs, and test files as needed.

* **TT-Metal executes ops** – the backend library handles memory management, scheduling, and interfacing with Tenstorrent hardware when execution is required.

* **Flatbuffers** – act as the portable, versioned model representation bridging the compiler, ttrt, TT-Metal, and the hardware.

Build modes:

* **Runtime mode** – ttrt fully executes the model on Tenstorrent hardware via TT-Metal.

* **Perf mode** – ttrt measures performance, traces ops, and simulates execution, using TT-Metal partially or in a non-executing mode.

## Building

1. The first step is building tt-mlir. When you get to the section in the instructions titled [Building the tt-mlir Project](https://github.com/tenstorrent/tt-mlir/blob/main/docs/src/getting-started.md#building-the-tt-mlir-project) you need to make a change to one of the steps. You cannot build TTRT unless you flag the build for one of: 
* Building runtime mode - `-DTTMLIR_ENABLE_RUNTIME=ON`
* Building perf mode - `-DTTMLIR_ENABLE_RUNTIME=ON / -DTT_RUNTIME_ENABLE_PERF_TRACE=ON`
...
The complete build instructions for tt-mlir (add one of the modifications described above) can be found here -
[Build ttmlir](./getting-started.md). 
...
Every time `ttrt` is built, it creates a whl (wheel) file in `build/tools/ttrt/build`. Ex filename: `ttrt-0.0.235-cp311-cp311-linux_x86_64.whl`. You can take this wheel and install it in any docker container or Python environment, independent of the full tt-mlir build. 
...
3. Install the wheel (replace the example command with your version of the wheel):
```bash
pip install build/tools/ttrt/build/ttrt-0.0.235-cp311-cp311-linux_x86_64.whl
```

## Generating a flatbuffer
ttrt works with flatbuffers (a serialized, memory-efficient data structure designed to be read and used directly without unpacking or parsing). You need to generate the flatbuffers, so this section provides a list of different ways yo ucan do that. 
...
This section shows you the basic commands available. They can be accessed in the command line interface (CLI) by typing `ttrt --help`. 

* `ttrt query` - Query for information about the current system.
* `ttrt read` - Read information from a flatbuffer binary.
* `ttrt run` - Run a flatbuffer binary. 
* `ttrt perf` - Run a performance trace and collect performance data.
* `ttrt check` - Check a flatbuffer binary against a system desc file. 
...
## Command line usage
You can retrieve more information about any `ttrt` command by typing `ttrt -command- --help` (for example, `ttrt read --help`). All artifacts are saved under `ttrt-artifacts` folder under `TT_MLIR_HOME` environment variable. By default, all logging is printed to the terminal. You can specify a log file to dump output to.

### query 
Available `query` commands include:

* `ttrt query --clean-artifacts` - Clean all the artifacts from previous runs.
* `ttrt query --save-artifacts` - Save all artifacts during a run. 
* `ttrt query --log-file LOG_FILE` - Specify the log file to dump `ttrt` output to. 
* `ttrt query --artifact-dir ARTIFACT_DIR` - Provides a directory path to save artifacts to.
...
* `ttrt query --result-file RESULT_FILE` - Test the file to save results to. 

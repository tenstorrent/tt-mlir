...
[`ttir-builder`] (./ttir-builder.md) is a tool for creating ttir ops and converting them into modules, `mlir` files, and flatbuffers.
...
5. A potential application for this callback function is implementing a golden callback. TTRT achieves this by first storing the golden data within the flatbuffer binary. This is used by [`ttir-builder`] (./ttir-builder.md) to construct flatbuffers with embedded golden input/output/intermediate data. See `runtime/tools/ttrt/ttrt/common/golden.py` for how ttrt implement the golden callback function.
```bash

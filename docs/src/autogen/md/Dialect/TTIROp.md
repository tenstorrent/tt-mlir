...
Auto-generated reference of operations in the `TTIR` dialect (176 ops).
...
Fused AdamW optimizer step. Returns the updated parameter and moment estimates.
`ttnn.adamw` writes all of them in place, so `TTIRToTTNN` maps each result back
onto the operand it updates.

Inputs:
- `param` (Tensor): parameter tensor being optimized.
...
Outputs: the updated `param`, `exp_avg`, `exp_avg_sq` and, when
`max_exp_avg_sq` is given, the updated `max_exp_avg_sq`.
...
## `ttir.all_to_all_dispatch_metadata`
...
## `ttir.chunked_scaled_dot_product_attention`
...
## `ttir.distributed_layer_norm`

Distributed layer normalization with all-gather operation

Fused distributed layer normalization across mesh devices. Each device
...
Fused distributed RMS normalization across mesh devices. Each device
...
For each element x, returns the fractional part x - trunc(x), matching common
floating-point library semantics.
...
Output:
  `combine_output`                 A2A selective-reduce-combined result.

The op always runs the full (combine) path; `cluster_axis` selects the
mesh axis the A2A combine reduces over. The metal kernel internally
allocates and frees its routing/tilize/matmul intermediates; only the
combine result is exposed in the IR.

## `ttir.moe_expert_token_remap`
...
Paged flash Multi-Latent Attention (MLA) for the decode phase. This is a
...
## `ttir.paged_scaled_dot_product_attention_decode`
...
## `ttir.scaled_dot_product_attention_decode`
...
Training-oriented fused scaled dot-product attention backward pass, backed by
the ttml metal op `ttml::metal::sdpa_bw`. Given the upstream gradient and the
forward pass tensors (output, Q, K, V and log-sum-exp intermediates), it
computes the gradients w.r.t. query, key and value.

Inputs:
- `grad_output` (Tensor): `[B x Hq x S x Dv]`, gradient w.r.t. the forward
  output.
- `attn_output` (Tensor): `[B x Hq x S x Dv]`, the forward output.
- `query` (Tensor): `[B x Hq x S x D]`.
- `key` (Tensor): `[B x Hkv x S x D]`.
- `value` (Tensor): `[B x Hkv x S x Dv]`.
- `intermediates` (Tensor): `[B x Hq x S x 32]` log-sum-exp from the forward
  pass.
- `attention_mask` (Optional Tensor): `[1 x 1 x S x S]`, only valid when
  `mask_type` is `arbitrary`.

Attributes:
- `mask_type`: `none`, `causal` or `arbitrary` (default). `arbitrary`
  requires `attention_mask`; `none`/`causal` forbid it.
- `dropout_probability`: dropout probability (currently must be 0.0).

Outputs:
- `grad_query` (Tensor): `[B x Hq x S x D]`.
- `grad_key` (Tensor): `[B x Hkv x S x D]`.
- `grad_value` (Tensor): `[B x Hkv x S x Dv]`.

## `ttir.sdpa_fw`

Scaled dot-product attention forward (ttml).

Training-oriented fused scaled dot-product attention forward pass, backed by
the ttml metal op `ttml::metal::sdpa_fw`. Computes
`softmax(Q @ K^T / sqrt(D) + mask) @ V` and, optionally, the per-row
log-sum-exp intermediates needed by the backward pass.
...
Attributes:
- `mask_type`: `none`, `causal` (default) or `arbitrary`. `arbitrary`
  requires `attention_mask`; `none`/`causal` forbid it.
- `dropout_probability`: dropout probability (currently must be 0.0).
- `return_intermediates`: when true, also returns the log-sum-exp
  intermediates.

Outputs:
- `output` (Tensor): `[B x Hq x S x Dv]`.
- `intermediates` (Optional Tensor): `[B x Hq x S x 32]` log-sum-exp, present
  iff `return_intermediates` is true.

## `ttir.selective_reduce_combine`
...
For each element, returns 0.0 if the sign bit is clear and 1.0 if set,
matching IEEE 754 signbit semantics in the element type of the result tensor.
...
## `ttir.split_query_key_value_and_split_heads`
...
Opaque tt-lang custom kernel placeholder.

Represents an invocation of a user-defined tt-lang operation registered via

...
public:
  ...
  // N150 benchmark: reserve a DRAM trace region so metal-trace capture works.
  // (Default codegen sets this to 0, which makes trace capture fail.)
  // Override with env TRACE_REGION_SIZE (bytes). Default 128 MB.
  static std::size_t traceRegionSize() {
    if (const char *e = std::getenv("TRACE_REGION_SIZE"))
      return static_cast<std::size_t>(std::stoull(e));
    return static_cast<std::size_t>(128) << 20;
  }
...
// TODO(amilovanovic): Remove this once the following issue if fixed in

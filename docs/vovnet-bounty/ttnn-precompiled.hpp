...
extern "C" {
inline void setDevice(ttnn::MeshDevice *device) { DeviceGetter::setInstance(device); }
}
...
inline uint32_t getScalarFromTensor(const ttnn::Tensor &tensor) {
...
inline ::ttnn::Tensor loadTensor(const std::string &filePath, ttnn::Layout layout,

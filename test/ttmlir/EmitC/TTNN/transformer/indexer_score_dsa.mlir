...
// seq_shard_axes -- the mesh axes the query sequence is sharded over -- is the
// 9th positional parameter, so the four intervening parameters are emitted at
// their ttnn defaults. An unset cluster_axis becomes std::nullopt, which is
// what the kernel treats as "flat device enumeration".
...
module {
  ...
  // A named mesh axis lands in the last argument slot as a one-element list.

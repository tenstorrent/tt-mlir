...
module {
  func.func @forward(%cache: tensor<32x8x128x128xbf16>, %indices: tensor<1xi64>, %fill_value: tensor<32x8x1x128xbf16>) -> tensor<32x8x128x128xbf16> {
    ...
    %updated = "stablehlo.scatter"(%cache, %indices, %fill_value) <{indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [0, 1, 3], inserted_window_dims = [2], scatter_dims_to_operand_dims = [2], index_vector_dim = 1>, unique_indices = false}> ({
    ...
    return %updated : tensor<32x8x128x128xbf16>
  }
}

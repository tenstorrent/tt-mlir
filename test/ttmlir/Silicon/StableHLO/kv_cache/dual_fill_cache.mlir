...
module {
  func.func @main(%arg0: tensor<5xi32>, %arg1: tensor<1x32x64x64xbf16>, %arg2: tensor<1x32x64x64xbf16>, %arg3: tensor<1x32x5x64xbf16>, %arg4: tensor<1x32x5x64xbf16>) -> (tensor<1x32x64x64xbf16>, tensor<1x32x64x64xbf16>) {
    ...
    %48 = "stablehlo.scatter"(%arg2, %46, %arg4) <{indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0, 1, 2, 3], scatter_dims_to_operand_dims = [0, 1, 2, 3], index_vector_dim = 4>, unique_indices = false}> ({
    ^bb0(%arg5: tensor<bf16>, %arg6: tensor<bf16>):
      stablehlo.return %arg6 : tensor<bf16>
    }) : (tensor<1x32x64x64xbf16>, tensor<1x32x5x64x4xi64>, tensor<1x32x5x64xbf16>) -> tensor<1x32x64x64xbf16>
    %49 = stablehlo.convert %cst : (tensor<1xi64>) -> tensor<1xbf16>
    %50 = stablehlo.reshape %49 : (tensor<1xbf16>) -> tensor<bf16>
    %51 = stablehlo.broadcast_in_dim %47, dims = [0, 1, 2, 3] : (tensor<1x32x64x64xbf16>) -> tensor<1x32x64x64xbf16>
    %52 = stablehlo.broadcast_in_dim %50, dims = [] : (tensor<bf16>) -> tensor<1x32x64x64xbf16>
    %53 = stablehlo.add %51, %52 : tensor<1x32x64x64xbf16>
    %54 = stablehlo.broadcast_in_dim %48, dims = [0, 1, 2, 3] : (tensor<1x32x64x64xbf16>) -> tensor<1x32x64x64xbf16>
    %55 = stablehlo.add %54, %52 : tensor<1x32x64x64xbf16>
    return %53, %55 : tensor<1x32x64x64xbf16>, tensor<1x32x64x64xbf16>
  }
}

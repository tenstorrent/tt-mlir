...
module @multichip_add attributes {ttcore.meshes = #ttcore.meshes<[<"mesh" = 1x2>]>} {
  func.func @main(%arg0: tensor<64x128xf32> {ttcore.shard_status = #ttcore.shard_status<unsharded>}, %arg1: tensor<64x128xf32> {ttcore.shard_status = #ttcore.shard_status<unsharded>}) -> (tensor<64x128xf32> {ttcore.shard_status = #ttcore.shard_status<unsharded>}) {
    ...
    %1 = "ttir.mesh_shard"(%arg1) <{shard_dims = array<i64: -1, 1>, shard_direction = #ttcore.shard_direction<full_to_shard>, shard_shape = array<i64: 1, 2>, shard_type = #ttcore.shard_type<devices>}> : (tensor<64x128xf32>) -> tensor<64x64xf32>
    // CHECK: "ttnn.distribute_tensor"

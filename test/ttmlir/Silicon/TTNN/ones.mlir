...
module {
  ...
  func.func @ones_6d() -> tensor<32x64x128x32x64x128xbf16> {
    // CHECK: {{.*}} = "ttnn.ones"() {{.*}}
    %0 = "ttir.ones"() <{shape = array<i32:32, 64, 128, 32, 64, 128>}> : () -> tensor<32x64x128x32x64x128xbf16>
    return %0 : tensor<32x64x128x32x64x128xbf16>
  }

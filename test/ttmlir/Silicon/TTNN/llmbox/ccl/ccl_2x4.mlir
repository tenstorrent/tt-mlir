...
func.func public @all_gather_cluster0(%arg0: tensor<1x1x8192x512xf32>) -> (tensor<1x1x16384x512xf32> {jax.result_info = ""}) {
  ...
  %3 = "ttir.all_gather"(%1, %2) <{all_gather_dim = 2 : si32, channel_handle = 1 : si32, replica_groups = dense<[[0, 4], [1, 5], [2, 6], [3, 7]]> : tensor<4x2xi64>, use_global_device_ids, cluster_axis = 0 : si32}> : (tensor<1x1x4096x128xf32>, tensor<1x1x8192x128xf32>) -> tensor<1x1x8192x128xf32>
...
func.func @all_gather_cluster1(%arg0: tensor<1x1x256x512xf32>) -> tensor<1x1x256x512xf32> {
  ...
  %3 = "ttir.all_gather"(%1, %2) <{all_gather_dim = 3 : si32, replica_groups = dense<[[0, 1, 2, 3], [4, 5, 6, 7]]> : tensor<2x4xi64>, cluster_axis = 1 : si32}> : (tensor<1x1x128x128xf32>, tensor<1x1x128x512xf32>) -> tensor<1x1x128x512xf32>
  // CHECK: %[[C:.*]] = "ttnn.all_gather"[[C:.*]]

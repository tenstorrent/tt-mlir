...
func.func @scatter_simple_3(%arg0: tensor<71x32xbf16>, %arg1: tensor<71x4x2xi64>, %arg2: tensor<71x4xbf16>) -> tensor<71x32xbf16> {
  ...
  // CHECK: "ttnn.multiply"({{.*}}) <{activations = [], dtype = #ttcore.supportedDataTypes<si32>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
  // CHECK: "ttnn.slice_static"({{.*}}) <{begins = [0 : i32, 0 : i32, 1 : i32], ends = [71 : i32, 4 : i32, 2 : i32], step = [1 : i32, 1 : i32, 1 : i32]}>
  // CHECK: "ttnn.add"({{.*}}) <{activations = [], dtype = #ttcore.supportedDataTypes<si32>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
  // flatten indices:

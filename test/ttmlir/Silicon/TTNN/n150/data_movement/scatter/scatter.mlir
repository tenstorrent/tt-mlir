...
func.func @scatter_simple_3(%arg0: tensor<71x32xbf16>, %arg1: tensor<71x4x2xi64>, %arg2: tensor<71x4xbf16>) -> tensor<71x32xbf16> {
  ...
  // CHECK: "ttnn.multiply"({{.*}}) <{dtype = #ttcore.supportedDataTypes<si32>, lhs_activations = [], post_activations = [], rhs_activations = []}>
  // CHECK: "ttnn.slice_static"({{.*}}) <{begins = [0 : i32, 0 : i32, 1 : i32], ends = [71 : i32, 4 : i32, 2 : i32], step = [1 : i32, 1 : i32, 1 : i32]}>
  // CHECK: "ttnn.add"({{.*}}) <{dtype = #ttcore.supportedDataTypes<si32>, lhs_activations = [], post_activations = [], rhs_activations = []}>
  // flatten indices:

// REQUIRES: opmodel
// RUN: ttmlir-opt --ttir-to-ttnn-backend-pipeline="system-desc-path=%system_desc_path% enable-optimizer=true memory-layout-analysis-enabled=true max-legal-layouts=32 insert-memreconfig=max_pool2d_17.dc.max_pool2d.2=0 override-output-layout=max_pool2d_17.dc.max_pool2d.2=dram:interleaved:row_major:8x8:bf16" -o resnet50_first_module_ttnn.mlir %s --mlir-print-debuginfo
// RUN: FileCheck %s --input-file=resnet50_first_module_ttnn.mlir
// RUN: ttmlir-translate --ttnn-to-flatbuffer resnet50_first_module_ttnn.mlir > %t.ttnn

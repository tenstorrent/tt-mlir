// REQUIRES: opmodel
// RUN: ttmlir-opt --ttir-to-ttnn-backend-pipeline="enable-optimizer=true memory-layout-analysis-enabled=true memreconfig-enabled=true insert-memreconfig=relu=0 override-output-layout=relu=tile row-major-enabled=true" -o shard_transpose.mlir %s
...
module attributes {} {
  func.func @main(%arg0: tensor<1x3x224x224xf32>) -> tensor<1x224x3x224xf32> {
    ...
    // CHECK: #[[LAYOUT_HS:.*]] = #ttnn.ttnn_layout{{.*}}<62x1{{.*}}memref<11x224xf32, #l1_>, <height_sharded>>

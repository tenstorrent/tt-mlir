...
#ttnn_layout = #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<2x4x!ttcore.tile<32x32, f32>, #ttnn.buffer_type<dram>>, <interleaved>, exactGrid = true>
#vgm_inv = affine_map<(d0, d1) -> (0, d0 - 2, d1)>
#vgm_fwd = affine_map<(d0, d1) -> (d0 + 2, d1)>
module  {
  // CHECK-LABEL: func.func @single_exp
  // CHECK: "ttnn.generic"
  func.func @single_exp(%shared_tensor: tensor<64x128xf32, #ttnn_layout>) -> (tensor<64x128xf32, #ttnn_layout>, tensor<64x128xf32, #ttnn_layout>) attributes {tt.function_type = "forward_device"} {
    ...
    %result_reg_0_metal, %result_reg_1_metal = d2m.spatial {grid_ranges = #ttcore.core_range_set<[#ttcore.core_range<(0, 0), (1, 1)>, #ttcore.core_range<(2, 0), (3, 1)>]>}
    ...
    outs(%out_tensor0_metal, %out_tensor1_metal : tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>, tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>) {
      ^region0():
        %result_reg_0 = d2m.generic {block_factors = [1, 1], grid = #ttcore.grid<1x1>, indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = [#ttcore.iterator_type<parallel>, #ttcore.iterator_type<parallel>], threads = [#d2m.thread<unified>]}
        ...
        ^unified0(%cb0: !d2m.cb<tensor<2x4x!ttcore.tile<32x32, f32>>>, %cb1: !d2m.cb<tensor<2x4x!ttcore.tile<32x32, f32>>>):
          %block0_0 = d2m.block_index(0) : index
          %block0_1 = d2m.block_index(1) : index
          %buf_0_2 = tensor.empty() : tensor<2x4x!ttcore.tile<32x32, f32>>
          %reg_0_2 = d2m.remote_load %buf_0_2 %shared_tensor_metal_0[%block0_0, %block0_1] : tensor<2x4x!ttcore.tile<32x32, f32>>, tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout> -> tensor<2x4x!ttcore.tile<32x32, f32>>
          %reg_0_3 = tensor.empty() : tensor<2x4x!ttcore.tile<32x32, f32>>
          %reg_0_4 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%reg_0_2 : tensor<2x4x!ttcore.tile<32x32, f32>>) outs(%reg_0_3 : tensor<2x4x!ttcore.tile<32x32, f32>>) {
          ...
          %block0_0_2 = d2m.block_index(0) : index
          %block0_1_3 = d2m.block_index(1) : index
          %out_0 = d2m.remote_store %out_tensor0_metal[%block0_0_2, %block0_1_3] %reg_0_4 : tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>, tensor<2x4x!ttcore.tile<32x32, f32>> -> tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>
          d2m.yield %out_0 : (tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>)
    ...
    {
      ^region1():
        %result_reg_1 = d2m.generic {block_factors = [1, 1], grid = #ttcore.grid<1x1>, indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = [#ttcore.iterator_type<parallel>, #ttcore.iterator_type<parallel>], threads = [#d2m.thread<unified>]}
        ...
        ^unified0(%cb0: !d2m.cb<tensor<2x4x!ttcore.tile<32x32, f32>>>, %cb1: !d2m.cb<tensor<2x4x!ttcore.tile<32x32, f32>>>):
          %block1_0 = d2m.block_index(0) : index
          %block1_1 = d2m.block_index(1) : index
          %buf_1_2 = tensor.empty() : tensor<2x4x!ttcore.tile<32x32, f32>>
          %reg_1_2 = d2m.remote_load %buf_1_2 %shared_tensor_metal_1[%block1_0, %block1_1] : tensor<2x4x!ttcore.tile<32x32, f32>>, tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout> -> tensor<2x4x!ttcore.tile<32x32, f32>>
          %reg_1_3 = tensor.empty() : tensor<2x4x!ttcore.tile<32x32, f32>>
          %reg_1_4 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%reg_1_2 : tensor<2x4x!ttcore.tile<32x32, f32>>) outs(%reg_1_3 : tensor<2x4x!ttcore.tile<32x32, f32>>) {
          ^bb0(%in: !ttcore.tile<32x32, f32>, %out: !ttcore.tile<32x32, f32>):
            %reg_1_5 = "d2m.tile_exp"(%in) : (!ttcore.tile<32x32, f32>) -> !ttcore.tile<32x32, f32>
            linalg.yield %reg_1_5 : !ttcore.tile<32x32, f32>
          } -> tensor<2x4x!ttcore.tile<32x32, f32>>
          %block1_0_2 = d2m.block_index(0) : index
          %block1_1_3 = d2m.block_index(1) : index
          %out_1 = d2m.remote_store %out_tensor1_metal[%block1_0_2, %block1_1_3] %reg_1_4 : tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>, tensor<2x4x!ttcore.tile<32x32, f32>> -> tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>
          d2m.yield %out_1 : (tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>)
        } : tensor<1x1x2x4x!ttcore.tile<32x32, f32>, #ttmetal_layout>

...
module {
  func.func @ozeros_2d() -> tensor<32x128xbf16> {
    // CHECK: {{.*}} = "ttnn.zeros"({{.*}}) {{.*}}
    %0 = "ttir.zeros"() <{shape = array<i32:32, 128>, dtype = bf16}> : () -> tensor<32x128xbf16>
    return %0 : tensor<32x128xbf16>
  }

  func.func @zeros_3d() -> tensor<32x64x128xbf16> {
    // CHECK: {{.*}} = "ttnn.zeros"({{.*}}) {{.*}}
    %0 = "ttir.zeros"() <{shape = array<i32:32, 64, 128>, dtype = bf16}> : () -> tensor<32x64x128xbf16>
  ...
  func.func @zeros_f32() -> tensor<32x64x128xf32> {
    ...
    %0 = "ttir.zeros"() <{shape = array<i32:32, 64, 128>, dtype = f32}> : () -> tensor<32x64x128xf32>

...
#l1_height_sharded_batch_8 = #ttnn.ttnn_layout<(d0, d1, d2, d3) -> (d0 * 256 + d1 * 32 + d2, d3), <8x1>, memref<1x1x!ttcore.tile<32x32, bf16>, #l1>, <height_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0,0), (7,0)>]>>
...
module {
  ...
  func.func @forward_with_batch_slice(%input: tensor<1x1x32x3072xbf16, #dram_interleaved_encoding_in>) -> (tensor<1x8x32x32xbf16, #l1_height_sharded_batch_8>, tensor<1x8x32x32xbf16, #l1_height_sharded_batch_8>, tensor<1x8x32x32xbf16, #l1_height_sharded_batch_8>) {
    ...
    %1 = "ttnn.constant"(%0) <{layout = #ttnn.layout<tile>, value = dense<[8]> : tensor<1xui32>}> : (!ttnn.device) -> tensor<1xui32, #dram_interleaved_encoding_in_unary_tensor>

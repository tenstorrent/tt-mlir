// RUN: ttmlir-opt --convert-ttnn-to-ttir --ttir-to-ttmetal-pipeline="system-desc-path=%system_desc_path% ttnn-mode=true dst-allocation-strategy=legacy" %s | FileCheck %s --check-prefixes=COMMON,LEGACY | tee %t.mlir
// RUN: ttmlir-opt --convert-ttnn-to-ttir --ttir-to-ttmetal-pipeline="system-desc-path=%system_desc_path% ttnn-mode=true dst-allocation-strategy=graph-coloring-greedy" %s | FileCheck %s --check-prefixes=COMMON,GREEDY | tee %t.mlir
// RUN: ttmlir-opt --convert-ttnn-to-ttir --ttir-to-ttmetal-pipeline="system-desc-path=%system_desc_path% ttnn-mode=true dst-allocation-strategy=graph-coloring-cb" %s | FileCheck %s --check-prefixes=COMMON,CB | tee %t.mlir
...
module {
  ...
  // COMMON-LABEL: func.func @test_composite_fuse
  ...
  // COMMON-LABEL: func.func @test_composite_no_fuse

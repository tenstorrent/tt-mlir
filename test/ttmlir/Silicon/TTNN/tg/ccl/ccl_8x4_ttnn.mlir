// RUN: ttmlir-opt --ttcore-register-device="system-desc-path=%system_desc_path% mesh-shape=2,4" -o %t.ttnn.mlir %s
// RUN: ttmlir-translate --ttnn-to-flatbuffer %-o %t.ttnn t.ttnn.mlir
#dram = #ttnn.buffer_type<dram>
#system_memory = #ttnn.buffer_type<system_memory>
#ttnn_layout1 = #ttnn.ttnn_layout<(d0, d1, d2, d3) -> (d0 * 1024 + d1 * 1024 + d2, d3), <1x1>, memref<1024x512xf32, #system_memory>>
...
module @point_to_point_test attributes {} {
  func.func public @point_to_point_provide_output_tensor(%arg0: tensor<1x1x1024x512xf32, #ttnn_layout1>) -> (tensor<1x1x1024x512xf32, #ttnn_layout1> {}) {
    %0 = "ttnn.get_device"() <{mesh_offset = #ttnn<mesh_offset 0x0>, mesh_shape = #ttnn<mesh_shape 8x4>}> : () -> !ttnn.device
    %1 = "ttnn.mesh_shard"(%arg0, %0) <{shard_dims = array<i64: 2, 3>, shard_direction = #ttcore.shard_direction<full_to_shard>, shard_shape = array<i64: 1, 1, 8, 4>, shard_type = #ttcore.shard_type<devices>}> : (tensor<1x1x1024x512xf32, #ttnn_layout1>, !ttnn.device) -> tensor<1x1x128x128xf32, #ttnn_layout2>
    "ttnn.deallocate"(%arg0) <{force = false}> : (tensor<1x1x1024x512xf32, #ttnn_layout1>) -> ()
    ...
    %9 = "ttnn.mesh_shard"(%8, %0) <{shard_dims = array<i64: 2, 3>, shard_direction = #ttcore.shard_direction<shard_to_full>, shard_shape = array<i64: 1, 1, 8, 4>, shard_type = #ttcore.shard_type<devices>}> : (tensor<1x1x128x128xf32, #ttnn_layout2>, !ttnn.device) -> tensor<1x1x1024x512xf32, #ttnn_layout1>
    "ttnn.deallocate"(%8) <{force = false}> : (tensor<1x1x128x128xf32, #ttnn_layout2>) -> ()
    return %9 : tensor<1x1x1024x512xf32, #ttnn_layout1>
  }
  func.func public @point_to_point_case_generate_output_tensor(%arg0: tensor<1x1x1024x512xf32, #ttnn_layout1>) -> (tensor<1x1x1024x512xf32, #ttnn_layout1> {}) {
    %0 = "ttnn.get_device"() <{mesh_offset = #ttnn<mesh_offset 0x0>, mesh_shape = #ttnn<mesh_shape 8x4>}> : () -> !ttnn.device
    %1 = "ttnn.mesh_shard"(%arg0, %0) <{shard_dims = array<i64: 2, 3>, shard_direction = #ttcore.shard_direction<full_to_shard>, shard_shape = array<i64: 1, 1, 8, 4>, shard_type = #ttcore.shard_type<devices>}> : (tensor<1x1x1024x512xf32, #ttnn_layout1>, !ttnn.device) -> tensor<1x1x128x128xf32, #ttnn_layout2>
    "ttnn.deallocate"(%arg0) <{force = false}> : (tensor<1x1x1024x512xf32, #ttnn_layout1>) -> ()
    %2 = "ttnn.to_layout"(%1) <{layout = #ttnn.layout<tile>}> : (tensor<1x1x128x128xf32, #ttnn_layout2>) -> tensor<1x1x128x128xf32, #ttnn_layout3>

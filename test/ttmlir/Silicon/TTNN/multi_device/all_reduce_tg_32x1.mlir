// RUN: ttmlir-opt --ttir-to-ttnn-backend-pipeline="system-desc-path=%system_desc_path% mesh-shape=32,1" %s > %t.mlir
...
module @jit_fwd_four attributes {mhlo.num_partitions = 32 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<8192x784xf32>, %arg1: tensor<784x2048xf32>, %arg2: tensor<2048xf32>, %arg3: tensor<2048x1024xf32>, %arg4: tensor<1024xf32>) -> (tensor<8192x1024xf32> {jax.result_info = ""}) {
    %0 = tensor.empty() : tensor<256x784xf32>
    %1 = "ttir.mesh_shard"(%arg0, %0) <{shard_direction = #tt.shard_direction<full_to_shard>, shard_shape = #tt.grid<32x1>, shard_type = #tt.shard_type<devices>}> : (tensor<8192x784xf32>, tensor<256x784xf32>) -> tensor<256x784xf32>
    ...
    %12 = "ttir.mesh_shard"(%10, %11) <{shard_direction = #tt.shard_direction<shard_to_full>, shard_shape = #tt.grid<32x1>, shard_type = #tt.shard_type<devices>}> : (tensor<256x1024xf32>, tensor<8192x1024xf32>) -> tensor<8192x1024xf32>
    // CHECK: %[[C:.*]] = "ttnn.mesh_shard"[[C:.*]]
  ...
  func.func private @shmap_body(%arg0: tensor<256x784xf32>, %arg1: tensor<784x2048xf32>, %arg2: tensor<2048xf32>, %arg3: tensor<2048x1024xf32>, %arg4: tensor<1024xf32>) -> (tensor<256x1024xf32> {jax.result_info = "[('batch',), None]"}) {
    %0 = "ttir.dot_general"(%arg0, %arg1) <{batch_dims_lhs = array<i64>, batch_dims_rhs = array<i64>, contract_dims_lhs = array<i64: 1>, contract_dims_rhs = array<i64: 0>}> : (tensor<256x784xf32>, tensor<784x2048xf32>) -> tensor<256x2048xf32>
    ...
    %6 = "ttir.broadcast"(%4, %5) <{broadcast_dimensions = array<i32: 256, 1>}> : (tensor<1x2048xf32>, tensor<256x2048xf32>) -> tensor<256x2048xf32>
    %7 = tensor.empty() : tensor<256x2048xf32>
    %8 = "ttir.add"(%0, %6, %7) <{operandSegmentSizes = array<i32: 2, 1>}> : (tensor<256x2048xf32>, tensor<256x2048xf32>, tensor<256x2048xf32>) -> tensor<256x2048xf32>
    %9 = "ttir.dot_general"(%8, %arg3) <{batch_dims_lhs = array<i64>, batch_dims_rhs = array<i64>, contract_dims_lhs = array<i64: 1>, contract_dims_rhs = array<i64: 0>}> : (tensor<256x2048xf32>, tensor<2048x1024xf32>) -> tensor<256x1024xf32>
    ...
    %14 = tensor.empty() : tensor<256x1024xf32>
    %15 = "ttir.broadcast"(%13, %14) <{broadcast_dimensions = array<i32: 256, 1>}> : (tensor<1x1024xf32>, tensor<256x1024xf32>) -> tensor<256x1024xf32>
    %16 = tensor.empty() : tensor<256x1024xf32>
    %17 = "ttir.add"(%9, %15, %16) <{operandSegmentSizes = array<i32: 2, 1>}> : (tensor<256x1024xf32>, tensor<256x1024xf32>, tensor<256x1024xf32>) -> tensor<256x1024xf32>
    return %17 : tensor<256x1024xf32>
  }
}

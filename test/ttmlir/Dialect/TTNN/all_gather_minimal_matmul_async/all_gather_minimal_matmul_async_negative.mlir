...
// Negative unit tests for the ttnn.all_gather_minimal_matmul_async verifier.
// Verifier only inspects operand shapes, so plain (unencoded) tensors suffice.
...
module {
  func.func @wrong_semaphore_count(%input: tensor<32x128xbf16>, %weight: tensor<128x64xbf16>) -> tensor<32x64xbf16> {
    ...
    // CHECK: error: 'ttnn.all_gather_minimal_matmul_async' op expects exactly two multi-device global semaphores, got 1
    %0 = "ttnn.all_gather_minimal_matmul_async"(%input, %weight, %sem, %device) <{
      cluster_axis = 1 : ui32,
...
module {
  func.func @addcmul_only_one(%input: tensor<32x128xbf16>, %weight: tensor<128x64xbf16>, %res: tensor<32x64xbf16>) -> tensor<32x64xbf16> {
    %device = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    // CHECK: error: 'ttnn.all_gather_minimal_matmul_async' op addcmul_input1 and addcmul_input2 must both be present or both absent
    %0 = "ttnn.all_gather_minimal_matmul_async"(%input, %weight, %res, %device) <{
      cluster_axis = 1 : ui32,
...
module {
  func.func @bias_wrong_width(%input: tensor<32x128xbf16>, %weight: tensor<128x64xbf16>, %bias: tensor<1x32xbf16>) -> tensor<32x64xbf16> {
    %device = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    // CHECK: error: 'ttnn.all_gather_minimal_matmul_async' op bias last dimension (32) must match weight's last dimension N (64)
    %0 = "ttnn.all_gather_minimal_matmul_async"(%input, %weight, %bias, %device) <{
      cluster_axis = 1 : ui32,
...
module {
  func.func @input_rank_too_low(%input: tensor<128xbf16>, %weight: tensor<128x64xbf16>) -> tensor<32x64xbf16> {
    %device = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    // CHECK: error: 'ttnn.all_gather_minimal_matmul_async' op input tensor must have rank >= 2

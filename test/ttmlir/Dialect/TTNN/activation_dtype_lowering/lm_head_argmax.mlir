...
// LM-head + argmax: matmul -> reshape -> all_gather (dim 0, expanding) ->
// sum (dim 0, collapsing) -> reshape -> all_gather (dim 2) -> argmax.
// The matmul's result encoding is rewritten to bfp_bf8, propagating through
// view / CCL / sum. A ttnn.to_layout(RowMajor, bf16) is inserted before
// argmax; that op lowers (via the DecomposeLayouts bfp_bf8 -> bf16 fast
// path) to a single tt-metal untilize kernel that does the bfp_bf8 -> bf16
// upcast inline as part of de-tiling.
...
module attributes {} {
  ...
  func.func @lm_head_argmax(
    ...
    // The pass inserts ttnn.to_layout(RowMajor, bf16) immediately before
    // argmax (no separate ttnn.typecast). The DecomposeLayouts fast-path
    // lowers this to a single tt-metal untilize kernel.
    // CHECK: %[[CAST:.*]] = "ttnn.to_layout"
    // CHECK-SAME: bf16

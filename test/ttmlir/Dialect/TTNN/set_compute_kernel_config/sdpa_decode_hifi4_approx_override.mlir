...
func.func @test_sdpa_decode_hifi4_forces_approx_off(%query: tensor<1x32x8x128xbf16>, %key: tensor<32x1x2048x128xbf16>, %value: tensor<32x1x2048x128xbf16>) -> tensor<1x32x8x128xbf16> {
  // CHECK: ttnn.scaled_dot_product_attention_decode
  // CHECK-SAME: compute_config = #ttnn.device_compute_kernel_config<math_fidelity = hifi4, math_approx_mode = false, fp32_dest_acc_en = true>
  // CHECK-SAME: program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = <8, 8>, q_chunk_size = 128, k_chunk_size = 128, exp_approx_mode = false>

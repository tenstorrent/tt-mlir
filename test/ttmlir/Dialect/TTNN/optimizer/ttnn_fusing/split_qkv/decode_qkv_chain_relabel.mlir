...
// Decode head split where Q has a partial-RoPE-shaped chain (norm feeding two
// head_dim slices recombined by a concat) between the split and the relabel.
// The relabel must sink through concat(dim3) + slice(dim3) + multi-use rms_norm
// so all three outputs reach a relabel and the decode head op forms.
module {
  // CHECK-LABEL: func.func @decode_qkv_chain_relabel
  // CHECK: "ttnn.nlp_create_qkv_heads_decode"
  // CHECK-NOT: "ttnn.split_query_key_value_and_split_heads"
  func.func @decode_qkv_chain_relabel(
    ...
    // Q: norm -> {slice[0:64], slice[64:128]} -> concat(swapped) -> relabel
    ...
    %qa = "ttir.slice_static"(%qn0) <{begins = [0 : i32, 0 : i32, 0 : i32, 0 : i32], ends = [16 : i32, 12 : i32, 1 : i32, 64 : i32], step = [1 : i32, 1 : i32, 1 : i32, 1 : i32]}> : (tensor<16x12x1x128xbf16>) -> tensor<16x12x1x64xbf16>
    %qb = "ttir.slice_static"(%qn0) <{begins = [0 : i32, 0 : i32, 0 : i32, 64 : i32], ends = [16 : i32, 12 : i32, 1 : i32, 128 : i32], step = [1 : i32, 1 : i32, 1 : i32, 1 : i32]}> : (tensor<16x12x1x128xbf16>) -> tensor<16x12x1x64xbf16>
    %qc = "ttir.concat"(%qb, %qa) <{dim = 3 : si32}> : (tensor<16x12x1x64xbf16>, tensor<16x12x1x64xbf16>) -> tensor<16x12x1x128xbf16>
    %qd = "ttir.permute"(%qc) <{permutation = array<i64: 2, 0, 1, 3>}> : (tensor<16x12x1x128xbf16>) -> tensor<1x16x12x128xbf16>
    // K: norm -> relabel

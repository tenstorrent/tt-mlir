...
module {
  func.func @forward(%arg0: tensor<1x32x32x64xbf16>, %arg1: tensor<64x64x3x3xbf16>, %arg2: tensor<1x1x1x64xbf16>) -> tensor<1x32x32x64xbf16> {
    ...
    // CHECK: %{{.*}} = "ttnn.conv2d"{{.*}} conv2d_config = #ttnn.conv2d_config<dtype = #tt.supportedDataTypes<bf16>, weights_dtype = #tt.supportedDataTypes<bf16>, activation = "relu", input_channels_alignment = 32 : ui32, deallocate_activation = false, reallocate_halo_output = true, act_block_h_override = 0 : ui32, act_block_w_div = 1 : ui32, reshard_if_not_optimal = false, override_sharding_config = false, transpose_shards = true, output_layout = #ttnn.layout<row_major>, preprocess_weights_on_device = false, always_preprocess_weights = false, enable_act_double_buffer = false, enable_weights_double_buffer = false, enable_split_reader = false, enable_subblock_padding = false>{{.*}}

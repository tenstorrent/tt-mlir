...
module @test_sparse_matmul_tile_dims attributes {} {
  ...
  // Down: preserve legacy [BD, S, E, *] factoring when sparse_matmul is
  // wrapped by frontend flatten/restore reshapes.
  func.func public @sparse_matmul_down_untiled_preserve_factoring(
    %input: tensor<2x128x4x2880xbf16>,
    %weight: tensor<1x4x2880x2880xbf16>,
    %sparsity: tensor<1x1x8x4xbf16>
  ) -> tensor<2x128x4x2880xbf16> {
    // CHECK-LABEL: func.func public @sparse_matmul_down_untiled_preserve_factoring
    // CHECK: "ttnn.sparse_matmul"
    // CHECK-SAME: tensor<8x4x32x2880xbf16
    // CHECK-SAME: tensor<1x4x2880x2880xbf16
    // CHECK-SAME: -> tensor<8x4x32x2880xbf16
    // CHECK: "ttnn.reshape"(%{{.*}}) <{shape = [2 : i32, 128 : i32, 4 : i32, 2880 : i32]}>
    // CHECK-NOT: shape = [256 : i32, 4 : i32, 1 : i32, 2880 : i32]
    %flat = "ttnn.reshape"(%input) <{shape = [256 : i32, 4 : i32, 1 : i32, 2880 : i32]}>
      : (tensor<2x128x4x2880xbf16>) -> tensor<256x4x1x2880xbf16>
    %mm = "ttnn.sparse_matmul"(%flat, %weight, %sparsity) <{
      is_input_a_sparse = true,
      is_input_b_sparse = false,
      nnz = 0 : i64
    }> : (tensor<256x4x1x2880xbf16>, tensor<1x4x2880x2880xbf16>, tensor<1x1x8x4xbf16>) -> tensor<256x4x1x2880xbf16>
    %restored = "ttnn.reshape"(%mm) <{shape = [2 : i32, 128 : i32, 4 : i32, 2880 : i32]}>
      : (tensor<256x4x1x2880xbf16>) -> tensor<2x128x4x2880xbf16>
    return %restored : tensor<2x128x4x2880xbf16>
  }

...
// Test: Non-(1,1,32,M) shape should be decomposed into primitive ops.
module @test_distributed_rms_norm_decomposition attributes {} {
  ...
  func.func public @test_no_decompose_supported_shape(
    ...
    // The (1,1,32,M) shape should NOT be decomposed — it stays as distributed_rms_norm.
  ...
  func.func public @test_decompose_with_residual(
    ...
    // With residual: first op should be add(input, residual)
    // CHECK: "ttnn.add"
    // CHECK: "ttnn.multiply"
    // CHECK: "ttnn.mean"
    // CHECK: "ttnn.all_gather"

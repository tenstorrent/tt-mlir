...
#ttnn_layout = #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<1x32xi32, #system_memory>>
...
#ttnn_layout6 = #ttnn.ttnn_layout<(d0, d1, d2, d3) -> (d0 * 2048 + d1 * 64 + d2, d3), <1x1>, memref<2048x64xi32, #system_memory>>
...
module @moreh_cumsum attributes {tt.device = #device, tt.system_desc = #system_desc} {
  func.func public @test_cumsum_layout_reshape(%arg0: tensor<1x32xi32, #ttnn_layout>) -> tensor<1x32xi32, #ttnn_layout> {
    // CHECK-LABEL: func.func public @test_cumsum_layout_reshape(
    %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x1>}> : () -> !tt.device<#device>
    %1 = "ttnn.to_layout"(%arg0) <{layout = #ttnn.layout<tile>}> : (tensor<1x32xi32, #ttnn_layout>) -> tensor<1x32xi32, #ttnn_layout1>
    %2 = "ttnn.to_device"(%1, %0) <{memory_config = #ttnn.memory_config<#dram, <<1x1>>, <interleaved>>}> : (tensor<1x32xi32, #ttnn_layout1>, !tt.device<#device>) -> tensor<1x32xi32, #ttnn_layout2>
    "ttnn.deallocate"(%1) <{force = false}> : (tensor<1x32xi32, #ttnn_layout1>) -> ()
    %3 = "ttnn.empty"(%0) <{dtype = #tt.supportedDataTypes<u32>, layout = #ttnn.layout<tile>, memory_config = #ttnn.memory_config<#dram, <<1x1>>, <interleaved>>, shape = #ttnn.shape<1x32>}> : (!tt.device<#device>) -> tensor<1x32xi32, #ttnn_layout2>
    // CHECK: %[[RESHAPE:[0-9]+]] = "ttnn.reshape"
    // CHECK-SAME: {shape = [1 : i32, 32 : i32, 1 : i32, 1 : i32]}
    // CHECK-SAME: tensor<1x32xi32
    // CHECK-SAME: -> tensor<1x32x1x1xi32
    ...
    // CHECK-SAME: tensor<1x32x1x1xf32
    // CHECK-SAME: -> tensor<1x32x1x1xf32
    %4 = "ttnn.moreh_cumsum"(%2, %3) <{dim = 0 : i64}> : (tensor<1x32xi32, #ttnn_layout2>, tensor<1x32xi32, #ttnn_layout2>) -> tensor<1x32xi32, #ttnn_layout2>
    ...
    // CHECK-SAME: tensor<1x32x1x1xi32
    // CHECK-SAME: -> tensor<1x32xi32
    "ttnn.deallocate"(%2) <{force = false}> : (tensor<1x32xi32, #ttnn_layout2>) -> ()
    %5 = "ttnn.from_device"(%4) : (tensor<1x32xi32, #ttnn_layout2>) -> tensor<1x32xi32, #ttnn_layout1>
    "ttnn.deallocate"(%3) <{force = false}> : (tensor<1x32xi32, #ttnn_layout2>) -> ()
    %6 = "ttnn.to_layout"(%5) <{layout = #ttnn.layout<row_major>}> : (tensor<1x32xi32, #ttnn_layout1>) -> tensor<1x32xi32, #ttnn_layout>
    "ttnn.deallocate"(%5) <{force = false}> : (tensor<1x32xi32, #ttnn_layout1>) -> ()
    return %6 : tensor<1x32xi32, #ttnn_layout>
  }
  ...
  func.func public @test_cumsum_layout(%arg0: tensor<1x32x64x64xi32, #ttnn_layout6>) -> tensor<1x32x64x64xi32, #ttnn_layout6> {
    // CHECK-LABEL: func.func public @test_cumsum_layout(
    %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x1>}> : () -> !tt.device<#device>
    %1 = "ttnn.to_layout"(%arg0) <{layout = #ttnn.layout<tile>}> : (tensor<1x32x64x64xi32, #ttnn_layout6>) -> tensor<1x32x64x64xi32, #ttnn_layout7>
    %2 = "ttnn.to_device"(%1, %0) <{memory_config = #ttnn.memory_config<#dram, <<64x2>>, <interleaved>>}> : (tensor<1x32x64x64xi32, #ttnn_layout7>, !tt.device<#device>) -> tensor<1x32x64x64xi32, #ttnn_layout8>
    "ttnn.deallocate"(%1) <{force = false}> : (tensor<1x32x64x64xi32, #ttnn_layout7>) -> ()
    // CHECK: %[[EMPTY:[0-9]]] = "ttnn.empty"
    // CHECK-SAME: -> tensor<1x32x64x64xi32
    %3 = "ttnn.empty"(%0) <{dtype = #tt.supportedDataTypes<u32>, layout = #ttnn.layout<tile>, memory_config = #ttnn.memory_config<#dram, <<64x2>>, <interleaved>>, shape = #ttnn.shape<1x32x64x64>}> : (!tt.device<#device>) -> tensor<1x32x64x64xi32, #ttnn_layout8>
    ...
    // CHECK-SAME: tensor<1x32x64x64xf32
    // CHECK-SAME: -> tensor<1x32x64x64xf32
    %4 = "ttnn.moreh_cumsum"(%2, %3) <{dim = 1 : i64}> : (tensor<1x32x64x64xi32, #ttnn_layout8>, tensor<1x32x64x64xi32, #ttnn_layout8>) -> tensor<1x32x64x64xi32, #ttnn_layout8>
    // CHECK: "ttnn.to_layout"(%[[CUMSUM]],
    // CHECK-SAME: dtype = #tt.supportedDataTypes<u32
    // CHECK-SAME: tensor<1x32x64x64xf32
    // CHECK-SAME: -> tensor<1x32x64x64xi32
    "ttnn.deallocate"(%2) <{force = false}> : (tensor<1x32x64x64xi32, #ttnn_layout8>) -> ()
    %5 = "ttnn.from_device"(%4) : (tensor<1x32x64x64xi32, #ttnn_layout8>) -> tensor<1x32x64x64xi32, #ttnn_layout7>
    "ttnn.deallocate"(%3) <{force = false}> : (tensor<1x32x64x64xi32, #ttnn_layout8>) -> ()
    %6 = "ttnn.to_layout"(%5) <{layout = #ttnn.layout<row_major>}> : (tensor<1x32x64x64xi32, #ttnn_layout7>) -> tensor<1x32x64x64xi32, #ttnn_layout6>
    "ttnn.deallocate"(%5) <{force = false}> : (tensor<1x32x64x64xi32, #ttnn_layout7>) -> ()
    return %6 : tensor<1x32x64x64xi32, #ttnn_layout6>
  }
}

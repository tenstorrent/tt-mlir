...
module @test_distributed_layer_norm_decomposition attributes {} {
  func.func public @test_decompose_basic(
    ...
    %1 = "ttnn.distributed_layer_norm"(%arg0, %0) <{cluster_axis = 1 : ui32, epsilon = 1.000000e-05 : f32, operandSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0, 1>}> : (tensor<1x1x64x128xbf16, #ttnn_layout>, !ttnn.device) -> tensor<1x1x64x128xbf16, #ttnn_layout>
  ...
  func.func public @test_decompose_with_weight_and_bias(
    ...
    // CHECK-NOT: "ttnn.distributed_layer_norm"
    %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    %1 = "ttnn.distributed_layer_norm"(%arg0, %arg1, %arg2, %0) <{cluster_axis = 1 : ui32, epsilon = 1.000000e-05 : f32, operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1>}> : (tensor<1x1x64x128xbf16, #ttnn_layout>, tensor<128xbf16, #ttnn_layout_weight>, tensor<128xbf16, #ttnn_layout_weight>, !ttnn.device) -> tensor<1x1x64x128xbf16, #ttnn_layout>
    return %1 : tensor<1x1x64x128xbf16, #ttnn_layout>
  }
  ...
  func.func public @test_decompose_with_residual(
    ...
    // CHECK-NOT: "ttnn.distributed_layer_norm"
    %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    %1 = "ttnn.distributed_layer_norm"(%arg0, %arg1, %0) <{cluster_axis = 1 : ui32, epsilon = 1.000000e-05 : f32, operandSegmentSizes = array<i32: 1, 0, 0, 1, 0, 0, 1>}> : (tensor<1x1x64x128xbf16, #ttnn_layout>, tensor<1x1x64x128xbf16, #ttnn_layout>, !ttnn.device) -> tensor<1x1x64x128xbf16, #ttnn_layout>
    return %1 : tensor<1x1x64x128xbf16, #ttnn_layout>
  }
  ...
  func.func public @test_reshape_rank3_to_pre_all_gather_shape(
    ...
    %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    %1 = "ttnn.distributed_layer_norm"(%arg0, %0) <{cluster_axis = 1 : ui32, epsilon = 1.000000e-05 : f32, operandSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0, 1>}> : (tensor<1x64x128xbf16, #ttnn_layout_rank3>, !ttnn.device) -> tensor<1x64x128xbf16, #ttnn_layout_rank3>
  ...
  func.func public @test_reshape_rank3_with_weight_and_bias(
    ...
    // Rank-3 AdaLN form keeps distributed_layer_norm for the DiT fused kernel.
    // Activation is promoted to [1,1,N,H]; 1D gamma/beta become [1,H].
    // CHECK-NOT: "ttnn.layer_norm_pre_all_gather"
    // CHECK-NOT: "ttnn.layer_norm_post_all_gather"
    // CHECK: "ttnn.reshape"
    // CHECK-SAME: shape = [1 : i32, 1 : i32, 64 : i32, 128 : i32]
    // CHECK: "ttnn.reshape"
    // CHECK-SAME: shape = [1 : i32, 128 : i32]
    // CHECK: "ttnn.reshape"
    // CHECK-SAME: shape = [1 : i32, 128 : i32]
    // CHECK: "ttnn.distributed_layer_norm"
    // CHECK: "ttnn.reshape"({{.*}}) <{shape = [1 : i32, 64 : i32, 128 : i32]}>
    %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x2>}> : () -> !ttnn.device
    %1 = "ttnn.distributed_layer_norm"(%arg0, %arg1, %arg2, %0) <{cluster_axis = 1 : ui32, epsilon = 1.000000e-05 : f32, operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1>}> : (tensor<1x64x128xbf16, #ttnn_layout_rank3>, tensor<128xbf16, #ttnn_layout_weight>, tensor<128xbf16, #ttnn_layout_weight>, !ttnn.device) -> tensor<1x64x128xbf16, #ttnn_layout_rank3>
    return %1 : tensor<1x64x128xbf16, #ttnn_layout_rank3>
  }

  func.func public @test_reshape_rank3_f32_with_weight_and_bias(
      %arg0: tensor<1x64x128xf32, #ttnn_layout_rank3_f32>,
      %arg1: tensor<128xf32, #ttnn_layout_weight_f32>,
      %arg2: tensor<128xf32, #ttnn_layout_weight_f32>) -> tensor<1x64x128xf32, #ttnn_layout_rank3_f32> {
    // CHECK-LABEL: func.func public @test_reshape_rank3_f32_with_weight_and_bias
    // f32 AdaLN is reshaped then typecast to TILE bf16 around the fused op.
    // CHECK-NOT: "ttnn.layer_norm_pre_all_gather"
    // CHECK-NOT: "ttnn.layer_norm_post_all_gather"
    // CHECK: "ttnn.reshape"
    // CHECK-SAME: shape = [1 : i32, 1 : i32, 64 : i32, 128 : i32]

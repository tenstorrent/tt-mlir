...
// which lit doesn't do. The "missing kernel_id" / "import failed" /
// "raising resolver" paths are unit-tested separately in
// `pjrt_implementation/src/api/unit_tests/tt_lang_bridge_tests.cc`,
// which now drives this pass via the tt-mlir compiler API.
...
// it. The artifact `[1, 2, 3]` survives the pass unchanged. This
// also implicitly exercises the "no work to do" branch -- the pass
// short-circuits without trying to import tt_torch.tt_lang.
// CHECK: ttnn.tt_lang_op
// CHECK-SAME: kernel_artifact = array<i8: 1, 2, 3>
...
func.func @prebaked_kernel_artifact_is_preserved(
  ...
  %b = "ttnn.to_device"(%arg1, %device) <{memory_config = #ttnn.memory_config<#dram, <interleaved>>}> : (tensor<32x32xf32, #host>, !ttnn.device) -> tensor<32x32xf32, #dev>
  %out = "ttnn.to_device"(%arg2, %device) <{memory_config = #ttnn.memory_config<#dram, <interleaved>>}> : (tensor<32x32xf32, #host>, !ttnn.device) -> tensor<32x32xf32, #dev>
  %0 = "ttnn.tt_lang_op"(%a, %b, %out) <{
    kernel_id = "test.prebaked::v1",
    version_tag = "1.0",
    arg_roles = "in,in,out",
    shard_spec = "",
    kernel_artifact = array<i8: 1, 2, 3>

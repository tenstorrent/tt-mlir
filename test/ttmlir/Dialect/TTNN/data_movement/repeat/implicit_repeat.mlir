...
module {
  func.func @main(%arg0: tensor<1x1x32xf32>, %arg1: tensor<1x1x1xf32>) -> tensor<1x16x32xf32> {
    // CHECK: "ttnn.repeat"
    // CHECK-SAME: repeat_dims = #ttnn.shape<1x16x32>
    // CHECK: %{{[0-9]+}} = "ttnn.multiply"(%arg0, %{{[0-9]+}})
    // CHECK: %{{[0-9]+}} = "ttnn.maximum"(%arg0, %{{[0-9]+}})
    %0 = ttir.empty() : tensor<1x16x32xf32>
    ...
    %6 = ttir.empty() : tensor<1x16x32xf32>
    %7 = "ttir.maximum"(%1, %5, %6) : (tensor<1x16x32xf32>, tensor<1x16x32xf32>, tensor<1x16x32xf32>) -> tensor<1x16x32xf32>

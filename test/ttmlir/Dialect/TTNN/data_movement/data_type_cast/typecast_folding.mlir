...
module attributes {} {
    ...
    func.func @typecast_folding_consecutive_typecasts_with_multiple_uses(%arg0: tensor<64x128xf32, #ttnn_layout_device_tile_f32>) -> (tensor<64x128xbf16, #ttnn_layout_device_tile_bf16>, tensor<64x128xi32, #ttnn_layout_device_tile_si32>) {
        ...
        %0 = "ttnn.typecast"(%arg0) <{dtype = #ttcore.supportedDataTypes<si32>}> : (tensor<64x128xf32, #ttnn_layout_device_tile_f32>) -> tensor<64x128xi32, #ttnn_layout_device_tile_si32>
        %1 = "ttnn.typecast"(%0) <{dtype = #ttcore.supportedDataTypes<bf16>}> : (tensor<64x128xi32, #ttnn_layout_device_tile_si32>) -> tensor<64x128xbf16, #ttnn_layout_device_tile_bf16>
        %2 = "ttnn.add"(%0, %0) <{dtype = #ttcore.supportedDataTypes<si32>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<64x128xi32, #ttnn_layout_device_tile_si32>, tensor<64x128xi32, #ttnn_layout_device_tile_si32>) -> tensor<64x128xi32, #ttnn_layout_device_tile_si32>
        return %1, %2 : tensor<64x128xbf16, #ttnn_layout_device_tile_bf16>, tensor<64x128xi32, #ttnn_layout_device_tile_si32>
    }

    // Host memory typecast folding tests.
    // Test case to verify the folding of typecast operation on host memory.
    ...
    // Test case to verify consecutive typecast op folding.
    func.func @typecast_folding_consecutive_typecasts_host_memory(%arg0: tensor<64x128xf32, #ttnn_layout_host_rm_f32>) -> tensor<64x128xbf16, #ttnn_layout_host_rm_bf16> {
        // Verify that we fold two consecutive typecast ops into a single one.
        // CHECK: ttnn.typecast
        // CHECK-NEXT: return
        ...
        // Test case to verify that we do not fold consecutive typecast ops if the first typecast have more than a single use.
        func.func @typecast_folding_consecutive_typecasts_with_multiple_uses_host_memory(%arg0: tensor<64x128xf32, #ttnn_layout_host_rm_f32>) -> (tensor<64x128xbf16, #ttnn_layout_host_rm_bf16>, tensor<64x128xi32, #ttnn_layout_host_rm_si32>) {
        // Verify that both typecasts exists.
        // CHECK: ttnn.typecast
        // CHECK: ttnn.typecast
        %0 = "ttnn.typecast"(%arg0) <{dtype = #ttcore.supportedDataTypes<si32>}> : (tensor<64x128xf32, #ttnn_layout_host_rm_f32>) -> tensor<64x128xi32, #ttnn_layout_host_rm_si32>
        %1 = "ttnn.typecast"(%0) <{dtype = #ttcore.supportedDataTypes<bf16>}> : (tensor<64x128xi32, #ttnn_layout_host_rm_si32>) -> tensor<64x128xbf16, #ttnn_layout_host_rm_bf16>
        %2 = "ttnn.add"(%0, %0) <{dtype = #ttcore.supportedDataTypes<si32>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<64x128xi32, #ttnn_layout_host_rm_si32>, tensor<64x128xi32, #ttnn_layout_host_rm_si32>) -> tensor<64x128xi32, #ttnn_layout_host_rm_si32>

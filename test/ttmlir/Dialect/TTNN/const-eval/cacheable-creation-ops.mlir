...
module {
  ...
  func.func @standalone_full(%arg0: tensor<32x32xbf16, #ttnn_layout> {ttcore.argument_type = #ttcore.argument_type<input>}) -> tensor<32x32xbf16, #ttnn_layout> {
    ...
    // CHECK: %[[RESULT:.*]] = "ttnn.add"(%arg0, %[[CACHED]]) <{activations = [], dtype = #ttcore.supportedDataTypes<bf16>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
    %result = "ttnn.add"(%arg0, %full) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32x32xbf16, #ttnn_layout>, tensor<32x32xbf16, #ttnn_layout>) -> tensor<32x32xbf16, #ttnn_layout>
  ...
  func.func @multiple_creation_ops(%arg0: tensor<32x32xbf16, #ttnn_layout> {ttcore.argument_type = #ttcore.argument_type<input>}) -> tensor<32x32xbf16, #ttnn_layout> {
    ...
    // CHECK: %[[ADD1:.*]] = "ttnn.add"(%arg0, %[[CACHED1]]) <{activations = [], dtype = #ttcore.supportedDataTypes<bf16>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
    %add1 = "ttnn.add"(%arg0, %full1) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32x32xbf16, #ttnn_layout>, tensor<32x32xbf16, #ttnn_layout>) -> tensor<32x32xbf16, #ttnn_layout>
    // CHECK: %[[ADD2:.*]] = "ttnn.add"(%[[ADD1]], %[[CACHED2]]) <{activations = [], dtype = #ttcore.supportedDataTypes<bf16>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
    %add2 = "ttnn.add"(%add1, %full2) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32x32xbf16, #ttnn_layout>, tensor<32x32xbf16, #ttnn_layout>) -> tensor<32x32xbf16, #ttnn_layout>
  ...
  func.func @merged_with_const_eval(
    ...
    %sub = "ttnn.subtract"(%arg1, %full) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32x32xbf16, #ttnn_layout>, tensor<32x32xbf16, #ttnn_layout>) -> tensor<32x32xbf16, #ttnn_layout>
    // CHECK: %[[RESULT:.*]] = "ttnn.add"(%arg0, %[[CACHED]]) <{activations = [], dtype = #ttcore.supportedDataTypes<bf16>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
    %result = "ttnn.add"(%arg0, %sub) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32x32xbf16, #ttnn_layout>, tensor<32x32xbf16, #ttnn_layout>) -> tensor<32x32xbf16, #ttnn_layout>
    return %result : tensor<32x32xbf16, #ttnn_layout>
  }
  ...
  func.func @arange_op(%arg0: tensor<32xbf16, #ttnn_layout_1d> {ttcore.argument_type = #ttcore.argument_type<input>}) -> tensor<32xbf16, #ttnn_layout_1d> {
    ...
    // CHECK: %[[RESULT:.*]] = "ttnn.add"(%arg0, %[[CACHED]]) <{activations = [], dtype = #ttcore.supportedDataTypes<bf16>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
    %result = "ttnn.add"(%arg0, %arange) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32xbf16, #ttnn_layout_1d>, tensor<32xbf16, #ttnn_layout_1d>) -> tensor<32xbf16, #ttnn_layout_1d>
  ...
  func.func @constant_op(%arg0: tensor<32x32xbf16, #ttnn_layout> {ttcore.argument_type = #ttcore.argument_type<input>}) -> tensor<32x32xbf16, #ttnn_layout> {
    ...
    // CHECK: %[[RESULT:.*]] = "ttnn.multiply"(%arg0, %[[CACHED]]) <{activations = [], dtype = #ttcore.supportedDataTypes<bf16>, input_tensor_a_activations = [], input_tensor_b_activations = []}>
    %result = "ttnn.multiply"(%arg0, %constant) <{dtype = #ttcore.supportedDataTypes<bf16>, activations = [], input_tensor_a_activations = [], input_tensor_b_activations = []}> : (tensor<32x32xbf16, #ttnn_layout>, tensor<32x32xbf16, #ttnn_layout>) -> tensor<32x32xbf16, #ttnn_layout>
    return %result : tensor<32x32xbf16, #ttnn_layout>
  }

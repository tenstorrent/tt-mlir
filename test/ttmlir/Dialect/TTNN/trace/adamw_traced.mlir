...
// ttnn.adamw takes its step-varying scalars as single-element device tensors
// (ttml::metal::adamw_tensor_scalars reads them on device, per launch), so the
// op involves no host readback and is safe to trace: a replayed trace re-reads
// whatever the scalar tensors hold at that step. The whole training step,
// forward compute and optimizer update alike, lands inside the trace region.
...
module {
  ttcore.device_module {
    builtin.module attributes {ttcore.system_desc = #system_desc} {
      ...
      // The forward compute and the optimizer step are both hoisted.
      ...
      func.func @fwd_then_step(%param: tensor<1x1x64x64xf32, #ttnn_layout>, %grad: tensor<1x1x64x64xf32, #ttnn_layout>,
                               ...
                               %decay_factor: tensor<1xf32, #scalar_layout>) -> tensor<1x1x64x64xf32, #ttnn_layout> attributes {tt.function_type = "forward_device"} {
        // The trace call covers both ops; no adamw is left behind it in the
        // outer function.
        ...
        "ttnn.adamw"(%param, %grad, %exp_avg, %exp_avg_sq, %step_size, %inv_sqrt_bc2, %decay_factor) <{

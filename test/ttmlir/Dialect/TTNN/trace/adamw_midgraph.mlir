...
// An adamw op with hoistable compute after it. adamw cannot be traced (see
// adamw_not_traced.mlir) and cannot be sunk above the trace either, since it
// writes the parameters. So the trace region ends before it: only the compute
// ahead of the optimizer step is hoisted, and everything from the step onwards
// runs unwrapped on every step. Updating beta^t on device after the step is the
...
module {
  ttcore.device_module {
    builtin.module attributes {ttcore.system_desc = #system_desc} {
      ...
      // The trace covers the add ahead of the step, and nothing else: neither
      // the adamw nor the multiply that follows it.
      // CHECK-LABEL: func.func private @trace_0_step_then_fwd
      // CHECK: "ttnn.add"
      // CHECK-NOT: "ttnn.adamw"
      // CHECK-NOT: "ttnn.multiply"

      // CHECK-LABEL: func.func @step_then_fwd(
      func.func @step_then_fwd(%param: tensor<1x1x64x64xf32, #ttnn_layout>, %grad: tensor<1x1x64x64xf32, #ttnn_layout>,
        ...
        %add = "ttnn.add"(%param, %grad) : (tensor<1x1x64x64xf32, #ttnn_layout>, tensor<1x1x64x64xf32, #ttnn_layout>) -> tensor<1x1x64x64xf32, #ttnn_layout>
        // CHECK: "ttnn.adamw"
        ...
        // CHECK: "ttnn.multiply"
        %mul = "ttnn.multiply"(%add, %param) : (tensor<1x1x64x64xf32, #ttnn_layout>, tensor<1x1x64x64xf32, #ttnn_layout>) -> tensor<1x1x64x64xf32, #ttnn_layout>
        return %mul : tensor<1x1x64x64xf32, #ttnn_layout>
      }
    }
  }
}

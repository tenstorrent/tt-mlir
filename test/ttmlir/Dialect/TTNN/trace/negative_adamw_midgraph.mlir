...
// RUN: not ttmlir-opt --ttnn-trace-hoist-transform %s 2>&1 | FileCheck %s

// An adamw op between two hoistable compute ops cannot be left out of the trace
// region, so the pass rejects it instead of capturing a readback that would
// only ever run on the capturing step. See adamw_not_traced.mlir for the shape
// that works.
  ...
  ttcore.device_module {
    builtin.module attributes {ttcore.system_desc = #system_desc} {
      ...
      func.func @step_then_fwd(%param: tensor<1x1x64x64xf32, #ttnn_layout>, %grad: tensor<1x1x64x64xf32, #ttnn_layout>,
        ...
        // CHECK: error: {{ttnn.adamw cannot be traced}}

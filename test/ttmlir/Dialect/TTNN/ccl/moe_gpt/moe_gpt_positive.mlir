...
module attributes {} {
  ...
  // tilize_out / tilize_out_rm are internal staging buffers (see
  // moe_gpt_program_factory.cpp) — `MoeGptLayoutRewritePattern` forces them
  // to L1 HEIGHT_SHARDED on `{numWorkerCores, 1}`.  They're not part of the
  // op's public contract, so the func only returns the routing-metadata
  // outputs (token_counts, activation_records, token_indices).

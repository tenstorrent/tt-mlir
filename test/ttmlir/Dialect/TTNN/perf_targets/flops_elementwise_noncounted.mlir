...
// Boolean/comparison elementwise ops do no floating-point work and must NOT be
// counted. Only the float add is counted (262144); ttnn.eq (u32 result) is not,
// so elementwise stays 262144 rather than 524288.
...
#f = #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<16x16x!ttcore.tile<32x32, bf16>, #dram>, <interleaved>>
#b = #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<16x16x!ttcore.tile<32x32, u32>, #dram>, <interleaved>>
module attributes {ttcore.system_desc = #system_desc} {
  func.func @forward(%x: tensor<512x512xbf16, #f>, %y: tensor<512x512xbf16, #f>) -> tensor<512x512xui32, #b>
      attributes {tt.function_type = "forward_device"} {
    %add = "ttnn.add"(%x, %y) : (tensor<512x512xbf16, #f>, tensor<512x512xbf16, #f>) -> tensor<512x512xbf16, #f>
    %eq = "ttnn.eq"(%add, %y) : (tensor<512x512xbf16, #f>, tensor<512x512xbf16, #f>) -> tensor<512x512xui32, #b>
    return %eq : tensor<512x512xui32, #b>
  }
}

// CHECK:      "flops": {
// CHECK:        "flops_by_category": {
// CHECK:          "elementwise": 262144
// CHECK:        "per_op": [
// CHECK:          "operation": "ttnn.add"
// CHECK-NOT:      "operation": "ttnn.eq"

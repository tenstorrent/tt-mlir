...
// The MFU denominator on a mesh. total_flops stays per-chip (one device's SPMD
// program), and the peak is emitted both per-chip and scaled by num_chips_used so
// a consumer pairs it with a per-chip or mesh-wide numerator without re-deriving
// anything.
//
//   matmul M=2048 K=128 N=128 -> 2*M*K*N = 67,108,864 (per chip)
//   config-less matmul runs HiFi4, so effective_peak == the HiFi4 entry:
//     64 cores * 1.0 GHz * 2*32^3 / 64 = 65,536,000,000,000
//   mesh_peak = 65,536,000,000,000 * 4 = 262,144,000,000,000
...
module attributes {ttcore.system_desc = #system_desc} {
  ttcore.device @default_device = <workerGrid = #ttcore.grid<8x8, virt_to_physical_map = (d0, d1) -> (0, d0, d1), physical_to_virt_map = (d0, d1, d2) -> (d1, d2)>, dramGrid = #ttcore.grid<1x12>, l1Map = (d0, d1, d2)[s0] -> (0, d0, d1, d2 + s0), dramMap = (d0, d1, d2)[s0, s1] -> (0, 0, 0, d0 * s1 + d1 * s1 + d2 + s0), meshShape = 2x2, chipIds = [0, 1, 2, 3]>
...
// Keys within the "flops" object are emitted alphabetically.
// CHECK:      "flops": {
// CHECK:        "effective_peak_flops_per_sec": 65536000000000
// CHECK:        "mesh_peak_flops_per_sec": 262144000000000
// CHECK:        "mesh_shape": [
// CHECK-NEXT:     2,
// CHECK-NEXT:     2
// CHECK-NEXT:   ]
// CHECK:        "num_chips_used": 4
// total_flops is one chip's program, not the mesh's.

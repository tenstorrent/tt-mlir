...
// Per-op FLOP accounting for a flash-MLA prefill (causal, no sliding window).
//   Q=[B=1,Hq=16,Sq=32,Dk=128], K=[..,Sk=32,..], head_dim_v(Dv)=64.
//   flops = 2*B*Hq*Sq*Sk*(Dk+Dv) / 2 (causal)
...
#q = #ttnn.ttnn_layout<(d0, d1, d2, d3) -> (d0 * 512 + d1 * 32 + d2, d3), <1x1>, memref<16x4x!ttcore.tile<32x32, bf16>, #dram>, <interleaved>>
#k = #ttnn.ttnn_layout<(d0, d1, d2, d3) -> (d0 * 32 + d1 * 32 + d2, d3), <1x1>, memref<1x4x!ttcore.tile<32x32, bf16>, #dram>, <interleaved>>
#o = #ttnn.ttnn_layout<(d0, d1, d2, d3) -> (d0 * 512 + d1 * 32 + d2, d3), <1x1>, memref<16x2x!ttcore.tile<32x32, bf16>, #dram>, <interleaved>>

module attributes {ttcore.system_desc = #system_desc} {
  func.func @forward(%query: tensor<1x16x32x128xbf16, #q>, %key: tensor<1x1x32x128xbf16, #k>) -> tensor<1x16x32x64xbf16, #o>
      attributes {tt.function_type = "forward_device"} {
    %0 = "ttnn.flash_mla_prefill"(%query, %key) <{operandSegmentSizes = array<i32: 1, 1, 0, 0>, head_dim_v = 64 : ui32, is_causal = true}> : (tensor<1x16x32x128xbf16, #q>, tensor<1x1x32x128xbf16, #k>) -> tensor<1x16x32x64xbf16, #o>
    return %0 : tensor<1x16x32x64xbf16, #o>
  }
}

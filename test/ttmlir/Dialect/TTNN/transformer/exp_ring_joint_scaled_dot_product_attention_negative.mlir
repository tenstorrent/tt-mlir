...
module attributes {} {
  func.func @ring_sdpa_gathered_kv(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x1024x128xbf16>, %v: tensor<1x40x1024x128xbf16>) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 0, 0, 0>, joint_strategy = "rear", logical_n = 1024 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x1024x128xbf16>, tensor<1x40x1024x128xbf16>) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_partial_joint(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %jq: tensor<1x40x512x128xbf16>) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %jq) <{operandSegmentSizes = array<i32: 1, 1, 1, 1, 0, 0, 0, 0, 0>, joint_strategy = "rear", logical_n = 512 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_partial_buffers(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %buf_k: tensor<1x40x1024x128xbf16>) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %buf_k) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1, 0, 0>, joint_strategy = "rear", logical_n = 1024 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x1024x128xbf16>) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_one_semaphore(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %buf_k: tensor<1x40x1024x128xbf16>, %buf_v: tensor<1x40x1024x128xbf16>, %ping: !ttnn.global_semaphore) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %buf_k, %buf_v, %ping) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1, 1, 1>, joint_strategy = "rear", logical_n = 1024 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x1024x128xbf16>, tensor<1x40x1024x128xbf16>, !ttnn.global_semaphore) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_semaphores_before_buffers(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %ping: !ttnn.global_semaphore, %pong: !ttnn.global_semaphore) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %ping, %pong) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 0, 0, 2>, joint_strategy = "rear", logical_n = 1024 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, !ttnn.global_semaphore, !ttnn.global_semaphore) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_bad_buffer_seq(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %buf_k: tensor<1x40x900x128xbf16>, %buf_v: tensor<1x40x900x128xbf16>, %ping: !ttnn.global_semaphore, %pong: !ttnn.global_semaphore) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %buf_k, %buf_v, %ping, %pong) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1, 1, 2>, joint_strategy = "rear", logical_n = 900 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x900x128xbf16>, tensor<1x40x900x128xbf16>, !ttnn.global_semaphore, !ttnn.global_semaphore) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_bad_buffer_heads(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %buf_k: tensor<1x20x1024x128xbf16>, %buf_v: tensor<1x20x1024x128xbf16>, %ping: !ttnn.global_semaphore, %pong: !ttnn.global_semaphore) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %buf_k, %buf_v, %ping, %pong) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1, 1, 2>, joint_strategy = "rear", logical_n = 1024 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x20x1024x128xbf16>, tensor<1x20x1024x128xbf16>, !ttnn.global_semaphore, !ttnn.global_semaphore) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
module attributes {} {
  func.func @ring_sdpa_logical_n_too_large(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %buf_k: tensor<1x40x1024x128xbf16>, %buf_v: tensor<1x40x1024x128xbf16>, %ping: !ttnn.global_semaphore, %pong: !ttnn.global_semaphore) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %buf_k, %buf_v, %ping, %pong) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1, 1, 2>, joint_strategy = "rear", logical_n = 2048 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x1024x128xbf16>, tensor<1x40x1024x128xbf16>, !ttnn.global_semaphore, !ttnn.global_semaphore) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
...
// logical_n may lag the gathered length by less than one shard, otherwise some
// device would hold only padding (tt-metal:
// TT_FATAL((N_global - logical_n) < N_local)).
module attributes {} {
  func.func @ring_sdpa_padding_exceeds_shard(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x40x512x128xbf16>, %v: tensor<1x40x512x128xbf16>, %buf_k: tensor<1x40x1024x128xbf16>, %buf_v: tensor<1x40x1024x128xbf16>, %ping: !ttnn.global_semaphore, %pong: !ttnn.global_semaphore) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v, %buf_k, %buf_v, %ping, %pong) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 1, 1, 2>, joint_strategy = "rear", logical_n = 512 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x512x128xbf16>, tensor<1x40x1024x128xbf16>, tensor<1x40x1024x128xbf16>, !ttnn.global_semaphore, !ttnn.global_semaphore) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
// CHECK: error: 'ttnn.exp_ring_joint_scaled_dot_product_attention' op the gap between the gathered sequence length (1024) and logical_n (512) must be smaller than the per-device sequence length (512)

// -----

// the ring kernel does not support GQA: query and kv head counts must be equal
module attributes {} {
  func.func @ring_sdpa_bad_head_ratio(%q: tensor<1x40x512x128xbf16>, %k: tensor<1x8x512x128xbf16>, %v: tensor<1x8x512x128xbf16>) -> tensor<1x40x512x128xbf16> {
    %0, %1, %2 = "ttnn.exp_ring_joint_scaled_dot_product_attention"(%q, %k, %v) <{operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 0, 0, 0>, joint_strategy = "rear", logical_n = 512 : i64, dim = 2 : si32, cluster_axis = 0 : ui32, program_config = #ttnn.sdpa_program_config<compute_with_storage_grid_size = #ttnn.core_coord<8, 8>, q_chunk_size = 128, k_chunk_size = 128>}> : (tensor<1x40x512x128xbf16>, tensor<1x8x512x128xbf16>, tensor<1x8x512x128xbf16>) -> (tensor<1x40x512x128xbf16>, tensor<1x40x0x128xbf16>, tensor<1x40x1024x1xbf16>)
    return %0 : tensor<1x40x512x128xbf16>
  }
}
// CHECK: error: 'ttnn.exp_ring_joint_scaled_dot_product_attention' op Query and key/value num heads must be equal

...
module {
  ...
  // CHECK: call @hoisted_forward_merge_const_eval_0_decl
  ...
  func.func @forward_merge(%arg0: tensor<32x32xbf16> {ttcore.argument_type = #ttcore.argument_type<input>},
    ...
    // All these ops are const-eval and merge into one subgraph
  ...
  // CHECK-LABEL: func.func private @hoisted_forward_merge_const_eval_0_decl{{.*}} -> tensor<32x32xf32

  // CHECK: ttcore.cpu_module {
  // CHECK: builtin.module {
  // Should have only ONE hoisted function (merged)
  // CHECK: llvm.func @hoisted_forward_merge_const_eval_0
}

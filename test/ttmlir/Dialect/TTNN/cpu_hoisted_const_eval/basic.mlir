...
module {
  ...
  // CHECK: call @cpu_hoisted_forward_const_eval_0_{{.*}}
  ...
  // CHECK: call @cpu_hoisted_forward_merge_return_multiple_values_const_eval_0_{{.*}}
  ...
  // CHECK-LABEL: func.func private @cpu_hoisted_forward_const_eval_0_{{.*}}{{.*}} -> tensor<32x32xf32
  // CHECK-LABEL: func.func private @cpu_hoisted_forward_merge_return_multiple_values_const_eval_0_{{.*}} -> (tensor<32x32xf32{{.*}}>, tensor<32x32xf32{{.*}}>)

  // CHECK: ttcore.cpu_module {
  // CHECK: builtin.module {
  // CHECK-LABEL: llvm.func @cpu_hoisted_forward_const_eval_0_{{.*}}(
  // CHECK-LABEL: llvm.func @cpu_hoisted_forward_merge_return_multiple_values_const_eval_0_{{.*}}(
}

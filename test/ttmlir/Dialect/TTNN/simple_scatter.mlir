...
module attributes {} {
  ...
  func.func @scatter_2(%arg0: tensor<1000x32xf32>, %arg1: tensor<10x1xi64>, %arg2: tensor<10x32xf32>) -> tensor<1000x32xf32> {
    ...
    // CHECK: %{{[0-9]+}} = "ttnn.scatter"({{.*}}) <{dim = 0 : i32}> : (tensor<1000x32xf32, {{.*}}>, tensor<10x32xsi32, {{.*}}>, tensor<10x32xf32, {{.*}}>) -> tensor<1000x32xf32, {{.*}}>
  ...
  func.func @scatter_3(%arg0: tensor<2050x768xf32>, %arg1: tensor<1x5x1xi32>, %arg2: tensor<1x5x768xf32>) -> tensor<2050x768xf32> {
    ...
    // CHECK: %{{[0-9]+}} = "ttnn.scatter"({{.*}}) <{dim = 0 : i32}> : (tensor<1x2050x768xf32, {{.*}}>, tensor<1x5x768xsi32, {{.*}}>, tensor<1x5x768xf32, {{.*}}>) -> tensor<2050x768xf32, {{.*}}>

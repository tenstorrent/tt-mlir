...
module {
  ...
  func.func @concatenate_heads_fusion_3(%arg0: tensor<1x12x256x64xbf16>) -> tensor<1x256x768xbf16> {
...
module {
  ...
  func.func @concatenate_heads_fusion_4(%arg0: tensor<1x12x197x64xbf16>) -> tensor<1x197x768xbf16> {
...
module {
  ...
  func.func @concatenate_heads_fusion_5(%arg0: tensor<2x12x197x64xbf16>) -> tensor<2x197x768xbf16> {
...
// Check that the fusing of permute and reshape into concatenate_heads do not work if batch_size != 1 and there is reshape fold.
module {
  // CHECK-LABEL: func.func @concatenate_heads_fusion_5
  func.func @concatenate_heads_fusion_5(%arg0: tensor<2x12x197x64xbf16>) -> tensor<197x1536xbf16> {
    // CHECK-NOT: ttir.concatenate_heads

    %0 = ttir.empty() : tensor<2x197x12x64xbf16>
    %1 = "ttir.permute"(%arg0, %0) <{permutation = array<i64: 0, 2, 1, 3>}> : (tensor<2x12x197x64xbf16>, tensor<2x197x12x64xbf16>) -> tensor<2x197x12x64xbf16>

    %2 = ttir.empty() : tensor<197x1536xbf16>
    %3 = "ttir.reshape"(%1, %2) <{shape = [197 : i32, 1536 : i32]}> : (tensor<2x197x12x64xbf16>, tensor<197x1536xbf16>) -> tensor<197x1536xbf16>

    return %3 : tensor<197x1536xbf16>
  }
}

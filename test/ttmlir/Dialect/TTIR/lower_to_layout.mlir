...
func.func @tilize(%arg0: tensor<256x768xf32, #layout>) -> tensor<256x768xf32, #layout1> {
  ...
  // CHECK: ttir.generic {block_factors = [1, 1], grid = #tt.grid<1x1>
...
func.func @reblock(%arg0: tensor<256x768xf32, #layout1>) -> tensor<256x768xf32, #layout2> {
  ...
  // CHECK: ttir.generic {block_factors = [1, 1], grid = #tt.grid<8x8>
  // CHECK: ^datamovement0
...
func.func @compound(%arg0: tensor<256x768xf32>) -> tensor<256x768xf32> {
  ...
  // CHECK-NEXT: ttir.generic {block_factors = [1, 1], grid = #tt.grid<8x8>
  ...
  // CHECK-NEXT: ttir.generic {block_factors = [1, 1], grid = #tt.grid<1x1>
  // untilize
  // CHECK: ttir.generic {block_factors = [1, 1], grid = #tt.grid<1x1>

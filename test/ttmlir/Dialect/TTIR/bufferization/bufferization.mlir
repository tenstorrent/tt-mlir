...
func.func @matmul() -> tensor<1x1x2x2x!tt.tile<32x32, f32>, #layout2> {
  ...
  // CHECK: {{^  ttir.generic.*}}
  %1 = "ttir.generic"(%arg0, %arg1, %0) <{block_factors = [1, 1, 1], grid = #tt.grid<1x1>, indexing_maps = [#map, #map1, #map2], iterator_types = [#parallel, #parallel, #reduction], threads = [#ttir.thread<compute>], operandSegmentSizes = array<i32: 2, 1>}> ({

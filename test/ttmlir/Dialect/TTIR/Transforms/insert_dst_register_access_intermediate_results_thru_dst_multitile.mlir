// RUN: ttmlir-opt --ttcore-register-device --ttir-insert-dst-register-access="use-tile-matmul=false" -o %t %s
...
module attributes {ttcore.system_desc = #ttcore.system_desc<[{role = host, target_triple = "x86_64-pc-linux"}], [{arch = <wormhole_b0>, grid = 8x8, coord_translation_offsets = 18x18, l1_size = 1499136, num_dram_channels = 12, dram_channel_size = 1073741824, noc_l1_address_align_bytes = 16, pcie_address_align_bytes = 32, noc_dram_address_align_bytes = 32, l1_unreserved_base = 100352, erisc_l1_unreserved_base = 98304, dram_unreserved_base = 2560032, dram_unreserved_end = 1073139712, supported_data_types = [<f32>, <f16>, <bf16>, <bfp_f8>, <bfp_bf8>, <bfp_f4>, <bfp_bf4>, <bfp_f2>, <bfp_bf2>, <u32>, <u16>, <u8>, <si32>], supported_tile_sizes = [ 4x16,  16x16,  32x16,  4x32,  16x32,  32x32], dst_register_size_tiles = 8, num_cbs = 32, num_compute_threads = 1, num_datamovement_threads = 2}], [0], [1 : i32], [ 0x0x0x0]>} {
  ...
  func.func @eltwise_unary_chain_multi_tile(%arg0: memref<128x128xbf16>) -> memref<128x128xbf16> {
    ...
    ttir.to_layout %arg0, %alloc_0 : memref<128x128xbf16> into memref<1x1x128x128xbf16, #ttcore.shard<256x2>, #ttcore.memory_space<l1>> hostInfo = <logical_shape = 128x128, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1>
    ...
    ^compute0(%cb0: memref<128x128xbf16, #ttcore.memory_space<l1>>, %cb1: memref<4x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<l1>>):
      "ttir.tile_tilize_block"(%cb0, %cb1) : (memref<128x128xbf16, #ttcore.memory_space<l1>>, memref<4x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<l1>>) -> ()
    }
    ...
    ^compute0(%cb0: memref<4x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<l1>>, %cb1: memref<4x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<l1>>):
      ...
      scf.for %arg1 = %c0 to %c4 step %c2 {
        scf.for %arg2 = %c0 to %c4 step %c4 {
          ...
          %dst = ttir.acquire_dst() : memref<1x2x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<dst>>
          ...
          affine.for %arg3 = 0 to 2 {
            affine.for %arg4 = 0 to 4 {
              ...
              // CHECK: %[[ABS_RESULT:.*]] = "ttir.tile_abs"(%[[DST0_VAL:.*]]) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
              ...
              %3 = "ttir.tile_sin"(%2) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
              // CHECK: %[[SIN_RESULT:.*]] = "ttir.tile_sin"(%[[DST_ABS]]) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
              ...
              %5 = "ttir.tile_negative"(%4) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
              // CHECK: %[[NEG_RESULT:.*]] = "ttir.tile_negative"(%[[DST_SIN]]) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
              ...
              %7 = "ttir.tile_exp"(%6) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
              // CHECK: %[[EXP_RESULT:.*]] = "ttir.tile_exp"(%[[DST_NEG]]) : (!ttcore.tile<32x32, bf16>) -> !ttcore.tile<32x32, bf16>
    ...
    ^compute0(%cb0: memref<4x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<l1>>, %cb1: memref<128x128xbf16, #ttcore.memory_space<l1>>):
      "ttir.tile_untilize_block"(%cb0, %cb1) : (memref<4x4x!ttcore.tile<32x32, bf16>, #ttcore.memory_space<l1>>, memref<128x128xbf16, #ttcore.memory_space<l1>>) -> ()
    }
    memref.dealloc %alloc_1 : memref<1x1x4x4x!ttcore.tile<32x32, bf16>, #ttcore.shard<8192x2048>, #ttcore.memory_space<l1>>
    ttir.to_layout %alloc_3, %alloc_2 : memref<1x1x128x128xbf16, #ttcore.shard<256x2>, #ttcore.memory_space<l1>> into memref<128x128xbf16> hostInfo = <logical_shape = 128x128, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1>

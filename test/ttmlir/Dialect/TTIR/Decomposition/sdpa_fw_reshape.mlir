...
module {
  ...
  // Rank normalization preserves the unaligned sequence extent. Forced
  // promotion must therefore reject the resulting typed TTNN operation.
  // CHECK-LABEL: func.func @sdpa_fw_rank3_unaligned_sequence
  func.func @sdpa_fw_rank3_unaligned_sequence(
      %query: tensor<8x31x64xbf16>, %key: tensor<8x31x64xbf16>,
      %value: tensor<8x31x64xbf16>) -> tensor<8x31x64xbf16> {
    // CHECK: "ttcore.composite"
    // CHECK-SAME: composite_name = "sdpa_fw"
    // CHECK-SAME: -> tensor<1x8x31x64xbf16>
    // ALIGNMENT: 'ttnn.sdpa_fw' op query sequence dimension (dim 2) must be a multiple of TILE_HEIGHT (32), but got 31
    %0 = "ttcore.composite"(%query, %key, %value) <{
        composite_name = "sdpa_fw",
        decomposition = @sdpa_fw_rank3_unaligned_sequence_decomposition,
        composite_attributes = {
          mask_type = #ttcore.attention_mask_type<causal>,
          dropout_probability = 0.000000e+00 : f32,
          return_intermediates = false}}>
        : (tensor<8x31x64xbf16>, tensor<8x31x64xbf16>,
           tensor<8x31x64xbf16>) -> tensor<8x31x64xbf16>
    return %0 : tensor<8x31x64xbf16>
  }
  ...
  func.func private @sdpa_fw_rank3_unaligned_sequence_decomposition(
      %query: tensor<8x31x64xbf16>, %key: tensor<8x31x64xbf16>,
      %value: tensor<8x31x64xbf16>) -> tensor<8x31x64xbf16> {
    return %query : tensor<8x31x64xbf16>
  }
}

...
module {
  ...
  func.func @reduce_C(%arg0: memref<1x1x64x96xf32, #ttcore.shard<384x4>, #l1_>) ->memref<2x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096>, #l1_> {
    ...
    "ttir.to_layout"(%0, %alloc) : (memref<32x32xf32>, memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096>, #l1_>) -> ()
    ...
    "ttir.to_layout"(%arg0, %alloc_0) : (memref<1x1x64x96xf32, #ttcore.shard<384x4>, #l1_>, memref<2x3x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096>, #l1_>) -> ()
    ...
    // CHECK: %[[STREAM_A:.+]] = "ttir.stream_layout"(%{{[^}]+}}, %{{[^}]+}}) : (memref<2x3x1x1x!ttcore.tile
    // CHECK: %[[STREAM_B:.+]] = "ttir.stream_layout"(%{{[^}]+}}, %{{[^}]+}}) : (memref<1x1x1x1x!ttcore.tile
    // CHECK: ins(%[[STREAM_A]], %[[STREAM_B]] : memref

    ttir.generic {block_factors = [1, 1], grid = #ttcore.grid<2x1>, indexing_maps = [#map, #map1, #map2], iterator_types = [#parallel, #reduction], threads = [#ttir.thread<compute>]}
    ...
    ^compute0(%cb0: memref<1x1x!ttcore.tile<32x32, f32>, #l1_>, %cb1: memref<1x1x!ttcore.tile<32x32, f32>, #l1_>, %cb2: memref<1x1x!ttcore.tile<32x32, f32>, #l1_>):
      ...
      ^bb0(%in: !ttcore.tile<32x32, f32>, %in_3: !ttcore.tile<32x32, f32>, %out: !ttcore.tile<32x32, f32>):
        %1 = "ttir.tile_reduce_sum"(%in, %in_3, %out) <{reduce_dim = #ttir<reduce_dim C>}> : (!ttcore.tile<32x32, f32>, !ttcore.tile<32x32, f32>, !ttcore.tile<32x32, f32>) -> !ttcore.tile<32x32, f32>

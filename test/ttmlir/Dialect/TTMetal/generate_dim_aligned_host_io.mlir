...
module {
  ...
  func.func @test_bounce_buffer_insertion(%arg0: memref<3x43x7xf32>, %arg1: memref<7x43x7xf32>) -> memref<9x43x7xf32> {
    ...
    // CHECK-NEXT: "ttmetal.enqueue_write_buffer"(%[[ARG0_BOUNCE]], %[[DEV_TENSOR_0]])
    // CHECK-NOT: host_info
    "ttmetal.enqueue_write_buffer"(%arg0, %0) {host_info = #ttcore.metal_layout<logical_shape = 3x43x7, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1>} : (memref<3x43x7xf32>, memref<6x1x32x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>>) -> ()
    ...
    // CHECK-NEXT: "ttmetal.enqueue_write_buffer"(%[[ARG1_BOUNCE]], %[[DEV_TENSOR_1]])
    // CHECK-NOT: host_info
    "ttmetal.enqueue_write_buffer"(%arg1, %1) {host_info = #ttcore.metal_layout<logical_shape = 7x43x7, dim_alignments = 4x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1>} : (memref<7x43x7xf32>, memref<8x1x64x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>>) -> ()
    ...
    // CHECK-NEXT: "ttmetal.enqueue_read_buffer"(%[[DEV_TENSOR_2]], %[[ARG_OUT_BOUNCE]])
    // CHECK-NOT: host_info
    "ttmetal.enqueue_read_buffer"(%2, %alloc) {host_info = #ttcore.metal_layout<logical_shape = 9x43x7, dim_alignments = 4x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1>} : (memref<8x1x96x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>>, memref<9x43x7xf32>) -> ()
  ...
  func.func @test_skipped_bounce_buffer_insertion(%arg0: memref<8x32x32xf32>) -> memref<8x32x32xf32> {
    ...
    // CHECK-NOT: host_info
    "ttmetal.enqueue_write_buffer"(%arg0, %0) {host_info = #ttcore.metal_layout<logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1>} : (memref<8x32x32xf32>, memref<8x1x32x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>>) -> ()
    ...
    // CHECK-NOT: host_info
    "ttmetal.enqueue_read_buffer"(%1, %alloc) {host_info = #ttcore.metal_layout<logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1>} : (memref<8x1x32x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>>, memref<8x32x32xf32>) -> ()
    "ttmetal.finish"() : () -> ()

...
#layout_mismatched_alignments = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>

// -----

// Test: alignments must be positive (zero alignment)
// expected-error @+1 {{dim_alignments[0] must be positive, got 0}}
#layout_zero_alignment = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 0x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>
...
#layout_1d_intervals = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[0, 1]> : tensor<2xi64>, undef, l1, sharded, index_map = map(0)>
...
#layout_wrong_interval_size = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1, 2]]> : tensor<1x3xi64>, undef, l1, sharded, index_map = map(0)>
...
#layout_start_oob = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[[5, 6]]> : tensor<1x2xi64>, undef, l1, sharded, index_map = map(0)>
...
#layout_end_oob = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 10]]> : tensor<1x2xi64>, undef, l1, sharded, index_map = map(0)>
...
#layout_inverted_interval = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[[1, 0]]> : tensor<1x2xi64>, undef, l1, sharded, index_map = map(0)>
...
#layout_neg_start_oob = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[[-10, 2]]> : tensor<1x2xi64>, undef, l1, sharded, index_map = map(0)>

// -----

// Test: Valid layout (should parse without error)
#layout_valid = #ttcore.metal_layout<logical_shape = 32x32, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>

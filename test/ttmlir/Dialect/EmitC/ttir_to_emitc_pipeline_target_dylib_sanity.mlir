// This test checks that the (TTIR to EmitC pipeline with target dylib) is equivalent to (TTIR to TTNN common pipeline + TTNN common to EmitC pipeline with target dylib).
...
// RUN: ttmlir-opt --ttir-to-ttnn-common-pipeline="system-desc-path=%system_desc_path%" --ttcore-unwrap-device-module --ttnn-common-to-emitc-pipeline="target-dylib=true" -o %t_indirect.mlir %s

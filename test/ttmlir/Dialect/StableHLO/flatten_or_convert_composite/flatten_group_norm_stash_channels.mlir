// REQUIRES: stablehlo
// RUN: rm -rf %t.mlir
// RUN: ttmlir-opt --split-input-file --flatten-or-convert-composites -o %t.mlir %s
...
// stashed attributes hold the *global* num_groups, so the global channel count
// must be stashed alongside them -- it is the only way ReoutlineCompositePass can
// recover the group size once shapes have been localized.
module @jit_flatten_group_norm attributes {mhlo.num_partitions = 4 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func @main(%arg0: tensor<1x512x1x60x90xbf16> {sdy.sharding = #sdy.sharding<@mesh, [{}, {"_axis_0"}, {}, {}, {}]>, ttcore.shard_status = #ttcore.shard_status<presharded>}) -> tensor<1x512x1x60x90xbf16> {
    // The composite is gone, and the seed op records the global channel dim (512).

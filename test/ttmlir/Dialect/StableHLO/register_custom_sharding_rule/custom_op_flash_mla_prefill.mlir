...
// Shapes follow a real DeepSeek V3.1 MLA *prefill*, which runs as MHA: the
// compressed KV latent is up-projected to per-head K/V, so every query head
// has its own KV head (Hq == Hkv). The MQA form -- attention against the
// single shared latent -- is the *decode* path; the prefill sharding rule
// asserts that case never reaches it.

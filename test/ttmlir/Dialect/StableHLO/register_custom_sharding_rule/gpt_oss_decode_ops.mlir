...
// The tt-metal moe_gpt kernel consumes only the fused 6D weight layout,
// so the op takes 6 operands:
//   hidden, indices, scores, mapping, fused_w0_w1, fused_w2
// Fused weights are opaque to the shardy rule — their internal layout
// doesn't expose the logical H / I / E axes.
// =====================================================================
module @MoeGpt_BlocksInputH attributes {mhlo.cross_program_prefetches = [], mhlo.frontend_attributes = {xla.sdy.meshes = "{mesh = #sdy.mesh<[\22_axis_0\22=2]>}"}, mhlo.input_output_alias = [], mhlo.is_dynamic = false, mhlo.use_auto_spmd_partitioning = false} {
  func.func @main(%arg0: tensor<1x4x1x128xbf16> {mhlo.frontend_attributes = {xla.sdy.sharding = "#sdy.sharding<@mesh, [{}, {}, {}, {\22_axis_0\22}]>"}, mhlo.sharding = "{devices=[1,1,1,2]<=[2]}", ttcore.argument_type = #ttcore.argument_type<input>, ttir.name = "hidden"}, %arg1: tensor<1x4x1x2xi64> {mhlo.frontend_attributes = {xla.sdy.sharding = "#sdy.sharding<@mesh, [{}, {}, {}, {}]>"}, ttcore.argument_type = #ttcore.argument_type<input>, ttir.name = "indices"}, %arg2: tensor<1x4x1x2xbf16> {mhlo.frontend_attributes = {xla.sdy.sharding = "#sdy.sharding<@mesh, [{}, {}, {}, {}]>"}, ttcore.argument_type = #ttcore.argument_type<input>, ttir.name = "scores"}, %arg3: tensor<1x1x8x2xi64> {mhlo.frontend_attributes = {xla.sdy.sharding = "#sdy.sharding<@mesh, [{}, {}, {}, {}]>"}, ttcore.argument_type = #ttcore.argument_type<input>, ttir.name = "mapping"}, %arg4: tensor<12x1x8x4x160x128xbf16> {mhlo.frontend_attributes = {xla.sdy.sharding = "#sdy.sharding<@mesh, [{}, {}, {}, {}, {}, {}]>"}, ttcore.argument_type = #ttcore.argument_type<input>, ttir.name = "fused_w0_w1"}, %arg5: tensor<12x1x8x2x160x128xbf16> {mhlo.frontend_attributes = {xla.sdy.sharding = "#sdy.sharding<@mesh, [{}, {}, {}, {}, {}, {}]>"}, ttcore.argument_type = #ttcore.argument_type<input>, ttir.name = "fused_w2"}) -> tensor<8x1x4x128xbf16> {
    // CHECK: stablehlo.all_gather
    // CHECK: stablehlo.custom_call @tt.moe_gpt
    %0:5 = stablehlo.custom_call @tt.moe_gpt(%arg0, %arg1, %arg2, %arg3, %arg4, %arg5) {api_version = 0 : i32, mhlo.frontend_attributes = {cluster_axis = "1", num_devices = "2", num_experts = "8", num_experts_per_tok = "2"}} : (tensor<1x4x1x128xbf16>, tensor<1x4x1x2xi64>, tensor<1x4x1x2xbf16>, tensor<1x1x8x2xi64>, tensor<12x1x8x4x160x128xbf16>, tensor<12x1x8x2x160x128xbf16>) -> (tensor<1x1x8x2xi64>, tensor<1x4x1x2xi64>, tensor<1x4x1x2xbf16>, tensor<1x4x1x2xbf16>, tensor<8x1x4x128xbf16>)

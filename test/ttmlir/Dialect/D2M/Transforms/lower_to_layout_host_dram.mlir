...
// CHECK-LABEL: func.func @dram_to_dram_reblock_bounces
func.func @dram_to_dram_reblock_bounces(%arg0: tensor<1x1x256x256xf32, #dram>) -> tensor<2x2x128x128xf32, #dram> {
  %0 = d2m.empty() : tensor<2x2x128x128xf32, #dram>

  // Pure DRAM->DRAM datamovement reblocking still needs a bounce through an
  // intermediate buffer: first DRAM->L1, then L1->DRAM. Generics with local
  // tile compute, like same-placement tilize/untilize below, can use DRAM
  // operands directly.
  // CHECK-COUNT-2: d2m.generic
  // CHECK-NOT: d2m.to_host
  // CHECK-NOT: d2m.to_device

  %1 = d2m.to_layout %arg0, %0 : tensor<1x1x256x256xf32, #dram> into tensor<2x2x128x128xf32, #dram>
    -> tensor<2x2x128x128xf32, #dram>

  return %1 : tensor<2x2x128x128xf32, #dram>
}

// Tiled variants: tilize/untilize operate on local shard buffers inside a
// generic, so DRAM operands are legal. The global staging remains in DRAM:
// host->tiled DRAM writes scalar data to DRAM then tilizes into tiled DRAM, and
// tiled DRAM->host untilizes into scalar DRAM then reads to host.

#dram_tiled = #ttcore.metal_layout<logical_shape = 256x256, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, dram, sharded>
...
func.func @host_to_dram_tiled(%arg0: tensor<256x256xf32>) -> tensor<1x1x8x8x!ttcore.tile<32x32, f32>, #dram_tiled> {
  ...
  // Tilize path: scalar DRAM -> tiled DRAM in one generic.
  // CHECK: d2m.generic

...
module {
  ...
  func.func @test_two_mcast_remote_loads(%arg0: memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #dram>,
    ...
    // CHECK: d2m.remote_load{{.*}}into %{{.*}} {preallocated_semaphores = [2, 3]}
  ...
  // Test 5: ScatterOp should get 1 preallocated semaphore (not 2)
  // CHECK-LABEL: func.func @test_scatter_semaphore
  func.func @test_scatter_semaphore(%arg0: memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>) {
    %alloc = memref.alloc() {alignment = 64 : i64} : memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>

    // CHECK: d2m.generic
    // Scatter needs 1 semaphore (scatterDone)
    // CHECK: ^{{.*}}(%[[SEM:.*]]: !d2m.semaphore):
    // CHECK: d2m.scatter {{.*}} {preallocated_semaphores = [0]}
    // CHECK: ^{{.*}}(%{{.*}}: !d2m.semaphore):
    d2m.generic {block_factors = [1, 1], grid = #ttcore.grid<4x4>, indexing_maps = [#map, #map], iterator_types = [#parallel, #reduction], threads = [#d2m.thread<datamovement>, #d2m.thread<compute>]}
    ...
    ^datamovement0:
      ...
      %c1 = arith.constant 1 : index
      %c4 = arith.constant 4 : index
      d2m.scatter %cb0 into %cb1 mcore[%c0, %c0] mshape[%c1, %c4] : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>> into !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
    }, {
    ^compute0:
      %cb0 = d2m.get_cb(0) : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
      %cb1 = d2m.get_cb(1) : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
  ...
  // Test 6: Scatter + mcast remote_load in same generic
  // CHECK-LABEL: func.func @test_scatter_and_mcast_remote_load
  func.func @test_scatter_and_mcast_remote_load(%arg0: memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #dram>,
                                                %arg1: memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>) {
    %alloc = memref.alloc() {alignment = 64 : i64} : memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>

    // CHECK: d2m.generic
    // remote_load gets [0,1], scatter gets [2]
    // CHECK: ^{{.*}}(%{{.*}}: !d2m.semaphore, %{{.*}}: !d2m.semaphore, %{{.*}}: !d2m.semaphore):
    // CHECK: d2m.remote_load{{.*}} {preallocated_semaphores = [0, 1]}
    // CHECK: d2m.scatter{{.*}} {preallocated_semaphores = [2]}
    d2m.generic {block_factors = [1, 1], grid = #ttcore.grid<4x4>, indexing_maps = [#map, #map, #map], iterator_types = [#parallel, #reduction], threads = [#d2m.thread<datamovement>, #d2m.thread<compute>]}
        ins(%arg0, %arg1 : memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #dram>, memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>)
        outs(%alloc : memref<4x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>)  {
    ^datamovement0:
      %cb0 = d2m.get_cb(0) : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
      %cb1 = d2m.get_cb(1) : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
      ...
      d2m.scatter %cb1 into %cb2 mcore[%c0, %c0] mshape[%c1, %c4] : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>> into !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
    }, {
    ^compute0:
      %cb0 = d2m.get_cb(0) : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
      %cb1 = d2m.get_cb(1) : !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1>>
  ...
  // Test 7: Generic with no remote loads at all - nothing happens

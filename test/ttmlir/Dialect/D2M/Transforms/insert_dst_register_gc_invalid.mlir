...
// Test: DST with aliased block arguments (same value passed multiple times).
func.func @aliased_block_arguments() {
  %dst = d2m.acquire_dst() : memref<1x!ttcore.tile<32x32, f32>, #dst_>
...
// Test: Large number of acquire_dst operations (stress test).
...
// Test: DST with type mismatch (liveness doesn't care about types).
func.func @type_mismatch_dst() {
...
// Test: Function with multiple blocks and complex control flow.

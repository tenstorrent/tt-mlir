// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine --d2m-insert-dst-register-access="max-dst-physical-size-tiles=32" --d2m-insert-tile-matmul-block="use-tile-matmul=true" --canonicalize -o %t %s
...
module {
  ...
  func.func @generic_matmul(
    ...
    ^unified0:
      ...
      // CHECK: %[[C_VAL:.*]] = affine.load %[[DST]][%[[I]] * 2 + %[[J]]] : memref<8x!ttcore.tile<32x32, f32>, #dst>

      // Check matmul operation uses values from correct memory spaces
      // CHECK: %[[MATMUL_RESULT:.*]] = "d2m.tile_matmul"(%[[A_VAL]], %[[B_VAL]], %[[C_VAL]]) {{<\{transpose_b = false\}>}} : (!ttcore.tile<32x32, f32>, !ttcore.tile<32x32, f32>, !ttcore.tile<32x32, f32>) -> !ttcore.tile<32x32, f32>

      // Check result is stored back to dst with linearized index
      // CHECK: affine.store %[[MATMUL_RESULT]], %[[DST]][%[[I]] * 2 + %[[J]]] : memref<8x!ttcore.tile<32x32, f32>, #dst>
      %c0 = arith.constant 0 : index

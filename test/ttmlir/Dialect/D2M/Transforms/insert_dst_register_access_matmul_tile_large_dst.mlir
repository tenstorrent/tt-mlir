// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine='use-tile-matmul=true' --d2m-insert-dst-register-access='use-tile-matmul=true max-dst-physical-size-tiles=32 allocation-strategy=basic' --canonicalize %s | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine='use-tile-matmul=true' --d2m-insert-dst-register-access='use-tile-matmul=true max-dst-physical-size-tiles=32 allocation-strategy=greedy' --canonicalize %s | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine='use-tile-matmul=true' --d2m-insert-dst-register-access='use-tile-matmul=true max-dst-physical-size-tiles=32 allocation-strategy=chaitin-briggs' --canonicalize %s | FileCheck %s

// RUN: ttmlir-opt --ttcore-register-device --d2m-tag-linalg-generics --d2m-generic-tile-compute-loops --d2m-insert-dst-register-access="use-tile-matmul=false" --canonicalize -o %t %s

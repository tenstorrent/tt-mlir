...
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine --d2m-insert-dst-register-gc="coloring-strategy=greedy use-tile-matmul=true" --canonicalize %s --split-input-file | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine --d2m-insert-dst-register-gc="coloring-strategy=chaitin-briggs use-tile-matmul=true" --canonicalize %s --split-input-file | FileCheck %s
//

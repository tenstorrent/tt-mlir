...
module {
  func.func @eq(%arg0: memref<4x4xf32>, %arg1: memref<4x4xf32>) -> memref<4x4xf32> {
    ...
    d2m.to_layout %alloc_2, %alloc_1 : memref<4x4xf32, #ttcore.host_layout<logical_shape = 4x4, host_strides = 32x1, host_volume = 1024>> into memref<1x1x32x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>> hostInfo = <logical_shape = 4x4, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1>
    ...
    ^compute0(%cb0: memref<32x32xf32, #ttcore.memory_space<l1>>, %cb1: memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>):
      "d2m.tile_tilize_block"(%cb0, %cb1) : (memref<32x32xf32, #ttcore.memory_space<l1>>, memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>) -> ()
    }
    ...
    d2m.to_layout %alloc_5, %alloc_4 : memref<4x4xf32, #ttcore.host_layout<logical_shape = 4x4, host_strides = 32x1, host_volume = 1024>> into memref<1x1x32x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>> hostInfo = <logical_shape = 4x4, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1>
    d2m.generic {block_factors = [1, 1], grid = #ttcore.grid<1x1>, indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = [#ttcore.iterator_type<parallel>, #ttcore.iterator_type<parallel>], threads = [#d2m.thread<compute>]}
    ...
    ^compute0(%cb0: memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>, %cb1: memref<32x32xf32, #ttcore.memory_space<l1>>):
      "d2m.tile_untilize_block"(%cb0, %cb1) : (memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>, memref<32x32xf32, #ttcore.memory_space<l1>>) -> ()
    }
    ...
    d2m.to_layout %alloc_8, %alloc_9 : memref<1x1x32x32xf32, #ttcore.shard<128x4>, #ttcore.memory_space<l1>> into memref<4x4xf32, #ttcore.host_layout<logical_shape = 4x4, host_strides = 32x1, host_volume = 1024>> hostInfo = <logical_shape = 4x4, dim_alignments = 32x32, collapsed_intervals = dense<[[0, 1], [1, 2]]> : tensor<2x2xi64>, undef, l1>

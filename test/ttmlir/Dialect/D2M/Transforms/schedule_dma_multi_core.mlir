// RUN: ttmlir-opt --ttcore-register-device --d2m-schedule-dma="num-dm-cores=6" -o %t %s
...
module {
  ...
  // Test 4: Three remote inputs - should split across 3 DM cores
  ...
  func.func @test_three_remote_loads(%arg0: memref<2x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #dram>,
    ...
    // CHECK: d2m.generic
    // 3 CBs with DMA work -> split into 3 DM cores
    // CHECK-SAME: threads = [#d2m.thread<datamovement, dm_core = 0>, #d2m.thread<datamovement, dm_core = 1>, #d2m.thread<datamovement, dm_core = 2>, #d2m.thread<compute>]
    d2m.generic {block_factors = [], grid = #ttcore.grid<2x4>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement>, #d2m.thread<compute>]}
  ...
  func.func @test_three_loads_one_store_balanced(%arg0: memref<2x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #dram>,
    ...
    // CHECK: d2m.generic
    // 4 CBs with DMA work -> split into 4 HW threads
    // CHECK-SAME: threads = [#d2m.thread<datamovement, dm_core = 0>, #d2m.thread<datamovement, dm_core = 1>, #d2m.thread<datamovement, dm_core = 2>, #d2m.thread<datamovement, dm_core = 3>, #d2m.thread<compute>]
    d2m.generic {block_factors = [], grid = #ttcore.grid<2x4>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement>, #d2m.thread<compute>]}

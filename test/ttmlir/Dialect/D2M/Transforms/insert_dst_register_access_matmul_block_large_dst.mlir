// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine --d2m-insert-dst-register-access="use-tile-matmul=false max-dst-physical-size-tiles=32" --canonicalize %s | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine --d2m-insert-dst-register-gc="coloring-strategy=greedy use-tile-matmul=false max-dst-physical-size-tiles=32" --canonicalize %s | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine --d2m-insert-dst-register-gc="coloring-strategy=chaitin-briggs use-tile-matmul=false max-dst-physical-size-tiles=32" --canonicalize %s | FileCheck %s
...
module {
  ...
  func.func @generic_matmul(
    ...
    ^compute0(%arg0_cb: !d2m.cb<memref<3x3x!ttcore.tile<32x32, f32>, #l1_>>, %arg1_cb: !d2m.cb<memref<3x2x!ttcore.tile<32x32, f32>, #l1_>>, %arg2_cb: !d2m.cb<memref<3x2x!ttcore.tile<32x32, f32>, #l1_>>):
      ...
      // CHECK: %[[FINAL_VAL:.*]] = affine.load %[[DST]][[[DSTIDX2:[0-9]+]], %[[WB_I]], %[[WB_J]]] : memref<[[DSTSIZE3:[0-9]+]]x3x2x!ttcore.tile<32x32, f32>, #dst>

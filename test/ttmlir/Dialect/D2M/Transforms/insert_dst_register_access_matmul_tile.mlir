// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine="use-tile-matmul=true" --d2m-insert-dst-register-access="use-tile-matmul=true" --canonicalize %s | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine="use-tile-matmul=true" --d2m-insert-dst-register-gc="coloring-strategy=greedy use-tile-matmul=true" --canonicalize %s | FileCheck %s
// RUN: ttmlir-opt --ttcore-register-device --d2m-linalg-to-affine="use-tile-matmul=true" --d2m-insert-dst-register-gc="coloring-strategy=chaitin-briggs use-tile-matmul=true" --canonicalize %s | FileCheck %s
...
module {
  ...
  func.func @generic_matmul(
    ...
    ^compute0(%arg0_cb: !d2m.cb<memref<3x3x!ttcore.tile<32x32, f16>, #l1_>>, %arg1_cb: !d2m.cb<memref<3x2x!ttcore.tile<32x32, f16>, #l1_>>, %arg2_cb: !d2m.cb<memref<3x2x!ttcore.tile<32x32, f16>, #l1_>>):
      ...
      // Check combined initialization and computation loop structure (3D loop)
      // CHECK: affine.for %[[INIT_I:.*]] = 0 to 3 {
      // CHECK-NEXT: affine.for %[[INIT_J:.*]] = 0 to 2 {
      // CHECK-NEXT: affine.for %[[INIT_K:.*]] = 0 to 3 {

      // Check for iteration index and conditional initialization inside the loop
      ...
      // CHECK: affine.store %[[INIT_VAL]], %[[DST]][{{.*}}, %[[INIT_I]], %[[INIT_J]]] : memref<{{.*}}x!ttcore.tile<32x32, f16>, #dst>
      ...
      // CHECK: %[[C_VAL:.*]] = affine.load %[[DST]][{{.*}}, %[[I]], %[[J]]] : memref<{{.*}}x!ttcore.tile<32x32, f16>, #dst>
      ...
      // CHECK: affine.store %[[MATMUL_RESULT]], %[[DST]][{{.*}}, %[[I]], %[[J]]] : memref<{{.*}}x!ttcore.tile<32x32, f16>, #dst>
      %c0 = arith.constant 0 : index
      ...
      // CHECK: %[[FINAL_VAL:.*]] = affine.load %[[DST]][{{.*}}, %[[WB_I]], %[[WB_J]]] : memref<{{.*}}x!ttcore.tile<32x32, f16>, #dst>

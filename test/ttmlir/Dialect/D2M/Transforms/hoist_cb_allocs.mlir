...
func.func @hoist_cb_dealloc(
  ...
  ^unified0():
    ...
    %alloc_cb = memref.alloc() {address = 1024 : i64, alignment = 16 : i64, d2m.synchronized_buffer = 2} : memref<4x4x!ttcore.tile<32x32, f32>, #l1>
...
func.func @hoist_multiple_cbs_dealloc(
  ...
  ^unified0():
    %c0 = d2m.core_index(0) : index
    %c1 = d2m.core_index(1) : index
    %alloc_cb0 = memref.alloc() {address = 1024 : i64, alignment = 16 : i64, d2m.synchronized_buffer = 2} : memref<4x4x!ttcore.tile<32x32, f32>, #l1>
    d2m.remote_load %alloc_cb0 %stream0[%c0, %c1] : memref<4x4x!ttcore.tile<32x32, f32>, #l1>, memref<2x2x4x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #l1>
    %alloc_cb1 = memref.alloc() {address = 2048 : i64, alignment = 16 : i64, d2m.synchronized_buffer = 2} : memref<4x4x!ttcore.tile<32x32, f32>, #l1>
    d2m.remote_load %alloc_cb1 %stream1[%c0, %c1] : memref<4x4x!ttcore.tile<32x32, f32>, #l1>, memref<2x2x4x4x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #l1>
    d2m.remote_store %out[%c0, %c1] %alloc_cb0 : memref<2x2x4x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1>, memref<4x4x!ttcore.tile<32x32, f32>, #l1>
  }
  return
}

// RUN: ttmlir-opt --ttcore-register-device "--d2m-allocate=test-assume-l1-capacity=2097152 test-buffer-size-policy=min" -o %t %s
// RUN: FileCheck %s --input-file=%t

// Verify that allocator-driven reblocking repairs get_cb types using operand_index rather than the CB hardware port number.

#l1 = #ttcore.memory_space<l1>
#mapL = affine_map<(d0, d1, d2) -> (d0, d2)>
#mapR = affine_map<(d0, d1, d2) -> (d2, d1)>
#mapO = affine_map<(d0, d1, d2) -> (d0, d1)>
#parallel = #ttcore.iterator_type<parallel>
#reduction = #ttcore.iterator_type<reduction>

module {
  // CHECK-LABEL: func.func @main()
  // CHECK: d2m.generic {block_factors = [1, 1, 16], grid = #ttcore.grid<1x1>
  // CHECK: %[[CB_LHS:.*]] = d2m.get_cb(23) operand_index = 0 : <memref<16x1x!ttcore.tile<32x32, f32>, #{{.+}}>>
  // CHECK: %[[CB_RHS:.*]] = d2m.get_cb(29) operand_index = 1 : <memref<1x16x!ttcore.tile<32x32, f32>, #{{.+}}>>
  // CHECK: %[[CB_OUT:.*]] = d2m.get_cb(31) operand_index = 2 : <memref<16x16x!ttcore.tile<32x32, f32>, #{{.+}}>>
  // CHECK: %[[WAIT_LHS:.*]] = d2m.wait %[[CB_LHS]] : <memref<16x1x!ttcore.tile<32x32, f32>, #{{.+}}>> -> memref<16x1x!ttcore.tile<32x32, f32>, #{{.+}}>
  // CHECK: %[[WAIT_RHS:.*]] = d2m.wait %[[CB_RHS]] : <memref<1x16x!ttcore.tile<32x32, f32>, #{{.+}}>> -> memref<1x16x!ttcore.tile<32x32, f32>, #{{.+}}>
  // CHECK: %[[RESERVE_OUT:.*]] = d2m.reserve %[[CB_OUT]] : <memref<16x16x!ttcore.tile<32x32, f32>, #{{.+}}>> -> memref<16x16x!ttcore.tile<32x32, f32>, #{{.+}}>
  func.func @main() -> memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1> {
    %lhs = memref.alloc() : memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>
    %rhs = memref.alloc() : memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>
    %r = memref.alloc() : memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>
    d2m.generic {block_factors = [1, 1, 1], grid = #ttcore.grid<1x1>, indexing_maps = [#mapL, #mapR, #mapO], iterator_types = [#parallel, #parallel, #reduction], threads = [#d2m.thread<compute>]}
        ins(%lhs, %rhs : memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>, memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>)
        outs(%r : memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>) {
    ^compute0():
      %cb0 = d2m.get_cb(23) operand_index = 0 : !d2m.cb<memref<16x16x!ttcore.tile<32x32, f32>, #l1>>
      %cb1 = d2m.get_cb(29) operand_index = 1 : !d2m.cb<memref<16x16x!ttcore.tile<32x32, f32>, #l1>>
      %cb2 = d2m.get_cb(31) operand_index = 2 : !d2m.cb<memref<16x16x!ttcore.tile<32x32, f32>, #l1>>
      %0 = d2m.wait %cb0 : !d2m.cb<memref<16x16x!ttcore.tile<32x32, f32>, #l1>> -> memref<16x16x!ttcore.tile<32x32, f32>, #l1>
      %1 = d2m.wait %cb1 : !d2m.cb<memref<16x16x!ttcore.tile<32x32, f32>, #l1>> -> memref<16x16x!ttcore.tile<32x32, f32>, #l1>
      %2 = d2m.reserve %cb2 : !d2m.cb<memref<16x16x!ttcore.tile<32x32, f32>, #l1>> -> memref<16x16x!ttcore.tile<32x32, f32>, #l1>
      "d2m.tile_matmul_block"(%0, %1, %2) : (memref<16x16x!ttcore.tile<32x32, f32>, #l1>, memref<16x16x!ttcore.tile<32x32, f32>, #l1>, memref<16x16x!ttcore.tile<32x32, f32>, #l1>) -> ()
    }
    return %r : memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096, 1>, #l1>
  }
}

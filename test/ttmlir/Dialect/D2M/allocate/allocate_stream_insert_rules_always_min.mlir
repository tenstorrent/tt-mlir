// RUN: ttmlir-opt --ttcore-register-device "--d2m-allocate=stream-insert-policy=always test-buffer-size-policy=min" -o %t %s
...
module {
  ...
  func.func @test_generic_accept_operand_streams() {
    ...
    // CHECK: %[[STREAM_LHS:.*]] = "d2m.stream_layout"({{.+}}) <{remapping = #map{{.*}}}>
    // CHECK: %[[STREAM_RHS:.*]] = "d2m.stream_layout"({{.+}}) <{remapping = #map{{.*}}}>
    // CHECK: ins(%{{.+}}, %{{.+}} : memref<{{.+}}#ttcore.view<4>, #l1>, memref<{{.+}}#ttcore.view<4>, #l1>)
  ...
  // Verify that "DMA-only" generics do have streams allocated for operands with views.

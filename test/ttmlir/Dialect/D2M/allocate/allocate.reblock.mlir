...
module {
  func.func @main() -> memref<1x1x16x16x!ttcore.tile<32x32, f32>, #ttcore.shard<65536x4096>, #l1> {
    ...
    ^compute0(%cb0: memref<16x16x!ttcore.tile<32x32, f32>, #l1>, %cb1: memref<16x16x!ttcore.tile<32x32, f32>, #l1>, %cb2: memref<16x16x!ttcore.tile<32x32, f32>, #l1>):
      linalg.generic {indexing_maps = [#mapL, #mapR, #mapO], iterator_types = ["parallel", "parallel", "reduction"]}
        ins(%cb0, %cb1 : memref<16x16x!ttcore.tile<32x32, f32>, #l1>, memref<16x16x!ttcore.tile<32x32, f32>, #l1>)
        outs(%cb2 : memref<16x16x!ttcore.tile<32x32, f32>, #l1>) {
      ^bb0(%in1: !ttcore.tile<32x32, f32>, %in2: !ttcore.tile<32x32, f32>, %out: !ttcore.tile<32x32, f32>):
        %0 = "d2m.tile_matmul"(%in1, %in2, %out) : (!ttcore.tile<32x32, f32>, !ttcore.tile<32x32, f32>, !ttcore.tile<32x32, f32>) -> !ttcore.tile<32x32, f32>

// RUN: ttmlir-opt --ttcore-register-device "--d2m-reblock-generics=test-buffer-size-policy=max" "--d2m-allocate=stream-insert-policy=always" -o %t %s
...
module {
  ...
  // Verify no aliasing for all generic inputs.
  ...
  // Verify no aliasing for all generic inputs (%lhs and %rhs).

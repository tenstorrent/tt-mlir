...
module {
  ...
  // Verify no aliasing for all generic inputs (and alias for output).
  ...
  // treated as aliased (no CB alloc) buffer if one of the operands (input or output) is aliase (in this case, %out).
  // CHECK-LABEL: func @test_shared_load_store_buffer_no_cb_if_one_operand_aliased
  // CHECK: d2m.generic
  // CHECK-NOT: %[[BUFFER:.*]] = memref.alloc() {{.*}} : memref<32x32xf32, #l1>
  // CHECK: d2m.operand_alias
  // CHECK: d2m.remote_load {{.*}} : memref<32x32xf32, #l1>
  // CHECK: d2m.remote_store {{.*}} : memref<1x1x32x32xf32, {{.*}}#l1>
  func.func @test_shared_load_store_buffer_no_cb_if_one_operand_aliased(%arg0: memref<1x1x32x32xf32, #ttcore.shard<128x4, 1>, #ttcore.memory_space<dram>>) {
    ...
    ^unified0:
      ...
      affine.for %iter0 = 0 to %bf0 {
        affine.for %iter1 = 0 to %bf1 {
          %buffer = memref.alloc() {d2m.synchronized_buffer = 2} : memref<32x32xf32, #ttcore.memory_space<l1>>
  ...
  // CHECK: d2m.generic
  // CHECK: %[[BUFFER:.*]] = memref.alloc(){{.*}} : memref<1x1x!ttcore.tile<32x32, f32>, {{.*}}#l1>
  // CHECK: %[[BUFFER_OUT:.*]] = memref.alloc(){{.*}} : memref<1x1x!ttcore.tile<32x32, f32>, {{.*}}#l1>
  // CHECK: d2m.remote_load %[[BUFFER]] {{.*}} : memref<1x1x!ttcore.tile<32x32, f32>, {{.*}}#l1>, memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #dram>
  // CHECK: d2m.remote_store {{.*}} %[[BUFFER_OUT]] : memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #dram>, memref<1x1x!ttcore.tile<32x32, f32>, {{.*}}#l1>
  func.func @test_shared_load_store_buffer_has_cb_if_both_operands_are_streamed(%arg0: memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<dram>>) {
    ...
    ^unified0:
      ...
      affine.for %iter0 = 0 to %bf0 {
        affine.for %iter1 = 0 to %bf1 {
          %buffer = memref.alloc() {d2m.synchronized_buffer = 2} : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>
          %buffer_out = memref.alloc() {d2m.synchronized_buffer = 2} : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>
          ...
          linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%buffer : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>) outs(%buffer_out : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>) {
          ...
          d2m.remote_store %out[%iter0, %iter1] %buffer_out : memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<dram>>, memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.memory_space<l1>>
        } {d2m.blocking_loop = 1}
      } {d2m.blocking_loop = 0}
    }
    return
  ...
  func.func @test_ttnn_arg_cast_bridge(%arg0: tensor<32x32xf32, #dram_layout>, %arg1: tensor<32x32xf32, #l1_layout>)  {
    ...
    ^unified0:
      ...
      affine.for %iter0 = 0 to %bf0 {
        affine.for %iter1 = 0 to %bf1 {
          %buffer = memref.alloc() {d2m.synchronized_buffer = 2} : memref<1x1x!ttcore.tile<32x32, f32>, #dram>
          d2m.remote_load %buffer %arg0_cast[%iter0, %iter1] : memref<1x1x!ttcore.tile<32x32, f32>, #dram>, memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #dram>
          d2m.remote_store %arg1_cast[%iter0, %iter1] %buffer : memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #l1>, memref<1x1x!ttcore.tile<32x32, f32>, #dram>
        } {d2m.blocking_loop = 1}
      } {d2m.blocking_loop = 0}
    }
    return

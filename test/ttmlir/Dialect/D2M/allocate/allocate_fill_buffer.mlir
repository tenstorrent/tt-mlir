...
// d2m.fill_buffer must receive `address` and `alignment` attrs from the
// allocator like memref.alloc.
// CHECK-LABEL: func.func @test_fill_buffer
func.func @test_fill_buffer() -> memref<1x2x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<l1>> {
  // CHECK: %[[ALLOC_IN:.+]] = memref.alloc() {address = {{[0-9]+}} : i64, alignment = {{[0-9]+}} : i64}{{.+}} #l1>
  %alloc_0 = memref.alloc() : memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<l1>>
  // CHECK: %[[FILL:.+]] = d2m.fill_buffer
  // CHECK-SAME: address = {{[0-9]+}} : i64
  // CHECK-SAME: alignment = {{[0-9]+}} : i64
  // CHECK-SAME: value = 1.000000e+00 : f32
  %fill = d2m.fill_buffer() {value = 1.0 : f32, fixed_shard = array<i64: 1, 1, 1, 1>} : memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<l1>>

  %0 = "d2m.composite_view"(%alloc_0, %fill) <{dim = 1 : si32}> : (memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<l1>>, memref<1x1x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #ttcore.memory_space<l1>>) -> memref<1x2x1x1x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #ttcore.memory_space<l1>>

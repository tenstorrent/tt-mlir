...
// --- Test 1: Single scratch_allocate replaced by subview of d2m.scratch alloc ---
...
func.func @single_scratch_store_load() {
  ...
  ^bb0():
    ...
    %scratch = memref.alloc() {d2m.scratch} : memref<1x8x!ttcore.tile<32x32, f32>, #ttcore.cb_layout<32768x4096, 1>, #l1>
    %c0 = arith.constant 0 : index
    // CHECK: %[[SCRATCH:.*]] = memref.alloc() {d2m.scratch}
...
// --- Test 3: Generic without scratch alloc is unchanged ---
...
func.func @scratch_bf16() {
  ...
  ^bb0():
    ...
    %alloc_cb9 = memref.alloc() : memref<2x2x!ttcore.tile<32x32, bf16>, #l1>
    %scratch = memref.alloc() {d2m.scratch} : memref<1x4x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<8192x2048, 1>, #l1>
    %c0 = arith.constant 0 : index
    // CHECK: %[[SCRATCH:.*]] = memref.alloc() {d2m.scratch}

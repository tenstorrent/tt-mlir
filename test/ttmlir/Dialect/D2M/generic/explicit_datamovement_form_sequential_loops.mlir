...
// rows [0, M/2), and the second loop nest processes rows [M/2, M).
// Uses a 1x1 grid with both operands wrapped in stream_layout.
//
// Loop structure:
//   for m = 0 to M/2 - 1:
//     for n = 0 to N - 1:
...
// CHECK-LABEL: func.func @explicit_datamovement_split_loops_with_streams
func.func @explicit_datamovement_split_loops_with_streams(
  ...
  // First loop nest: rows [0, 1)
  // CHECK: scf.for
  // CHECK: d2m.remote_load
  // CHECK: linalg.generic
  // CHECK: d2m.tile_abs
  // CHECK: d2m.remote_store
  // Second loop nest: rows [1, 2)
  // CHECK: scf.for
  // CHECK: d2m.remote_load
  // CHECK: linalg.generic
  // CHECK: d2m.tile_abs
  ...
  ^unified0(%cb_in: !d2m.cb<tensor<2x2x!ttcore.tile<32x32, f32>>>,
    ...
    // First loop nest: process rows [0, M/2) = [0, 1)
    // for m = 0 to M/2 - 1:
    //   for n = 0 to N - 1:
    ...
    // Second loop nest: process rows [M/2, M) = [1, 2)
    // for m = M/2 to M - 1:
    //   for n = 0 to N - 1:
    scf.for %i = %c1 to %c2 step %c1 {
      scf.for %j = %c0 to %c3 step %c1 {
        %buffer_in = tensor.empty() : tensor<2x2x!ttcore.tile<32x32, f32>>
        %buffer_out = tensor.empty() : tensor<2x2x!ttcore.tile<32x32, f32>>

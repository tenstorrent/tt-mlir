...
!stT = memref<1x64x4x4x!ttcore.tile<32x32, f32>, #ttcore.view<map(4)>, #ttcore.memory_space<l1>>
...
func.func @virtual_grid_noblocking(%arg0 : memref<1x64x4x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096>, #ttcore.memory_space<l1>>)  {
  ...
  %physSbA = memref.alloc() {address = 361888 : i64, alignment = 16 : i64} : !physT
  %virtInA = d2m.view_layout %physInA : !physT -> !virtT
  %virtSbA = d2m.view_layout %physSbA : !physT -> !virtT

  %streamIn = "d2m.stream_layout"(%virtInA, %virtSbA) : (!virtT, !virtT) -> !stT

  // output stream
  %physOut   = memref.alloc() {address = 558496 : i64, alignment = 16 : i64} : !physT
  %physSbOut = memref.alloc() {address = 361888 : i64, alignment = 16 : i64} : !physT
  %virtOut   = d2m.view_layout %physOut : !physT -> !virtT
  %virtSbOut = d2m.view_layout %physSbOut : !physT -> !virtT

  %streamOut = "d2m.stream_layout"(%virtOut, %virtSbOut) : (!virtT, !virtT) -> !stT

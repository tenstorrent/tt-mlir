// RUN: ttmlir-opt %s -ttir-elementwise-fusion -split-input-file | FileCheck %s --check-prefix=NEG
...
module {
  func.func @main(%arg0: tensor<4x4xf32>, %arg1: tensor<4x4xf32>) -> tensor<4x4xf32> {
    ...
    // Mismatched grid prevents fusion
    %cons = ttir.generic {grid = #ttcore.grid<1x2>, block_factors = [1, 1], indexing_maps = [#map, #map, #map], iterator_types = [#parallel, #parallel], threads = [#ttir.thread<compute>]}
...
// NEG-LABEL: func.func @main
// NEG: ttir.generic {{.*}}grid = #ttcore.grid<1x1>
// NEG: ttir.generic {{.*}}grid = #ttcore.grid<1x2>

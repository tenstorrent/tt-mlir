...
module {
  ...
  func.func @generic_with_global_semaphore(%arg0: tensor<1x1x1x1x!ttcore.tile<32x32, f32>, #layout>) -> tensor<1x1x1x1x!ttcore.tile<32x32, f32>, #layout> {
    ...
    %5 = d2m.generic {block_factors = [], grid = #ttcore.grid<1x1>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<unified>]}
     ...
     {
    // CHECK: %6 = d2m.get_global_semaphore operand_index = 2 resolution_stage = compile : !d2m.global_semaphore
    ^unified0():
...
module {
  func.func @test() {
    ...
    {
    // CHECK: %4 = d2m.get_local_semaphore operand_index = 3 resolution_stage = compile : !d2m.local_semaphore
    // CHECK: %5 = d2m.get_local_semaphore operand_index = 2 resolution_stage = compile : !d2m.local_semaphore
    ^datamovement0():
    ...
    // CHECK: %4 = d2m.get_local_semaphore operand_index = 5 resolution_stage = compile : !d2m.local_semaphore
    // CHECK: %5 = d2m.get_local_semaphore operand_index = 4 resolution_stage = compile : !d2m.local_semaphore
    ^datamovement1():
      %cb0 = d2m.get_cb(0) operand_index = 0 : <memref<8x8x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #l1>>
      %cb1 = d2m.get_cb(1) operand_index = 1 : <memref<8x8x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 1>, #l1>>
      %c7 = arith.constant 7 : index
...
module {
  func.func @test(%arg0: index) {
    ...
    d2m.generic {block_factors = [], grid = #ttcore.grid<1x1>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<unified>]}
     ...
     {
    // CHECK: %0 = d2m.get_scalar operand_index = 2 resolution_stage = compile : index
    ^unified0():
      %cb0 = d2m.get_cb(0) operand_index = 0 : <memref<32x32xf32, #l1>>
      %cb1 = d2m.get_cb(1) operand_index = 1 : <memref<1x1x!ttcore.tile<32x32, f32>, #l1>>
      %c0 = arith.constant 0 : index
...
module {
  func.func @test(%arg0: i32, %arg1: si32, %arg2: ui16, %arg3: si16, %arg4: ui8, %arg5: si8, %arg6: i1, %arg7: f32, %arg8: bf16, %arg9: f16) {
    ...
    d2m.generic {block_factors = [], grid = #ttcore.grid<1x1>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<unified>]}
     ...
     {
    ^unified0():
      // CHECK: %0 = d2m.get_scalar operand_index = 11 resolution_stage =  compile : f16
      // CHECK: %1 = d2m.get_scalar operand_index = 10 resolution_stage =  compile : bf16
      // CHECK: %2 = d2m.get_scalar operand_index = 9 resolution_stage =  compile : f32
      // CHECK: %3 = d2m.get_scalar operand_index = 8 resolution_stage =  compile : i1
      // CHECK: %4 = d2m.get_scalar operand_index = 7 resolution_stage =  compile : si8
      // CHECK: %5 = d2m.get_scalar operand_index = 6 resolution_stage =  compile : ui8
      // CHECK: %6 = d2m.get_scalar operand_index = 5 resolution_stage =  compile : si16
      // CHECK: %7 = d2m.get_scalar operand_index = 4 resolution_stage =  compile : ui16
      // CHECK: %8 = d2m.get_scalar operand_index = 3 resolution_stage =  compile : si32
      // CHECK: %9 = d2m.get_scalar operand_index = 2 resolution_stage =  compile : i32
  ...
  func.func @add(%arg0: memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<0x0, 1>, #l1>, %arg1: memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<0x0, 1>, #l1>) -> memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<0x0, 1>, #l1> {
    ...
    d2m.generic {block_factors = [], grid = #ttcore.grid<1x1>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement>, #d2m.thread<compute>]}
        ...
        outs(%alloc : memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<0x0, 1>, #l1>)
     {
      ...
      %c0 = arith.constant 0 : index
      // CHECK: %4 = d2m.get_buffer operand_index = 0 resolution_stage = compile : memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<0x0, 1>, #l1>

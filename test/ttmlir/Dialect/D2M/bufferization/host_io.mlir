...
module {
  ...
  func.func @test_bounce_buffer_insertion(%arg0: tensor<3x43x7xf32>, %arg1: tensor<7x43x7xf32>) -> tensor<9x43x7xf32> {
    ...
    %1 = d2m.to_layout %arg0, %0 : tensor<3x43x7xf32> into tensor<6x1x32x32xf32, #ttcore.metal_layout<logical_shape = 3x43x7, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>> hostInfo = <logical_shape = 3x43x7, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)> -> tensor<6x1x32x32xf32, #ttcore.metal_layout<logical_shape = 3x43x7, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>>
    ...
    %6 = d2m.to_layout %5, %4 : tensor<8x1x96x32xf32, #ttcore.metal_layout<logical_shape = 9x43x7, dim_alignments = 256x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>> into tensor<9x43x7xf32> hostInfo = <logical_shape = 9x43x7, dim_alignments = 256x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)> -> tensor<9x43x7xf32>
  ...
  func.func @test_skipped_bounce_buffer_insertion(%arg0: tensor<8x32x32xf32>) -> tensor<8x32x32xf32> {
    ...
    %1 = d2m.to_layout %arg0, %0 : tensor<8x32x32xf32> into tensor<8x1x32x32xf32, #ttcore.metal_layout<logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>> hostInfo = <logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)> -> tensor<8x1x32x32xf32, #ttcore.metal_layout<logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>>
    ...
    %4 = d2m.to_layout %3, %2 : tensor<8x1x32x32xf32, #ttcore.metal_layout<logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)>> into tensor<8x32x32xf32> hostInfo = <logical_shape = 8x32x32, dim_alignments = 1x32x32, collapsed_intervals = dense<[[0, 2], [2, 3]]> : tensor<2x2xi64>, undef, l1, sharded, index_map = map(0)> -> tensor<8x32x32xf32>

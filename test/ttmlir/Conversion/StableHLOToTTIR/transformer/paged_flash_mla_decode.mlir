...
module @paged_flash_mla_decode attributes {} {
  ...
  func.func public @mla_decode_value_mask(%query: tensor<1x8x16x64xbf16>, %key: tensor<128x2x32x64xbf16>, %value: tensor<128x2x32x32xbf16>, %page_table: tensor<8x4xi32>, %attn_mask: tensor<8x16x1x128xbf16>) -> tensor<1x8x16x32xbf16> {
    ...
    %0 = stablehlo.custom_call @tt.paged_flash_mla_decode(%query, %key, %value, %page_table, %attn_mask) {api_version = 0 : i32, mhlo.frontend_attributes = {head_dim_v = "32", has_attention_mask = "True", has_attention_sink = "False", has_cur_pos_tensor = "False", has_value = "True", is_causal = "False"}} : (tensor<1x8x16x64xbf16>, tensor<128x2x32x64xbf16>, tensor<128x2x32x32xbf16>, tensor<8x4xi32>, tensor<8x16x1x128xbf16>) -> tensor<1x8x16x32xbf16>
  ...
  func.func public @mla_decode_sink_scale(%query: tensor<1x8x16x64xbf16>, %key: tensor<128x2x32x64xbf16>, %value: tensor<128x2x32x64xbf16>, %page_table: tensor<8x4xi32>, %cur_pos: tensor<8xi32>, %sink: tensor<16xbf16>) -> tensor<1x8x16x64xbf16> {

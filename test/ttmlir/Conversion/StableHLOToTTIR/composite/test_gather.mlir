...
module @gather_composite_tests attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  // Test: Complex gather — complex types are lowered before this pass, so the
  // index is reshaped and broadcast to match the input rank for ttir.gather.
  ...
  func.func @gather_complex_dim1(%arg0: tensor<1x512x16xcomplex<f32>>, %arg1: tensor<1x4352x16xi64>) -> tensor<1x4352x16xcomplex<f32>> {
    // CHECK: "ttir.typecast"
    // CHECK-SAME: -> tensor<1x4352x16xui32>
    // CHECK: "ttir.reshape"
    // CHECK-SAME: (tensor<1x4352x16xui32>) -> tensor<1x4352x16x1xui32>
    // CHECK: "ttir.broadcast"
    // CHECK-SAME: broadcast_dimensions = array<i64: 1, 1, 1, 2>
    // CHECK-SAME: (tensor<1x4352x16x1xui32>) -> tensor<1x4352x16x2xui32>
    // CHECK: "ttir.gather"
    // CHECK-SAME: dim = 1 : i32

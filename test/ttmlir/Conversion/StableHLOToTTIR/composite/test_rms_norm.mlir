...
module @jit__rms_norm attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func public @main(%arg0: tensor<4x32xf32> {ttcore.argument_type = #ttcore.argument_type<input>, ttcore.shard_status = #ttcore.shard_status<unsharded>, ttir.name = "args_0"}) -> (tensor<4x32xf32> {jax.result_info = "result", ttcore.shard_status = #ttcore.shard_status<unsharded>}) {
    ...
    %2 = stablehlo.composite "tenstorrent.rms_norm" %1 {composite_attributes = {epsilon = 9.99999974E-6 : f32, normalized_shape = dense<32> : tensor<1xi64>}, decomposition = @tenstorrent.rms_norm.impl} : (tensor<4x32xf32>) -> tensor<4x32xf32>
  ...
  func.func private @tenstorrent.rms_norm.impl(%arg0: tensor<4x32xf32> {ttcore.argument_type = #ttcore.argument_type<input>, ttcore.shard_status = #ttcore.shard_status<unsharded>}) -> (tensor<4x32xf32> {ttcore.shard_status = #ttcore.shard_status<unsharded>}) {
    %cst = stablehlo.constant dense<9.99999974E-6> : tensor<4x1xf32> 

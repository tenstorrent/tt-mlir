...
// After refactor: index_map is now on stream_layout remapping attribute, not on metal_layout.
// Only 6 unique layouts (no separate layouts with index_map).
...
// CHECK-SAME: l1, sharded>
// CHECK: #map = affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 * 4 + d2 + (d1 * 4 + d3) floordiv 32) mod 32, (d1 * 4 + d3) mod 32)>
...
func.func @test_lower_interleaved_dram(
  ...
  // CHECK: %[[RESULT:.*]] = d2m.generic{{.*}}grid = #ttcore.grid<8x8>

...
// CHECK-SAME: index_map = (d0, d1, d2, d3) -> (((d0 + d1) floordiv 8) mod 8, (d0 + d1) mod 8, d2, d3)>
// CHECK: #layout17 = #ttcore.metal_layout<logical_shape = 4096x32, dim_alignments = 32x32, collapsed_intervals
...
func.func @test_lower_height_sharded_l1(
  ...
  // CHECK: ins(%[[VIEW1]] : tensor<6x1x2x1x!ttcore.tile<32x32, bf16>, #layout2>)
  // CHECK: outs(%[[EMPTY0]] : tensor<6x1x2x1x!ttcore.tile<32x32, bf16>, #layout1>)
...
func.func @test_lower_height_sharded_l1_1(
  ...
  // CHECK: %[[VIEW0:.*]] = d2m.view_layout %[[CAST0]] : tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout3> -> tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout1>
  // CHECK: %[[EMPTY0:.*]] = d2m.empty() : tensor<6x1x2x1x!ttcore.tile<32x32, bf16>, #layout1>
  // CHECK: %[[VIEW1:.*]] = d2m.view_layout %[[VIEW0]] : tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout1> -> tensor<6x1x2x1x!ttcore.tile<32x32, bf16>, #layout2>
  // CHECK: %[[RESULT:.*]] = d2m.generic
  // CHECK-SAME: grid = #ttcore.grid<6x1>
  ...
  // CHECK: %[[CAST1:.*]] = ttir.ttnn_metal_layout_cast %[[EMPTY1]] : tensor<384x32xbf16, #ttnn_layout1> -> tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout3>
  // CHECK: %[[LAYOUT:.*]] = d2m.to_layout %[[RESULT]], %[[CAST1]] : tensor<6x1x2x1x!ttcore.tile<32x32, bf16>, #layout1> into tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout3> -> tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout3>
  // CHECK: %[[CAST2:.*]] = ttir.ttnn_metal_layout_cast %[[LAYOUT]] : tensor<12x1x1x1x!ttcore.tile<32x32, bf16>, #layout3> -> tensor<384x32xbf16, #ttnn_layout1>
...
func.func @test_lower_height_sharded_l1_2(
  ...
  // CHECK: %[[VIEW0:.*]] = d2m.view_layout %[[CAST0]] : tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout4> -> tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout5>
  // CHECK: %[[EMPTY0:.*]] = d2m.empty() : tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout5>
  // CHECK: %[[VIEW1:.*]] = d2m.view_layout %[[VIEW0]] : tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout5> -> tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout6>
  // CHECK: %[[RESULT:.*]] = d2m.generic
  // CHECK-SAME: grid = #ttcore.grid<6x2>
  // CHECK: ins(%[[VIEW1]] : tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout6>)
  // CHECK: outs(%[[EMPTY0]] : tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout5>)
  // CHECK-DAG: d2m.tile_abs
  // CHECK: %[[EMPTY1:.*]] = d2m.empty() : tensor<384x64xbf16, #ttnn_layout2>
  // CHECK: %[[CAST1:.*]] = ttir.ttnn_metal_layout_cast %[[EMPTY1]] : tensor<384x64xbf16, #ttnn_layout2> -> tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout4>
  // CHECK: %[[LAYOUT:.*]] = d2m.to_layout %[[RESULT]], %[[CAST1]] : tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout5> into tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout4> -> tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout4>
  // CHECK: %[[CAST2:.*]] = ttir.ttnn_metal_layout_cast %[[LAYOUT]] : tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout4> -> tensor<384x64xbf16, #ttnn_layout2>
...
func.func @test_lower_height_sharded_l1_3(
  ...
  // CHECK: %[[VIEW0:.*]] = d2m.view_layout %[[CAST0]] : tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout7> -> tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout5>
  // CHECK: %[[EMPTY0:.*]] = d2m.empty() : tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout5>
  // CHECK: %[[VIEW1:.*]] = d2m.view_layout %[[VIEW0]] : tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout5> -> tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout6>
  // CHECK: %[[RESULT:.*]] = d2m.generic
  // CHECK-SAME: grid = #ttcore.grid<6x2>
  ...
  // CHECK: %[[CAST1:.*]] = ttir.ttnn_metal_layout_cast %[[EMPTY1]] : tensor<384x64xbf16, #ttnn_layout3> -> tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout7>
  // CHECK: %[[LAYOUT:.*]] = d2m.to_layout %[[RESULT]], %[[CAST1]] : tensor<6x2x2x1x!ttcore.tile<32x32, bf16>, #layout5> into tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout7> -> tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout7>
  // CHECK: %[[CAST2:.*]] = ttir.ttnn_metal_layout_cast %[[LAYOUT]] : tensor<12x1x1x2x!ttcore.tile<32x32, bf16>, #layout7> -> tensor<384x64xbf16, #ttnn_layout3>
...
func.func @test_lower_height_sharded_l1_4(
  ...
  // CHECK: %[[VIEW0:.*]] = d2m.view_layout %[[CAST0]] : tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout8> -> tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout9>
  // CHECK: %[[EMPTY0:.*]] = d2m.empty() : tensor<8x1x3x1x!ttcore.tile<32x32, bf16>, #layout9>
  // CHECK: %[[VIEW1:.*]] = d2m.view_layout %[[VIEW0]] : tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout9> -> tensor<8x1x3x1x!ttcore.tile<32x32, bf16>, #layout10>
  // CHECK: %[[RESULT:.*]] = d2m.generic
  // CHECK-SAME: grid = #ttcore.grid<8x1>
  // CHECK: ins(%[[VIEW1]] : tensor<8x1x3x1x!ttcore.tile<32x32, bf16>, #layout10>)
  // CHECK: outs(%[[EMPTY0]] : tensor<8x1x3x1x!ttcore.tile<32x32, bf16>, #layout9>)
  // CHECK-DAG: d2m.tile_abs
  // CHECK: %[[EMPTY1:.*]] = d2m.empty() : tensor<2x384x32xbf16, #ttnn_layout4>
  // CHECK: %[[CAST1:.*]] = ttir.ttnn_metal_layout_cast %[[EMPTY1]] : tensor<2x384x32xbf16, #ttnn_layout4> -> tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout8>
  // CHECK: %[[LAYOUT:.*]] = d2m.to_layout %[[RESULT]], %[[CAST1]] : tensor<8x1x3x1x!ttcore.tile<32x32, bf16>, #layout9> into tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout8> -> tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout8>
  // CHECK: %[[CAST2:.*]] = ttir.ttnn_metal_layout_cast %[[LAYOUT]] : tensor<12x1x2x1x!ttcore.tile<32x32, bf16>, #layout8> -> tensor<2x384x32xbf16, #ttnn_layout4>
...
func.func @test_lower_height_sharded_l1_5(
  ...
  // CHECK: %[[VIEW0:.*]] = d2m.view_layout %[[CAST0]] : tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout11> -> tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout12>
  // CHECK: %[[EMPTY0:.*]] = d2m.empty() : tensor<8x1x6x1x!ttcore.tile<32x32, bf16>, #layout12>
  // CHECK: %[[VIEW1:.*]] = d2m.view_layout %[[VIEW0]] : tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout12> -> tensor<8x1x6x1x!ttcore.tile<32x32, bf16>, #layout13>
  // CHECK: %[[RESULT:.*]] = d2m.generic
  // CHECK-SAME: grid = #ttcore.grid<8x1>
  // CHECK: ins(%[[VIEW1]] : tensor<8x1x6x1x!ttcore.tile<32x32, bf16>, #layout13>)
  // CHECK: outs(%[[EMPTY0]] : tensor<8x1x6x1x!ttcore.tile<32x32, bf16>, #layout12>)
  // CHECK-DAG: d2m.tile_abs
  // CHECK: %[[EMPTY1:.*]] = d2m.empty() : tensor<2x2x384x32xbf16, #ttnn_layout5>
  // CHECK: %[[CAST1:.*]] = ttir.ttnn_metal_layout_cast %[[EMPTY1]] : tensor<2x2x384x32xbf16, #ttnn_layout5> -> tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout11>
  // CHECK: %[[LAYOUT:.*]] = d2m.to_layout %[[RESULT]], %[[CAST1]] : tensor<8x1x6x1x!ttcore.tile<32x32, bf16>, #layout12> into tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout11> -> tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout11>
  // CHECK: %[[CAST2:.*]] = ttir.ttnn_metal_layout_cast %[[LAYOUT]] : tensor<12x1x4x1x!ttcore.tile<32x32, bf16>, #layout11> -> tensor<2x2x384x32xbf16, #ttnn_layout5>

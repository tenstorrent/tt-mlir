...
func.func @test_local_to_local_remote_core(%arg0: memref<2x2x!tt.tile<32x32, f32>, #l1_>, %arg1: memref<2x2x!tt.tile<32x32, f32>, #l1_>) {
  ...
  // CHECK: ttkernel.get_noc_addr_xy
  // CHECK: ttkernel.noc_async_write
  %0 = ttir.dma %arg0, %dst[%c0, %c1] : (memref<2x2x!tt.tile<32x32, f32>, #l1_>, memref<1x2x2x2x!tt.tile<32x32, f32>, #tt.stream<(d0, d1, d2, d3) -> (d0, d1, d2 * 16384 + d3 * 4096)>, #l1_>) -> !ttir.mem_tx
  return
}

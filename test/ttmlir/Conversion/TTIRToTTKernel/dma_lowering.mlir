...
func.func @test_local_to_local_same_core(%arg0: memref<2x2x!tt.tile<32x32, f32>, #l1_>, %arg1: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
  // CHECK: ttkernel.get_read_ptr
  // CHECK: ttkernel.get_write_ptr
  // CHECK: ttkernel.get_noc_addr_xy
  // CHECK: ttkernel.noc_async_write
  %c0 = arith.constant 0 : index
  %0 = ttir.dma %arg0[%c0, %c0], %arg1[%c0, %c0] : (memref<2x2x!tt.tile<32x32, f32>, #l1_>, memref<2x2x!tt.tile<32x32, f32>, #l1_>) -> !ttir.mem_tx
  return
}

// Test 2: Local to local DMA between different cores
// CHECK-LABEL: func.func @test_local_to_local_remote_core
func.func @test_local_to_local_remote_core(%arg0: memref<2x2x!tt.tile<32x32, f32>, #l1_>, %arg1: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
...
func.func @test_local_to_remote_multicast_regular(%arg0: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  ...
  %0 = ttir.dma %arg0[%c0, %c0], %arg0[%c0, %c0] core[%c1, %c2] mcast[%c3, %c4] : (memref<2x2x!tt.tile<32x32, f32>, #l1_>, memref<2x2x!tt.tile<32x32, f32>, #l1_>) -> !ttir.mem_tx
  return
}
...
func.func @test_local_to_remote_multicast_loopback(%arg0: memref<2x2x!tt.tile<32x32, f32>, #l1_>, %arg1: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %c3 = arith.constant 3 : index
  ...
  %0 = ttir.dma %arg0[%c0, %c0], %arg1[%c0, %c0] core[%c1, %c2] mcast[%c3, %c4] : (memref<2x2x!tt.tile<32x32, f32>, #l1_>, memref<2x2x!tt.tile<32x32, f32>, #l1_>) -> !ttir.mem_tx
  return
}

// Test 5: Remote to local DMA (L1 from another core)
// CHECK-LABEL: func.func @test_remote_l1_to_local
func.func @test_remote_l1_to_local(%dst: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
...
func.func @test_dma_wait_read(%dst: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %src = ttir.get_global_operand(0) : memref<1x2x2x2x!tt.tile<32x32, f32>, #tt.stream<(d0, d1, d2, d3) -> (d0, d1, d2 * 16384 + d3 * 4096)>, #l1_>
  %0 = ttir.dma %src[%c0, %c1, %c0, %c0], %dst[%c0, %c0] : (memref<1x2x2x2x!tt.tile<32x32, f32>, #tt.stream<(d0, d1, d2, d3) -> (d0, d1, d2 * 16384 + d3 * 4096)>, #l1_>, memref<2x2x!tt.tile<32x32, f32>, #l1_>) -> !ttir.mem_tx
...
func.func @test_dma_wait_write(%arg0: memref<2x2x!tt.tile<32x32, f32>, #l1_>, %arg1: memref<2x2x!tt.tile<32x32, f32>, #l1_>) attributes {ttir.thread_type = 1 : i32} {
  %c0 = arith.constant 0 : index
  %0 = ttir.dma %arg0[%c0, %c0], %arg1[%c0, %c0] : (memref<2x2x!tt.tile<32x32, f32>, #l1_>, memref<2x2x!tt.tile<32x32, f32>, #l1_>) -> !ttir.mem_tx

...
module attributes {ttcore.system_desc = #ttcore.system_desc<[{role = host, target_triple = "x86_64-pc-linux"}], [{arch = <wormhole_b0>, grid = 8x8, coord_translation_offsets = 18x18, l1_size = 1499136, num_dram_channels = 12, dram_channel_size = 1073741824, noc_l1_address_align_bytes = 16, pcie_address_align_bytes = 32, noc_dram_address_align_bytes = 32, l1_unreserved_base = 103712, erisc_l1_unreserved_base = 98304, dram_unreserved_base = 1920032, dram_unreserved_end = 1073119552, supported_data_types = [<f32>, <f16>, <bf16>, <bfp_f8>, <bfp_bf8>, <bfp_f4>, <bfp_bf4>, <bfp_f2>, <bfp_bf2>, <u32>, <u16>, <u8>, <si32>], supported_tile_sizes = [ 4x16,  16x16,  32x16,  4x32,  16x32,  32x32], dst_physical_size_tiles = 16, num_cbs = 64, num_compute_threads = 1, num_datamovement_threads = 2}], [0], [1 : i32], [ 0x0x0x0]>} {
  ...
  func.func @abs_test(%arg0: tensor<64x128xf32, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<2x4x!ttcore.tile<32x32, f32>, #ttnn.buffer_type<dram>>, <interleaved>, exactGrid = true>>) -> tensor<64x128xf32, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<2x4x!ttcore.tile<32x32, f32>, #ttnn.buffer_type<dram>>, <interleaved>, exactGrid = true>> attributes {tt.function_type = "forward_device"} {
    ...
    %alloc = memref.alloc() : memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 2>, #ttcore.memory_space<l1>>
    %stream = "d2m.stream_layout"(%cast, %alloc) <{remapping = affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 + d1 floordiv 4) mod 2, d1 mod 4)>}> : (memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.interleaved<16384x4096>, #ttcore.memory_space<dram>>, memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 2>, #ttcore.memory_space<l1>>) -> memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #ttcore.memory_space<dram>>
    %alloc_1 = memref.alloc() : memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 2>, #ttcore.memory_space<l1>>
    %stream_2 = "d2m.stream_layout"(%cast_0, %alloc_1) <{remapping = affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 + d1 floordiv 4) mod 2, d1 mod 4)>}> : (memref<1x1x2x4x!ttcore.tile<32x32, f32>, #ttcore.interleaved<16384x4096>, #ttcore.memory_space<dram>>, memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.shard<4096x4096, 2>, #ttcore.memory_space<l1>>) -> memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #ttcore.memory_space<dram>>
    %alloc_3 = memref.alloc() {address = 103712 : i64, alignment = 16 : i64, d2m.cb_for_operand = 0 : i64} : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.cb_layout<4096x4096, 2, grid = [2x4]>, #ttcore.memory_space<l1>>
    %alloc_4 = memref.alloc() {address = 111904 : i64, alignment = 16 : i64, d2m.cb_for_operand = 1 : i64} : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.cb_layout<4096x4096, 2, grid = [2x4]>, #ttcore.memory_space<l1>>
    d2m.generic {block_factors = [], grid = #ttcore.grid<2x4>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement, noc = 0, @datamovement_kernel0>, #d2m.thread<datamovement, noc = 1, @datamovement_kernel1>, #d2m.thread<compute, noc = 0, @compute_kernel2>]}
        ins(%stream : memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #ttcore.memory_space<dram>>)
        outs(%stream_2 : memref<2x4x1x1x!ttcore.tile<32x32, f32>, #ttcore.view<4>, #ttcore.memory_space<dram>>)
        additionalArgs(%alloc_3, %alloc_4 : memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.cb_layout<4096x4096, 2, grid = [2x4]>, #ttcore.memory_space<l1>>, memref<1x1x!ttcore.tile<32x32, f32>, #ttcore.cb_layout<4096x4096, 2, grid = [2x4]>, #ttcore.memory_space<l1>>)

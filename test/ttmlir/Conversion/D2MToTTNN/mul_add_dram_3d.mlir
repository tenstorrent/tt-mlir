...
// CHECK: "ttnn.empty"{{.*}}<block_sharded>{{.*}}core_range<(0,0), (7,7)>{{.*}}shape = #ttnn.shape<1024x2048>
// CHECK: "ttnn.empty"{{.*}}<block_sharded>{{.*}}core_range<(0,0), (7,7)>{{.*}}shape = #ttnn.shape<1024x2048>
// CHECK: "ttnn.empty"{{.*}}<block_sharded>{{.*}}core_range<(0,0), (7,7)>{{.*}}shape = #ttnn.shape<1024x2048>
// CHECK: "ttnn.empty"{{.*}}<block_sharded>{{.*}}core_range<(0,0), (7,7)>{{.*}}shape = #ttnn.shape<1024x2048>
...
module attributes {ttcore.system_desc = #ttcore.system_desc<[{role = host, target_triple = "x86_64-pc-linux"}], [{arch = <wormhole_b0>, grid = 8x8, coord_translation_offsets = 18x18, l1_size = 1499136, num_dram_channels = 12, dram_channel_size = 1073741824, noc_l1_address_align_bytes = 16, pcie_address_align_bytes = 32, noc_dram_address_align_bytes = 32, l1_unreserved_base = 103712, erisc_l1_unreserved_base = 98304, dram_unreserved_base = 1920032, dram_unreserved_end = 1073119552, supported_data_types = [<f32>, <f16>, <bf16>, <bfp_f8>, <bfp_bf8>, <bfp_f4>, <bfp_bf4>, <bfp_f2>, <bfp_bf2>, <u32>, <u16>, <u8>, <si32>], supported_tile_sizes = [ 4x16,  16x16,  32x16,  4x32,  16x32,  32x32], dst_physical_size_tiles = 16, num_cbs = 64, num_compute_threads = 1, num_datamovement_threads = 2}], [0], [1 : i32], [ 0x0x0x0]>} {
  ...
  func.func @mul_add(%arg0: tensor<2x512x2048xbf16, #ttnn.ttnn_layout<(d0, d1, d2) -> (d0 * 512 + d1, d2), <1x1>, memref<32x64x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<dram>>, <interleaved>, exactGrid = true>>, %arg1: tensor<2x512x2048xbf16, #ttnn.ttnn_layout<(d0, d1, d2) -> (d0 * 512 + d1, d2), <1x1>, memref<32x64x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<dram>>, <interleaved>, exactGrid = true>>, %arg2: tensor<2x512x2048xbf16, #ttnn.ttnn_layout<(d0, d1, d2) -> (d0 * 512 + d1, d2), <1x1>, memref<32x64x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<dram>>, <interleaved>, exactGrid = true>>) -> tensor<2x512x2048xbf16, #ttnn.ttnn_layout<(d0, d1, d2) -> (d0 * 512 + d1, d2), <8x8>, memref<4x8x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <block_sharded>, exactGrid = true>> attributes {tt.function_type = "forward_device"} {
    ...
    %alloc_1 = memref.alloc() : memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 2>, #ttcore.memory_space<l1>>
    %stream = "d2m.stream_layout"(%cast, %alloc_1) <{remapping = affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 * 4 + d2 + (d1 * 8 + d3) floordiv 64) mod 32, (d1 * 8 + d3) mod 64)>}> : (memref<1x1x32x64x!ttcore.tile<32x32, bf16>, #ttcore.interleaved<131072x2048>, #ttcore.memory_space<dram>>, memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 2>, #ttcore.memory_space<l1>>) -> memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<dram>>
    %alloc_2 = memref.alloc() : memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 2>, #ttcore.memory_space<l1>>
    %stream_3 = "d2m.stream_layout"(%cast_0, %alloc_2) <{remapping = affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 * 4 + d2 + (d1 * 8 + d3) floordiv 64) mod 32, (d1 * 8 + d3) mod 64)>}> : (memref<1x1x32x64x!ttcore.tile<32x32, bf16>, #ttcore.interleaved<131072x2048>, #ttcore.memory_space<dram>>, memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 2>, #ttcore.memory_space<l1>>) -> memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<dram>>
    %alloc_4 = memref.alloc() {address = 103712 : i64, alignment = 16 : i64, d2m.cb_for_operand = 0 : i64} : memref<4x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2, grid = [8x8]>, #ttcore.memory_space<l1>>
    %alloc_5 = memref.alloc() {address = 234784 : i64, alignment = 16 : i64, d2m.cb_for_operand = 1 : i64} : memref<4x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2, grid = [8x8]>, #ttcore.memory_space<l1>>
    ...
    %stream_9 = "d2m.stream_layout"(%cast_6, %alloc_8) <{remapping = affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 * 4 + d2 + (d1 * 8 + d3) floordiv 64) mod 32, (d1 * 8 + d3) mod 64)>}> : (memref<1x1x32x64x!ttcore.tile<32x32, bf16>, #ttcore.interleaved<131072x2048>, #ttcore.memory_space<dram>>, memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 2>, #ttcore.memory_space<l1>>) -> memref<8x8x4x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<dram>>
    %alloc_10 = memref.alloc() {address = 103712 : i64, alignment = 16 : i64, d2m.cb_for_operand = 1 : i64} : memref<4x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2, grid = [8x8]>, #ttcore.memory_space<l1>>

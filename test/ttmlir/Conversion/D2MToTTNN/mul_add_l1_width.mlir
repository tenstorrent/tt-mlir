...
// CHECK: %[[MUL_OUT:.*]] = "ttnn.empty"{{.*}}shape = #ttnn.shape<128x2048>{{.*}}<4x8>{{.*}}<block_sharded>
...
// CHECK: %[[ADD_OUT:.*]] = "ttnn.empty"{{.*}}shape = #ttnn.shape<128x2048>{{.*}}<4x8>{{.*}}<block_sharded>
...
module attributes {ttcore.system_desc = #ttcore.system_desc<[{role = host, target_triple = "x86_64-pc-linux"}], [{arch = <wormhole_b0>, grid = 8x8, coord_translation_offsets = 18x18, l1_size = 1499136, num_dram_channels = 12, dram_channel_size = 1073741824, noc_l1_address_align_bytes = 16, pcie_address_align_bytes = 32, noc_dram_address_align_bytes = 32, l1_unreserved_base = 103712, erisc_l1_unreserved_base = 98304, dram_unreserved_base = 1920032, dram_unreserved_end = 1073119552, supported_data_types = [<f32>, <f16>, <bf16>, <bfp_f8>, <bfp_bf8>, <bfp_f4>, <bfp_bf4>, <bfp_f2>, <bfp_bf2>, <u32>, <u16>, <u8>, <si32>], supported_tile_sizes = [ 4x16,  16x16,  32x16,  4x32,  16x32,  32x32], dst_physical_size_tiles = 16, num_cbs = 64, num_compute_threads = 1, num_datamovement_threads = 2, dram_grid = 1x12, dram_bank_to_logical_worker_noc0 = [(7, 3), (0, 0), (3, 0), (4, 0), (0, 4), (7, 7), (1, 4), (6, 4), (5, 4), (2, 6), (3, 4), (4, 4)], dram_bank_to_logical_worker_noc1 = [(7, 3), (0, 0), (3, 0), (4, 0), (0, 4), (7, 7), (1, 4), (6, 4), (5, 4), (2, 6), (3, 4), (4, 4)]}], [0], [1 : i32], [ 0x0x0x0]>} {
  ...
  func.func @mul_add(%arg0: tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <8x8>, memref<4x1x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <width_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 7)>]>>>, %arg1: tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <8x8>, memref<4x1x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <width_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 7)>]>>>, %arg2: tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <8x8>, memref<4x1x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <width_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 7)>]>>>) -> tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <4x8>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <block_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 3)>]>>> attributes {tt.function_type = "forward_device"} {
    %cast = ttir.ttnn_metal_layout_cast %arg1 {virtual_grid_forward_mapping = affine_map<(d0, d1, d2, d3) -> ((d1 floordiv 8) mod 8, d1 mod 8, d2, d3)>, virtual_grid_inverse_mapping = affine_map<(d0, d1) -> (0, 0, (d1 + d0 * 8) mod 64)>} : tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <8x8>, memref<4x1x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <width_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 7)>]>>> -> memref<1x64x4x1x!ttcore.tile<32x32, bf16>, #ttcore.shard<2048x2048, 1>, #ttcore.memory_space<l1>>
    ...
    %view = d2m.view_layout %cast remapping = affine_map<(d0, d1, d2, d3) -> (0, (d1 * 8 + d3) mod 64, (d0 + (d1 * 8 + d3) floordiv 64) mod 4, 0)> : memref<1x64x4x1x!ttcore.tile<32x32, bf16>, #ttcore.shard<2048x2048, 1>, #ttcore.memory_space<l1>> -> memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<l1>>
    %view_1 = d2m.view_layout %cast_0 remapping = affine_map<(d0, d1, d2, d3) -> (0, (d1 * 8 + d3) mod 64, (d0 + (d1 * 8 + d3) floordiv 64) mod 4, 0)> : memref<1x64x4x1x!ttcore.tile<32x32, bf16>, #ttcore.shard<2048x2048, 1>, #ttcore.memory_space<l1>> -> memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<l1>>
    %alloc_4 = memref.alloc() {address = 103712 : i64, alignment = 16 : i64} : memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2>, #ttcore.memory_space<l1>>
    %alloc_5 = memref.alloc() {address = 136480 : i64, alignment = 16 : i64} : memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2>, #ttcore.memory_space<l1>>
    %alias_2 = d2m.operand_alias %alloc : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>> -> memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 1>, #l1>
    d2m.generic {block_factors = [], grid = #ttcore.grid<4x8>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement, @datamovement_kernel0, processor = 1>, #d2m.thread<datamovement, @datamovement_kernel1, processor = 0>, #d2m.thread<compute, @compute_kernel2>]}
        ins(%view, %view_1 : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<l1>>, memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<l1>>)
        outs(%alloc : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>>)
        additionalArgs(%alloc_4, %alloc_5, %alias_2 : memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2>, #ttcore.memory_space<l1>>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2>, #ttcore.memory_space<l1>>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 1>, #l1>)
    ...
    %view_2 = d2m.view_layout %cast_6 remapping = affine_map<(d0, d1, d2, d3) -> (0, (d1 * 8 + d3) mod 64, (d0 + (d1 * 8 + d3) floordiv 64) mod 4, 0)> : memref<1x64x4x1x!ttcore.tile<32x32, bf16>, #ttcore.shard<2048x2048, 1>, #ttcore.memory_space<l1>> -> memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<l1>>
    %alloc_10 = memref.alloc() {address = 103712 : i64, alignment = 16 : i64} : memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2>, #ttcore.memory_space<l1>>
    %alias_1_0 = d2m.operand_alias %alloc : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>> -> memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 1>, #l1>
    %alias_1_2 = d2m.operand_alias %alloc_7 : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>> -> memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 1>, #l1>
    d2m.generic {block_factors = [], grid = #ttcore.grid<4x8>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement, @datamovement_kernel3, processor = 1>, #d2m.thread<compute, @compute_kernel4>]}
        ins(%alloc, %view_2 : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>>, memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.view<4>, #ttcore.memory_space<l1>>)
        outs(%alloc_7 : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>>)
        additionalArgs(%alias_1_0, %alloc_10, %alias_1_2 : memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 1>, #l1>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 2>, #ttcore.memory_space<l1>>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttcore.cb_layout<16384x2048, 1>, #l1>)

    memref.dealloc %alloc : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>>
    %cast_11 = ttir.ttnn_metal_layout_cast %alloc_7 : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>> -> tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <4x8>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <block_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 3)>]>>>
    memref.dealloc %alloc_7 : memref<4x8x1x8x!ttcore.tile<32x32, bf16>, #ttcore.shard<16384x2048, 1>, #ttcore.memory_space<l1>>
    return %cast_11 : tensor<128x2048xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <4x8>, memref<1x8x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <block_sharded>, core_ranges = #ttnn.core_range_set<[#ttnn.core_range<(0, 0), (7, 3)>]>>>
  }

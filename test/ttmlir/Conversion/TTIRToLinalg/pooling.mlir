...
// MaxPool2dOp with asymmetric padding
module {
  // Input: 1x32x32x64, kernel: 3x3, stride: 1x1, padding: 1x2x3x4
  // OutputH: (32 + 1 + 3 - 3) / 1 + 1 = 34
  // OutputW: (32 + 2 + 4 - 3) / 1 + 1 = 36
  func.func @max_pool2d_asymmetric_padding(%arg0: tensor<1x32x32x64xbf16>) -> tensor<1x34x36x64xbf16> {
    // CHECK: tensor.pad
    // CHECK: linalg.pooling_nhwc_max
    %1 = "ttir.max_pool2d"(%arg0) <{
      kernel = array<i32: 3, 3>,
      ...
      ceil_mode = false
    }> : (tensor<1x32x32x64xbf16>) -> tensor<1x34x36x64xbf16>
    return %1 : tensor<1x34x36x64xbf16>
  }
}

// MaxPool2dOp with asymmetric kernel and stride
module {
  // Input: 1x32x32x64, kernel: 2x3, stride: 1x2, no padding
  // OutputH: (32 - 2) / 1 + 1 = 31
  // OutputW: (32 - 3) / 2 + 1 = 15
  func.func @max_pool2d_asymmetric_kernel_stride(%arg0: tensor<1x32x32x64xbf16>) -> tensor<1x31x15x64xbf16> {
    // CHECK: linalg.pooling_nhwc_max
    %1 = "ttir.max_pool2d"(%arg0) <{
      kernel = array<i32: 2, 3>,
      stride = array<i32: 1, 2>,
      dilation = array<i32: 1, 1>,
      padding = array<i32: 0, 0, 0, 0>,
      ceil_mode = false
    }> : (tensor<1x32x32x64xbf16>) -> tensor<1x31x15x64xbf16>
    return %1 : tensor<1x31x15x64xbf16>
  }
}

// MaxPool2dOp with flattened input: batch=1, 32x32 input, 3x3 kernel, stride 1.
// Input is (1, 1, 1*32*32, 64) = (1, 1, 1024, 64).
// Output is (1, 1, 1*30*30, 64) = (1, 1, 900, 64).
module {
...
// MaxPool2dOp with flattened input and batch > 1.
// Input is (1, 1, 2*8*8, 32) = (1, 1, 128, 32); FlatInfo: N=2, H=8, W=8.
// Output: (8 - 3) / 2 + 1 = 3, so (1, 1, 2*3*3, 32) = (1, 1, 18, 32).
module {
  func.func @max_pool2d_flattened_batch(%arg0: tensor<1x1x128x32xbf16>) -> tensor<1x1x18x32xbf16> {
    // CHECK: tosa.reshape
    // CHECK: linalg.pooling_nhwc_max
    // CHECK: tosa.reshape
    %1 = "ttir.max_pool2d"(%arg0) <{
      ...
      padding = array<i32: 0, 0, 0, 0>,
      ceil_mode = false,
      flattened_compat_info = #ttir<flattened_compat batch_size = 2, input_height = 8, input_width = 8>
    }> : (tensor<1x1x128x32xbf16>) -> tensor<1x1x18x32xbf16>
    return %1 : tensor<1x1x18x32xbf16>
  }
}

// MaxPool2dOp with flattened input and padding.
// Input is (1, 1, 1*16*16, 64) = (1, 1, 256, 64); FlatInfo: N=1, H=16, W=16.
// Output: (16 + 1 + 1 - 3) / 1 + 1 = 16, so (1, 1, 256, 64).
module {
  func.func @max_pool2d_flattened_padding(%arg0: tensor<1x1x256x64xbf16>) -> tensor<1x1x256x64xbf16> {
    // CHECK: tosa.reshape
    // CHECK: tensor.pad
    // CHECK: linalg.pooling_nhwc_max
    // CHECK: tosa.reshape
    %1 = "ttir.max_pool2d"(%arg0) <{
      ...
      padding = array<i32: 1, 1, 1, 1>,
      ceil_mode = false,
      flattened_compat_info = #ttir<flattened_compat batch_size = 1, input_height = 16, input_width = 16>
    }> : (tensor<1x1x256x64xbf16>) -> tensor<1x1x256x64xbf16>
    return %1 : tensor<1x1x256x64xbf16>
  }
}
...
// AvgPool2dOp with asymmetric padding, count_include_pad=false
module {
  // Input: 1x32x32x64, kernel: 3x3, stride: 1x1, padding: 1x2x3x4
  // OutputH: (32 + 1 + 3 - 3) / 1 + 1 = 34
  // OutputW: (32 + 2 + 4 - 3) / 1 + 1 = 36
  func.func @avg_pool2d_asymmetric_padding(%arg0: tensor<1x32x32x64xbf16>) -> tensor<1x34x36x64xbf16> {
    // CHECK: tensor.pad
    // CHECK: linalg.pooling_nhwc_sum
    // CHECK: tensor.pad
    // CHECK: linalg.pooling_nhwc_sum
...
// AvgPool2dOp with asymmetric kernel and stride
module {
  // Input: 1x32x32x64, kernel: 2x4, stride: 1x2, no padding
  // OutputH: (32 - 2) / 1 + 1 = 31
  // OutputW: (32 - 4) / 2 + 1 = 15
  func.func @avg_pool2d_asymmetric_kernel_stride(%arg0: tensor<1x32x32x64xbf16>) -> tensor<1x31x15x64xbf16> {
    // CHECK: linalg.pooling_nhwc_sum
    // CHECK: linalg.pooling_nhwc_sum
    // CHECK: linalg.div
    %1 = "ttir.avg_pool2d"(%arg0) <{
      kernel = array<i32: 2, 4>,
      stride = array<i32: 1, 2>,
      dilation = array<i32: 1, 1>,
      padding = array<i32: 0, 0, 0, 0>,
      ceil_mode = false,
...
// AvgPool2dOp with flattened input: batch=1, 32x32 input, 3x3 kernel, stride 1.
// Input is (1, 1, 1024, 64). Output is (1, 1, 900, 64).
module {
...
// AvgPool2dOp with flattened input, padding, and count_include_pad=false.
// Input is (1, 1, 1024, 64); FlatInfo: N=1, H=32, W=32.
// Output: (32 + 1 + 1 - 3) / 1 + 1 = 32, so (1, 1, 1024, 64).
module {
  func.func @avg_pool2d_flattened_exclude_pad(%arg0: tensor<1x1x1024x64xbf16>) -> tensor<1x1x1024x64xbf16> {
    // CHECK: tosa.reshape
    // CHECK: tensor.pad
    // CHECK: linalg.pooling_nhwc_sum
    // CHECK: tensor.pad
    ...
    %1 = "ttir.avg_pool2d"(%arg0) <{
      ...
      count_include_pad = false,
      flattened_compat_info = #ttir<flattened_compat batch_size = 1, input_height = 32, input_width = 32>
    }> : (tensor<1x1x1024x64xbf16>) -> tensor<1x1x1024x64xbf16>
...
// MaxPool2dWithIndicesOp with asymmetric padding
module {
  // Input: 1x8x8x32, kernel: 3x3, stride: 1x1, padding: 1x2x3x0
  // OutputH: (8 + 1 + 3 - 3) / 1 + 1 = 10
  // OutputW: (8 + 2 + 0 - 3) / 1 + 1 = 8
  func.func @max_pool2d_with_indices_asymmetric_padding(%arg0: tensor<1x8x8x32xbf16>) -> (tensor<1x10x8x32xbf16>, tensor<1x10x8x32xi32>) {
    // CHECK-LABEL: func.func @max_pool2d_with_indices_asymmetric_padding
    // CHECK: tensor.pad
    // CHECK: linalg.generic
    // CHECK: arith.cmpf ogt
    // CHECK: arith.select
    %0 = tensor.empty() : tensor<1x10x8x32xbf16>
    %1 = tensor.empty() : tensor<1x10x8x32xi32>
    %2:2 = "ttir.max_pool2d_with_indices"(%arg0, %0, %1) <{
      kernel = array<i32: 3, 3>,
      stride = array<i32: 1, 1>,
      dilation = array<i32: 1, 1>,
      padding = array<i32: 1, 2, 3, 0>,
      ceil_mode = false
    }> : (tensor<1x8x8x32xbf16>, tensor<1x10x8x32xbf16>, tensor<1x10x8x32xi32>) -> (tensor<1x10x8x32xbf16>, tensor<1x10x8x32xi32>)
    return %2#0, %2#1 : tensor<1x10x8x32xbf16>, tensor<1x10x8x32xi32>
  }
}

// MaxPool2dWithIndicesOp with flattened input: batch=1, 8x8 input, 3x3 kernel, stride 2.
// Input is (1, 1, 1*8*8, 32) = (1, 1, 64, 32).
// Output: (8 - 3) / 2 + 1 = 3, so (1, 1, 1*3*3, 32) = (1, 1, 9, 32).
module {
  func.func @max_pool2d_with_indices_flattened(%arg0: tensor<1x1x64x32xbf16>) -> (tensor<1x1x9x32xbf16>, tensor<1x1x9x32xi32>) {
    // CHECK-LABEL: func.func @max_pool2d_with_indices_flattened
    // CHECK: tosa.reshape
    // CHECK: linalg.generic
    // CHECK: arith.cmpf ogt
    // CHECK: arith.select
    // CHECK: tosa.reshape
    // CHECK: tosa.reshape
    %0 = tensor.empty() : tensor<1x1x9x32xbf16>
    %1 = tensor.empty() : tensor<1x1x9x32xi32>
    %result, %result_indices = "ttir.max_pool2d_with_indices"(%arg0, %0, %1) <{
      kernel = array<i32: 3, 3>,
      stride = array<i32: 2, 2>,
      dilation = array<i32: 1, 1>,
      padding = array<i32: 0, 0, 0, 0>,
      ceil_mode = false,
      flattened_compat_info = #ttir<flattened_compat batch_size = 1, input_height = 8, input_width = 8>
    }> : (tensor<1x1x64x32xbf16>, tensor<1x1x9x32xbf16>, tensor<1x1x9x32xi32>) -> (tensor<1x1x9x32xbf16>, tensor<1x1x9x32xi32>)
    return %result, %result_indices : tensor<1x1x9x32xbf16>, tensor<1x1x9x32xi32>
  }
}

// MaxPool2dWithIndicesOp with flattened input and batch > 1.
// Input is (1, 1, 2*8*8, 32) = (1, 1, 128, 32); FlatInfo: N=2, H=8, W=8.
// Output: (8 - 3) / 2 + 1 = 3, so (1, 1, 2*3*3, 32) = (1, 1, 18, 32).
module {
  func.func @max_pool2d_with_indices_flattened_batch(%arg0: tensor<1x1x128x32xbf16>) -> (tensor<1x1x18x32xbf16>, tensor<1x1x18x32xi32>) {
    // CHECK-LABEL: func.func @max_pool2d_with_indices_flattened_batch
    // CHECK: tosa.reshape
    // CHECK: linalg.generic
    // CHECK: arith.cmpf ogt
    // CHECK: arith.select
    // CHECK: tosa.reshape
    // CHECK: tosa.reshape
    %0 = tensor.empty() : tensor<1x1x18x32xbf16>
    %1 = tensor.empty() : tensor<1x1x18x32xi32>
    %result, %result_indices = "ttir.max_pool2d_with_indices"(%arg0, %0, %1) <{
      kernel = array<i32: 3, 3>,
      stride = array<i32: 2, 2>,
      dilation = array<i32: 1, 1>,
      ...
      ceil_mode = false,
      flattened_compat_info = #ttir<flattened_compat batch_size = 2, input_height = 8, input_width = 8>
    }> : (tensor<1x1x128x32xbf16>, tensor<1x1x18x32xbf16>, tensor<1x1x18x32xi32>) -> (tensor<1x1x18x32xbf16>, tensor<1x1x18x32xi32>)
    return %result, %result_indices : tensor<1x1x18x32xbf16>, tensor<1x1x18x32xi32>
  }
}

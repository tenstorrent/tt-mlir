// RUN: ttmlir-opt --ttcore-register-device --convert-ttir-to-ttkernel --convert-ttir-to-ttmetal %s > %t.mlir
...
module {
  func.func @generic0(%arg0: memref<1x1x8x24x!ttcore.tile<32x32, f32>, #ttcore.shard<98304x4096>, #l1_>, %arg1: memref<1x1x24x32x!ttcore.tile<32x32, f32>, #ttcore.shard<131072x4096>, #l1_>) -> memref<8x8x1x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096>, #l1_> {
    ...
    ttir.generic {grid = #ttcore.grid<8x8>, indexing_maps = [], iterator_types = [], threads = [#ttir.thread<datamovement, @datamovement_kernel0>, #ttir.thread<datamovement, @datamovement_kernel1>, #ttir.thread<compute, @compute_kernel2>]}

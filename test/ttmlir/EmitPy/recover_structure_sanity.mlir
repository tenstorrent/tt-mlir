// RUN: ttmlir-opt --ttir-to-emitpy-pipeline="try-recover-structure=true" -o %t.mlir %s
// RUN: ttmlir-translate --mlir-to-python -o %t.py %t.mlir --allow-unregistered-dialect
...
// Check that the structure has been recovered and that the generated functions contain proper ops.
//
// CHECK: def _main
// CHECK: def FirstModule
// CHECK: ttnn.matmul
// CHECK: def SecondModule
// CHECK: ttnn.matmul
// CHECK: def forward
// CHECK: ttnn.multiply
// CHECK: def main

...
# Control flow tests
# ------------------------------------------------------------
@pytest.mark.parametrize("shape, max_grid", TEST_SHAPE_GRIDS)
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@pytest.mark.parametrize("use_exp", [True, False])
def test_if_else_branch(device, shape, max_grid, dtype, use_exp):
    ...
    run_op_test(
        device, shape, max_grid, dtype, op_wrapper, 1, buffer_type=ttnn.BufferType.L1
    )
...
def test_nested_if(device, shape, max_grid, dtype, mode):
...
def test_if_with_ops_before_after(device, shape, max_grid, dtype, apply_exp):
    """Test if statement with operations before and after."""
...
def test_multiple_sequential_ifs(device, shape, max_grid, dtype, apply_exp, apply_cos):
...
def test_for_loop_simple(device, shape, max_grid, dtype, iterations):
...
def test_for_loop_with_index(device, shape, max_grid, dtype, iterations):
...
def test_nested_for_loops(device, shape, max_grid, dtype, outer, inner):
    """Test nested for loops."""
...
def test_for_loop_with_if(device, shape, max_grid, dtype, iterations):
    """Test for loop with if statement inside."""
...
def test_if_inside_for_multiple_branches(device, shape, max_grid, dtype, iterations):
    """Test for loop with multiple if branches inside."""

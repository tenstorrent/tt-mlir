...
def gptoss_subgraph(left_input, right_input, left_min, left_max, right_min, right_max):
    # use externally created per-element min/max tensors
...
def test_oss_subgraph(device):
    ...
    # create min/max tensors outside the JIT and pass them in
    shape = (4, 32, 3072)
    left_min = create_dram_tensor(
        device, shape, torch.float16, input_transform=lambda t: t.fill_(-7.0)
    )
    left_max = create_dram_tensor(
        device, shape, torch.float16, input_transform=lambda t: t.fill_(7.0)
    )
    right_min = create_dram_tensor(
        device, shape, torch.float16, input_transform=lambda t: t.fill_(float("-inf"))
    )
    ...
    output = gptoss_subgraph(
        left_input, right_input, left_min, left_max, right_min, right_max
    )
...
@ttnn_jit.jit(debug=True)
def fusion_example(input_a):
    out = ttnn.exp(input_a)

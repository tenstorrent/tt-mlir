...
matmul -> all_gather -> sum -> all_gather -> argmax. The matmul's result
encoding is rewritten to bfp_bf8 and propagates through view / CCL / sum;
a ttnn.to_layout(RowMajor, bf16) is inserted before argmax, which lowers
to a single tt-metal untilize kernel via the DecomposeLayouts fast path.

...
def test_rand(
    ...
    def module(builder: TTNNBuilder):
        ...
        def rand(builder: TTNNBuilder, unit_attrs: Optional[List[str]] = None):
            mesh_shape = ttnn.ir.MeshShapeAttr.get(builder._ctx, 1, 1)
            mesh_offset = ttnn.ir.MeshOffsetAttr.get(builder._ctx, 0, 0)
            device2 = ttnn.get_device(mesh_shape=mesh_shape, mesh_offset=mesh_offset)
    ...
    cp = create_custom_ttir_pipeline_fn("")
    compile_and_execute_ttnn(
        module,
        test_base=request.node.name,
        device=device,

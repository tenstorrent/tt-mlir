...
module attributes {} {
  ...
  func.func private @datamovement_kernel0() attributes {ttkernel.arg_spec = #ttkernel.arg_spec< ct_args = [<arg_type = cb_port, operand_index = 0>]>, ttkernel.thread = #ttkernel.thread<noc>} {
    ...
    // Src device sends its shard to the destination device.
    %is_src = arith.cmpi eq, %my_device_id, %src_dev_id : i16
    scf.if %is_src {
      "ttkernel.experimental.fabric_fast_write_any_len"(%fabric_connection_manager, %dst_mesh_id, %dst_dev_id, %noc_addr, %src_ptr, %len_bytes) : (!ttkernel.fabric_connection_manager, i16, i16, !ttkernel.noc_addr, i32, i32) -> ()
    }

...
module attributes {} {
  ...
  func.func @test_fabric_unicast(%arg0: !full_tensor_layout) -> !full_tensor_layout {
    ...
    "ttmetal.enqueue_program"(%1, %semaphore, %1) <{cb_ports = array<i64: 0>, dfb_ids = array<i64>, kernelConfigs = [#ttmetal.noc_config<@datamovement_kernel0, #ttmetal.core_range<0x0, 1x1>, #ttmetal.kernel_args< ct_args = [<cb_port[0]>, <global_semaphore[1]>]>, noc0>], operandSegmentSizes = array<i32: 2, 1, 0>, fabricConnectionConfig = #ttcore.fabric_connection_config<noc_index = noc0, topology = insert_topology, cluster_axis = insert_cluster_axis, routing_mode = insert_routing_mode, num_links = 1>}> : (!l1_shard_layout, !ttmetal.global_semaphore, !l1_shard_layout) -> ()

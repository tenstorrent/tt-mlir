...
def test_distributed_layer_norm(
    ...
    def module(builder: TTIRBuilder):
        @builder.func(shapes, [torch.bfloat16] * len(shapes))
        def distributed_layer_norm_test(*inputs, unit_attrs: Optional[List[str]] = None):
            builder = inputs[-1]
    ...
    # Non-tile-padded shapes (last dim not a multiple of 32) trigger slightly
    # lower PCC due to the distributed kernel computing statistics over
    # tile-padded zeros, which shifts the effective mean/variance.
    is_tile_padded = all(d % 32 == 0 for d in shape)
    pcc = 0.99 if is_tile_padded else 0.98

...
shapes_list = [
    # Identity reshapes
    # [(64, 64), (64, 64)],  # 2D identity
    # [(3, 32, 64), (3, 32, 64)],  # 3D identity
    # [(2, 3, 32, 64), (2, 3, 32, 64)],  # 4D identity
    # Dimension-changing reshapes
    # [(64, 64), (32, 128)],  # 2D -> 2D
    # [(128, 64), (32, 256)],  # 2D -> 2D
    [(96, 64), (3, 32, 64)],  # 2D -> 3D
    # [(256, 96), (2, 4, 32, 96)],  # 2D -> 4D
    # [(384, 64), (3, 2, 2, 32, 64)],  # 2D -> 5D
    # [(5, 32, 32), (160, 32)],  # 3D -> 2D
    # [(7, 32, 96), (21, 32, 32)],  # 3D -> 3D
    # [(6, 32, 64), (2, 3, 32, 64)],  # 3D -> 4D
    # [(12, 64, 32), (2, 2, 3, 64, 32)],  # 3D -> 5D
    # [(15, 32, 32), (480, 32)],  # 4D -> 2D
    # [(2, 7, 32, 64), (7, 64, 64)],  # 4D -> 3D
    # [(3, 3, 64, 32), (3, 2, 32, 96)],  # 4D -> 4D
    # [(2, 3, 64, 64), (2, 3, 2, 32, 64)],  # 4D -> 5D
    # [(2, 3, 4, 32, 32), (768, 32)],  # 5D -> 2D
    # [(5, 3, 32, 64), (10, 96, 32)],  # 5D -> 3D
    # [(2, 3, 4, 64, 32), (6, 2, 128, 32)],  # 5D -> 4D
    # [(2, 2, 3, 32, 64), (3, 4, 2, 32, 32)],  # 5D -> 5D
    # [(2, 2, 3, 5, 32, 32), (1920, 32)],  # 6D -> 2D
    # [(2, 3, 2, 4, 32, 64), (12, 64, 128)],  # 6D -> 3D
    # [(2, 2, 3, 2, 64, 32), (6, 2, 64, 64)],  # 6D -> 4D
    # [(2, 2, 2, 3, 2, 32, 64), (24, 64, 64)],  # 7D -> 3D
    # [(2, 2, 3, 2, 2, 32, 32), (6, 4, 32, 64)],  # 7D -> 4D
    # [(2, 2, 2, 2, 3, 2, 32, 32), (48, 64, 32)],  # 8D -> 3D
    # [(2, 2, 2, 2, 2, 3, 32, 32), (8, 6, 32, 64)],  # 8D -> 4D
    # # Funky reshapes - last dimension changes dramatically
    # [(128, 32), (64, 64)],  # 2D: last dim 32->64
    # [(64, 128), (128, 64)],  # 2D: last dim 128->64
    # [(32, 192), (96, 64)],  # 2D: last dim 192->64
    # [(128, 64), (64, 128)],  # 2D: both last dims change 128x64 -> 64x128
    # [(3, 64, 32), (2, 32, 96)],  # 3D: last dim 32->96
    # [(4, 32, 96), (3, 128, 32)],  # 3D: last dim 96->32
    # [(2, 128, 64), (4, 64, 64)],  # 3D: both last 2 dims change
    # [(5, 64, 64), (10, 64, 32)],  # 3D: last dim 64->32
    # [(6, 64, 96), (4, 96, 96)],  # 3D: 64x96 -> 96x96
    # [(3, 5, 64, 32), (5, 3, 32, 64)],  # 4D: swap last 2 dims
    # [(2, 4, 96, 32), (3, 2, 64, 64)],  # 4D: 96x32 -> 64x64
    # [(3, 2, 32, 128), (2, 3, 128, 32)],  # 4D: last dim 128->32
    # [(2, 2, 3, 32, 64), (4, 3, 32, 64)],  # 5D: keep last 2 dims same
    # [(3, 2, 2, 64, 64), (6, 4, 32, 64)],  # 5D: second-to-last 64->32
    # [(2, 3, 64, 96), (2, 192, 96)],  # 4D->3D: 64x96 -> 192x96
    # [(5, 2, 64, 64), (10, 32, 128)],  # 4D->3D: 64x64 -> 32x128
    # [(2, 2, 3, 32, 96), (6, 32, 192)],  # 5D->3D: 32x96 -> 32x192
    # [(3, 2, 3, 64, 32), (6, 96, 64)],  # 5D->3D: 64x32 -> 96x64
]
...
@pytest.mark.parametrize("dtype", [torch.float32], ids=["f32"])
@pytest.mark.parametrize("target", ["ttmetal"])
def test_reshape(shapes: List[Shape], dtype: torch.dtype, target: str, request, device):
    """Test reshape followed by absolute value operation on 1x1 core grid"""
    input_shape, output_shape = shapes

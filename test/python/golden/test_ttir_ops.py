...
def test_repeat_interleave(shapes: List[Shape], repeats: int, dim: int):
    def repeat_interleave(in0: Operand, in1: Operand, builder: TTIRBuilder):
        return builder.repeat_interleave(in0, repeats=repeats, dim=dim)
...
def test_pad(shape: Shape, padding: List[int], value):
    def pad(in0: Operand, builder: TTIRBuilder):
        return builder.pad(in0, padding=padding, value=value)
    _compile_to_flatbuffer(pad, inputs_shapes=[shape], test_name="test_pad")
...
def prod(in0: Operand, in1: Operand, builder: TTIRBuilder):
    return builder.prod(in0, in1, [1], False)
...
@pytest.mark.parametrize("test_fn", [add, multiply, subtract, eq, ne, le, lt,
                                     ...
                                     power, matmul, hoisted_add, logical_and, logical_or, logical_xor])
def test_binary_ops(
    test_fn: Callable,
    shape: Shape,
    dtype: torch.dtype):

...
@pytest.mark.parametrize("target", ["ttnn" | SkipIf("sim")])
def test_silu_bw_composite(target: str, request, device):
    shape = (1, 1, 32, 64)
    dtypes = [torch.bfloat16, torch.bfloat16]

    def module(builder: TTIRBuilder):
        @builder.func([shape, shape], dtypes)
        def silu_bw_decomp(
            input: Operand,
            grad_output: Operand,
            builder: TTIRBuilder,
        ...
        silu_bw_decomp.sym_visibility = StringAttr.get("private")
        builder._nested_funcs.append(silu_bw_decomp.name.value)

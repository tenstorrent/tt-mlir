...
def test_compile_full_module(print_results: bool = False):
    ttnn_module_str = """
        ...
        module attributes {tt.device = #device, tt.system_desc = #system_desc} {
            func.func @main() -> tensor<1x128xf32, #ttnn_layout> {
                %0 = "ttnn.get_device"() <{mesh_shape = #ttnn<mesh_shape 1x1>}> : () -> !tt.device<#device>
                %1 = "ttnn.empty"(%0) <{dtype = #tt.supportedDataTypes<f32>, layout = #ttnn.layout<tile>, memory_config = #ttnn.memory_config<#dram, <<1x4>>, <interleaved>>, shape = #ttnn.shape<1x128>}> : (!tt.device<#device>) -> tensor<1x128xf32, #ttnn_layout>

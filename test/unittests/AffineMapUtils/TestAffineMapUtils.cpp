...
TEST(AffineMapUtilsTest, CanAnalyzeQwenFullyIndexedAffineMapsWithoutSampling) {
  using namespace mlir;
  MLIRContext context;

  struct QwenAffineMapRepro {
    llvm::StringLiteral name;
    llvm::StringLiteral map;
  };

  // These are the affine-map definitions from qwen.d2m.pre-fully-indexed.mlir.
  // Keep this as a parser-level repro so the test does not run the full IR.
  constexpr QwenAffineMapRepro
      testCases
          [] =
              {
                  {"#map",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d3 floordiv 32) mod 4, (d0 * 8 + d2 + d3 floordiv 128) mod 32, d3 mod 32)>)mlir"},
                  {"#map1",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 2 + (d2 * 2560 + d1 * 320 + d3) floordiv 48578560, (d1 + d3 floordiv 320) mod 8, (d2 + (d1 * 320 + d3) floordiv 2560) mod 18976, d3 mod 320)>)mlir"},
                  {"#map2",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d1 * 5 + d3) floordiv 80, ((d1 * 5 + d3) floordiv 10) mod 8, 0, (d1 * 5 + d3) mod 10)>)mlir"},
                  {"#map3",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> ((d2 floordiv 8 + d1 * 2) mod 8, d2 mod 8, d3, d4, d5)>)mlir"},
                  {"#map4",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, ((d1 + d0 * 8) floordiv 16) mod 4, (d1 + d0 * 8) mod 16)>)mlir"},
                  {"#map5",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d0 + (d1 * 10 + d3) floordiv 80) mod 4, (d1 * 2 + d3 floordiv 5) mod 16, 0, 0, d3 mod 5)>)mlir"},
                  {"#map6",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2, d3, d4, d5)>)mlir"},
                  {"#map7",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (((d1 + d2) floordiv 2) mod 2, (d1 + d2) mod 2, d3, d4, d5)>)mlir"},
                  {"#map8",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 + d0 * 2) mod 4, 0)>)mlir"},
                  {"#map9",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> ((d2 floordiv 2) mod 2, d2 mod 2, d3, d4, d5)>)mlir"},
                  {"#map10",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, (d1 + d0 * 2) mod 4)>)mlir"},
                  {"#map11",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, d2 mod 4, 0, 0, 0, 0)>)mlir"},
                  {"#map12",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, d5 mod 4, 0, 0, 0)>)mlir"},
                  {"#map13",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, 0, (d1 mod 4) floordiv 128, ((d1 mod 4) mod 128) floordiv 128, (d1 mod 4) mod 128)>)mlir"},
                  {"#map14",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, d0 mod 4, 0, 0)>)mlir"},
                  {"#map15",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, (d4 + d5 floordiv 80) mod 4, (d5 floordiv 5) mod 16, 0, 0, d5 mod 5)>)mlir"},
                  {"#map16",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, 0, (((d0 + (d1 * 10 + d3) floordiv 80) mod 4) * 2560 + (d1 * 10 + d3) mod 80) floordiv 327680, ((((d0 + (d1 * 10 + d3) floordiv 80) mod 4) * 2560 + (d1 * 10 + d3) mod 80) mod 327680) floordiv 2560, ((d1 * 10 + d3) mod 80) mod 2560)>)mlir"},
                  {"#map17",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d1 floordiv 8) mod 5, d1 mod 8, d2, d3)>)mlir"},
                  {"#map18",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 + d0 * 8) mod 40)>)mlir"},
                  {"#map19",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, ((d1 * 5 + d3) floordiv 2) mod 40, 0, (d1 * 5 + d3) mod 2)>)mlir"},
                  {"#map20",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 81920 + d2 * 2560 + d1 * 320 + d3) floordiv 1966080, (d1 + d3 floordiv 320) mod 8, (d0 * 32 + d2 + (d1 * 320 + d3) floordiv 2560) mod 768, d3 mod 320)>)mlir"},
                  {"#map21",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 80 + d1 * 10 + d3) floordiv 1920, (d1 + d3 floordiv 10) mod 8, (d0 + (d1 * 10 + d3) floordiv 80) mod 24, d3 mod 10)>)mlir"},
                  {"#map22",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 24 + d2 + (d1 * 10 + d3) floordiv 80, (d1 + d3 floordiv 10) mod 8, 0, d3 mod 10)>)mlir"},
                  {"#map23",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d2 * 80 + d3) floordiv 1920, (d3 floordiv 10) mod 8, (d2 + d3 floordiv 80) mod 24, d3 mod 10)>)mlir"},
                  {"#map24",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d1 * 24 + d3) mod 192, (d0 * 10 + d2 + (d1 * 24 + d3) floordiv 192) mod 80)>)mlir"},
                  {"#map25",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 192 + d1 * 4 + d3) floordiv 1920, ((d1 * 4 + d3) floordiv 24) mod 8, (d0 + (d1 * 4 + d3) floordiv 192) mod 10, (d1 * 4 + d3) mod 24)>)mlir"},
                  {"#map26",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 10 + d2 + (d1 * 24 + d3) floordiv 192, (d1 * 6 + d3 floordiv 4) mod 48, 0, d3 mod 4)>)mlir"},
                  {"#map27",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 192 + d1 * 4 + d3) floordiv 1920, ((d1 * 4 + d3) floordiv 24) mod 8, (d2 + (d1 * 4 + d3) floordiv 192) mod 10, (d1 * 4 + d3) mod 24)>)mlir"},
                  {"#map28",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d1 * 4 + d3) floordiv 192, ((d1 * 4 + d3) floordiv 24) mod 8, 0, (d1 * 4 + d3) mod 24)>)mlir"},
                  {"#map29",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d2 + d3 floordiv 192, (d3 floordiv 24) mod 8, 0, d3 mod 24)>)mlir"},
                  {"#map30",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, (d4 + (d2 * 3 + d5) floordiv 192) mod 4 + ((d2 * 3 + d5) mod 192) floordiv 6144, ((d2 * 3 + d5) mod 192) mod 6144)>)mlir"},
                  {"#map31",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> ((d2 floordiv 8) mod 8, d2 mod 8, d3, d4, d5)>)mlir"},
                  {"#map32",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, (d1 + d0 * 8) mod 64)>)mlir"},
                  {"#map33",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, (d2 floordiv 3) mod 64, 0, (d4 + d2 floordiv 192) mod 4, d2 mod 3)>)mlir"},
                  {"#map34",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, ((d2 * 32 + d5) floordiv 96) mod 64, 0, (d4 + (d2 * 32 + d5) floordiv 6144) mod 128, (d2 * 32 + d5) mod 96)>)mlir"},
                  {"#map35",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, (d5 floordiv 96) mod 64, 0, (d4 + d5 floordiv 6144) mod 128, d5 mod 96)>)mlir"},
                  {"#map36",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (d4 + (d2 * 64 + d5) floordiv 4096) mod 128, (d2 * 64 + d5) mod 4096)>)mlir"},
                  {"#map37",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (d4 + (d2 * 2 + d5) floordiv 128, ((d2 * 2 + d5) floordiv 16) mod 8, 0, (d2 * 2 + d5) mod 16)>)mlir"},
                  {"#map38",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d1 * 4 + d3) floordiv 128, ((d1 * 4 + d3) floordiv 16) mod 8, 0, (d1 * 4 + d3) mod 16)>)mlir"},
                  {"#map39",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 4096 + d1 * 128 + d3) floordiv 131072, ((d1 * 128 + d3) floordiv 512) mod 8, (d2 + (d1 * 128 + d3) floordiv 4096) mod 32, (d1 * 128 + d3) mod 512)>)mlir"},
                  {"#map40",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 4096 + d1 * 512 + d3) floordiv 131072, (d1 * 4 + d3 floordiv 128) mod 32, (d2 + (d1 * 512 + d3) floordiv 4096) mod 32, d3 mod 128)>)mlir"},
                  {"#map41",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d1 * 8 + d3 floordiv 64) mod 64, 0, (d0 * 32 + d2 + (d1 * 512 + d3) floordiv 4096) mod 128, d3 mod 64)>)mlir"},
                  {"#map42",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, (d5 floordiv 64) mod 64, 0, (d4 + d5 floordiv 4096) mod 128, d5 mod 64)>)mlir"},
                  {"#map43",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, (((d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128) * 4096 + ((d6 + (d3 * 32 + d7) floordiv 128) mod 32) * 128 + (d3 * 32 + d7) mod 128) floordiv 524288, ((((d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128) * 4096 + ((d6 + (d3 * 32 + d7) floordiv 128) mod 32) * 128 + (d3 * 32 + d7) mod 128) mod 524288) floordiv 4096, (((d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128) * 4096 + ((d6 + (d3 * 32 + d7) floordiv 128) mod 32) * 128 + (d3 * 32 + d7) mod 128) mod 4096)>)mlir"},
                  {"#map44",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d3 + d2 * 4 + d1 * 4) floordiv 8) mod 8, (d3 + d2 * 4 + d1 * 4) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map45",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 4 + d0 * 2) mod 16, 0, d1 mod 4)>)mlir"},
                  {"#map46",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 16384 + d5 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 32768) mod 16, 0, (d3 + d7 floordiv 32) mod 4, 0, (d1 * 4 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 8, (d6 + (d3 * 32 + d7) floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map47",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 16 + d5 * 4 + d3) floordiv 32) mod 16, 0, d3 mod 4, 0, (d1 * 4 + d5 + d3 floordiv 4) mod 8, 0, 0)>)mlir"},
                  {"#map48",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, (d1 * 2 + (d5 * 4 + d3) floordiv 16) mod 32, 0, d3 mod 4, 0, (d5 + d3 floordiv 4) mod 4, 0, 0)>)mlir"},
                  {"#map49",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> ((d1 * 32 + d5 * 4 + d3) floordiv 64, d3 mod 4, (d1 * 8 + d5 + d3 floordiv 4) mod 16, 0)>)mlir"},
                  {"#map50",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>)mlir"},
                  {"#map51",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 16 + d2 * 4 + d1) floordiv 64, d1 mod 4, (d0 * 4 + d2 + d1 floordiv 4) mod 16, 0)>)mlir"},
                  {"#map52",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 4 + (d2 * 4 + d1) floordiv 16, d1 mod 4, (d2 + d1 floordiv 4) mod 4, 0)>)mlir"},
                  {"#map53",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d0 * 2 + (d2 * 4 + d1) floordiv 32) mod 16, 0, d1 mod 4, 0, (d2 + d1 floordiv 4) mod 8, 0, 0)>)mlir"},
                  {"#map54",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d2 + d3 + d1) floordiv 8) mod 8, (d2 + d3 + d1) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map55",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 + d0 * 8) mod 64, 0, 0)>)mlir"},
                  {"#map56",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 8 + d5 * 4 + d3) floordiv 32) mod 16, 0, d3 mod 4, 0, (d1 * 2 + d5 + d3 floordiv 4) mod 8, 0, 0)>)mlir"},
                  {"#map57",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (d0, d1, d2, d3, d4, d5, d6, d7)>)mlir"},
                  {"#map58",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 1024 + d6 * 32 + d7) floordiv 2048) mod 64, 0, 0, 0, (d5 + (d6 * 32 + d7) floordiv 1024) mod 2, (d6 + d7 floordiv 32) mod 32, d7 mod 32)>)mlir"},
                  {"#map59",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (((d1 * 32 + d4 + d5 floordiv 32) mod 128) * 32 + d5 mod 32) floordiv 4096, ((((d1 * 32 + d4 + d5 floordiv 32) mod 128) * 32 + d5 mod 32) mod 4096) floordiv 32, (d5 mod 32) floordiv 32, d5 mod 32)>)mlir"},
                  {"#map60",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (d1, 0, 0, 0)>)mlir"},
                  {"#map61",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d0 + (d2 * 32 + d3) floordiv 1024) mod 4, 0, 0, (d2 + d3 floordiv 32) mod 32, d3 mod 32)>)mlir"},
                  {"#map62",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, ((d4 * 32 + d5) floordiv 1024) mod 4, 0, 0, (d4 + d5 floordiv 32) mod 32, d5 mod 32)>)mlir"},
                  {"#map63",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, (((d1 * 2 + d5 + (d6 * 32 + d7) floordiv 1024) mod 128) * 32 + ((d6 + d7 floordiv 32) mod 32) * 32 + d7 mod 32) floordiv 4096, ((((d1 * 2 + d5 + (d6 * 32 + d7) floordiv 1024) mod 128) * 32 + ((d6 + d7 floordiv 32) mod 32) * 32 + d7 mod 32) mod 4096) floordiv 32, d7 mod 32)>)mlir"},
                  {"#map64",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d2 * 32 + d6 + d7 floordiv 32) mod 128, (d1 * 2 + d5 + (d2 * 1024 + d6 * 32 + d7) floordiv 4096) mod 32, d7 mod 32)>)mlir"},
                  {"#map65",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d2 + d3 + d1 * 4) floordiv 8) mod 8, (d2 + d3 + d1 * 4) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map66",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 4 + d0 * 2) mod 16, d1 mod 4, 0)>)mlir"},
                  {"#map67",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 4096 + d6 * 128 + d7) floordiv 32768) mod 16, 0, (d7 floordiv 32) mod 4, 0, (d5 + (d6 * 128 + d7) floordiv 4096) mod 8, (d6 + d7 floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map68",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 128, (d1 * 8 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 32, (d3 * 32 + d7) mod 128)>)mlir"},
                  {"#map69",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d3 + d2 * 4) floordiv 8 + d1 * 2) mod 8, (d3 + d2 * 4) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map70",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, ((d1 + d0 * 8) floordiv 16) mod 4, (d1 floordiv 4 + d0 * 2) mod 4, d1 mod 4)>)mlir"},
                  {"#map71",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 65536 + d5 * 16384 + d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 131072) mod 4, (d2 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 4, (d3 + d7 floordiv 32) mod 4, 0, (d1 * 4 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 8, (d6 + (d3 * 32 + d7) floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map72",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 64 + d5 * 16 + d2 * 4 + d3) floordiv 128) mod 4, (d2 + d3 floordiv 4) mod 4, d3 mod 4, 0, (d1 * 4 + d5 + (d2 * 4 + d3) floordiv 16) mod 8, 0, 0)>)mlir"},
                  {"#map73",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, (d1 * 2 + (d5 * 16 + d2 * 4 + d3) floordiv 64) mod 8, (d2 + d3 floordiv 4) mod 4, d3 mod 4, 0, (d5 + (d2 * 4 + d3) floordiv 16) mod 4, 0, 0)>)mlir"},
                  {"#map74",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, (d1 * 4 + (d5 * 4 + d2) floordiv 8) mod 16, d2 mod 4, 0, 0, (d5 + d2 floordiv 4) mod 2, 0, 0)>)mlir"},
                  {"#map75",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 16 + d5 * 4 + d2) floordiv 32) mod 4, d2 mod 4, 0, 0, (d1 * 4 + d5 + d2 floordiv 4) mod 8, 0, 0)>)mlir"},
                  {"#map76",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (d1 * 2 + (d5 * 16 + d2 * 4 + d3) floordiv 64, d3 mod 4, (d5 * 4 + d2 + d3 floordiv 4) mod 16, 0)>)mlir"},
                  {"#map77",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, d3 mod 4, 0, 0)>)mlir"},
                  {"#map78",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, (d3 mod 4) floordiv 128, (d3 mod 4) mod 128)>)mlir"},
                  {"#map79",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> ((d3 floordiv 2) mod 2, d3 mod 2, d4, d5, d6, d7)>)mlir"},
                  {"#map80",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, 0, (d1 + d0 * 2) mod 4)>)mlir"},
                  {"#map81",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, ((d0 * 64 + d2 * 4 + d1) floordiv 128) mod 4, (d2 + d1 floordiv 4) mod 4, d1 mod 4, 0, (d0 * 4 + (d2 * 4 + d1) floordiv 16) mod 8, 0, 0)>)mlir"},
                  {"#map82",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, (d1 * 2 + (d5 * 16384 + d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 65536) mod 8, (d2 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 4, (d3 + d7 floordiv 32) mod 4, 0, (d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 4, (d6 + (d3 * 32 + d7) floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map83",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 16384 + d6 * 128 + d7) floordiv 131072) mod 4, ((d6 * 128 + d7) floordiv 4096) mod 4, (d7 floordiv 32) mod 4, 0, (d5 + (d6 * 128 + d7) floordiv 16384) mod 8, (d6 + d7 floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map84",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d1 * 4 + d5 + (d2 * 2048 + d6 * 64 + d3 * 32 + d7) floordiv 8192) mod 32, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 64) mod 128, (d3 * 32 + d7) mod 64)>)mlir"},
                  {"#map85",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d3 + d2 * 2) floordiv 8 + d1) mod 8, (d3 + d2 * 2) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map86",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 8 + d0) mod 8, (d1 floordiv 2) mod 4, d1 mod 2)>)mlir"},
                  {"#map87",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 2 + (d2 * 128 + d1 * 32 + d3) floordiv 655360, (d1 + d3 floordiv 32) mod 4, (d2 + (d1 * 32 + d3) floordiv 128) mod 5120, d3 mod 32)>)mlir"},
                  {"#map88",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> ((d2 floordiv 4 + d1) mod 4, d2 mod 4, d3, d4, d5)>)mlir"},
                  {"#map89",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 4 + d0) mod 4, d1 mod 4)>)mlir"},
                  {"#map90",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d0 + (d2 * 128 + d1 * 32 + d3) floordiv 4096) mod 4, (d1 + d3 floordiv 32) mod 4, 0, (d2 + (d1 * 32 + d3) floordiv 128) mod 32, d3 mod 32)>)mlir"},
                  {"#map91",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, ((d4 * 128 + d5) floordiv 4096) mod 4, (d5 floordiv 32) mod 4, 0, (d4 + d5 floordiv 128) mod 32, d5 mod 32)>)mlir"},
                  {"#map92",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (d1 * 32 + d4 + (d2 * 32 + d5) floordiv 64) mod 128, (d2 * 32 + d5) mod 64)>)mlir"},
                  {"#map93",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (((d2 + d1 * 2) floordiv 4) mod 2, (d2 + d1 * 2) mod 4, d3, d4, d5)>)mlir"},
                  {"#map94",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 2 + d0 * 2) mod 4, d1 mod 2)>)mlir"},
                  {"#map95",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, ((d4 * 64 + d5) floordiv 2048) mod 4, (d5 floordiv 32) mod 2, 0, (d4 + d5 floordiv 64) mod 32, d5 mod 32)>)mlir"},
                  {"#map96",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, (((d1 * 4 + d5 + (d6 * 64 + d3 * 32 + d7) floordiv 2048) mod 128) * 64 + ((d6 + (d3 * 32 + d7) floordiv 64) mod 32) * 64 + (d3 * 32 + d7) mod 64) floordiv 8192, ((((d1 * 4 + d5 + (d6 * 64 + d3 * 32 + d7) floordiv 2048) mod 128) * 64 + ((d6 + (d3 * 32 + d7) floordiv 64) mod 32) * 64 + (d3 * 32 + d7) mod 64) mod 8192) floordiv 64, (d3 * 32 + d7) mod 64)>)mlir"},
                  {"#map97",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d3 + d2 * 2 + d1 * 2) floordiv 8) mod 8, (d3 + d2 * 2 + d1 * 2) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map98",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 2 + d0 * 4) mod 32, 0, d1 mod 2)>)mlir"},
                  {"#map99",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 2048 + d6 * 64 + d7) floordiv 8192) mod 32, 0, (d7 floordiv 32) mod 2, 0, (d5 + (d6 * 64 + d7) floordiv 2048) mod 4, (d6 + d7 floordiv 64) mod 32, d7 mod 32)>)mlir"},
                  {"#map100",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 64) mod 128, 0, (d3 * 32 + d7) mod 64)>)mlir"},
                  {"#map101",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d3 + d2 * 2) floordiv 4) mod 2, (d3 + d2 * 2) mod 4, d4, d5, d6, d7)>)mlir"},
                  {"#map102",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, (d1 floordiv 2 + d0 * 2) mod 4, d1 mod 2)>)mlir"},
                  {"#map103",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d1 * 4 + d5 + (d2 * 2048 + d6 * 64 + d3 * 32 + d7) floordiv 8192) mod 32, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 64) mod 128, (d3 * 32 + d7) mod 64 + 64)>)mlir"},
                  {"#map104",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (d1 * 32 + d4 + (d2 * 32 + d5) floordiv 64) mod 128, (d2 * 32 + d5) mod 64 + 64)>)mlir"},
                  {"#map105",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (d1 + (d5 * 8 + d2 * 2 + d3) floordiv 32, d3 mod 2, (d5 * 4 + d2 + d3 floordiv 2) mod 16, 0)>)mlir"},
                  {"#map106",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 8 + d2 * 2 + d1) floordiv 32, d1 mod 2, (d0 * 4 + d2 + d1 floordiv 2) mod 16, 0)>)mlir"},
                  {"#map107",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 4 + (d2 * 2 + d1) floordiv 8, d1 mod 2, (d2 + d1 floordiv 2) mod 4, 0)>)mlir"},
                  {"#map108",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d0 + (d2 * 2 + d1) floordiv 32) mod 8, (d2 + d1 floordiv 2) mod 4, d1 mod 2, 0, ((d2 * 2 + d1) floordiv 8) mod 4, 0, 0)>)mlir"},
                  {"#map109",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0)>)mlir"},
                  {"#map110",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((((d3 + d2 * 2) floordiv 8 + d1) mod 8) * 8 + (d3 + d2 * 2) mod 8 + d4 + (d5 * 2048 + d6 * 2048 + d7 * 2048) floordiv 8192) mod 12)>)mlir"},
                  {"#map111",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> ((((((d3 + d2 * 2) floordiv 8 + d1) mod 8) * 8 + (d3 + d2 * 2) mod 8 + d4 + (d5 * 2048 + d6 * 2048 + d7 * 2048) floordiv 8192) floordiv 12) * 8192 + (d5 * 2048 + d6 * 2048 + d7 * 2048) mod 8192)>)mlir"},
                  {"#map112",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 16384 + d1 * 2048 + d2 * 2048 + d3 * 2048)>)mlir"},
                  {"#map113",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (d1 * 64 + d4 + (d2 * 32 + d5) floordiv 1024) mod 128, (d2 * 32 + d5) mod 1024 + 4096)>)mlir"},
                  {"#map114",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> ((d2 floordiv 8 + d1 * 4) mod 8, d2 mod 8, d3, d4, d5)>)mlir"},
                  {"#map115",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, ((d1 + d0 * 8) floordiv 32) mod 2, (d1 + d0 * 8) mod 32)>)mlir"},
                  {"#map116",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (d1 * 2 + d4 + d2 floordiv 32, (d2 floordiv 4) mod 8, 0, d2 mod 4)>)mlir"},
                  {"#map117",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, ((d0 * 32768 + d2 * 1024 + d1 * 128 + d3) floordiv 65536) mod 2, (d1 * 4 + d3 floordiv 32) mod 32, 0, (d0 * 32 + d2 + (d1 * 128 + d3) floordiv 1024) mod 64, d3 mod 32)>)mlir"},
                  {"#map118",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, ((d4 * 1024 + d5) floordiv 65536) mod 2, (d5 floordiv 32) mod 32, 0, (d4 + d5 floordiv 1024) mod 64, d5 mod 32)>)mlir"},
                  {"#map119",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, (((d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128) * 1024 + ((d6 + (d3 * 32 + d7) floordiv 128) mod 32) * 128 + (d3 * 32 + d7) mod 128) floordiv 131072, ((((d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128) * 1024 + ((d6 + (d3 * 32 + d7) floordiv 128) mod 32) * 128 + (d3 * 32 + d7) mod 128) mod 131072) floordiv 1024, (((d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128) * 1024 + ((d6 + (d3 * 32 + d7) floordiv 128) mod 32) * 128 + (d3 * 32 + d7) mod 128) mod 1024)>)mlir"},
                  {"#map120",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (((d1 * 32 + d4 + d5 floordiv 32) mod 128) * 8 + d5 mod 32) floordiv 1024, ((((d1 * 32 + d4 + d5 floordiv 32) mod 128) * 8 + d5 mod 32) mod 1024) floordiv 8, (d5 mod 8) floordiv 8, d5 mod 8)>)mlir"},
                  {"#map121",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, (((d1 * 2 + d5 + (d6 * 32 + d7) floordiv 1024) mod 128) * 8 + ((d6 + d7 floordiv 32) mod 32) * 8 + d7 mod 32) floordiv 1024, ((((d1 * 2 + d5 + (d6 * 32 + d7) floordiv 1024) mod 128) * 8 + ((d6 + d7 floordiv 32) mod 32) * 8 + d7 mod 32) mod 1024) floordiv 8, d7 mod 8)>)mlir"},
                  {"#map122",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d2 * 32 + d6 + d7 floordiv 32) mod 128, (d1 + (d2 * 1024 + d6 * 32 + d7) floordiv 4096) mod 8, d7 mod 32)>)mlir"},
                  {"#map123",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d2 + d3 + d1 * 4) floordiv 8) mod 4, (d2 + d3 + d1 * 4) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map124",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, (d1 floordiv 4 + d0 * 2) mod 8, d1 mod 4, 0)>)mlir"},
                  {"#map125",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 128, (d1 * 2 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 8, (d3 * 32 + d7) mod 128)>)mlir"},
                  {"#map126",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, (d1 * 2 + d5 + d2 floordiv 4) mod 8, d2 mod 4, 0, 0, 0, 0, 0)>)mlir"},
                  {"#map127",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (d1 * 2 + d5 + (d2 * 4 + d3) floordiv 16, d3 mod 4, (d2 + d3 floordiv 4) mod 4, 0)>)mlir"},
                  {"#map128",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, ((d0 * 16 + d2 * 4 + d1) floordiv 32) mod 4, (d2 + d1 floordiv 4) mod 4, d1 mod 4, 0, (d0 + (d2 * 4 + d1) floordiv 16) mod 2, 0, 0)>)mlir"},
                  {"#map129",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 16384 + d6 * 128 + d7) floordiv 32768) mod 4, ((d6 * 128 + d7) floordiv 4096) mod 4, (d7 floordiv 32) mod 4, 0, (d5 + (d6 * 128 + d7) floordiv 16384) mod 2, (d6 + d7 floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map130",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d1 + (d2 * 2048 + d6 * 64 + d3 * 32 + d7) floordiv 8192) mod 8, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 64) mod 128, (d3 * 32 + d7) mod 64)>)mlir"},
                  {"#map131",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d1 + (d2 * 2048 + d6 * 64 + d3 * 32 + d7) floordiv 8192) mod 8, (d2 * 32 + d6 + (d3 * 32 + d7) floordiv 64) mod 128, (d3 * 32 + d7) mod 64 + 64)>)mlir"},
                  {"#map132",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (d1 + (d2 * 2 + d3) floordiv 8, d3 mod 2, (d2 + d3 floordiv 2) mod 4, 0)>)mlir"},
                  {"#map133",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d0 + (d2 * 2 + d1) floordiv 8) mod 8, (d2 + d1 floordiv 2) mod 4, d1 mod 2, 0, 0, 0, 0)>)mlir"},
                  {"#map134",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((((d3 + d2 * 2) floordiv 8 + d1) mod 8) * 8 + (d3 + d2 * 2) mod 8 + d4 + d5 + d6 + d7) mod 12)>)mlir"},
                  {"#map135",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> ((((((d3 + d2 * 2) floordiv 8 + d1) mod 8) * 8 + (d3 + d2 * 2) mod 8 + d4 + d5 + d6 + d7) floordiv 12) * 2048)>)mlir"},
                  {"#map136",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 4096 + d1 * 2048 + d2 * 2048 + d3 * 2048)>)mlir"},
                  {"#map137",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, 0, 0, (d1 * 64 + d4 + (d2 * 32 + d5) floordiv 1024) mod 128, (d2 * 32 + d5) mod 1024 + 5120)>)mlir"},
                  {"#map138",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, (((d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 65536) mod 8) * 65536 + ((d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 512) * 128 + (d3 * 32 + d7) mod 128) floordiv 524288, ((((d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 65536) mod 8) * 65536 + ((d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 512) * 128 + (d3 * 32 + d7) mod 128) mod 524288) floordiv 16384, ((((d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 65536) mod 8) * 65536 + ((d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 512) * 128 + (d3 * 32 + d7) mod 128) mod 16384) floordiv 128, (d3 * 32 + d7) mod 128)>)mlir"},
                  {"#map139",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (((d3 + d2 * 4) floordiv 8) mod 8, (d3 + d2 * 4) mod 8, d4, d5, d6, d7)>)mlir"},
                  {"#map140",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, (d1 floordiv 4 + d0 * 2) mod 16, d1 mod 4)>)mlir"},
                  {"#map141",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 16 + d6 * 4 + d7) floordiv 32) mod 4, (d6 + d7 floordiv 4) mod 4, d7 mod 4, 0, (d5 + (d6 * 4 + d7) floordiv 16) mod 2, 0, 0)>)mlir"},
                  {"#map142",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d1 * 2 + d5 + (d2 * 4 + d3) floordiv 16) mod 8, d3 mod 4, (d2 + d3 floordiv 4) mod 4)>)mlir"},
                  {"#map143",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, (d2 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 16, (d3 + d7 floordiv 32) mod 4, 0, (d1 * 4 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 65536) mod 8, (d6 + (d3 * 32 + d7) floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map144",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, (d2 + d3 floordiv 4) mod 16, d3 mod 4, 0, (d1 * 4 + d5 + (d2 * 4 + d3) floordiv 64) mod 8, 0, 0)>)mlir"},
                  {"#map145",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 64 + d2 * 4 + d3) floordiv 256) mod 2, (d2 + d3 floordiv 4) mod 16, d3 mod 4, 0, (d5 + (d2 * 4 + d3) floordiv 64) mod 4, 0, 0)>)mlir"},
                  {"#map146",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 16 + d2 * 4 + d3) floordiv 32) mod 4, (d2 + d3 floordiv 4) mod 4, d3 mod 4, 0, (d5 + (d2 * 4 + d3) floordiv 16) mod 2, 0, 0)>)mlir"},
                  {"#map147",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, (d2 + d3 floordiv 4) mod 4, d3 mod 4, 0, (d1 * 4 + d5 + (d2 * 4 + d3) floordiv 16) mod 8, 0, 0)>)mlir"},
                  {"#map148", R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d5 * 65536 + d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 262144) mod 2, (d2 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 16, (d3 + d7 floordiv 32) mod 4, 0, (d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 65536) mod 4, (d6 + (d3 * 32 + d7) floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map149",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, ((d6 * 128 + d7) floordiv 4096) mod 16, (d7 floordiv 32) mod 4, 0, (d5 + (d6 * 128 + d7) floordiv 65536) mod 8, (d6 + d7 floordiv 128) mod 32, d7 mod 32)>)mlir"},
                  {"#map150",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, (((d1 * 8 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 32) * 16384 + ((d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 128) * 128 + (d3 * 32 + d7) mod 128) floordiv 524288, ((((d1 * 8 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 32) * 16384 + ((d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 128) * 128 + (d3 * 32 + d7) mod 128) mod 524288) floordiv 65536, ((((d1 * 8 + d5 + (d2 * 4096 + d6 * 128 + d3 * 32 + d7) floordiv 16384) mod 32) * 16384 + ((d2 * 32 + d6 + (d3 * 32 + d7) floordiv 128) mod 128) * 128 + (d3 * 32 + d7) mod 128) mod 65536) floordiv 128, (d3 * 32 + d7) mod 128)>)mlir"},
                  {"#map151",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> ((d3 floordiv 4 + d2) mod 4, d3 mod 4, d4, d5, d6, d7)>)mlir"},
                  {"#map152",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, (d1 floordiv 4 + d0) mod 4, d1 mod 4)>)mlir"},
                  {"#map153",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, ((d1 * 32 + d5 * 16 + d2 * 4 + d3) floordiv 128) mod 4, (d2 + d3 floordiv 4) mod 4, d3 mod 4, 0, (d1 * 2 + d5 + (d2 * 4 + d3) floordiv 16) mod 8, 0, 0)>)mlir"},
                  {"#map154",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5, d6, d7) -> (0, 0, 0, 0, 0, (d6 + (d3 * 32 + d7) floordiv 128) mod 32, (d1 * 8 + d5 + (d6 * 128 + d3 * 32 + d7) floordiv 4096) mod 128, (d3 * 32 + d7) mod 128)>)mlir"},
                  {"#map155",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, 0, 0, (((d0 * 32 + d2 + (d1 * 512 + d3) floordiv 4096) mod 128) * 4096 + (d1 * 512 + d3) mod 4096) floordiv 524288, ((((d0 * 32 + d2 + (d1 * 512 + d3) floordiv 4096) mod 128) * 4096 + (d1 * 512 + d3) mod 4096) mod 524288) floordiv 4096, ((d1 * 512 + d3) mod 4096) floordiv 128, d3 mod 128)>)mlir"},
                  {"#map156",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 4096 + d1 * 32 + d3) floordiv 1310720, ((d1 * 32 + d3) floordiv 512) mod 8, (d2 + (d1 * 32 + d3) floordiv 4096) mod 320, (d1 * 32 + d3) mod 512)>)mlir"},
                  {"#map157",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 128 + d1) floordiv 1280, (d1 floordiv 16) mod 8, (d2 + d1 floordiv 128) mod 10, d1 mod 16)>)mlir"},
                  {"#map158",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 128 + d1 * 16 + d3) floordiv 1280, (d1 * 16 + d3) mod 128, (d2 + (d1 * 16 + d3) floordiv 128) mod 10, 0)>)mlir"},
                  {"#map159",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d2 * 128 + d3) floordiv 1280, (d3 floordiv 16) mod 8, (d2 + d3 floordiv 128) mod 10, d3 mod 16)>)mlir"},
                  {"#map160",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d1 * 10 + d3) mod 80, (d0 * 16 + d2 + (d1 * 10 + d3) floordiv 80) mod 128)>)mlir"},
                  {"#map161",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 160 + d2 * 80 + d1 * 2 + d3) floordiv 1280, ((d1 * 2 + d3) floordiv 10) mod 8, (d0 * 2 + d2 + (d1 * 2 + d3) floordiv 80) mod 16, (d1 * 2 + d3) mod 10)>)mlir"},
                  {"#map162",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 8 + (d2 * 80 + d1 * 10 + d3) floordiv 160, (d1 * 5 + d3 floordiv 2) mod 40, (d2 + (d1 * 10 + d3) floordiv 80) mod 2, d3 mod 2)>)mlir"},
                  {"#map163",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 80 + d1 * 5 + d3) floordiv 1280, ((d1 * 5 + d3) floordiv 10) mod 8, (d2 + (d1 * 5 + d3) floordiv 80) mod 16, (d1 * 5 + d3) mod 10)>)mlir"},
                  {"#map164",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d2 + d3 floordiv 80, (d3 floordiv 10) mod 8, 0, d3 mod 10)>)mlir"},
                  {"#map165",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, (d1 + (d2 * 5 + d5) floordiv 80) mod 4 + ((d2 * 5 + d5) mod 80) floordiv 2560, ((d2 * 5 + d5) mod 80) mod 2560)>)mlir"},
                  {"#map166",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (d1 + (d2 * 5 + d5) floordiv 80, ((d2 * 5 + d5) floordiv 10) mod 8, 0, (d2 * 5 + d5) mod 10)>)mlir"},
                  {"#map167",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 81920 + d2 * 2560 + d1 * 320 + d3) floordiv 6225920, (d1 + d3 floordiv 320) mod 8, (d0 * 32 + d2 + (d1 * 320 + d3) floordiv 2560) mod 2432, d3 mod 320)>)mlir"},
                  {"#map168",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 80 + d1 * 10 + d3) floordiv 6080, (d1 + d3 floordiv 10) mod 8, (d0 + (d1 * 10 + d3) floordiv 80) mod 76, d3 mod 10)>)mlir"},
                  {"#map169",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 76 + d2 + (d1 * 10 + d3) floordiv 80, (d1 + d3 floordiv 10) mod 8, 0, d3 mod 10)>)mlir"},
                  {"#map170",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d2 * 80 + d3) floordiv 6080, (d3 floordiv 10) mod 8, (d2 + d3 floordiv 80) mod 76, d3 mod 10)>)mlir"},
                  {"#map171",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d1 * 76 + d3) mod 608, (d0 * 10 + d2 + (d1 * 76 + d3) floordiv 608) mod 80)>)mlir"},
                  {"#map172",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 608 + d1 * 4 + d3) floordiv 6080, ((d1 * 4 + d3) floordiv 76) mod 8, (d0 + (d1 * 4 + d3) floordiv 608) mod 10, (d1 * 4 + d3) mod 76)>)mlir"},
                  {"#map173",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 10 + d2 + (d1 * 76 + d3) floordiv 608, (d1 * 19 + d3 floordiv 4) mod 152, 0, d3 mod 4)>)mlir"},
                  {"#map174",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 608 + d1 * 4 + d3) floordiv 6080, ((d1 * 4 + d3) floordiv 76) mod 8, (d2 + (d1 * 4 + d3) floordiv 608) mod 10, (d1 * 4 + d3) mod 76)>)mlir"},
                  {"#map175",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d1 * 4 + d3) floordiv 608, ((d1 * 4 + d3) floordiv 76) mod 8, 0, (d1 * 4 + d3) mod 76)>)mlir"},
                  {"#map176",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 19456 + d1 * 128 + d3) floordiv 622592, ((d1 * 128 + d3) floordiv 2432) mod 8, (d2 + (d1 * 128 + d3) floordiv 19456) mod 32, (d1 * 128 + d3) mod 2432)>)mlir"},
                  {"#map177",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 19456 + d1 * 2432 + d3) floordiv 622592, (d1 * 19 + d3 floordiv 128) mod 152, (d2 + (d1 * 2432 + d3) floordiv 19456) mod 32, d3 mod 128)>)mlir"},
                  {"#map178",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d2 * 19456 + d3) floordiv 622592, (d3 floordiv 2432) mod 8, (d2 + d3 floordiv 19456) mod 32, d3 mod 2432)>)mlir"},
                  {"#map179",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 * 32 + d2 + (d1 * 1216 + d3) floordiv 9728) mod 128, (d1 * 1216 + d3) mod 9728)>)mlir"},
                  {"#map180",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d1 * 19 + d3) floordiv 304, ((d1 * 19 + d3) floordiv 38) mod 8, 0, (d1 * 19 + d3) mod 38)>)mlir"},
                  {"#map181",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d0 * 32 + d2 + (d1 * 1216 + d3) floordiv 9728) mod 128, (d1 * 1216 + d3) mod 9728 + 9728)>)mlir"},
                  {"#map182",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 9728 + d1 * 32 + d3) floordiv 3112960, ((d1 * 32 + d3) floordiv 1216) mod 8, (d2 + (d1 * 32 + d3) floordiv 9728) mod 320, (d1 * 32 + d3) mod 1216)>)mlir"},
                  {"#map183",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 304 + d1) floordiv 3040, (d1 floordiv 38) mod 8, (d2 + d1 floordiv 304) mod 10, d1 mod 38)>)mlir"},
                  {"#map184",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 304 + d1 * 38 + d3) floordiv 3040, (d1 * 38 + d3) mod 304, (d2 + (d1 * 38 + d3) floordiv 304) mod 10, 0)>)mlir"},
                  {"#map185",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d2 * 304 + d3) floordiv 3040, (d3 floordiv 38) mod 8, (d2 + d3 floordiv 304) mod 10, d3 mod 38)>)mlir"},
                  {"#map186",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, 0, (d1 * 10 + d3) mod 80, (d0 * 38 + d2 + (d1 * 10 + d3) floordiv 80) mod 304)>)mlir"},
                  {"#map187",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> ((d0 * 160 + d2 * 80 + d1 * 2 + d3) floordiv 3040, ((d1 * 2 + d3) floordiv 10) mod 8, (d0 * 2 + d2 + (d1 * 2 + d3) floordiv 80) mod 38, (d1 * 2 + d3) mod 10)>)mlir"},
                  {"#map188",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 * 19 + (d2 * 80 + d1 * 10 + d3) floordiv 160, (d1 * 5 + d3 floordiv 2) mod 40, (d2 + (d1 * 10 + d3) floordiv 80) mod 2, d3 mod 2)>)mlir"},
                  {"#map189",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (d0 + (d2 * 80 + d1 * 5 + d3) floordiv 3040, ((d1 * 5 + d3) floordiv 10) mod 8, (d2 + (d1 * 5 + d3) floordiv 80) mod 38, (d1 * 5 + d3) mod 10)>)mlir"},
                  {"#map190",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, (d2 mod 4) floordiv 128, (d2 mod 4) mod 128)>)mlir"},
                  {"#map191",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, d1 mod 4, 0, 0, 0)>)mlir"},
                  {"#map192",
                   R"mlir(affine_map<(d0, d1, d2, d3) -> (0, (d3 floordiv 2) mod 40, 0, d3 mod 2)>)mlir"},
                  {"#map193",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, ((d2 * 2 + d5) mod 80) floordiv 2560, ((d2 * 2 + d5) mod 80) mod 2560)>)mlir"},
                  {"#map194",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> ((d2 floordiv 8) mod 5, d2 mod 8, d3, d4, d5)>)mlir"},
                  {"#map195",
                   R"mlir(affine_map<(d0, d1) -> (0, 0, 0, (d1 + d0 * 8) mod 40)>)mlir"},
                  {"#map196",
                   R"mlir(affine_map<(d0, d1, d2, d3, d4, d5) -> (0, 0, ((d2 * 5 + d5) floordiv 2) mod 40, 0, 0, (d2 * 5 + d5) mod 2)>)mlir"},
              };

  for (const QwenAffineMapRepro &testCase : testCases) {
    auto attr = llvm::dyn_cast_if_present<AffineMapAttr>(
        parseAttribute(testCase.map, &context));
    ASSERT_TRUE(attr) << "failed to parse " << testCase.name.str();

    AffineMap map = attr.getValue();
    SmallVector<int64_t> shape(map.getNumDims(), 16);
    std::optional<size_t> coalescingFactor =
        tryComputeCoalescingFactorAnalytically(
            map, shape, /*numGridDims=*/map.getNumDims() / 2,
            /*elemSizeBytes=*/1);
    EXPECT_TRUE(coalescingFactor.has_value())
        << "analysis fell back for " << testCase.name.str() << ": " << map;
  }
}

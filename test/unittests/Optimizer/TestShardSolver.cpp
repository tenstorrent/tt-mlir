...
TEST_F(ShardSolverBase, VerifyProduceMaxCoreUsage) {
  ...
  mlir::Operation *op =
      AddOp::create(builder, builder.getUnknownLoc(), lhs.getType(), lhs, rhs);
  ...
  rhs = op->getResult(0);
  op = ReluOp::create(builder, builder.getUnknownLoc(), rhs.getType(), rhs);
  prepareOpForShardSolver(op, opL1MemSpecs, l1ChainedOps);
  addConfigForOp(op, legalConfigs, BufferType::L1,
  ...
  op = AddOp::create(builder, builder.getUnknownLoc(), lhs.getType(), lhs, rhs);
  prepareOpForShardSolver(op, opL1MemSpecs, l1ChainedOps);
  addConfigForOp(op, legalConfigs, BufferType::L1,
                 TensorMemoryLayout::WidthSharded, 1, 4);
  addConfigForOp(op, legalConfigs, BufferType::L1,

...
// tensor-L1 policy now lives: it charges each op the live set plus its output,
// and deliberately not its static CB region -- whether the CB region collides
// with the live set is an address question, owned by tt-metal's own
// CB-vs-lowest-occupied-address check and by the headroom floor below.
//===----------------------------------------------------------------------===//

class StatefulBudgetCeilingTest : public L1SpillTestFixture {};

// opA (600 KiB) is live when opB (800 KiB output) is validated. The synthetic
// backend -- standing in for tt-metal, which sees only the physical L1 --
// reports success, but 1400 KiB > the 1300 KiB optimizer budget, so the ceiling
// must report OOM and the pass must evict opA.
TEST_F(StatefulBudgetCeilingTest, LiveSetPlusOutputBindsAgainstBudget) {
  l1BudgetPerCore = 1300 * kKiB;
  llvm::SmallVector<int64_t> shape = {1, 1, 1024, 1024};
  ...
  auto args = beginFunc({tt});
  auto *opA = addUnary(args[0], tt, /*l1UsageBytes=*/600 * kKiB);
  auto *opB = addUnary(opA->getResult(0), tt, /*l1UsageBytes=*/800 * kKiB);
  finishFunc({opB->getResult(0)});

  auto [obs] = runStateful();

  EXPECT_FALSE(obs->ooms.empty())
      << "live + output exceeds the optimizer budget";
  ASSERT_FALSE(obs->evictions.empty());
  EXPECT_EQ(obs->evictions[0].victim, opA);
...
// Buffers well under budget (300 + 300 KiB <= 1300 KiB) but a 900 KiB CB region
// that would blow it if summed in. Nothing may be spilled: the CB region grows
// up from l1UnreservedBase while these buffers sit near the top, so there is no
// overlap to report, and summing CB bytes into the tensor budget would only
// spill a hot activation for nothing.
TEST_F(StatefulBudgetCeilingTest, CBPeakDoesNotCountAgainstBudget) {
  l1BudgetPerCore = 1300 * kKiB;
  llvm::SmallVector<int64_t> shape = {1, 1, 1024, 1024};
  auto tt = tensorType(shape, makeL1Sharded(shape));
  ...
  auto args = beginFunc({tt});
  auto *opA = addUnary(args[0], tt, /*l1UsageBytes=*/300 * kKiB);
  auto *opB = addUnary(opA->getResult(0), tt, /*l1UsageBytes=*/300 * kKiB);
  setL1Usage(opB, /*l1=*/300 * kKiB, /*cb=*/900 * kKiB);
  finishFunc({opB->getResult(0)});

  auto [obs] = runStateful();

  EXPECT_TRUE(obs->ooms.empty())
      << "the op's own CB peak must not bind the byte ceiling";
  EXPECT_EQ(countSpills(), 0u);
  EXPECT_TRUE(resultIsL1(opA->getResult(0)));
}

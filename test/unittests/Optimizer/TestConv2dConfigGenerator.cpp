...
TEST_F(Conv2dConfigGeneratorTest, SingleFieldIteration) {
  Conv2dConfigAttr baseConfig = Conv2dConfigAttr::getEmpty(&context);
  Conv2dConfigSearchSpace space;
  space.dtype = {mlir::tt::DataType::BFloat16, mlir::tt::DataType::Float32,
                 mlir::tt::DataType::UInt32};
  Conv2dConfigGenerator gen(/*op=*/nullptr, baseConfig, space, filterOutFn);
  std::vector<mlir::tt::DataType> seen;
  ...
  std::vector<mlir::tt::DataType> expected = {mlir::tt::DataType::BFloat16,
...
TEST_F(Conv2dConfigGeneratorTest, MultipleFieldIteration) {
  ...
  Conv2dConfigGenerator gen(/*op=*/nullptr, baseConfig, space, filterOutFn);
  std::set<std::pair<mlir::tt::DataType, std::string>> seen;
  while (!gen.searchDone()) {
    auto config = gen.getNextConfig();
    ASSERT_TRUE(config);
    ASSERT_TRUE(config.getDtype().has_value());
  ...
  EXPECT_TRUE(seen.count({mlir::tt::DataType::BFloat16, "relu"}));
  EXPECT_TRUE(seen.count({mlir::tt::DataType::BFloat16, "gelu"}));
  EXPECT_TRUE(seen.count({mlir::tt::DataType::Float32, "relu"}));
  EXPECT_TRUE(seen.count({mlir::tt::DataType::Float32, "gelu"}));
}

TEST_F(Conv2dConfigGeneratorTest, FilterOut) {
  Conv2dConfigAttr baseConfig = Conv2dConfigAttr::getEmpty(&context);
  Conv2dConfigSearchSpace space;
  space.dtype = {mlir::tt::DataType::BFloat16, mlir::tt::DataType::UInt32};
...
TEST_F(Conv2dConfigGeneratorTest, EdgeCaseSingleConfig) {
  Conv2dConfigAttr baseConfig = Conv2dConfigAttr::getEmpty(&context);
  Conv2dConfigSearchSpace space;
  space.dtype = {mlir::tt::DataType::Float32};
  Conv2dConfigGenerator gen(nullptr, baseConfig, space, filterOutFn);
  auto config = gen.getNextConfig();
  ASSERT_TRUE(config);
  EXPECT_EQ(config.getDtype(), mlir::tt::DataType::Float32);
  EXPECT_TRUE(gen.searchDone());
...
TEST_F(Conv2dConfigGeneratorTest, NonEmptyBaseConfig) {
  Conv2dConfigAttr baseConfig = Conv2dConfigAttr::getEmpty(&context);
  baseConfig = baseConfig.withDtype(mlir::tt::DataType::Float32);

  Conv2dConfigSearchSpace space;
  space.dtype = {mlir::tt::DataType::Float32, mlir::tt::DataType::UInt32};
  space.activation = {"relu", "gelu"};
  space.weightsDtype = {mlir::tt::DataType::Float32,
                        mlir::tt::DataType::UInt32};

  Conv2dConfigGenerator gen(nullptr, baseConfig, space, filterOutFn);
  std::set<std::tuple<mlir::tt::DataType, std::string, mlir::tt::DataType>>
      seen;
  while (!gen.searchDone()) {
    auto config = gen.getNextConfig();
    ASSERT_TRUE(config);
  ...
  EXPECT_TRUE(seen.count(
      {mlir::tt::DataType::Float32, "relu", mlir::tt::DataType::UInt32}));
  EXPECT_TRUE(seen.count(
      {mlir::tt::DataType::Float32, "gelu", mlir::tt::DataType::UInt32}));
  EXPECT_TRUE(seen.count(
      {mlir::tt::DataType::Float32, "relu", mlir::tt::DataType::Float32}));
  EXPECT_TRUE(seen.count(
      {mlir::tt::DataType::Float32, "gelu", mlir::tt::DataType::Float32}));
}

...
public:
  ...
  llvm::SmallVector<int64_t> GetVirtualGridShape(
    ...
    auto tilePaddedShape = mlir::tt::ttnn::utils::getTilePaddedShape(tensorShape);
    size_t rank = tilePaddedShape.size();
    auto physicalShape = llvm::SmallVector<int64_t, 2>({1, tilePaddedShape[rank - 1]});
    for (size_t i = 0; i < rank - 1; ++i) {
      physicalShape[0] *= tilePaddedShape[i];
    }
    auto shapeInTiles = llvm::SmallVector<int64_t, 2>({physicalShape[0] / 32, physicalShape[1] / 32});

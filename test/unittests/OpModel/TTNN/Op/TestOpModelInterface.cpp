...
// Sanity-check that OpModel<Conv3dOp>::getOpRuntime returns *different* cycle
// estimates for *different* Conv3dConfigAttr inputs. This is a precondition
// for any optimizer scoring to work: if OpModel collapses all configs to the
// same number, no amount of tiebreaker rules can pick a winner.
//
// This is a soft floor: we assert distinguishability, not directionality.
// The strong "optimizer pick beats tt-metal default by N×" assertion lives
// in Phase 5 once scoring tiebreakers exist.
TEST_F(OpModelBase, Conv3dOpModelDiscriminatesConfigs) {
  llvm::SmallVector<int64_t> inputShape = {1, 8, 28, 28, 128};
  llvm::SmallVector<int64_t> weightShape = {3456, 32};
  llvm::SmallVector<int64_t> outputShape = {1, 6, 26, 26, 32};
  ...
  Conv3dOp conv3d = builder.create<Conv3dOp>(
      builder.getUnknownLoc(), outputType, input, weight, /*bias=*/nullptr,
      deviceOp, /*in_channels=*/128, /*out_channels=*/32, /*batch_size=*/1,
  ...
  // Config A: tt-metal default-style — t=h=w=1, c_in=c_out=32.
  auto configA = Conv3dConfigAttr::get(
      &context,
      /*weights_dtype=*/std::nullopt,
      /*t_out_block=*/std::optional<uint32_t>(1),
      /*w_out_block=*/std::optional<uint32_t>(1),
  ...
  // Config B: larger spatial blocks (h=2, w=2 — both divide H_out=W_out=26
  // and h*w=4 <= 256) and larger c_in_block=64.
  auto configB = Conv3dConfigAttr::get(
      &context,
      /*weights_dtype=*/std::nullopt,
      /*t_out_block=*/std::optional<uint32_t>(1),
      /*w_out_block=*/std::optional<uint32_t>(2),
      /*h_out_block=*/std::optional<uint32_t>(2),
      /*c_out_block=*/std::optional<uint32_t>(32),
      /*c_in_block=*/std::optional<uint32_t>(64),
      /*compute_with_storage_grid_size=*/std::optional<ttcore::GridAttr>());

  auto runtimeA = backend.getOpRuntime(
      getInputLayouts(conv3d),
      OpConfig(getOutputLayout(conv3d), Conv3dAttrs{configA, std::nullopt}));
  ASSERT_TRUE(static_cast<bool>(runtimeA))
      << llvm::toString(runtimeA.takeError());

  auto runtimeB = backend.getOpRuntime(
      getInputLayouts(conv3d),
      OpConfig(getOutputLayout(conv3d), Conv3dAttrs{configB, std::nullopt}));
  ASSERT_TRUE(static_cast<bool>(runtimeB))
      << llvm::toString(runtimeB.takeError());

  EXPECT_GT(runtimeA.get(), 0u);
  EXPECT_GT(runtimeB.get(), 0u);
  // If these are equal, OpModel is not discriminating configs and the
  // optimizer cannot make informed choices — any scoring rule degenerates
  // to arbitrary tiebreaker.
  EXPECT_NE(runtimeA.get(), runtimeB.get())
      << "OpModel<Conv3dOp>::getOpRuntime returned identical cycles for "
         "distinct configs A=(1,1,1,32,32) and B=(1,2,2,32,64). The "
         "optimizer cannot rank configs without a discriminating cost.";

  // Directional perf floor: B (larger spatial blocks, larger c_in_block)
  // must NOT be more than 5x slower than A. This is a loose floor — it
  // catches regressions where OpModel grossly mis-costs aggressive blocking.
  // The strong "B < A" assertion belongs in silicon perf tests (Phase 6)
  // because OpModel cycle estimates are not silicon truth.
  EXPECT_LE(runtimeB.get(), runtimeA.get() * 5)
      << "Aggressive blocking (B) is more than 5x slower than tt-metal "
         "default (A) per OpModel — config search likely picks degenerate "
         "configs. A="
      << runtimeA.get() << ", B=" << runtimeB.get();
}

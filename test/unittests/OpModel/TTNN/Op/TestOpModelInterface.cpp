...
TEST_F(OpModelBase, argMaxOp) {
  // create ArgMaxOp
  llvm::SmallVector<int64_t> tensorShapeA = {64, 64};

  // For now, use simple tiled layout for testing the interface
  // TODO: ArgMax requires ROW_MAJOR layout, but creating proper ROW_MAJOR
  // layouts in tests causes segfaults. This tests the interface with expected
  // layout failure.
  auto input = createEmptyTensor(tensorShapeA);
  ...
  // test argMax Op interface
  // Note: This will fail constraints due to layout mismatch, but tests the
  // interface
  auto constraintsExp = getOpConstraints(argMax.getOperation());
  if (constraintsExp) {
    auto l1 = constraintsExp.get();
    const auto &[cbSize, peakSize, outputSize, outputLayout] = l1;
    // If constraints somehow pass, expect some memory usage
    EXPECT_GT(cbSize, 0);
    EXPECT_GT(peakSize, 0);
    EXPECT_GT(outputSize, 0);
  } else {
    // Expected failure due to layout constraints - just verify we get an error
    auto error = constraintsExp.takeError();
    std::string errorMsg = llvm::toString(std::move(error));
    // Verify it's the expected ROW_MAJOR layout error
    EXPECT_TRUE(errorMsg.find("ROW_MAJOR") != std::string::npos ||
                errorMsg.find("layout") != std::string::npos ||
                errorMsg.find("last dim") != std::string::npos);
  }

  // Skip runtime testing since constraints will fail
  // auto runtimeExp = getOpRuntime(argMax.getOperation());
}

TEST_F(OpModelBase, prodOp) {
  // create ProdOp
  llvm::SmallVector<int64_t> tensorShapeA = {64, 64};
  ...
  auto runtimeExp = getOpRuntime(prod.getOperation());
  if (runtimeExp) {
    EXPECT_TRUE(runtimeExp.get() > 0);
  } else {
    FAIL() << llvm::toString(runtimeExp.takeError());

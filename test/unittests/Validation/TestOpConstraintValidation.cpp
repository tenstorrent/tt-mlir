...
// getUsableL1PerCore must return the same budget whether it is asked via an op
// inside the module or via the module itself. getParentOfType starts from
// getParentOp(), so called on the ModuleOp it used to return null and silently
// fall back to the defaults -- which meant the L1 spill pass's per-core budget
// (computed from the module) ignored both `ttnn.tensor_l1_usage_cap` and
// `ttnn.l1_const_eval_usage`. That budget is now the only carrier of those two
// policies on the stateful path, so it has to read them.
TEST_F(OpConstraintValidationTest, UsableL1PerCoreReadsPolicyFromModuleOp) {
  auto addOp = createMockAddOp();
  setL1UsageCap(0.5f);
  module->getOperation()->setAttr(
      utils::g_L1ConstEvalUsageAttrName,
      builder.getI64IntegerAttr(/*const-eval L1 reservation=*/1024));

  const uint64_t usableL1 =
      ttcore::getOpChipDescAttr(addOp.getOperation()).getUsableL1Size();
  const uint64_t expected =
      static_cast<uint64_t>(0.5f * usableL1) - /*reserved=*/1024;

  EXPECT_EQ(utils::getUsableL1PerCore(addOp.getOperation()), expected);
  EXPECT_EQ(utils::getUsableL1PerCore(module->getOperation()), expected);
}

// The peak-L1 byte check is the stateless path's only capacity model (both of
// tt-metal's captures run NO_DISPATCH there, so nothing is allocated and no
// OOM can ever be thrown), but on the stateful path tt-metal really allocates
// on a pre-seeded allocator and decides fit itself. The peak it reports there
// covers only the op's own trace-local allocations -- neither the live set nor
// the op's inputs, which are materialized weightless in a discarded first
// capture -- so comparing it against the L1 limit is strictly weaker than the
// backend's verdict. checkConstraintsResult must therefore skip the check for
// a stateful result and keep it for a stateless one. The optimizer's own byte
// budget (the tensorL1UsageCap and const-eval reservation) is enforced instead
// by MockAllocatorL1Tracker's ceiling, which has the live set in hand.
TEST_F(OpConstraintValidationTest, StatefulQuerySkipsPeakL1ByteCheck) {
  auto addOp = createMockAddOp();

  // 0.1% of L1: any nonzero peak is over the limit.
  setL1UsageCap(0.001f);

  auto makeConstraints = [&](size_t peak) {
    return llvm::Expected<op_model::OpConstraints>(op_model::OpConstraints(
        /*cbPeak=*/4096, /*tensorPeak=*/8192,
        /*peakMemory=*/peak, /*outputBuffer=*/8192,
        llvm::SmallVector<TTNNLayoutAttr>{createTiledLayout(
            {1, 1, 32, 32}, BufferType::L1, TensorMemoryLayout::Interleaved)}));
  };

  auto stateless = op_constraint_validation::checkConstraintsResult(
      addOp.getOperation(), makeConstraints(12288), /*additionalL1Usage=*/0,
      /*statefulQuery=*/false);
  EXPECT_EQ(stateless.status,
            op_constraint_validation::ValidationStatus::OutOfMemoryError);

  auto stateful = op_constraint_validation::checkConstraintsResult(
      addOp.getOperation(), makeConstraints(12288), /*additionalL1Usage=*/0,
      /*statefulQuery=*/true);
  EXPECT_TRUE(stateful.isSuccess());
  EXPECT_EQ(stateful.outputL1Usage, 8192u);
  EXPECT_EQ(stateful.cbPeakUsage, 4096u);

  // The same skip is what retires the spurious-spill class the stateful path
  // used to have: tt-metal's phase-2 trace carries deallocations for
  // pre-seeded buffers it never saw allocated, so its unsigned running
  // counters can wrap and report a 2^64-N peak for an op under no memory
  // pressure at all. Statelessly that peak is a real measurement and must
  // still be rejected; statefully it is never consulted.
  constexpr size_t kWrappedPeak = ~static_cast<size_t>(0) - 20480 + 1;
  EXPECT_TRUE(op_constraint_validation::checkConstraintsResult(
                  addOp.getOperation(), makeConstraints(kWrappedPeak),
                  /*additionalL1Usage=*/0, /*statefulQuery=*/true)
                  .isSuccess());
}

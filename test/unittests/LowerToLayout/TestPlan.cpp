...
TEST_F(CanonicalizeTest, HostToDeviceVirtualGridUsesNativeLayout) {
  auto collapsed = ttcore::MetalLayoutAttr::computeDefaultCollapsedIntervals(
      &context, /*rank=*/2);
  ...
  auto *step = std::get_if<HostToDeviceStep>(&plan.front());
  ASSERT_NE(step, nullptr);

  auto deviceLayout =
      mlir::cast<ttcore::MetalLayoutAttr>(step->output.type.getEncoding());
  EXPECT_EQ(deviceLayout.getMemorySpace(), ttcore::MemorySpace::DeviceL1);
  EXPECT_EQ(deviceLayout.getMemoryLayout(),
            ttcore::TensorMemoryLayout::Sharded);
  EXPECT_EQ(deviceLayout.getDimAlignments(), ArrayRef<int64_t>({32, 256}));
  EXPECT_EQ(deviceLayout.getCollapsedIntervals(), collapsed);

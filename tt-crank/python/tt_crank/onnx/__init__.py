...
module is the glue tests and users share: sessions on the EP, tt-backend tensors,
...
def tt_device() -> onnxruntime.OrtEpDevice:
...
def input(name: str, shape: list | None, dtype: int = DType.F32) -> onnx.ValueInfoProto:
    """Declare a graph input; dims may be symbolic (e.g. "N")."""
    return helper.make_tensor_value_info(name, dtype, shape)


def output(
    name: str, shape: list | None, dtype: int = DType.F32
) -> onnx.ValueInfoProto:
    """Declare a graph output; shape None lets shape inference fill it in."""
    return helper.make_tensor_value_info(name, dtype, shape)


def constant(name: str, arr: np.ndarray) -> onnx.TensorProto:
...
def model(
    ...
    proto = helper.make_model(graph, opset_imports=[helper.make_opsetid("", opset)])
    return shape_inference.infer_shapes(proto).SerializeToString()


def export_torch(module, dummy, dynamic_batch: bool = False) -> bytes:
    """Export a torch module to ONNX bytes at EP_OPSET_VERSION, with input `x` and
    output `y`; `dynamic_batch` makes dim 0 of both symbolic ("N")."""
    import torch  # a tt_crank dependency, kept off this module's import path

    dynamic_axes = {"x": {0: "N"}, "y": {0: "N"}} if dynamic_batch else None
    buf = io.BytesIO()
    torch.onnx.export(
        module.eval(),
        dummy,
...
def tt_memory_info() -> onnxruntime.OrtMemoryInfo:
    """Memory info of the tt backend, for allocating its OrtValues."""
    return tt_device().memory_info(onnxruntime.OrtDeviceMemoryType.DEFAULT)
...
def empty_tt_tensor(shape, dtype) -> onnxruntime.OrtValue:
    """An uninitialized OrtValue on the tt backend."""
...
def to_tt(arr: np.ndarray) -> onnxruntime.OrtValue:
    """Move `arr` to the tt backend."""
    dev = empty_tt_tensor(arr.shape, arr.dtype)
    onnxruntime.copy_tensors([host_ortvalue(arr)], [dev])
    return dev


def to_host(ov: onnxruntime.OrtValue) -> np.ndarray:
    """Move a tt-backend OrtValue back to a numpy array (bf16 as ml_dtypes.bfloat16)."""
...
def bind(
    ...
    """Bind `inputs` and output slots on the tt backend; returns (io_binding, outputs).
    Run with session.run_with_iobinding(io) as often as needed, read back with to_host."""
    ...
    for out in session.get_outputs():
        ...
        ov = empty_tt_tensor(out.shape, _numpy_dtype(out.type))
...
def run(
    ...
    """One run with inputs and outputs on the tt backend."""

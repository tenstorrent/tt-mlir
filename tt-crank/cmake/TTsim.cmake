...
# does in shell.
#
# Always builds the staging and wires the paths into libtt_crank.so. Whether
# tt-crank actually opens ttsim is a runtime decision driven by the
# TT_CRANK_USE_SIMULATOR env var — see src/engine/sim_env.cpp and the `sim`
# test preset in CMakePresets.json.
#
# Exposes (parent scope):
#   TT_CRANK_SIM_DIR          staged sim home dir
#   TT_CRANK_TT_METAL_HOME    tt-metal source tree the runtime needs at TT_METAL_RUNTIME_ROOT
...
set(TT_CRANK_SIM_ARCH "bh" CACHE STRING "Simulator arch (wh|bh)")
set_property(CACHE TT_CRANK_SIM_ARCH PROPERTY STRINGS wh bh)

if(TT_CRANK_SIM_ARCH STREQUAL "wh")
    set(_soc_filename "wormhole_b0_80_arch.yaml")
elseif(TT_CRANK_SIM_ARCH STREQUAL "bh")
...
file(MAKE_DIRECTORY "${TT_CRANK_SIM_DIR}")

# Cache the download by arch so flipping TT_CRANK_SIM_ARCH doesn't redownload.
set(_lib_cache "${CMAKE_BINARY_DIR}/_deps/libttsim_${TT_CRANK_SIM_ARCH}_${TT_CRANK_TTSIM_VERSION}.so")
if(NOT EXISTS "${_lib_cache}")
    message(STATUS "Downloading ttsim ${TT_CRANK_TTSIM_VERSION} (${TT_CRANK_SIM_ARCH})")
    file(DOWNLOAD
        "https://github.com/tenstorrent/ttsim/releases/download/${TT_CRANK_TTSIM_VERSION}/libttsim_${TT_CRANK_SIM_ARCH}.so"
        "${_lib_cache}"
...
add_custom_target(tt_crank_ttsim_stage DEPENDS "${_soc_staged}")
...
target_compile_definitions(tt_crank PRIVATE
    TT_CRANK_SIM_DIR="${TT_CRANK_SIM_DIR}"
    TT_CRANK_TT_METAL_HOME="${TT_CRANK_TT_METAL_HOME}"
)
add_dependencies(tt_crank tt_crank_ttsim_stage)

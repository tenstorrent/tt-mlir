...
Benchmarks live under `tests/python/torch/benchmarks/` and `tests/python/onnx/benchmarks/` and run as pytest targets. The shared flags below are always available; the torch-only ones (`--mode`, `--profiler`, `--profile-dir`, `--strict-no-fallback`, `--llm-*`) are registered by `tests/python/torch/benchmarks/conftest.py`, so pytest has to be invoked at-or-below that directory for them to be recognized.

```sh
pytest tt-crank/tests/python/torch/benchmarks/                        # run all torch benchmarks
pytest tt-crank/tests/python/torch/benchmarks/test_mnist_linear.py    # one benchmark
pytest tt-crank/tests/python/onnx/benchmarks/ --accuracy              # ONNX Runtime EP benchmarks
```

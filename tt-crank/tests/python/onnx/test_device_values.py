...
_GEMM_RELU = tt_onnx.model(
    [
        tt_onnx.node("Gemm", ["x", "w"], ["g"], transB=1),
        tt_onnx.node("Relu", ["g"], ["y"]),
    ],
    [tt_onnx.input("x", [32, 64])],
    [tt_onnx.output("y", [32, 128])],
    [tt_onnx.constant("w", tt_onnx.randn(128, 64))],
)
...
def test_device_ortvalue_roundtrip() -> None:
    ...
    np.testing.assert_array_equal(x, tt_onnx.to_host(tt_onnx.to_tt(x)))
...
def test_iobinding_device_output(session: ort.InferenceSession) -> None:
    ...
    y_dev = tt_onnx.empty_tt_tensor((32, 128), np.float32)
    io = session.io_binding()
    io.bind_ortvalue_input("x", tt_onnx.to_tt(x))
...
def test_device_to_device_copy() -> None:
    ...
    dst = tt_onnx.empty_tt_tensor(x.shape, np.float32)

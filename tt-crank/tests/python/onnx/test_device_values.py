...
def session() -> ort.InferenceSession:
    return tt_onnx.session_on_tt(_GEMM_RELU)


def test_device_ortvalue_roundtrip() -> None:
    x = tt_onnx.randn(32, 64)
    np.testing.assert_array_equal(x, tt_onnx.from_tt(tt_onnx.to_tt(x)))
...
def test_iobinding_device_input_reused(session: ort.InferenceSession) -> None:
    ...
    io.bind_ortvalue_input("x", tt_onnx.to_tt(x))
...
def test_iobinding_device_output(session: ort.InferenceSession) -> None:
    ...
    y_dev = tt_onnx.tt_empty((32, 128), np.float32)
    io = session.io_binding()
    io.bind_ortvalue_input("x", tt_onnx.to_tt(x))
    io.bind_ortvalue_output("y", y_dev)
    session.run_with_iobinding(io)
    np.testing.assert_allclose(y_plain, tt_onnx.from_tt(y_dev), atol=1e-5)


def test_device_to_device_copy() -> None:
    x = tt_onnx.randn(32, 32)
    dst = tt_onnx.tt_empty(x.shape, np.float32)
    ort.copy_tensors([tt_onnx.to_tt(x)], [dst])
    np.testing.assert_array_equal(x, tt_onnx.from_tt(dst))

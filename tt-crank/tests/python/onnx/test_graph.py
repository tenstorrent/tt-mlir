...
def test_initializer_shared_between_metadata_and_operand() -> None:
    ...
    model = tt_onnx.model(
        [
            tt_onnx.node("Clip", ["x", "lo", "hi"], ["c"]),
            tt_onnx.node("Mul", ["c", "lo"], ["y"]),
        ],
        [tt_onnx.input("x", [16, 16])],
        [tt_onnx.output("y", [16, 16])],
        [
            tt_onnx.constant("lo", np.float32(-0.5)),
            tt_onnx.constant("hi", np.float32(0.5)),
        ],
    )
...
def test_params_consumed_mid_graph() -> None:
    ...
    model = tt_onnx.model(
        [
            tt_onnx.node("Relu", ["x"], ["r"]),
            tt_onnx.node("Gemm", ["r", "w"], ["g"], transB=1),
            tt_onnx.node("Relu", ["g"], ["h"]),
            tt_onnx.node("Mul", ["h", "scale"], ["y"]),
        ],
        [tt_onnx.input("x", [32, 64])],
        [tt_onnx.output("y", [32, 128])],
        [
            tt_onnx.constant("w", tt_onnx.randn(128, 64) / 8),
            tt_onnx.constant("scale", tt_onnx.randn(128)),
        ],
    )

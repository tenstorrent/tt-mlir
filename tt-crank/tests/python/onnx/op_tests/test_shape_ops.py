...
def test_transpose_flatten_identity() -> None:
    model = make_model(
        [
            helper.make_node("Transpose", ["x"], ["t"], perm=[1, 0, 2]),
            helper.make_node("Flatten", ["t"], ["f"], axis=1),
            helper.make_node("Identity", ["f"], ["y"]),
        ],
        [vi("x", [4, 8, 32])],
        [vi("y", None)],
    )
...
def test_squeeze_unsqueeze() -> None:
    model = make_model(
        [
            helper.make_node("Unsqueeze", ["x", "axes"], ["u"]),
            helper.make_node("Squeeze", ["u", "axes"], ["y"]),
        ],
        [vi("x", [4, 8, 32])],
        [vi("y", None)],
...
def test_old_opset_attribute_forms() -> None:
    ...
    model = make_model(
        [
            helper.make_node("Clip", ["x"], ["c"], min=-0.5, max=0.5),
            helper.make_node("Clip", ["c"], ["c2"]),
            helper.make_node("Squeeze", ["c2"], ["y"], axes=[0]),
        ],
        [vi("x", [1, 16, 16])],
        [vi("y", None)],
        opset=9,

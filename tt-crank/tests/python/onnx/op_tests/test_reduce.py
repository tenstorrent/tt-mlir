...
def test_reduce_mean_axes_input(keepdims: int) -> None:
    ...
    model = tt_onnx.model(
        [tt_onnx.node("ReduceMean", ["x", "axes"], ["y"], keepdims=keepdims)],
        [tt_onnx.input("x", [2, 8, 4, 4])],
        [tt_onnx.output("y", None)],
        [tt_onnx.constant("axes", np.array([-1, -2], dtype=np.int64))],
    )
...
def test_reduce_mean_axes_attr() -> None:
    ...
    model = tt_onnx.model(
        [tt_onnx.node("ReduceMean", ["x"], ["y"], axes=[2, 3], keepdims=1)],
        [tt_onnx.input("x", [2, 8, 4, 4])],
        [tt_onnx.output("y", None)],
        opset=13,
    )

...
def test_binary_broadcast() -> None:
    model = tt_onnx.make_model(
        ...
        [tt_onnx.vi("a", [4, 32, 32]), tt_onnx.vi("b", [32])],
        [tt_onnx.vi("y", [4, 32, 32])],
    )
...
def test_clip(lo: float, hi: float) -> None:
    ...
    model = tt_onnx.make_model(
        [tt_onnx.make_node("Clip", ["x", "lo", "hi"], ["y"])],
        [tt_onnx.vi("x", [1, 8, 16, 16])],
        [tt_onnx.vi("y", None)],
        [
            tt_onnx.make_tensor("lo", np.float32(lo)),
            tt_onnx.make_tensor("hi", np.float32(hi)),
        ],
    )
...
def test_softmax() -> None:
    model = tt_onnx.make_model(
        [tt_onnx.make_node("Softmax", ["x"], ["y"], axis=-1)],
        [tt_onnx.vi("x", [8, 64])],
        [tt_onnx.vi("y", [8, 64])],
    )

...
def _gemm_relu() -> bytes:
    return tt_onnx.make_model(
        [
            tt_onnx.make_node("Gemm", ["x", "w"], ["y"], transB=1),
            tt_onnx.make_node("Relu", ["y"], ["z"]),
        ],
        [tt_onnx.vi("x", [4, 16])],
        [tt_onnx.vi("z", [4, 8])],
        [tt_onnx.make_tensor("w", tt_onnx.randn(8, 16) / 4)],
    )
...
def _gemm_relu_reshape() -> bytes:
    ...
    return tt_onnx.make_model(
        [
            tt_onnx.make_node("Gemm", ["x", "w", "b"], ["y"], transB=1),
            tt_onnx.make_node("Relu", ["y"], ["r"]),
            tt_onnx.make_node("Reshape", ["r", "shape"], ["z"]),
        ],
        [tt_onnx.vi("x", [4, 8])],
        [tt_onnx.vi("z", [2, 2, 16])],
        [
            tt_onnx.make_tensor("w", tt_onnx.randn(16, 8) / 4),
            tt_onnx.make_tensor("b", tt_onnx.randn(16) / 4),
            tt_onnx.make_tensor("shape", np.array([2, 2, 16], dtype=np.int64)),
        ],
    )

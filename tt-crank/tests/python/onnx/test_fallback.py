...
_TANH = tt_onnx.model(
    [tt_onnx.node("Tanh", ["x"], ["y"])],
    [tt_onnx.input("x", [16, 16])],
    [tt_onnx.output("y", [16, 16])],
)
...
def test_unsupported_op_falls_back_to_cpu() -> None:
    ...
    got = tt_onnx.session(_TANH, allow_cpu_fallback=True).run(None, inputs)
...
def test_zero_sized_dim_falls_back_to_cpu() -> None:
    ...
    model = tt_onnx.model(
        ...
        [tt_onnx.input("a", [0, 32]), tt_onnx.input("b", [0, 32])],
        [tt_onnx.output("y", [0, 32])],
    )
    inputs = {"a": tt_onnx.randn(0, 32), "b": tt_onnx.randn(0, 32)}
    (got,) = tt_onnx.session(model, allow_cpu_fallback=True).run(None, inputs)
...
def test_batch_norm_rank6_fails_session_creation() -> None:
    ...
    model = tt_onnx.model(
        [tt_onnx.node("BatchNormalization", ["x", "s", "b", "m", "v"], ["y"])],
        [tt_onnx.input("x", [2, 4, 2, 2, 8, 8])],
        [tt_onnx.output("y", None)],
        [
            tt_onnx.constant("s", tt_onnx.randn(4)),
            tt_onnx.constant("b", tt_onnx.randn(4)),
            tt_onnx.constant("m", tt_onnx.randn(4)),
            tt_onnx.constant("v", np.abs(tt_onnx.randn(4)) + 0.5),
        ],
    )

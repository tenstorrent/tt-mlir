...
struct OrtDataTransferImpl {
     ...
     * \since Version 1.23.
     */
    ORT_API_T(bool, CanCopy, _In_ const OrtDataTransferImpl *this_ptr, _In_ const OrtMemoryDevice *src_memory_device,
    ...
    ORT_API2_STATUS(CopyTensors, _In_ OrtDataTransferImpl *this_ptr,
                    ...
                    _In_reads_(num_tensors) OrtValue **dst_tensors, _In_reads_(num_tensors) OrtSyncStream **streams,
...
struct OrtExternalResourceImporterImpl {
    ...
    ORT_API2_STATUS(WaitSemaphore, _In_ OrtExternalResourceImporterImpl *this_ptr,
                    _In_ OrtExternalSemaphoreHandle *handle, _In_ OrtSyncStream *stream, _In_ uint64_t value);
...
struct OrtEpProfilerImpl {
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(EndProfiling, _In_ OrtEpProfilerImpl *this_ptr, _In_ OrtProfilingEventsContainer *events_container);
};
...
struct OrtNodeComputeInfo {
    ...
    OrtStatus *(ORT_API_CALL *CreateState)(_In_ OrtNodeComputeInfo *this_ptr,
                                           _In_ OrtNodeComputeContext *compute_context, _Outptr_ void **compute_state);
...
struct OrtKernelImpl {
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(PrePackWeight, _In_ OrtKernelImpl *this_ptr, _In_ const OrtValue *tensor, _In_ int input_index,
                    _Inout_ OrtAllocator *allocator, _In_opt_ OrtSharedPrePackedWeightCache *prepacked_weight_cache,
    ...
    ORT_API2_STATUS(SetSharedPrePackedWeight, _In_ OrtKernelImpl *this_ptr,
                    ...
                    _In_reads_(num_buffers) const size_t *buffer_data_sizes, _In_ size_t num_buffers,
...
typedef struct OrtResourceCount {
    ...
    static OrtResourceCount None() noexcept { return OrtResourceCount{}; }
    ...
    uint64_t AsTotalBytes() const noexcept { return value.total_bytes; }
#endif
} OrtResourceCount;
...
struct OrtEpApi {
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(CreateEpDevice, _In_ OrtEpFactory *ep_factory, _In_ const OrtHardwareDevice *hardware_device,
                    _In_opt_ const OrtKeyValuePairs *ep_metadata, _In_opt_ const OrtKeyValuePairs *ep_options,
    ...
    ORT_API2_STATUS(CreateHardwareDevice, _In_ OrtHardwareDeviceType type, _In_ uint32_t vendor_id,
                    _In_ uint32_t device_id, _In_ const char *vendor_name, _In_opt_ const OrtKeyValuePairs *metadata,
     ...
     * \param[in] kernel_def The kernel definition, which includes operator type, version, EP name, type constraints,
     * etc.
     * \param[in] kernel_create_func Function that creates an instance of the operator kernel as a OrtKernelImpl
     * instance.
    ...
    ORT_API2_STATUS(KernelDefBuilder_AddInputOutputAliases, _In_ OrtKernelDefBuilder *kernel_def_builder,
                    ...
                    _In_reads_(num_io_indices) int const *output_indices, _In_ size_t num_io_indices);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(KernelDef_GetSinceVersion, _In_ const OrtKernelDef *kernel_def, _Out_ int *start_version,
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(KernelDef_GetInputMemType, _In_ const OrtKernelDef *kernel_def, _In_ size_t input_index,
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(KernelDef_GetOutputMemType, _In_ const OrtKernelDef *kernel_def, _In_ size_t output_index,
                    _Out_ OrtMemType *mem_type);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(GetTensorDataType, _In_ ONNXTensorElementDataType elem_type, _Outptr_ const OrtDataType **out);
     ...
     * \param[in] buffer_data_ptrs An array of buffer data pointers that collectively hold the pre-packed data for a
     *                             single shared weight. Note that sometimes a single weight may have multiple
     * pre-packed buffers and it is up to the kernel implementation to determine how to split the data into multiple
     * buffers (if desired).
     * \param[in] buffer_data_sizes An array of buffer byte sizes, one per element in `buffer_data_ptrs`.
     * \param[in] num_buffers The number of buffers used to store the data for the shared pre-packed weight.
     *                        Specifies the number of elements in the `buffer_data_ptrs` and `buffer_data_sizes` arrays.
    ...
    ORT_API2_STATUS(DeviceEpIncompatibilityDetails_SetDetails, _Inout_ OrtDeviceEpIncompatibilityDetails *details,
                    _In_ uint32_t reasons_bitmask, _In_ int32_t error_code, _In_opt_z_ const char *notes);
    ...
    /** \brief Gets a new OrtKeyValuePairs instance containing a copy of all configuration entries set on the
     * environment.
     *
     ...
     *
     * \param[out] config_entries Output parameter set to the OrtKeyValuePairs instance containing all configuration
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(GetOpSchema, _In_ const char *name, _In_ int max_inclusive_version, _In_ const char *domain,
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(CreateProfilingEvent, _In_ OrtProfilingEventCategory category, _In_ int32_t process_id,
                    _In_ int32_t thread_id, _In_ const char *event_name, _In_ int64_t timestamp_us,
                    _In_ int64_t duration_us, _In_reads_(num_args) const char *const *arg_keys,
                    _In_reads_(num_args) const char *const *arg_values, _In_ size_t num_args,
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(ProfilingEvent_GetName, _In_ const OrtProfilingEvent *event, _Outptr_ const char **out);
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(ProfilingEvent_GetTimestampUs, _In_ const OrtProfilingEvent *event, _Out_ int64_t *out);
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(ProfilingEvent_GetDurationUs, _In_ const OrtProfilingEvent *event, _Out_ int64_t *out);
    ...
    ORT_API2_STATUS(ProfilingEventsContainer_AddEvents, _In_ OrtProfilingEventsContainer *events_container,
                    _In_reads_(num_events) const OrtProfilingEvent *const *events, _In_ size_t num_events);
...
struct OrtEp {
     ...
     *
     * \param[in] this_ptr The OrtEp instance.
     * \param[in] graph The OrtGraph instance for which to populate node support. The OrtGraph could be a nested
     * subgraph contained by a node (e.g., an If or Loop node). ONNX Runtime calls this function separately for each
     * nested subgraph.
     ...
     *
     * If the session is configured to generate a pre-compiled model, the execution provider must return EPContext
     * nodes, as OrtNode instances, that ONNX Runtime uses to create a pre-compiled model, known as an "EPContext
     * model". An EPContext model contains EPContext nodes. Each EPContext node encapsulates the pre-compiled binary
     * data for a OrtGraph compiled for a specific execution provider. For more details about the EPContext design,
     * refer to:
     *  \htmlonly
     ...
     *                              OrtApi::ReleaseNode. Should be ignored if the session is not configured to generate
     * an EPContext model.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Compile, _In_ OrtEp *this_ptr, _In_ const OrtGraph **graphs, _In_ const OrtNode **fused_nodes,
                    _In_ size_t count, _Out_writes_all_(count) OrtNodeComputeInfo **node_compute_infos,
     ...
     * \since Version 1.23.
     */
    ORT_API_T(void, ReleaseNodeComputeInfos, _In_ OrtEp *this_ptr, OrtNodeComputeInfo **node_compute_infos,
     ...
     *         If the EP prefers a non-default data layout (see `GetPreferredDataLayout()`), this function will be
     * called during layout transformation with `target_data_layout` set to the EP's preferred data layout.
     *
     * \note Implementation of this function is optional.
    ...
    ORT_API2_STATUS(ShouldConvertDataLayoutForOp, _In_ OrtEp *this_ptr, _In_z_ const char *domain,
                    _In_z_ const char *op_type, _In_ OrtEpDataLayout target_data_layout, _Outptr_ int *should_convert);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(SetDynamicOptions, _In_ OrtEp *this_ptr, _In_reads_(num_options) const char *const *option_keys,
                    _In_reads_(num_options) const char *const *option_values, _In_ size_t num_options);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(CreateSyncStreamForDevice, _In_ OrtEp *this_ptr, _In_ const OrtMemoryDevice *memory_device,
     ...
     * \since Version 1.23.
     */
    ORT_API_T(const char *, GetCompiledModelCompatibilityInfo, _In_ OrtEp *this_ptr, _In_ const OrtGraph *graph);
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(CreateProfiler, _In_ OrtEp *this_ptr, _Outptr_result_maybenull_ OrtEpProfilerImpl **profiler);
     ...
     * During OrtSession initialization, ORT calls OrtEp::IsGraphCaptureEnabled() on each EP in the order specified
     * during provider registration with the session. If an EP returns true, ORT validates that the graph is suitable
     * for graph capture, and if so, caches the EP for graph capture during the next run. The graph validation ensures
     ...
     * \since Version 1.26.
     */
    ORT_API_T(OrtGraphCaptureNodeAssignmentPolicy, GetGraphCaptureNodeAssignmentPolicy, _In_ const OrtEp *this_ptr);
     ...
     *
     * \param[in] this_ptr The OrtEp instance.
     * \param[out] device Set to the EP's default OrtMemoryDevice, or NULL to use the default behavior (described
     * above).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
...
typedef OrtStatus *(*CreateEpApiFactoriesFn)(_In_ const char *registered_name, _In_ const OrtApiBase *ort_api_base,
                                             _In_ const OrtLogger *default_logger, _Inout_ OrtEpFactory **factories,
...
struct OrtEpFactory {
     ...
     *                        i.e. usage is `ep_devices[0] = <ptr to OrtEpDevice created with
    ...
    ORT_API2_STATUS(GetSupportedDevices, _In_ OrtEpFactory *this_ptr,
                    _In_reads_(num_devices) const OrtHardwareDevice *const *devices, _In_ size_t num_devices,
                    _Inout_ OrtEpDevice **ep_devices, _In_ size_t max_ep_devices, _Out_ size_t *num_ep_devices);
    ...
    ORT_API2_STATUS(CreateEp, _In_ OrtEpFactory *this_ptr,
                    ...
                    _In_reads_(num_devices) const OrtKeyValuePairs *const *ep_metadata_pairs, _In_ size_t num_devices,
                    _In_ const OrtSessionOptions *session_options, _In_ const OrtLogger *logger, _Outptr_ OrtEp **ep);
    ...
    /** \brief Validate the compatibility of a compiled model with the execution provider factory for one or more
     * devices.
     *
     * Given a compatibility info string produced during model compilation, the EP factory should determine whether the
     * compiled model is compatible with the EP factory when targeting the provided hardware devices. All devices
     * provided must belong to the same execution provider instance that this factory creates.
     *
     * The EP factory implementation should consider the set of devices (e.g., multi-adapter or multi-GPU scenarios)
     * when evaluating compatibility and set `model_compatibility` accordingly.
     *
     * \param[in] this_ptr The OrtEpFactory instance.
     ...
     * \param[out] model_compatibility OrtCompiledModelCompatibility value describing the compatibility of the model
     * with the EP.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
    ...
    ORT_API2_STATUS(ValidateCompiledModelCompatibilityInfo, _In_ OrtEpFactory *this_ptr,
                    _In_reads_(num_devices) const OrtHardwareDevice *const *devices, _In_ size_t num_devices,
                    _In_ const char *compatibility_info, _Out_ OrtCompiledModelCompatibility *model_compatibility);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(CreateAllocator, _In_ OrtEpFactory *this_ptr, _In_ const OrtMemoryInfo *memory_info,
     ...
     *
     * \param[in] this_ptr The OrtEpFactory instance.
     * \return True if the factory creates execution providers that are stream aware and it implements
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(CreateSyncStreamForDevice, _In_ OrtEpFactory *this_ptr, _In_ const OrtMemoryDevice *memory_device,
                    _In_opt_ const OrtKeyValuePairs *stream_options, _Outptr_ OrtSyncStreamImpl **stream);
    ...
    ORT_API2_STATUS(GetHardwareDeviceIncompatibilityDetails, _In_ OrtEpFactory *this_ptr,
                    _In_ const OrtHardwareDevice *hw, _Inout_ OrtDeviceEpIncompatibilityDetails *details);
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(InitGraphicsInterop, _In_ OrtEpFactory *this_ptr, _In_ const OrtEpDevice *ep_device,
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(DeinitGraphicsInterop, _In_ OrtEpFactory *this_ptr, _In_ const OrtEpDevice *ep_device);
     ...
     * (e.g., "speed optimized" vs "memory optimized") that produce different compatibility strings. The EP can
     * implement this function to evaluate the candidate metadata and select the best compatible variant based on its
     * own criteria, the target device, and the session options.
     *
     ...
     * \since Version 1.28.
     */
    ORT_API2_STATUS(SelectBestModelCandidate, _In_ OrtEpFactory *this_ptr, _In_ const OrtHardwareDevice *device,
                    _In_reads_(num_candidates) const OrtKeyValuePairs *const *candidates, _In_ size_t num_candidates,
                    _In_opt_ const OrtSessionOptions *session_options, _Out_ size_t *selected_index);
};

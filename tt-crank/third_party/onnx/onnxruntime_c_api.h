 ...
 *
 * It is a set of header only wrapper classes around the C API. The goal is to turn the C style return value error codes
 * into C++ exceptions, and to automate memory management through standard C++ RAII principles.
 *
 * \addtogroup Global
 * ONNX Runtime C API
 * @{
...
#define ORT_API_T(RETURN_TYPE, NAME, ...) RETURN_TYPE(ORT_API_CALL *NAME)(__VA_ARGS__) NO_EXCEPTION
 ...
 *
 * Structure of function pointers that defines a memory allocator. This can be created and filled in by the user for
 * custom allocators.
 *
 * When an allocator is passed to any function, be sure that the allocator object is not destroyed until the last
 * allocated object using it is freed.
 */
...
typedef void(ORT_API_CALL *OrtLoggingFunction)(void *param, OrtLoggingLevel severity, const char *category,
 ...
 */
// Whenever this struct is updated, please also update the MakeKey function in onnxruntime / core / framework /
 ...
 */
typedef OrtStatus *(ORT_API_CALL *EpSelectionDelegate)(_In_ const OrtEpDevice **ep_devices, _In_ size_t num_devices,
                                                       _In_ const OrtKeyValuePairs *model_metadata,
                                                       _In_opt_ const OrtKeyValuePairs *runtime_metadata,
                                                       _Inout_ const OrtEpDevice **selected, _In_ size_t max_selected,
                                                       _Out_ size_t *num_selected, _In_ void *state);
 ...
 *                    ORT will release the OrtStatus* if not null.
 */
typedef OrtStatus *(ORT_API_CALL *OrtWriteBufferFunc)(_In_ void *state, _In_ const void *buffer,
...
typedef OrtStatus *(ORT_API_CALL *OrtGetInitializerLocationFunc)(
    _In_ void *state, _In_ const char *initializer_name, _In_ const OrtValue *initializer_value,
...
typedef struct OrtCUDAProviderOptions {
    ...
    OrtCUDAProviderOptions()
        : device_id{}, cudnn_conv_algo_search{OrtCudnnConvAlgoSearchExhaustive}, gpu_mem_limit{SIZE_MAX},
          arena_extend_strategy{}, do_copy_in_default_stream{1}, has_user_compute_stream{}, user_compute_stream{},
          default_memory_arena_cfg{}, tunable_op_enable{false}, tunable_op_tuning_enable{false},
...
typedef struct OrtROCMProviderOptions {
    ...
    OrtROCMProviderOptions()
        : device_id{}, miopen_conv_exhaustive_search{0}, gpu_mem_limit{SIZE_MAX}, arena_extend_strategy{},
          do_copy_in_default_stream{1}, has_user_compute_stream{}, user_compute_stream{}, default_memory_arena_cfg{},
          enable_hip_graph{false}, tunable_op_enable{false}, tunable_op_tuning_enable{false},
          tunable_op_max_tuning_duration_ms{} {}
#endif
...
typedef struct OrtTensorRTProviderOptions {
    ...
    int trt_force_sequential_engine_build; // force building TensorRT engine sequentially. Default 0 = false, nonzero =
                                           // true
                                           // This is the legacy struct and don't add new fields here.
                                           // For new field that can be represented by string, please add it in
    // include/onnxruntime/core/providers/tensorrt/tensorrt_provider_options.h For non-string field, need to create a
    // new separate api to handle it.
...
typedef struct OrtOpenVINOProviderOptions {
    ...
    OrtOpenVINOProviderOptions()
        : device_type{}, enable_npu_fast_compile{}, device_id{}, num_of_threads{}, cache_dir{}, context{},
          enable_opencl_throttling{}, enable_dynamic_shapes{} {}
#endif
...
struct OrtApiBase {
     ...
     * \return The ::OrtApi for the version requested, nullptr will be returned if this version is unsupported, for
 ...
 * Onnxruntime will throw exception on return value of nullptr or 0, indicating that the function failed to create a
 * thread
 */
typedef OrtCustomThreadHandle (*OrtCustomCreateThreadFn)(void *ort_custom_thread_creation_options,
                                                         OrtThreadWorkerFn ort_thread_worker_fn,
                                                         void *ort_worker_fn_param);
...
typedef enum OrtExternalMemoryHandleType {
    ORT_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_RESOURCE =
        0, /**< Shared HANDLE from ID3D12Device::CreateSharedHandle(resource) */
    ...
    ORT_EXTERNAL_MEMORY_HANDLE_TYPE_VK_MEMORY_WIN32 =
        2, /**< Shared HANDLE from vkGetMemoryWin32HandleKHR, non-dedicated allocation */
...
typedef enum OrtExternalSemaphoreType {
    ...
    ORT_EXTERNAL_SEMAPHORE_VK_TIMELINE_SEMAPHORE_WIN32 = 1, /**< Shared HANDLE from vkGetSemaphoreWin32HandleKHR of a
                                                               VkSemaphore created as VK_SEMAPHORE_TYPE_TIMELINE */
    ORT_EXTERNAL_SEMAPHORE_VK_TIMELINE_SEMAPHORE_OPAQUE_FD =
        2, /**< File descriptor from vkGetSemaphoreFdKHR of a VkSemaphore created as VK_SEMAPHORE_TYPE_TIMELINE */
...
typedef struct OrtEnvCreationOptions {
     ...
     *       Use OrtApi::SetSessionLogSeverityLevel to set a logging severity level for the creation of specific
     * session. Use OrtApi::RunOptionsSetRunLogSeverityLevel to set a logging severity level for a specific session run.
     *
     * \since Version 1.24.
     */
     ...
     *       using keys with the prefix 'ep_factory.\\<ep_name\\>.'. Ex: the key 'ep_factory.my_ep.some_ep_key'
     * represents a key named 'some_ep_key' that is meant to be consumed by an execution provider named 'my_ep'. Refer
     * to the specific execution provider's documentation for valid keys and values.
     *
     * \note An application may separately set session-level configuration options for execution providers via other
     * APIs such as SessionOptionsAppendExecutionProvider_V2, which store configuration entries within
     * OrtSessionOptions. If an environment-level configuration conflicts with a session-level configuration, then
...
struct OrtApi {
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateSessionFromArray, _In_ const OrtEnv *env, _In_ const void *model_data,
     ...
     * \param[out] out_options Returned newly created ::OrtSessionOptions. Must be freed with
     ...
     * \note Memory pattern optimization is only available when Sequential Execution mode is enabled (see
     ...
     *
     * Please see https://onnxruntime.ai/docs/performance/model-optimizations/graph-optimizations.html for an in-depth
     * explanation
     ...
     * The handle to the loaded library is returned in library_handle. It can be freed by the caller after all sessions
     *using the passed in session options are destroyed, or if an error occurs and it is non null.
     *
     * \param[in] options
     * \param[in] library_path
     * \param[out] library_handle OS specific handle to the loaded library (Use FreeLibrary on Windows, dlclose on
     *Linux, etc.. to unload)
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetInputTypeInfo, _In_ const OrtSession *session, size_t index,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOutputTypeInfo, _In_ const OrtSession *session, size_t index,
                    _Outptr_ OrtTypeInfo **type_info);

    /** \brief Get overridable initializer type information
     *
     * \param[in] session
     * \param[in] index Must be between 0 (inclusive) and what OrtApi::SessionGetOverridableInitializerCount returns
     * (exclusive)
     * \param[out] type_info Must be freed with OrtApi::ReleaseTypeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOverridableInitializerTypeInfo, _In_ const OrtSession *session, size_t index,
                    _Outptr_ OrtTypeInfo **type_info);
     ...
     * \param[in] index Must be between 0 (inclusive) and what OrtApi::SessionGetInputCount returns (exclusive)
     * \param[in] allocator
     * \param[out] value Set to a null terminated UTF-8 encoded string allocated using `allocator`. Must be freed using
     * `allocator`.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetInputName, _In_ const OrtSession *session, size_t index, _Inout_ OrtAllocator *allocator,
                    _Outptr_ char **value);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOutputName, _In_ const OrtSession *session, size_t index, _Inout_ OrtAllocator *allocator,
                    _Outptr_ char **value);
     ...
     *
     * If a currently executing session needs to be force terminated, this can be called from another thread to force it
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateTensorAsOrtValue, _Inout_ OrtAllocator *allocator, _In_ const int64_t *shape,
                    size_t shape_len, ONNXTensorElementDataType type, _Outptr_ OrtValue **out);
     ...
     *
     * \param[out] out Returns newly created ::OrtTensorTypeAndShapeInfo. Must be freed with
     * OrtApi::ReleaseTensorTypeAndShapeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * \param[in] info
     * \param[out] dim_values Array with `dim_values_length` elements. On return, filled with the dimensions stored in
     * the ::OrtTensorTypeAndShapeInfo
     ...
     *
     * \param[in] info
     * \param[in] dim_params Array with `dim_params_length` elements. On return filled with pointers to null terminated
     * strings of the dimension names
     ...
     * \param[out] out Newly created ::OrtTensorTypeAndShapeInfo. Must be freed with
     * OrtApi::ReleaseTensorTypeAndShapeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * Special case version of OrtApi::CreateMemoryInfo for CPU based memory. Same as using OrtApi::CreateMemoryInfo
     * with name = "Cpu" and id = 0.
     *
     * \param[in] type
     * \param[in] mem_type
     * \param[out] out
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CompareMemoryInfo, _In_ const OrtMemoryInfo *info1, _In_ const OrtMemoryInfo *info2,
                    _Out_ int *out);

    /** \brief Get name from ::OrtMemoryInfo
     *
     * \param[in] ptr
     * \param[out] out Writes null terminated string to this pointer. Do NOT free the returned pointer. It is valid for
     * the lifetime of the ::OrtMemoryInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * Override symbolic dimensions (by specific denotation strings) with actual values if known at session
     * initialization time to enable optimizations that can take advantage of fixed values (such as memory planning,
     * etc)
     *
     * \param[in] options
     ...
     * \param[out] data_container_size Size in bytes of the buffer pointed to by data_container. Must match the size of
     * the internal buffer.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * \param[in] type_info
     * \param[out] denotation Pointer to the null terminated denotation string is written to this pointer. This pointer
     * is valid until the object is destroyed or the name is changed, do not free.
     ...
     * \param[in] session
     * \param[in] allocator
     * \param[out] out Null terminated string of the filename, allocated using `allocator`. Must be freed using
     * `allocator`
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionEndProfiling, _In_ OrtSession *session, _Inout_ OrtAllocator *allocator,
                    _Outptr_ char **out);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(ModelMetadataGetDomain, _In_ const OrtModelMetadata *model_metadata,
                    _Inout_ OrtAllocator *allocator, _Outptr_ char **value);
    ...
    ORT_API2_STATUS(ModelMetadataGetCustomMetadataMapKeys, _In_ const OrtModelMetadata *model_metadata,
                    _Inout_ OrtAllocator *allocator, _Outptr_result_buffer_maybenull_(*num_keys) char ***keys,
     ...
     *
     */
    ORT_API2_STATUS(AddFreeDimensionOverrideByName, _Inout_ OrtSessionOptions *options, _In_ const char *dim_name,
                    _In_ int64_t dim_value);
     ...
     *
     * \note The providers in the list are not guaranteed to be usable. They may fail to load due to missing system
     * dependencies. For example, if the CUDA/cuDNN libraries are not installed, the CUDA provider will report an error
     * when it is added to the session options.
     *
     * \param[out] out_ptr Set to a pointer to an array of null terminated strings of the available providers. The
     * entries and the array itself must be freed using OrtApi::ReleaseAvailableProviders
     ...
     *
     * \param[in] value A string tensor
     * \param[in] s_len Number of bytes in the `s` buffer. Must match the value returned by
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetStringTensorElement, _In_ const OrtValue *value, size_t s_len, size_t index,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(AddSessionConfigEntry, _Inout_ OrtSessionOptions *options, _In_z_ const char *config_key,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(RunWithBinding, _Inout_ OrtSession *session, _In_ const OrtRunOptions *run_options,
     ...
     * You can either create an instance of ::OrtMemoryInfo with a device id or obtain one from the allocator that you
     * have created/are using This is useful when one or more outputs have dynamic shapes and, it is hard to
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(BindOutputToDevice, _Inout_ OrtIoBinding *binding_ptr, _In_ const char *name,
     ...
     * with bound outputs because the returned names are in order in which output ::OrtValue are returned. This is
     * useful if the order of outputs and their names is not known.
     *
     * \param[in] binding_ptr
     * \param[in] allocator Allocator used to allocate continuous buffers for output strings and lengths.
     * \param[out] buffer Returns an array of non-null terminated UTF-8 strings. The number of strings stored is
     * returned in the count parameter. This buffer is allocated using `allocator` and must be freed using it.
     ...
     *
     * Returns an array of pointers to individually allocated ::OrtValue%s that contain results of a model execution
     * with OrtApi::RunWithBinding The array contains the same number of ::OrtValue%s and they are in the same order as
     ...
     * The array is allocated using the specified instance of the allocator and must be freed using the same allocator
     * after all the ::OrtValue%s contained therein are individually released.
     *
     * \param[in] binding_ptr
     * \param[in] allocator Allocator used to allocate output array
     * \param[out] output Set to the allocated array of allocated ::OrtValue outputs. Set to nullptr if there are 0
     * outputs.
     ...
     *
     * For example, given a tensor with shape of [3,224,224], a pointer to the element at location [2,150,128] can be
     * retrieved
     *
     ...
     *
     * \param[in] value
     * \param[in] location_values Pointer to an array of index values that specify an element's location relative to its
     * shape
     * \param[in] location_values_count Number of elements in location_values. Must match the number of elements in the
     * tensor's shape.
     * \param[out] out Set to a pointer to the element specified
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(TensorAt, _Inout_ OrtValue *value, const int64_t *location_values, size_t location_values_count,
                    _Outptr_ void **out);
     ...
     *
     * This will configure the global thread pool options to be used in the call to
     * OrtApi::CreateEnvWithGlobalThreadPools. Allow spinning of thread pools when their queues are empty. This will set
     * the value for both inter_op and intra_op threadpools.
     *
     * \param[in] tp_options
     ...
     *   managed by the user (created using the OrtApi::CreateTensorWithDataAsOrtValue) and it must outlive the session
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateEnvWithCustomLoggerAndGlobalThreadPools, OrtLoggingFunction logging_function,
                    _In_opt_ void *logger_param, OrtLoggingLevel log_severity_level, _In_ const char *logid,
                    _In_ const struct OrtThreadingOptions *tp_options, _Outptr_ OrtEnv **out);
     ...
     *
     * If CUDA is not available (due to a non CUDA enabled build, or if CUDA is not installed on the system), this
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_CUDA, _In_ OrtSessionOptions *options,
                    _In_ const OrtCUDAProviderOptions *cuda_options);

    /** \brief Append ROCM execution provider to the session options
     *
     * If ROCM is not available (due to a non ROCM enabled build, or if ROCM is not installed on the system), this
     * function will return failure.
     *
     * \param[in] options
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_ROCM, _In_ OrtSessionOptions *options,
                    _In_ const OrtROCMProviderOptions *rocm_options);

    /** \brief Append OpenVINO execution provider to the session options
     *
     * If OpenVINO is not available (due to a non OpenVINO enabled build, or if OpenVINO is not installed on the
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_OpenVINO, _In_ OrtSessionOptions *options,
     ...
     * \note This option is not needed if the models used have no denormals. Having no denormals is recommended as this
     * option may hurt model accuracy.
     *
     * \param[in] tp_options
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     ...
     *
     * This will create the configuration of an arena that can eventually be used to define an arena based allocator's
     * behavior
     *
     * \param[in] max_mem Use 0 to allow ORT to choose the default
     * \param[in] arena_extend_strategy Use -1 to allow ORT to choose the default, 0 = kNextPowerOfTwo, 1 =
     * kSameAsRequested
     ...
     * \param[out] value Set to a null terminated string allocated using `allocator`.  The caller is responsible for
     * freeing it using `allocator`
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * If TensorRT is not available (due to a non TensorRT enabled build, or if TensorRT is not installed on the
     * system), this function will return failure.
     *
     * \param[in] options
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_TensorRT, _In_ OrtSessionOptions *options,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(AddRunConfigEntry, _Inout_ OrtRunOptions *options, _In_z_ const char *config_key,
                    _In_z_ const char *config_value);
     ...
     * creation time.
     *
     *  \param[out] out Newly created ::OrtPrepackedWeightsContainer. Must be freed with
     * OrtApi::ReleasePrepackedWeightsContainer
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateSessionWithPrepackedWeightsContainer, _In_ const OrtEnv *env,
                    _In_ const ORTCHAR_T *model_path, _In_ const OrtSessionOptions *options,
                    _Inout_ OrtPrepackedWeightsContainer *prepacked_weights_container, _Outptr_ OrtSession **out);
    ...
    ORT_API2_STATUS(CreateSessionFromArrayWithPrepackedWeightsContainer, _In_ const OrtEnv *env,
                    _In_ const void *model_data, size_t model_data_length, _In_ const OrtSessionOptions *options,
                    _Inout_ OrtPrepackedWeightsContainer *prepacked_weights_container, _Outptr_ OrtSession **out);
     ...
     *
     * For OrtApi::SessionOptionsAppendExecutionProvider_TensorRT, the user needs to instantiate
     * ::OrtTensorRTProviderOptions as well as allocate/release buffers for some members of
     * ::OrtTensorRTProviderOptions. Here, OrtApi::CreateTensorRTProviderOptions and
     * Ortapi::ReleaseTensorRTProviderOptions will do the memory management for you.
     *
     * \param[in] options
     * \param[in] tensorrt_options
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_TensorRT_V2, _In_ OrtSessionOptions *options,
     ...
     *
     * \param[out] out Newly created ::OrtTensorRTProviderOptionsV2. Must be released with
     ...
     * ::OrtTensorRTProviderOptionsV2 and value should be its related range. Recreates the options and only sets the
     * supplied values.
     *
     ...
     * \param allocator - a ptr to an instance of OrtAllocator obtained with OrtApi::CreateAllocator or
     * OrtApi::GetAllocatorWithDefaultOptions the specified allocator will be used to allocate continuous buffers for
     * output strings and lengths.
     * \param ptr - is a UTF-8 null terminated string allocated using 'allocator'. The caller is responsible for using
     * the same allocator to free it.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetTensorRTProviderOptionsAsString, _In_ const OrtTensorRTProviderOptionsV2 *tensorrt_options,
                    _Inout_ OrtAllocator *allocator, _Outptr_ char **ptr);

    /** \brief Release an ::OrtTensorRTProviderOptionsV2
     *
     * \note This is an exception in the naming convention of other Release* functions, as the name of the method does
     * not have the V2 suffix, but the type does
     */
     ...
     *  at the creation time has memory info that is not the same as mem_info argument to this function a X-device copy
     * will be performed. String data is assumed to be on CPU and will only be copied into a CPU allocated buffer.
    ...
    /**
     * This fills populates an empty tensor that was created using OrtApi::CreateSparseTensorAsOrtValue.
     * This will allocate required memory and copy the supplied NNZ values and BlockSparse indices into that memory
     * allocation. Memory allocation is performed using the allocator that was specified with
     ...
     * \param[in] indices_data pointer to a location of indices data. Shape will determine the length of the indices
     * data.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(FillSparseTensorBlockSparse, _Inout_ OrtValue *ort_value, _In_ const OrtMemoryInfo *data_mem_info,
                    _In_ const int64_t *values_shape, size_t values_shape_len, _In_ const void *values,
                    _In_ const int64_t *indices_shape_data, size_t indices_shape_len, _In_ const int32_t *indices_data);
     ...
     * \param[in,out] p_data pointer to a user allocated buffer with values. To create a full sparse tensor with no
     * non-zero values, pass nullptr
     * \param[in] dense_shape shape of the original dense tensor
     * \param[in] dense_shape_len number of shape dimensions being passed
    ...
    ORT_API2_STATUS(CreateSparseTensorWithValuesAsOrtValue, _In_ const OrtMemoryInfo *info, _Inout_ void *p_data,
                    _In_ const int64_t *dense_shape, size_t dense_shape_len, _In_ const int64_t *values_shape,
                    size_t values_shape_len, ONNXTensorElementDataType type, _Outptr_ OrtValue **out);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(UseBlockSparseIndices, _Inout_ OrtValue *ort_value, const int64_t *indices_shape,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetSparseTensorValuesTypeAndShape, _In_ const OrtValue *ort_value,
     ...
     * \param[out] out an instance of ::OrtTensorTypeAndShapeInfo. Must be freed by
     * OrtApi::ReleaseTensorTypeAndShapeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetSparseTensorIndicesTypeShape, _In_ const OrtValue *ort_value,
                    enum OrtSparseIndicesFormat indices_format, _Outptr_ OrtTensorTypeAndShapeInfo **out);

    /** \brief Returns indices data for the type of the indices specified by indices_format
     *
     * \param[in] ort_value ::OrtValue containing sparse tensor.
     * \param[in] indices_format One of the indices formats. It is an error to request a format that the sparse tensor
     * does not contain.
     * \param[out] num_indices Pointer to where the number of indices entries is returned
     * \param[out] indices Returned pointer to the indices data. Do not free the returned pointer as it refers to
     * internal data owned by the ::OrtValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetSparseTensorIndices, _In_ const OrtValue *ort_value, enum OrtSparseIndicesFormat indices_format,
     ...
     *             If retrieving the GPU compute stream is not relevant (GPU not enabled in the build, kernel
     * partitioned to some other EP), then a nullptr is returned as the output param. Do not free or mutate the returned
     * pointer as it refers to internal data owned by the underlying session. Only use it for custom kernel launching.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[out] mem_info ::OrtMemoryInfo of the tensor. Do NOT free the returned pointer. It is valid for the
     * lifetime of the ::OrtValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetExecutionProviderApi, _In_ const char *provider_name, _In_ uint32_t version,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsSetCustomCreateThreadFn, _Inout_ OrtSessionOptions *options,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsSetCustomThreadCreationOptions, _Inout_ OrtSessionOptions *options,
                    _In_ void *ort_custom_thread_creation_options);

    /** \brief Set custom thread join function
     *
     * \param[in] options Session options
     * \param[in] ort_custom_join_thread_fn Custom join thread function, must not be nullptr when
     * ort_custom_create_thread_fn is set
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionOptionsSetCustomJoinThreadFn, _Inout_ OrtSessionOptions *options,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetGlobalCustomCreateThreadFn, _Inout_ OrtThreadingOptions *tp_options,
                    _In_ OrtCustomCreateThreadFn ort_custom_create_thread_fn);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetGlobalCustomThreadCreationOptions, _Inout_ OrtThreadingOptions *tp_options,
                    _In_ void *ort_custom_thread_creation_options);

    /** \brief Set custom thread join function for global thread pools
     *
     * \param[inout] tp_options
     * \param[in] ort_custom_join_thread_fn Custom thread join function, must not be nullptr when global
     * ort_custom_create_thread_fn is set
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetGlobalCustomJoinThreadFn, _Inout_ OrtThreadingOptions *tp_options,
                    _In_ OrtCustomJoinThreadFn ort_custom_join_thread_fn);
    /// @}
     ...
     * Here, OrtApi::CreateCUDAProviderOptions and Ortapi::ReleaseCUDAProviderOptions will do the memory management for
     * you.
     *
     * \param[in] options
     * \param[in] cuda_options
     ...
     * \since Version 1.11.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_CUDA_V2, _In_ OrtSessionOptions *options,
     ...
     *
     * \param[out] out Newly created ::OrtCUDAProviderOptionsV2. Must be released with
     ...
     * to know the available keys and values. Key should be in null terminated string format of the member of
     * ::OrtCUDAProviderOptionsV2 and value should be its related range. Recreates the options and only sets the
     * supplied values.
     *
     ...
     * \param allocator - a ptr to an instance of OrtAllocator obtained with CreateAllocator() or
     * GetAllocatorWithDefaultOptions() the specified allocator will be used to allocate continuous buffers for output
     * strings and lengths.
     * \param ptr - is a UTF-8 null terminated string allocated using 'allocator'. The caller is responsible for using
     * the same allocator to free it.
     *
     ...
     * \since Version 1.11.
     */
    ORT_API2_STATUS(GetCUDAProviderOptionsAsString, _In_ const OrtCUDAProviderOptionsV2 *cuda_options,
                    _Inout_ OrtAllocator *allocator, _Outptr_ char **ptr);
     ...
     *
     * If MIGraphX is not available (due to a non MIGraphX enabled build, or if MIGraphX is not installed on the
     * system), this function will return failure.
     *
     * \param[in] options
     ...
     * \since Version 1.11.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_MIGraphX, _In_ OrtSessionOptions *options,
     ...
     * has an external data reference and has the same name, dimensions and data type as its replacement. The
     * replacement will occur before any of the optimizations take place. The data will be copied into the graph since
     * TensorProto can't refer to the user provided buffers.
     *
     ...
     * \since Version 1.12.
     */
    ORT_API2_STATUS(CreateOpAttr, _In_ const char *name, _In_ const void *data, _In_ int len, _In_ OrtOpAttrType type,
     ...
     * \since Version 1.12.
     */
    ORT_API2_STATUS(CreateOp, _In_ const OrtKernelInfo *info, _In_z_ const char *op_name, _In_z_ const char *domain,
                    int version, _In_reads_(type_constraint_count) const char **type_constraint_names,
                    _In_reads_(type_constraint_count) const ONNXTensorElementDataType *type_constraint_values,
                    int type_constraint_count, _In_reads_(attr_count) const OrtOpAttr *const *attr_values,
                    int attr_count, int input_count, int output_count, _Outptr_ OrtOp **ort_op);
     ...
     * \since Version 1.12.
     */
    ORT_API2_STATUS(InvokeOp, _In_ const OrtKernelContext *context, _In_ const OrtOp *ort_op,
                    _In_ const OrtValue *const *input_values, _In_ int input_count,
     ...
     *      -# "normal_high"
     *      -# "high"
     *   "htp_graph_finalization_optimization_mode": Set the optimization mode for graph finalization on the HTP
     *backend. Available options:
     *      -# "0": Default.
     *      -# "1": Faster preparation time, less optimal graph.
     *      -# "2": Longer preparation time, more optimal graph.
     *      -# "3": Longest preparation time, most likely even more optimal graph. See QNN SDK documentation for
     *specific details. "soc_model": The SoC model number. Refer to the QNN SDK documentation for valid values. Defaults
     *to "0" (unknown). "htp_arch": The minimum HTP architecture the driver will use to select compatible QNN operators.
     *      Available options:
     *      -# "0": Default (none).
     *      -# "68"
     *      -# "69"
     ...
     *   "json_qnn_graph_dir": Directory in which to dump QNN JSON graphs. If not specified, QNN graphs are dumped in
     *the program's current working directory. Ignored if "dump_json_qnn_graph" is not set. "op_packages": QNN UDO
     *op_package for QNN EP, allowed format:   
     ...
     *     interface is the symbol name to register the op life cycle functions, and target is the backend type. For
     *more details, refer to: https://docs.qualcomm.com/bundle/publicresource/topics/80-63442-50/op_packages.html
     *     [Advanced] "skip_qnn_version_check": Set to "1" to allow a different version of QNN to be used than what was
     *compiled into ONNX Runtime. Differences in operator support, accuracy, performance, and QNN's ABI may lead to
     *crashes, inaccurate results, and poor performance. Use with caution and test thoroughly.
     *
    ...
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider, _In_ OrtSessionOptions *options,
                    _In_ const char *provider_name, _In_reads_(num_keys) const char *const *provider_options_keys,
                    _In_reads_(num_keys) const char *const *provider_options_values, _In_ size_t num_keys);
     ...
     *
     * If CANN is not available (due to a non CANN enabled build, or if CANN is not installed on the system), this
     * function will return failure.
     *
     * \param[in] options
     ...
     * \since Version 1.13.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_CANN, _In_ OrtSessionOptions *options,
     ...
     * Semicolon isolates configurations among threads, while comma split processors where ith thread expected to attach
     * to. e.g. 1,2,3;4,5 specifies affinities for two threads, with the 1st thread attach to the 1st, 2nd, and 3rd
     * processor, and 2nd thread to the 4th and 5th. To ease the configuration, an "interval" is also allowed: e.g.
     * 1-8;8-16;17-24 orders that the 1st thread runs on first eight processors, 2nd thread runs on next eight
     * processors, and so forth. Note:
     ...
     * 2. For windows, ort will infer the group id from a logical processor id, for example, assuming there are two
     * groups with each has 64 logical processors, an id of 64 will be inferred as the last processor of the 1st group,
     * while 65 will be interpreted as the 1st processor of the second group. Hence 64-65 is an invalid configuration,
     * because a windows thread cannot be attached to processors across group boundary.
     *
     *  \since Version 1.14
     */
    ORT_API2_STATUS(SetGlobalIntraOpThreadAffinity, _Inout_ OrtThreadingOptions *tp_options,
     ...
     *                         loading utilities (e.g., LoadLibraryEx on Windows or dlopen on Linux/MacOS) for
     *information on the format of library names and search paths.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.14
     */
    ORT_API2_STATUS(RegisterCustomOpsLibrary_V2, _Inout_ OrtSessionOptions *options,
     ...
     * \param[out] out Memory location into which to write the UTF-8 null-terminated string representing the input's
     * name.
     ...
     * \since Version 1.14
     */
    ORT_API2_STATUS(HasSessionConfigEntry, _In_ const OrtSessionOptions *options, _In_z_ const char *config_key,
                    _Out_ int *out);
     ...
     * \since Version 1.14
     */
    ORT_API2_STATUS(GetSessionConfigEntry, _In_ const OrtSessionOptions *options, _In_z_ const char *config_key,
     ...
     * \since Version 1.15.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_Dnnl, _In_ OrtSessionOptions *options,
     ...
     * \since Version 1.15.
     */
    ORT_API2_STATUS(GetDnnlProviderOptionsAsString, _In_ const OrtDnnlProviderOptions *dnnl_options,
                    _Inout_ OrtAllocator *allocator, _Outptr_ char **ptr);
     ...
     * and a failure status is returned.
     *
     * Can be used in a custom operator's CreateKernel callback to get the name of the operator's node name in the
     * graph.
     *
     * \param[in] info An instance of ::OrtKernelInfo.
     ...
     * \since Version 1.15.
     */
    ORT_API2_STATUS(KernelInfoGetConstantInput_tensor, _In_ const OrtKernelInfo *info, size_t index,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetResizedStringTensorElementBuffer, _Inout_ OrtValue *value, _In_ size_t index,
                    _In_ size_t length_in_bytes, _Inout_ char **buffer);

    /** \brief Get Allocator from KernelContext for a specific memoryInfo. Please use C API ReleaseAllocator to release
     * out object
     *
     * \param[in] context OrtKernelContext instance
     ...
     * \since Version 1.15.
     */
    ORT_API2_STATUS(KernelContext_GetAllocator, _In_ const OrtKernelContext *context,
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(GetROCMProviderOptionsAsString, _In_ const OrtROCMProviderOptions *rocm_options,
                    _Inout_ OrtAllocator *allocator, _Outptr_ char **ptr);
     ...
     *  This API enhance CreateAndRegisterAllocator that it can create an allocator with specific type, not just CPU
     * allocator Enables sharing the allocator between multiple sessions that use the same env instance. Lifetime of the
     * created allocator will be valid for the duration of the environment. Returns an error if an allocator with the
     * same ::OrtMemoryInfo is already registered.
     *  \param[in] env OrtEnv instance
     ...
     *             regardless of whether the member (OrtValue*) is allocated by onnxruntime or preallocated by the
     * customer.
    ...
    ORT_API2_STATUS(RunAsync, _Inout_ OrtSession *session, _In_opt_ const OrtRunOptions *run_options,
                    ...
                    _In_reads_(output_names_len) const char *const *output_names, size_t output_names_len,
                    _Inout_updates_all_(output_names_len) OrtValue **output, _In_ RunAsyncCallbackFn run_async_callback,
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(UpdateTensorRTProviderOptionsWithValue, _Inout_ OrtTensorRTProviderOptionsV2 *tensorrt_options,
                    _In_ const char *key, _In_ void *value);

    /**
     * Get TensorRT EP provider option where its data type is pointer.
     * If the data type of the provider option can be represented by string please use
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(GetTensorRTProviderOptionsByName, _In_ const OrtTensorRTProviderOptionsV2 *tensorrt_options,
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(UpdateCUDAProviderOptionsWithValue, _Inout_ OrtCUDAProviderOptionsV2 *cuda_options,
                    _In_ const char *key, _In_ void *value);
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(GetCUDAProviderOptionsByName, _In_ const OrtCUDAProviderOptionsV2 *cuda_options,
                    _In_ const char *key, _Outptr_ void **ptr);
     ...
     *  This function allows a user to override this default session logger with a logger of their own choosing. This
     * way the user doesn't have to create a separate environment with a custom logger. This addresses the problem when
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ShapeInferContext_GetInputTypeShape, _In_ const OrtShapeInferContext *context, _In_ size_t index,
                    _Outptr_ OrtTensorTypeAndShapeInfo **info);

    /**
     * Get attribute from OrtShapeInferContext. Note that OrtShapeInferContext is a per-node context, one could only
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ShapeInferContext_GetAttribute, _In_ const OrtShapeInferContext *context,
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ShapeInferContext_SetOutputTypeShape, _In_ const OrtShapeInferContext *context, _In_ size_t index,
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(SetSymbolicDimensions, _In_ OrtTensorTypeAndShapeInfo *info, _In_ const char *dim_params[],
                    _In_ size_t dim_params_length);
     ...
     * \param[out] out Number of bytes required to save the data when the call failed, or the real number of bytes saved
     * to data on success
     *
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ReadOpAttr, _In_ const OrtOpAttr *op_attr, _In_ OrtOpAttrType type, _Inout_ void *data,
     ...
     * \param[in] num_batch Number of batches by which the "total" is to be divided in maximum. When zero, there is no
     * limit
     * \param[in] usr_data User data to be passed back to fn
     *
     * \since Version 1.17.
     */
    ORT_API2_STATUS(KernelContext_ParallelFor, _In_ const OrtKernelContext *context, _In_ void (*fn)(void *, size_t),
                    _In_ size_t total, _In_ size_t num_batch, _In_ void *usr_data);
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_OpenVINO_V2, _In_ OrtSessionOptions *options,
                    _In_reads_(num_keys) const char *const *provider_options_keys,
                    _In_reads_(num_keys) const char *const *provider_options_values, _In_ size_t num_keys);

    /** \brief Append VitisAI provider to session options
     *
     * If VitisAI is not available (due to a non VitisAI enabled build, or if VitisAI is not installed on the system),
     ...
     * \since Version 1.18.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_VitisAI, _In_ OrtSessionOptions *options,
                    _In_reads_(num_keys) const char *const *provider_options_keys,
                    _In_reads_(num_keys) const char *const *provider_options_values, _In_ size_t num_keys);
     ...
     * \since Version 1.18.
     */
    ORT_API2_STATUS(KernelContext_GetScratchBuffer, _In_ const OrtKernelContext *context,
                    _In_ const OrtMemoryInfo *mem_info, _In_ size_t count_or_bytes, _Outptr_ void **out);

    /** \brief Get allocator from KernelInfo for a specific memory type. Please use C API ReleaseAllocator to release
     * out object
     *
     * \param[in] info OrtKernelInfo instance
     ...
     * \since Version 1.18.
     */
    ORT_API2_STATUS(KernelInfoGetAllocator, _In_ const OrtKernelInfo *info, _In_ OrtMemType mem_type,
                    _Outptr_ OrtAllocator **out);
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(CreateTensorWithDataAndDeleterAsOrtValue, _In_ OrtAllocator *deleter, _In_ void *p_data,
                    size_t p_data_len, _In_ const int64_t *shape, size_t shape_len, ONNXTensorElementDataType type,
                    _Outptr_ OrtValue **out);
     ...
     *            The APIs above would not return any valid Session instance. This is the best case effort and the
     * result is not guaranteed. The session may have already been created and initialized before the cancellation
     * request was issued.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     */
    ORT_API2_STATUS(SessionOptionsSetLoadCancellationFlag, _Inout_ OrtSessionOptions *options, _In_ bool cancel);
     ...
     * \since Version 1.22.
     */
    void(ORT_API_CALL *GetKeyValuePairs)(_In_ const OrtKeyValuePairs *kvps, _Outptr_ const char *const **keys,
                                         _Outptr_ const char *const **values, _Out_ size_t *num_entries);
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(GetEpDevices, _In_ const OrtEnv *env, _Outptr_ const OrtEpDevice *const **ep_devices,
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_V2, _In_ OrtSessionOptions *session_options, _In_ OrtEnv *env,
                    ...
                    _In_reads_(num_op_options) const char *const *ep_option_vals, size_t num_ep_options);
    ...
    ORT_API2_STATUS(SessionOptionsSetEpSelectionPolicyDelegate, _In_ OrtSessionOptions *session_options,
                    _In_ EpSelectionDelegate delegate, _In_opt_ void *delegate_state);
    ...
    ORT_API2_STATUS(CreateMemoryInfo_V2, _In_ const char *name, _In_ enum OrtMemoryInfoDeviceType device_type,
                    ...
                    _In_ size_t alignment, enum OrtAllocatorType allocator_type, _Outptr_ OrtMemoryInfo **out);
    ...
    ORT_API2_STATUS(ValueInfo_GetValueConsumers, _In_ const OrtValueInfo *value_info,
                    ...
                    _Out_writes_all_(num_consumers) int64_t *input_indices, _In_ size_t num_consumers);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(ValueInfo_IsFromOuterScope, _In_ const OrtValueInfo *value_info, _Out_ bool *is_from_outer_scope);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Graph_GetInputs, _In_ const OrtGraph *graph, _Out_writes_(num_inputs) const OrtValueInfo **inputs,
    ...
    ORT_API2_STATUS(Graph_GetInitializers, _In_ const OrtGraph *graph,
                    _Out_writes_(num_initializers) const OrtValueInfo **initializers, _In_ size_t num_initializers);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Graph_GetNodes, _In_ const OrtGraph *graph, _Out_writes_(num_nodes) const OrtNode **nodes,
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Node_GetInputs, _In_ const OrtNode *node, _Out_writes_(num_inputs) const OrtValueInfo **inputs,
                    _In_ size_t num_inputs);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Node_GetOutputs, _In_ const OrtNode *node, _Out_writes_(num_outputs) const OrtValueInfo **outputs,
                    _In_ size_t num_outputs);

    /** \brief Returns the number of node implicit inputs.
     *
     * Certain operator types (e.g., If and Loop) contain nested subgraphs. The internal nodes within the nested
     * subgraphs may use values from the outer scope. Those "outer scope" values are considered implicit inputs to the
     * node that contains the subgraphs (e.g., the If or Loop node).
     *
     * \param[in] node The OrtNode instance.
     ...
     *
     * \param[in] node The OrtNode instance.
     * \param[out] attributes Pre-allocated array of `num_attributes` elements that is filled with the node's
     * attributes.
     ...
     *                       returned. For an invalid attribute name, `attribute` is set to NULL and an error status
     * with code ORT_NOT_FOUND is returned.
     *
     * \note ONNX Runtime automatically sets optional (unset) attributes to their default values if the default value
     * is a constant expression that does not depend on other tensor/model characteristics. Conv's 'kernel_shape'
     * attribute is an example of an optional attribute that does not have a constant default value. This function
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Node_GetSubgraphs, _In_ const OrtNode *node, _Out_writes_(num_subgraphs) const OrtGraph **subgraphs,
                    _In_ size_t num_subgraphs, _Out_writes_opt_(num_subgraphs) const char **attribute_names);
     ...
     * \since Version 1.23
     */
    ORT_API_T(const char *, GetRunConfigEntry, _In_ const OrtRunOptions *options, _In_z_ const char *config_key);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(GetSessionOptionsConfigEntries, _In_ const OrtSessionOptions *options,
    ...
    ORT_API2_STATUS(SessionGetMemoryInfoForInputs, _In_ const OrtSession *session,
                    _Out_writes_(num_inputs) const OrtMemoryInfo **inputs_memory_info, _In_ size_t num_inputs);
    ...
    ORT_API2_STATUS(SessionGetMemoryInfoForOutputs, _In_ const OrtSession *session,
                    _Out_writes_(num_outputs) const OrtMemoryInfo **outputs_memory_info, _In_ size_t num_outputs);
    ...
    ORT_API2_STATUS(SessionGetEpDeviceForInputs, _In_ const OrtSession *session,
                    _Out_writes_(num_inputs) const OrtEpDevice **inputs_ep_devices, _In_ size_t num_inputs);
    ...
    ORT_API2_STATUS(CreateSyncStreamForEpDevice, _In_ const OrtEpDevice *ep_device,
                    _In_opt_ const OrtKeyValuePairs *stream_options, _Outptr_ OrtSyncStream **stream);
     ...
     * \since Version 1.23
     */
    ORT_API2_STATUS(CopyTensors, _In_ const OrtEnv *env, _In_reads_(num_tensors) const OrtValue *const *src_tensors,
                    _In_reads_(num_tensors) OrtValue *const *dst_tensors, _In_opt_ OrtSyncStream *stream,
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(GetModelCompatibilityForEpDevices, _In_reads_(num_ep_devices) const OrtEpDevice *const *ep_devices,
                    _In_ size_t num_ep_devices, _In_ const char *compatibility_info,
     ...
     *
     * \param[in] filepath The relative path to the file that stores the initializer's data. ORT copies this path
     * string.
     ...
     * \since Version 1.24
     */
    ORT_API2_STATUS(KernelInfo_GetOperatorSinceVersion, _In_ const OrtKernelInfo *info, _Out_ int *since_version);
    ...
    ORT_API2_STATUS(SessionGetEpDeviceForOutputs, _In_ const OrtSession *session,
                    _Out_writes_(num_outputs) const OrtEpDevice **outputs_ep_devices, _In_ size_t num_outputs);
    ...
    ORT_API2_STATUS(GetHardwareDevices, _In_ const OrtEnv *env,
                    _Out_writes_(num_devices) const OrtHardwareDevice **devices, _In_ size_t num_devices);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(GetHardwareDeviceEpIncompatibilityDetails, _In_ const OrtEnv *env, _In_ const char *ep_name,
                    _In_ const OrtHardwareDevice *hw, _Outptr_ OrtDeviceEpIncompatibilityDetails **details);
    ...
    ORT_API2_STATUS(DeviceEpIncompatibilityDetails_GetReasonsBitmask,
                    _In_ const OrtDeviceEpIncompatibilityDetails *details, _Out_ uint32_t *reasons_bitmask);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(DeviceEpIncompatibilityDetails_GetNotes, _In_ const OrtDeviceEpIncompatibilityDetails *details,
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(DeviceEpIncompatibilityDetails_GetErrorCode, _In_ const OrtDeviceEpIncompatibilityDetails *details,
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(GetCompatibilityInfoFromModel, _In_ const ORTCHAR_T *model_path, _In_ const char *ep_type,
                    _Inout_ OrtAllocator *allocator, _Outptr_result_maybenull_ char **compatibility_info);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(GetCompatibilityInfoFromModelBytes, _In_reads_(model_data_length) const void *model_data,
                    _In_ size_t model_data_length, _In_ const char *ep_type, _Inout_ OrtAllocator *allocator,
    ...
    ORT_API2_STATUS(Session_GetEpGraphAssignmentInfo, _In_ const OrtSession *session,
                    _Outptr_ const OrtEpAssignedSubgraph *const **ep_subgraphs, _Out_ size_t *num_ep_subgraphs);
    ...
    ORT_API2_STATUS(GetTensorElementTypeAndShapeDataReference, _In_ const OrtValue *value,
                    _Out_ ONNXTensorElementDataType *elem_type, _Outptr_result_maybenull_ const int64_t **shape_data,
     ...
     * \since Version 1.25.
     */
    ORT_API2_STATUS(RunOptionsEnableProfiling, _Inout_ OrtRunOptions *options,
    ...
    ORT_API2_STATUS(KernelInfoGetAttributeArray_string, _In_ const OrtKernelInfo *info, _In_ const char *name,
                    _Inout_ OrtAllocator *allocator, _Outptr_result_buffer_maybenull_(*size) char ***out,
                    _Out_ size_t *size);
     ...
     *
     * Experimental functions are not part of the stable ABI and may be added or removed between releases without
     * notice. Use the companion header onnxruntime_experimental_c_api.h for typedefs, name constants, and (for C++)
     * typed accessors.
     *
...
struct OrtCustomOp {
    ...
    OrtStatusPtr(ORT_API_CALL *CreateKernelV2)(_In_ const struct OrtCustomOp *op, _In_ const OrtApi *api,
                                               _In_ const OrtKernelInfo *info, _Out_ void **kernel);
...
struct OrtModelEditorApi {
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(CreateNode, _In_ const char *operator_name, _In_ const char *domain_name,
                    _In_ const char *node_name, _In_reads_(input_names_len) const char *const *input_names,
                    size_t input_names_len, _In_reads_(output_names_len) const char *const *output_names,
                    size_t output_names_len, _In_reads_(attribs_len) _In_opt_ OrtOpAttr **attributes,
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(SetGraphInputs, _Inout_ OrtGraph *graph, _In_reads_(inputs_len) _Inout_ OrtValueInfo **inputs,
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(SetGraphOutputs, _Inout_ OrtGraph *graph, _In_reads_(outputs_len) _Inout_ OrtValueInfo **outputs,
     ...
     *
     * Pre-existing memory:
     *    Use OrtApi::CreateTensorWithDataAsOrtValue or OrtApi::CreateTensorWithDataAndDeleterAsOrtValue to create an
     * OrtValue with a tensor that contains a pointer to the existing data. Set `data_is_external` to true.
     *
     *    The pointer must remain valid for the duration of the inference session.
     *    If using OrtApi::CreateTensorWithDataAsOrtValue you are responsible for freeing the memory after the inference
     * session is released. If using OrtApi::CreateTensorWithDataAndDeleterAsOrtValue, ORT will free the memory using
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(CreateModel, _In_reads_(opset_entries_len) const char *const *domain_names,
                    _In_reads_(opset_entries_len) const int *opset_versions, size_t opset_entries_len,
     ...
     * Graph inputs/outputs should be updated with SetGraphInputs() and SetGraphOutputs() as needed to reflect changes
     * made by the new nodes. The list of graph inputs/outputs should be for the overall model and not just the new
     * nodes.
     *
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(CreateModelEditorSessionFromArray, _In_ const OrtEnv *env, _In_ const void *model_data,
                    size_t model_data_length, _In_ const OrtSessionOptions *options, _Outptr_ OrtSession **out);
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(SessionGetOpsetForDomain, _In_ const OrtSession *session, _In_ const char *domain,
                    _Out_ int *opset);
     ...
     *
     * \param[in] session OrtSession to update. Session must have been created using
     ...
     *
     * \param[in] session OrtSession to finalize. Session must have been created using
     CreateModelEditorSession[FromArray].
...
struct OrtCompileApi {
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetOutputModelExternalInitializersFile,
                    ...
                    _In_ const ORTCHAR_T *external_initializers_file_path, size_t external_initializers_size_threshold);
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetOutputModelBuffer,
                    _In_ OrtModelCompilationOptions *model_compile_options, _Inout_ OrtAllocator *allocator,
                    _Outptr_ void **output_model_buffer_ptr, _Out_ size_t *output_model_buffer_size_ptr);
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(ModelCompilationOptions_SetEpContextEmbedMode,
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetEpContextBinaryInformation,
                    _In_ OrtModelCompilationOptions *model_compile_options, _In_ const ORTCHAR_T *output_directory,
     ...
     * ONNX bytes.
     *
     * The provided write function may be called repeatedly until then entire output model has been written out. Each
     * call to the write function is expected to consume the entire input buffer.
     *
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetOutputModelWriteFunc,
                    _In_ OrtModelCompilationOptions *model_compile_options, _In_ OrtWriteBufferFunc write_func,
                    _In_ void *state);

    /** \brief Sets a OrtGetInitializerLocationFunc function that is called by ORT for every initializer in the
     * generated model. Allows implementer to specify whether initializers should be stored within the model or
     * externally.
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetWeightlessEnabled,
                    _In_ OrtModelCompilationOptions *model_compile_options, _In_ bool use_weightless);
};
...
struct OrtInteropApi {
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(CreateExternalResourceImporterForDevice, _In_ const OrtEpDevice *ep_device,
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(CanImportMemory, _In_ const OrtExternalResourceImporter *importer,
                    _In_ OrtExternalMemoryHandleType handle_type, _Out_ bool *out_supported);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(ImportMemory, _In_ OrtExternalResourceImporter *importer,
                    _In_ const OrtExternalMemoryDescriptor *desc, _Outptr_ OrtExternalMemoryHandle **out_handle);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(CreateTensorFromMemory, _In_ OrtExternalResourceImporter *importer,
                    _In_ const OrtExternalMemoryHandle *mem_handle, _In_ const OrtExternalTensorDescriptor *tensor_desc,
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(CanImportSemaphore, _In_ const OrtExternalResourceImporter *importer,
                    _In_ OrtExternalSemaphoreType type, _Out_ bool *out_supported);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(ImportSemaphore, _In_ OrtExternalResourceImporter *importer,
                    _In_ const OrtExternalSemaphoreDescriptor *desc, _Outptr_ OrtExternalSemaphoreHandle **out_handle);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(WaitSemaphore, _In_ OrtExternalResourceImporter *importer,
                    _In_ OrtExternalSemaphoreHandle *semaphore_handle, _In_ OrtSyncStream *stream, _In_ uint64_t value);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(SignalSemaphore, _In_ OrtExternalResourceImporter *importer,
                    _In_ OrtExternalSemaphoreHandle *semaphore_handle, _In_ OrtSyncStream *stream, _In_ uint64_t value);
...
/*
 * This is the old way to add the CUDA provider to the session, please use SessionOptionsAppendExecutionProvider_CUDA
 * above to access the latest functionality This function always exists, but will only succeed if Onnxruntime was built
 * with CUDA support and the CUDA provider shared library exists
 *
 ...
 * SessionOptionsAppendExecutionProvider_TensorRT_V2 above to access the latest functionality This function always
 * exists, but will only succeed if Onnxruntime was built with TensorRT support and the TensorRT provider shared library
 * exists
 *
 * \param device_id CUDA device id, starts from zero.
 */

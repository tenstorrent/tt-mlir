...
ttnn::Tensor mesh_shard(ttnn::Tensor &input, ::ttnn::MeshDevice &meshDevice,
  ...
  if (meshShardType == MeshShardType::Identity) {
    ...
    assert(input.storage_type() == ::ttnn::StorageType::HOST &&
           "Input of ttnn::mesh_shard should be host tensor for replicate and devices operations.");
  }
  ...
  if (meshShardDirection == MeshShardDirection::FullToShard) {
    ...
    if (meshShardType == MeshShardType::Replicate) {
      ...
      auto getNonOverlappingDim = [&]() -> int {
        ...
        assert(false && "All dimensions are overlapping, cannot find non-overlapping dimension for mesh composer.");
        return -1;
      };
...
ttnn::Tensor all_gather(ttnn::Tensor &input,
  ...
  std::optional<::tt::tt_metal::MemoryConfig> outputMemoryConfig = memory_config;
  ...
  return ::ttnn::experimental::all_gather_async(
      input, dim, cluster_axis, meshDevice,
      ::ttnn::ccl::Topology::Linear, semaphores, std::nullopt,
      outputMemoryConfig, std::make_optional(static_cast<size_t>(num_links)),
      std::nullopt);
}
...
ttnn::Tensor point_to_point(ttnn::Tensor &input,
  ...
  auto calcIdFromCoords =
      [&](const std::vector<uint32_t> &coords) -> size_t {

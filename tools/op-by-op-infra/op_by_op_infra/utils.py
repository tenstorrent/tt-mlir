...
# These ops transform the constant value but the result is still effectively constant.
...
def _get_constant_chain(operand) -> List:
    ...
    If the operand's defining op is a constant, returns [constant_op].
    If the operand comes from a chain of constant-preserving ops that starts
    with a constant, returns the full chain [constant_op, ..., final_op].
    Otherwise returns an empty list.

    Example: For broadcast_in_dim(convert(constant)), returns
    [constant_op, convert_op, broadcast_in_dim_op].
    """
    ...
    while current_op and hasattr(current_op, "name"):
        if current_op.name in CONSTANT_PRESERVING_OPS:
            ...
            # These ops have single operand at index 0
            if len(current_op.operands) > 0:
                current_op = current_op.operands[0].owner
            else:
                break
        elif current_op.name in PRESERVED_OP_NAMES:
            # Found the root constant
            chain.append(current_op)
            # Reverse to get [constant, ..., final_preserving_op]
            chain.reverse()
            return chain
        else:
            # Chain doesn't start with a constant
            break
...
def _get_captured_values_from_regions(op) -> List:
    ...
    In MLIR, regions can reference values from the enclosing scope. These are
    "captured" values. This function traverses all regions and blocks to find
    operands that are used but not defined within the region.

    Block arguments are excluded as they are defined by the region itself.

    Returns list of captured operand values (not names).
    """
    captured = []
...
class OpWrapper:
    ...
    def __init__(
        ...
        # This includes both top-level operands AND captured values from regions
        ...
        for original_name, operand in unique_operands.items():
            ...
            if chain:
                ...
                for chain_op in chain:
                    if chain_op.results:
                        chain_original_names.append(chain_op.results[0].get_name())
                    else:
                        chain_original_names.append(None)
        ...
        for chain, chain_original_names, pres_idx in preserved_chains:
            ...
            for i, chain_op in enumerate(chain):
                op_str = str(chain_op)
                # Replace all chain names in this op string
        ...
        # Store results (operands already deduplicated)
    ...
    def as_module_str(self) -> str:
        ...
        ```
        """
        # Operands are already deduplicated in the constructor

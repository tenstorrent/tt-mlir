...
class MemoryAnalyzer:
    ...
    def __init__(self, device):
        ...
        # Note: The memory_view().block_table shows addresses relative to the start of the 'allocatable' region and should not be used by allocator.
        # However, get_statistics().largest_free_block_addrs returns addresses (relative to bank base address) that can hold the largest_free_block_bytes (the actual hardware address offset within the L1/DRAM bank).
        # Since D2M only accepts a single [start, end) range, we just collect one of the largest free blocks for one bank.
        assert (

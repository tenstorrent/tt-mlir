...
def _emit_kernel_generic(
    ...
    def _call_error(msg, hint=None, cause=None):
        ...
        return D2mJitError(
            ...
            line=(
                kernel._source_firstlineno
                + (kernel._ast.body[0].lineno - 1)
                if kernel._ast.body
            ...
            snippet_line=(
                kernel._ast.body[0].lineno if kernel._ast.body else None
            ),
            hint=hint,
            cause=cause,
        )
    ...
    if num_outs < 1:
        raise _call_error(f"num_outs must be >= 1 (got {num_outs})",
                          cause=ValueError())

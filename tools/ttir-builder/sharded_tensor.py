...
class ShardedTensor:
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        # If no ShardedTensor subclass is involved, let PyTorch handle it
        if not any(issubclass(t, cls) for t in types):
            return NotImplemented

        # Collect all ShardedTensor inputs
        st_inputs = [a for a in _walk(args, kwargs) if isinstance(a, cls)]
        n_shards_set = {len(st._shards) for st in st_inputs}
        if len(n_shards_set) != 1:
            raise RuntimeError("all ShardedTensors must have the same number of shards")
        n = n_shards_set.pop()

        # Helper to fetch i-th physical tensor for each arg
        ...
        out_shards: List[torch.Tensor] = [
            func(*_take(args, i), **_take(kwargs, i)) for i in range(n)
        ]

        # Wrap back when all outputs are uniform tensors
        ...
        # Otherwise propagate the list (mixed outputs)
        return out_shards

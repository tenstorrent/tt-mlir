...
def _reduce_values(torch_fn, t, dim, keepdim):
    ...
    if isinstance(dim, (list, tuple)):
        return _reduce_iterative(
            lambda x, **kw: torch_fn(x, **kw).values, t, dim, keepdim
        )
...
def gelu_bw(grad, input_tensor, approximate="none", **_):
    ...
    input_tensor = (
        input_tensor.expand(broadcast_shape).clone().detach().requires_grad_(True)
    )
...
def dot_general(
    ...
    **_,
    ...
    lhs_free = [
        i for i in range(lhs_rank) if i not in lhs_batch and i not in lhs_contract
    ]
    rhs_free = [
        i for i in range(rhs_rank) if i not in rhs_batch and i not in rhs_contract
    ]
    ...
    out_shape = (
        [lhs.shape[d] for d in lhs_batch]
        + [lhs.shape[d] for d in lhs_free]
        + [rhs.shape[d] for d in rhs_free]
    )
...
def gather(
    ...
    **_,
    ...
    indices_np = start_indices.numpy().astype(int)
    ...
    indices_shape = list(indices_np.shape)
    if index_vector_dim < len(indices_shape):
        batch_dims_shape = (
            indices_shape[:index_vector_dim] + indices_shape[index_vector_dim + 1 :]
        )
        index_depth = indices_shape[index_vector_dim]
    else:
        batch_dims_shape = indices_shape
        index_depth = 1

    num_batch = 1
    for s in batch_dims_shape:
        num_batch *= s

    if index_vector_dim < len(indices_shape):
        flat_indices = (
            indices_np.reshape(-1, index_depth)
            if index_depth > 0
            else indices_np.reshape(-1, 1) * 0
        )
    else:
        flat_indices = indices_np.reshape(-1, 1)

    full_slice_shape = list(slice_sizes)
    collapsed_set = set(collapsed_slice_dims)
    result_slice_shape = [
        s for i, s in enumerate(full_slice_shape) if i not in collapsed_set
    ]

    results = []
    for b_idx in range(num_batch):
        idx_vec = flat_indices[b_idx]
        slices = [builtins.slice(None)] * len(input_np.shape)
        for k, dim in enumerate(start_index_map):
            start = int(idx_vec[k])
            start = builtins.max(
                0, builtins.min(start, input_np.shape[dim] - slice_sizes[dim])
            )
            slices[dim] = builtins.slice(start, start + slice_sizes[dim])
        piece = input_np[tuple(slices)]
        for d in sorted(collapsed_slice_dims, reverse=True):
            piece = np.squeeze(piece, axis=d)
        results.append(piece)

    result_np = np.stack(results).reshape(batch_dims_shape + result_slice_shape)

    out_rank = len(batch_dims_shape) + len(result_slice_shape)
    batch_out_dims = [i for i in range(out_rank) if i not in offset_dims]
    perm = [0] * out_rank
    for i, d in enumerate(batch_out_dims):
        perm[d] = i
    for i, d in enumerate(offset_dims):
        perm[d] = len(batch_out_dims) + i
    inv_perm = [0] * out_rank
    for i, p in enumerate(perm):
        inv_perm[p] = i
    result_np = np.transpose(result_np, inv_perm)

    return torch.from_numpy(result_np.copy()).to(input.dtype)

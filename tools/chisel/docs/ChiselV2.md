...
The current tool for debugging numerical accuracy is the builder, which computes golden intermediates at compile time. The training team encountered several problems with this approach that block debugging of larger models:
...
The primary motivation for this redesign is to eliminate the complexity of cross-dialect op correlation, support multi-program and multi-chip workloads, and provide an easy way to integrate into the frontend with minimal effort.
...
The TTNN flatbuffer binary contains an `mlir` field (`TTNNBinary.mlir.source`) that stores the full TTNN MLIR text as a plain string. Chisel reads this field at runtime via `ttrt.binary` and parses it into a first-class MLIR `Module` object using the `ttmlir` nanobindings:
...
This gives Chisel structured access to the TTNN IR — operations, SSA values, attributes, locations, and types — rather than working with raw text. The parsed module is used by the `Registry` to walk the op graph, resolve operand definitions, and extract op metadata (e.g., input/output shapes, op attributes) needed for golden lookup and CSV reporting.
...
Currently, tensor comparison logic (PCC, atol, rtol) lives in `tools/builder/base/builder_runtime.py` and is tightly coupled to the builder. Since Chisel V2 needs the same metrics for golden-vs-device comparison, we propose extracting this logic into a shared `tools/golden/metrics.py` module that both builder and Chisel can import from.

#### Proposed unified API

The shared module provides the following pure-torch (no numpy) functions:

- **`compute_pcc(golden, calculated)`** — Pearson Correlation Coefficient. Measures linear correlation between tensors. PCC of 1.0 indicates perfect match.
- **`compute_atol(golden, calculated)`** — Max absolute error: `max(|golden - calculated|)`.
- **`compute_rtol(golden, calculated)`** — Max relative error: `max(|(golden - calculated) / calculated|)`.
- **`compute_metrics(golden, calculated)`** — Full comparison dict returning PCC, atol, rtol, allclose, MAE, RMSE, and cosine similarity.
- **`align_shapes(golden, calculated)`** — Opt-in shape alignment (squeeze/broadcast/permute/flatten) for minor layout differences between golden and device outputs.

#### Edge case handling

| Case | Behavior |
| :---- | :---- |
| Single-element tensors | Falls back to `torch.isclose` instead of PCC |
| Constant tensors (zero variance) | Falls back to `torch.equal`; returns 1.0 if identical, 0.0 otherwise |
| Empty tensors | Returns PCC 1.0 if shapes match |
| All-NaN tensors | Returns PCC 1.0 if both are all-NaN, 0.0 if only one is |
| bfloat16 / low-precision dtypes | Automatically upcast to float32 before comparison |

#### Migration

Builder's `check_outputs()` keeps its existing signature for backward compatibility but internally delegates to `golden.metrics.compute_metrics()`. Chisel V2 imports directly from `golden.metrics` — no local metrics code needed.
...
The initial plan is that the developer reviews the CSV report manually and selects operations with low PCC. As we gather data from real workloads we plan to automate this process in several stages:
...
- **Automated op skipping**: Once bad ops are identified, Chisel could automatically apply op skipping (replacing device output with golden) to isolate whether a specific op is the source of downstream numerical errors. The output would be what ops were skipped.
...
- **Debug metadata propagation**: Debug dialect operations could annotate TTNN ops with additional metadata (e.g., original source locations from the frontend framework, variable names, layer identifiers). Chisel could read these annotations and include them in the CSV report, making it easier to map low-PCC ops back to user-level model code.
- **Selective instrumentation**: Instead of writing every op comparision, debug dialect markers could designate specific regions of the graph for Chisel to instrument. This would reduce overhead for large models where only a subset of ops is under investigation.

...
#ttnn_layout = #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <1x1>, memref<64x1x!ttcore.tile<32x32, f32>, #dram>, <interleaved>>
module {
  ttcore.device_module {
    builtin.module attributes {ttcore.system_desc = #system_desc, ttnn.l1_const_eval_usage = 1024 : ui64} {
      ...
      func.func @sm(%arg0: tensor<2048x32xf32, #ttnn_layout>) -> tensor<2048x32xf32, #ttnn_layout> attributes {tt.function_type = "forward_device"} {
        %0 = "ttnn.softmax"(%arg0) <{compute_config = #ttnn.device_compute_kernel_config<math_fidelity = hifi4, fp32_dest_acc_en = true>, dimension = 1 : si32, numericStable = true}> : (tensor<2048x32xf32, #ttnn_layout>) -> tensor<2048x32xf32, #ttnn_layout>
        "ttnn.deallocate"(%arg0) <{force = false}> : (tensor<2048x32xf32, #ttnn_layout>) -> ()
        return %0 : tensor<2048x32xf32, #ttnn_layout>
      }
    }
  }
}

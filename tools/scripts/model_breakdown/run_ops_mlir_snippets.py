...
# Repo layout: tools/scripts/model_breakdown/<this file> -> tt-mlir root is parents[3]
...
def _is_attributes_dict(text: str, brace_pos: int) -> bool:
    prefix = text[:brace_pos].rstrip()
    return prefix.endswith(" attributes")


def _find_func_body_start(text: str, start_pos: int) -> int | None:
    pos = start_pos
    while pos < len(text):
        ...
        if not _is_attributes_dict(text, brace_pos):
            return brace_pos

        close_pos = _find_matching_brace(text, brace_pos)
...
def extract_functions_from_mlir(mlir_content: str) -> list[tuple[str, str]]:
    """Extract each top-level ``func.func`` as its own wrapped module."""
    ...
    for match in func_start_pattern.finditer(mlir_content):
        ...
        brace_pos = _find_func_body_start(mlir_content, match.end())
        if brace_pos is None:
            continue

        end_pos = _find_matching_brace(mlir_content, brace_pos)
        if end_pos is None:
            continue

        func_body = mlir_content[start_pos:end_pos]
        func_mlir = f"module {{\n  {func_body}\n}}"
...
def _extract_diagnostics(raw: str) -> List[str]:
    ...
    for line in raw.splitlines():
        ...
        if clean.startswith("Warning:"):
            continue
        if not any(pattern.search(clean) for pattern in _DIAG_PATTERNS):
            continue
        if clean in seen:
            continue
        seen.add(clean)
...
def _process_file(
    ...
    """
    content = ops_path.read_text(encoding="utf-8", errors="replace").strip()
    if not content:
...
def _list_functions(ops_files: List[Path], func_filter: Optional[str]) -> int:
    for ops_path in ops_files:
        content = ops_path.read_text(encoding="utf-8", errors="replace").strip()
        snippets = extract_functions_from_mlir(content)
        if func_filter:
            snippets = [(name, mlir) for name, mlir in snippets if func_filter in name]

        if len(ops_files) > 1:
            print(f"{ops_path}:")
        for func_name, _ in snippets:
            print(func_name if len(ops_files) == 1 else f"  {func_name}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "-l",
        action="store_true",
        help="List matching function names without compiling",
    )
    parser.add_argument(
        "--fail-fast",
        action="store_true",
    ...
    if args.list:
        return _list_functions(ops_files, args.func)

...
def split_output_types(out: str) -> list[str]:
    out = out.strip()
    if out.startswith("(") and out.endswith(")"):
        return split_input_types(out[1:-1])
    return [out]
...
def _format_op_and_return(
    ...
    """Format a generic TTIR op and return, including multi-result ops.

    MLIR generic assembly binds multi-result ops as ``%0:N`` and each result is
    used as ``%0#i``. Binding the op as just ``%0`` is invalid for ops like
    ``ttir.sort`` and ``ttir.topk`` that return values and indices.
    """
    output_types = split_output_types(out_t)
    num_results = len(output_types)
    result_binding = "%0" if num_results == 1 else f"%0:{num_results}"
    op_line = (
        f'{result_binding} = "ttir.{mnemonic}"('
        + ", ".join(operands)
        + f"){attr} : ({inp_types_s}) -> {out_t}"
    )

    if num_results == 1:
        return op_line, f"return %0 : {out_t}"

    return_values = ", ".join(f"%0#{i}" for i in range(num_results))
    return_types = ", ".join(output_types)
    return op_line, f"return {return_values} : {return_types}"
...
def emit_func(
    ...
    if n == 0:
        ...
        op_line, return_line = _format_op_and_return(
            mnemonic, [], attr, parsed["inp_types_s"], out_t
        )
        return f"  {sig} {{\n    {op_line}\n    {return_line}\n  }}"
    ...
    op_line, return_line = _format_op_and_return(
        mnemonic, op_operand_names, attr, parsed["inp_types_s"], out_t
    )
    body_lines.append(f"    {op_line}")
    body_lines.append(f"    {return_line}")

...
class CMakeBuild(build_ext):
    ...
    def run(self) -> None:
        """Build tt-mlir with D2M-JIT enabled and copy MLIR libraries into wheel"""
        ...
        if self.is_editable_install_():
            raise Exception("Editable install not supported for d2m-jit")
        ...
        if not self.dev_build:
            ...
            build_cmd = ["cmake", "--build", str(build_dir), "--", "d2m-jit-deps"]
    ...
    def write_build_metadata(self):
        ...
        build_metadata_file = self.install_dir / "d2m_jit" / "_build_metadata.py"
...
def get_readme():
    ...
    return """# d2m-jit
D2M Just-In-Time Compilation Interface for TT-MLIR

This package provides a Python interface for just-in-time compilation
of D2M operations using the TT-MLIR compiler infrastructure.

## Installation
pip install d2m-jit## Usage
import d2m_jit
# Use d2m-jit APIs
...
d2m_jit_ext = Extension("d2m_jit._build_trigger", sources=[])
...
│                       ├── _d2m_jit.cpython-310-x86_64-linux-gnu.so
│                       ├── libJITCPP.so
│                       └── ttmlir/
│                           ├── dialects/
│                           ├── _mlir_libs/
...
setup(
    ...
    description="D2M Just-In-Time Compilation Interface for TT-MLIR",
    long_description=get_readme(),
    long_description_content_type="text/markdown",
    packages=["d2m_jit", "d2m_jit._src"],
    package_dir={"d2m_jit": "."},
    ext_modules=[d2m_jit_ext],

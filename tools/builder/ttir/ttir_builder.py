...
class TTIRBuilder(Builder):
    ...
    def pooling(
        ...
        Performs a pooling operation over the input tensor using the specified
        method (e.g., max or average). Pooling reduces spatial dimensions by
        aggregating values within sliding windows.

        The output spatial dimensions are determined by the input dimensions,
        window size, strides, padding, and dilations. The operation supports
        both NCHW and NHWC data layouts, which are automatically detected
        based on the window dimensions.

        Parameters
        ----------
        in0 : Operand
            Input tensor. Typically a 4D tensor in NCHW or NHWC format.
        pooling_method : str
            Pooling method to apply. Common values are "Max" for max pooling
            or "Average" for average pooling.
        window_dimensions : list of int
            Size of the pooling window for each dimension. For NCHW format,
            use [1, 1, kH, kW]. For NHWC format, use [1, kH, kW, 1].
        window_strides : list of int
            Stride of the pooling window for each dimension.
        padding : list of int
            Padding to apply before and after each spatial dimension.
            Format is [before_dim0, after_dim0, before_dim1, after_dim1, ...].
        window_dilations : list of int
            Dilation factor for the pooling window in each dimension.
            A dilation of 1 means no dilation (standard pooling).
        base_dilations : list of int, optional
            Dilation applied to the input tensor. If None, defaults to
            [1, 1, ...] (no dilation).
        output_type : torch.dtype, optional
            Data type for the output tensor. If None, uses the input tensor's
            dtype.
        ...
        -------
        OpResult
            Pooled output tensor with reduced spatial dimensions.

        Notes
        -----
        The layout (NCHW vs NHWC) is inferred from `window_dimensions`:
        - NCHW: spatial dims at indices [2, 3]
        - NHWC: spatial dims at indices [1, 2]

        See Also
        --------
        max_pool2d_with_indices : Max pooling with index tracking.
        """

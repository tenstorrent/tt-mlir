...
class TTIRBuilder(Builder):
    ...
    def hardsigmoid(
        ...
        """
        Creates ``ttir.hardsigmoid``.

        *Elementwise hard sigmoid activation operation.*

        Computes the hard sigmoid function for each element in the input tensor.
        Hard sigmoid is a piecewise linear approximation of the sigmoid function
        defined as: hardsigmoid(x) = max(0, min(1, (x + 3) / 6))

        This activation function is computationally efficient compared to the
        standard sigmoid function while providing similar behavior.

        .. code-block:: mlir

            // Compute hard sigmoid of all elements
            %result = ttir.hardsigmoid(%input, %output) : tensor<4xf32>, tensor<4xf32> -> tensor<4xf32>
            // Input tensor:
            // [-3.0, -1.0, 0.0, 3.0]
            // Output tensor:
            // [0.0, 0.333, 0.5, 1.0]
        ...
        -------
        (*OpView*)
            A tensor containing the hard sigmoid activation values of each element in the input tensor
        """
        return self._op_proxy(ttir.HardsigmoidOp, [in0], unit_attrs)

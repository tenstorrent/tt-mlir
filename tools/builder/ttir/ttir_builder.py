    ...
    def get_metal_tensor_layout(
        ...
        with self._ctx, self._loc:
            ...
            layout = ttcore.ir.MetalLayoutAttr.get(
                self._ctx, logical_shape, worker_grid, oobVal, memorySpace
            )
            ...
            for l, g in zip(logical_shape, grid_shape):
                assert (
                    l % g == 0
                ), f"Logical shape {l} must be divisible by grid shape {g}"
                shard_shape.append(l // g)

            # Sharded shape = grid + logical shape.
            device_shape = grid_shape + shard_shape

            elemType = F32Type.get(self._ctx)

            if tiled:
                elemType = ttcore.ir.TileType.get(
                    self._ctx, 32, 32, ttcore.DataType.Float32
                )
                device_shape[-2] //= 32
                device_shape[-1] //= 32

            return RankedTensorType.get(
                device_shape, elemType, layout, Location.unknown(self._ctx)
            )

...
class TTIRBuilder(Builder, metaclass=TTIRBuilderMeta):
            ...
            with InsertionPoint(scatter_in_dim_module.body):
                ...
                def decorated_func(*inputs):
                    ...
                    if not self._disable_golden_check:
                        ...
                        scatter_in_dim_builder._set_golden_tensor(queried_input0, input0)
                        scatter_in_dim_builder._set_golden_tensor(queried_index0, input_index)
                        scatter_in_dim_builder._set_golden_tensor(queried_source0, input_source)
                        scatter_in_dim_builder._set_input_ordering([queried_input0, queried_index0, queried_source0])
    ...
    def max_pool2d_with_indices(
        ...
        input0 = self._get_golden_tensor(in0)
        op_golden_function = get_golden_function(ttir_op)
        golden_outputs = op_golden_function(input0, kernel_attr, stride_attr, padding_attr, dilation_attr, ceil_mode_attr)
        result = self._create_ranked_tensor_type(golden_outputs[0].shape, output_type)
        result_indices = self._create_ranked_tensor_type(golden_outputs[1].shape, self._get_type_from_torch_dtype(torch.int64))
        output = self._get_empty_op(result)
    ...
    def max_pool2d_with_indices_parser(
        ...
        ttir_op = self.get_opview_from_parser(TTIRBuilder.max_pool2d_with_indices_parser)
        ...
        with old_ctx, old_loc:
            ...
            with InsertionPoint(max_pool2d_with_indices_module.body):
                ...
                def decorated_func(*inputs):
                    ...
                    output_indices = max_pool2d_with_indices_builder._get_empty_op(result_indices)
                    kernel_attr = old_op.kernel
                    stride_attr = old_op.stride
                    dilation_attr = old_op.dilation
                    padding_attr = old_op.padding
                    ...
                    if not self._disable_golden_check:
                        ...
                        op_golden_function = get_golden_function(ttir_op)
                        golden_outputs = op_golden_function(input0, kernel_attr, stride_attr, padding_attr, dilation_attr, ceil_mode_attr)
                        max_pool2d_with_indices_builder._set_golden_tensor(new_op, golden_outputs)
                        max_pool2d_with_indices_builder._set_output_ordering([new_op])
                        max_pool2d_with_indices_builder._set_golden_tensor(queried_input0, input0)
                        max_pool2d_with_indices_builder._set_input_ordering([queried_input0])
    ...
    def floor(
        ...
        *Elementwise floor operation.*
        ...
        loc : *Optional[str]*, optional
            Location name for the operation
        unit_attrs : *Optional[List[str]]*, optional
            Optional list of unit attributes
        ...
        with old_ctx, old_loc:
            ...
            with InsertionPoint(exp_module.body):
                ...
                def decorated_func(*inputs):
                    ...
                    new_op = ttir_op(result, in0, output, loc=old_op.location)
    ...
    def slice(
        ...
        """
        Creates ``ttir.slice_static``.

        *Tensor slice operation with constant parameters.*

        Extracts a sub-tensor (slice) from the input tensor across one or more dimensions.

        .. code-block:: mlir

            // Extract a 2x2 slice from a 4x4 tensor
            %result = ttir.slice_static(%input, %output) {
                begins = [1, 1], ends = [3, 3], step = [1, 1]
        ...
        ends : List[int]
            Ending indices (exclusive) for each dimension
        step : List[int], optional
            Step sizes for each dimension (default: 1 for each)
        output_type : *Optional[torch.dtype]*
            Optional output type
        loc : *Optional[str]*
            Optional location string
                        ...
                        for block in entry.body:
                            for op in block.operations:
                                ...
                                if isinstance(op, func.ReturnOp):
                                    ...
                                    if isinstance(parsed_op, ttir.MaxPool2dWithIndicesOp):
                                        global_dict[op.result] = parsed_op.result
                                        global_dict[op.result_indices] = parsed_op.result_indices
                                    else:

...
class TTIRBuilder(Builder):
    ...
    ############### ttir.LayerNormForwardOp ###############

    @tag(ttir.LayerNormForwardOp)
    def layernorm_fw(
        self,
        input: Operand,
        weight: Operand,
        ...
        ttir_op = self.get_opview_from_method(TTIRBuilder.layernorm_fw)
        epsilon_attr = FloatAttr.get_f32(epsilon)
        ...
        rstd_type = None
        if return_mean_rstd:
            mean_type = self._create_ranked_tensor_type(
                golden_output[1].shape, mlir_output_type
            )
            rstd_type = self._create_ranked_tensor_type(
                golden_output[2].shape, mlir_output_type
            )
        ...
        op = ttir_op(
            ...
            return_mean_rstd=return_mean_rstd,
            loc=loc,
        )

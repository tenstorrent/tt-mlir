...
def ttir_max_pool2d_with_indices(
    ...
    assert padding[0] == padding[2], "Asymmetric padding not supported in height dimension"
    assert padding[1] == padding[3], "Asymmetric padding not supported in width dimension"
    ...
    return output_nhwc.to(torch.float32), indices_nhwc.to(torch.int64)
...
def ttir_scatter_in_dim_golden(
    ...
    return torch.scatter(input_tensor, dim_value, index_copy, source).to(torch.float32)

def ttir_reverse_golden(input_tensor: GoldenMapTensor, dimensions_attr: DenseI64ArrayAttr) -> GoldenMapTensor:

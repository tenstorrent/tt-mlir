...
std::string ProfilerManager::getCaptureCommand() {
  ...
  return "/code/jan-2/tt-mlir/build/runtime/profiler/capture-release -o " + outputPath + " -a " + m_address + " -p " + std::to_string(m_port) + " -f";
}
...
std::string ProfilerManager::getCSVExportTimesCommand() {
  ...
  return "/code/jan-2/tt-mlir/build/runtime/profiler/csvexport-release -u -p TT_DNN -x " + childCalls + " " + tracyFilePath;
}

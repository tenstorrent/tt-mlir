...
// In case that operation does not have an output tensor, returns nullopt instead.
std::optional<TensorRef> getOpOutputRef(OpContext opContextHandle,
                                        CallbackContext programContextHandle);

// Returns the vector of references to the input tensors of the current operation
...
// TODO(ndrakulicTT): Check what happens if the tensor is not live, and other cases 

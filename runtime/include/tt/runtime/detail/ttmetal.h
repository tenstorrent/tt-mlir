...
Tensor createBorrowedHostTensor(std::shared_ptr<void> data, TensorDesc const &desc);
...
inline Tensor createBorrowedHostTensor(void *data, TensorDesc const &desc) {
  return ttmetal::createBorrowedHostTensor(utils::unsafe_borrow_shared(data), desc);
}

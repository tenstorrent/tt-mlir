...
def golden(context=None, opContext=None):
    ...
    try:
        ...
        golden_tensor_list = golden_tensor.get_data()
        output_tensor_list = output_tensor.get_data()

        if len(golden_tensor_list) == 0:
        ...
        if len(golden_tensor_list) != len(output_tensor_list):
            print(
        ...
        golden_tensor_torch = torch.tensor(
            golden_tensor_list, dtype=torch.float32
        ).flatten()
        output_tensor_torch = torch.tensor(
            output_tensor_list, dtype=torch.float32
        ).flatten()

...
from chisel.core.tensors import get_op_outputs
import argparse
import pathlib
import numpy as np
import torch
...
from chisel.utils.mapping import ttir_dtype_maps, ttrt_dtype_maps
...
class ChiselContext:
    ...
    def compare_outputs(self, op_location: Tuple[int, int]):
        ...
        if len(get_op_outputs(last_device_op)) == 0 or len(get_op_outputs(last_golden_op)) == 0:
            return

        device_output_name = get_op_outputs(last_device_op)[0].get_name()
        golden_output_name = get_op_outputs(last_golden_op)[0].get_name()
    ...
    def preop(self, binary, programContext, opContext):
        ...
        try:
            ...
            print(f"Skipping op {op_location}")
            return
        if op is None:
            return 
        if len(get_op_inputs(op)) == 0:
            return

        op_inputs = get_op_inputs(op)

        for i, input_ref in enumerate(input_refs):
            input_tensor = retrieve_tensor_from_pool(programContext, input_ref)
            if input_tensor is None:
                continue
            torch_tensor = get_torch_tensor(input_tensor)

            print(f"Input {i} is {torch_tensor}")
            input_name = op_inputs[i].get_name()
            first_time = False
            if input_name in [ '%arg7' , '%arg11']:
                if input_name not in self.cache_tensor: 
                    first_time = True
                self.cache_tensor[input_name] = input_ref

            if input_name not in self.device_tensor_pool:
                continue

            tensor_value = self.device_tensor_pool[input_name]
            # tensor_value.data = torch_tensor
            # tensor_value.execution_data = torch_tensor
            # tensor_value.tensor = input_tensor
            # tensor_value.tensor_ref = input_ref
            if tensor_value.execution_data is not None:
                input_tensor = retrieve_tensor_from_pool(programContext, input_ref)
                update_device_tensor(
                    programContext, input_ref, input_tensor, tensor_value.execution_data if first_time and input_name not in ['%arg7' , '%arg11'] else torch.zeros(tensor_value.execution_data.shape, dtype=tensor_value.execution_data.dtype)
                )
                tensor_value.tensor_ref = input_ref
                tensor_value.execution_data = None
                tensor_value.tensor = retrieve_tensor_from_pool(programContext, input_ref)
                tensor_value.data = get_torch_tensor(tensor_value.tensor)

    def load_inputs(self):
        for arg in self.device_ir_module.get_function_inputs():
            tensor_path = self.input_dir / "tensors" / f"{arg.get_name().replace('%arg', '')}.pt"
            print(tensor_path)
            assert (
                tensor_path.exists()
            ), f"Input tensor {arg.get_name()} not found on path {tensor_path}"
            tensor = torch.load(tensor_path) * 2
            tensor = tensor.to(ttir_dtype_maps[str(arg.type.element_type)])
            print(
                f"Loaded tensor {arg.get_name()} with shape {tensor.shape} with dtype {tensor.dtype}"
            )
            ...
            self.device_tensor_pool[arg_name].execution_data = self.device_tensor_pool[
                arg_name
            ].data
    ...
    def postop(self, binary, programContext, opContext):
        ...
        if "cache" in device_op.name:
            for k, v in self.cache_tensor.items():
                tensor = retrieve_tensor_from_pool(programContext, v)
                torch_tensor = get_torch_tensor(tensor)
                print(f"Input {k} is {torch_tensor}")
                import pdb; pdb.set_trace()
        if len(get_op_outputs(device_op)) == 0:
            return
        ...
        output_name = get_op_outputs(device_op)[0].get_name()
...
def main():
    ...
    parser.add_argument("--main_fn", type=str, default="main")

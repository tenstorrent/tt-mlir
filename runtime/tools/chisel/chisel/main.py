...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Program index for execution"
    )
...
def main():
    ...
    try:
        ...
        raise PermissionError(f"Cannot create directory for output: {args.output_dir}: {e}")
    
    # Validate report path directory is writable
    try:
        args.report_path.parent.mkdir(parents=True, exist_ok=True)
    except (PermissionError, OSError) as e:
        raise PermissionError(f"Cannot create directory for report file {args.report_path}: {e}")

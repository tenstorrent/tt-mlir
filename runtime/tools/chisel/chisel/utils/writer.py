...
class ReportWriter:
    ...
    def _format_tensors(self, tensors: List[Operation], kind: ExecutionType):
        return "\n".join(sorted([self._format_tensor(tensor, kind) for tensor in tensors]))

    def write_row(self, **kwargs):
        assert "location" in kwargs
        assert "golden_ops" in kwargs
        assert "device_ops" in kwargs
        kwargs["golden_ops"] = self._format_ops(kwargs["golden_ops"], ExecutionType.GOLDEN)
        kwargs["device_ops"] = self._format_ops(kwargs["device_ops"], ExecutionType.DEVICE)
        kwargs["golden_output"] = self._format_tensor(kwargs["golden_output"], ExecutionType.GOLDEN)
        kwargs["device_output"] = self._format_tensor(kwargs["device_output"], ExecutionType.DEVICE)
        if "golden_inputs" in kwargs:
            kwargs["golden_inputs"] = self._format_tensors(kwargs["golden_inputs"], ExecutionType.GOLDEN)
        if "device_inputs" in kwargs:
            kwargs["device_inputs"] = self._format_tensors(kwargs["device_inputs"], ExecutionType.DEVICE)

...
ttmlir-opt --allow-unregistered-dialect --mlir-print-debuginfo --undo-const-eval --ttir-to-ttnn-backend-pipeline="enable-erase-inverse-ops-pass=false enable-const-eval=false system-desc-path=/proj_sw/user_dev/sgligorijevic/jaxbringups/tt-mlir/ttrt-artifacts/system_desc.ttsys" $folder/ttir.mlir -o $folder/ttnn.mlir
echo "Done ttir -> ttnn"

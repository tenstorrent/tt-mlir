...
TEST(ComputeMeshFabricConfig, EightDevices2x4LinearRowsDegenerateCols) {
  ...
  auto result = computeMeshFabricConfig({makeChannel(0, 1), makeChannel(1, 2),
                                         makeChannel(2, 3), makeChannel(4, 5),
                                         makeChannel(5, 6), makeChannel(6, 7),
                                         makeChannel(0, 4), makeChannel(1, 5),
                                         makeChannel(2, 6), makeChannel(3, 7)},
                                        {2, 4}, {0, 1, 2, 3, 4, 5, 6, 7});
...
TEST(ComputeMeshFabricConfig, TwelveDevices3x4RingRowsLinearColsTorusX) {
  ...
  auto result = computeMeshFabricConfig(
      {// row rings (wrap col0<->col3 on each row)
       ...
       makeChannel(0, 3), makeChannel(4, 5), makeChannel(5, 6),
       makeChannel(6, 7), makeChannel(4, 7), makeChannel(8, 9),
       makeChannel(9, 10), makeChannel(10, 11), makeChannel(8, 11),
       // columns linear (no row0<->row2 wrap)
       makeChannel(0, 4), makeChannel(4, 8), makeChannel(1, 5),
       makeChannel(5, 9), makeChannel(2, 6), makeChannel(6, 10),
       makeChannel(3, 7), makeChannel(7, 11)},
...
TEST(ComputeMeshFabricConfig, TwelveDevices4x3LinearRowsRingColsTorusY) {
  ...
  auto result = computeMeshFabricConfig(
      {// rows linear (no col0<->col2 wrap)
       makeChannel(0, 1), makeChannel(1, 2), makeChannel(3, 4),
       makeChannel(4, 5), makeChannel(6, 7), makeChannel(7, 8),
       makeChannel(9, 10), makeChannel(10, 11),
       // column rings (wrap row0<->row3 on each column)
       makeChannel(0, 3), makeChannel(3, 6), makeChannel(6, 9),
       makeChannel(0, 9), makeChannel(1, 4), makeChannel(4, 7),
       makeChannel(7, 10), makeChannel(1, 10), makeChannel(2, 5),
       makeChannel(5, 8), makeChannel(8, 11), makeChannel(2, 11)},

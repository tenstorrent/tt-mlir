...
def create_multi_device_input(shape, dtype, mesh_shape):
...
def create_grouped_multi_device_input(shape, dtype, mesh_shape, num_distinct):
    """Create a multi-device host tensor cycling through num_distinct buffers.

    Shard i borrows buffer i % num_distinct, so shards share host data
    without the tensor being fully replicated. Returns the runtime tensor and
    the per-shard torch tensor list (with repeats).
    """
    runtime_dtype = Binary.Program.to_data_type(dtype)
    num_devices = mesh_shape[0] * mesh_shape[1]
    distinct_tensors = [torch.randn(shape, dtype=dtype) for _ in range(num_distinct)]
    torch_shards = [distinct_tensors[i % num_distinct] for i in range(num_devices)]

    runtime_tensor = ttrt.runtime.create_multi_device_borrowed_host_tensor(
        [shard.data_ptr() for shard in torch_shards],
        list(shape),
        list(torch_shards[0].stride()),
        torch_shards[0].element_size(),
...
def create_replicated_multi_device_input(shape, dtype, mesh_shape):
    """Create a multi-device host tensor whose shards all borrow the same data.

    Every mesh coordinate shares one host buffer, which is how replicated
    tensors reach the runtime; the CPU-hoisted op dedupes on the shard data
    pointers and executes once for all shards.
    """
    runtime_tensor, torch_shards = create_grouped_multi_device_input(
        shape, dtype, mesh_shape, num_distinct=1
    )
    return runtime_tensor, torch_shards[0]
...
def run_and_verify_output_shards(
    helper, mesh_device, runtime_inputs, expected_output_shards
):
    """Submit program 0 and verify output shards match expected values."""

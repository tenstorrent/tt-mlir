...
def test_mnist_linear_logits_single_cq(helper: Helper, request, num_loops):
    binary_path = os.path.join(
    ...
    test_config = ProgramTestConfig(
        ...
        description="mnist linear logits trace test single cq",
    )
...
@pytest.mark.parametrize("num_loops", [16])
def test_mnist_linear_logits_two_cqs(helper: Helper, request, num_loops):
    binary_path = os.path.join(
        FLATBUFFER_BASE_PATH, "mnist_linear_logits.mlir.tmp.ttnn"
    )
    assert os.path.exists(binary_path), f"Binary file not found: {binary_path}"
    ...
    with DeviceContext(
        ...
        for i in range(num_loops - 1):
            output = test_runner.run_program(
                device, inputs_runtime_with_layout, blocking_to_host=False
            )
            assert debug_stats.get_stat("TraceCacheMiss") == 1
            assert debug_stats.get_stat("CapturedTrace") == 1
            assert debug_stats.get_stat("ExecutedTrace") == i + 1
        end_time = time.perf_counter() * 1000
        ttrt.runtime.wait(output)
        ttrt.runtime.memcpy(output_torch.data_ptr(), output)

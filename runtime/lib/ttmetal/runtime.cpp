...
static void stridedMemcpy(const TensorDesc &dst, const TensorDesc &src,
  ...
  const auto srcIndices =
      getStridedRowStartIndices(src.shape, src.physicalStrides);
  const auto dstIndices =
      getStridedRowStartIndices(dst.shape, dst.physicalStrides);

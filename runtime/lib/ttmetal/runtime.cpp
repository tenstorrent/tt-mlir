...
Device openMeshDevice(const MeshDeviceOptions &options) {
  LOG_ASSERT(options.meshShape.size() == 2, "Mesh shape must be 2D for now");
  ::tt::tt_metal::distributed::MeshShape shape{options.meshShape[0],
                                               options.meshShape[1]};

  std::optional<::tt::tt_metal::distributed::MeshCoordinate> offset =
      std::nullopt;
  if (options.meshOffset.has_value()) {
    const std::vector<uint32_t> &meshOffset = options.meshOffset.value();
    LOG_ASSERT(meshOffset.size() == 2, "Mesh offset must be 2D for now");

    offset = ::tt::tt_metal::distributed::MeshCoordinate{meshOffset[0],
                                                         meshOffset[1]};
  }

  std::vector<int> deviceIds;
  if (options.deviceIds.has_value()) {
    deviceIds = options.deviceIds.value();
    LOG_ASSERT(deviceIds.size() == shape.mesh_size(),
               "Number of devices must match mesh size");
  }

  size_t numHWCQs = options.numHWCQs.value_or(1);
  ...
  ::tt::tt_metal::distributed::MeshDeviceConfig meshConfig;
  meshConfig.mesh_shape = shape;
  meshConfig.offset = offset;
  meshConfig.physical_device_ids = deviceIds;

  std::shared_ptr<::tt::tt_metal::distributed::MeshDevice> meshDevice =
      ::tt::tt_metal::distributed::MeshDevice::create(
          meshConfig, l1SmallSize, DEFAULT_TRACE_REGION_SIZE, numHWCQs,
          dispatchCoreType);

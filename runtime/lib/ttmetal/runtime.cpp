...
Tensor toLayout(Tensor tensor, Device, Layout layout, std::optional<bool>) {
  return std::visit(utils::overloaded{
                        [&](const TensorDesc &desc) {
                          // TODO(wenbinlyuTT): implement toLayout.
                          return tensor;
                        },
...
std::array<size_t, 2> computePhysicalShape2D(const TensorDesc &desc) {
  // TODO(wenbinlyuTT): is padding the 2nd dim handled correctly? Like 128x1 or
  // 120x3?
...
// 2. At the end of each logical row, if W % 32 != 0. This is done by incremeing
// the physical row start index with the stride of roundUP(W, 32).
static std::vector<std::tuple<size_t, size_t>>
computeLogicalToPhysicalMapping(const TensorDesc &desc) {
  ...
  std::vector<std::tuple<size_t, size_t>> mapping;
...
static void alignAndPadPhysical2DTensorData(const void *src, void *dst,
                                            const TensorDesc &desc) {
  ...
  const std::byte *srcBytePtr = static_cast<const std::byte *>(src);
  std::byte *dstBytePtr = static_cast<std::byte *>(dst);
  ...
  for (const auto &[logicalIdxStart, physicalIdxStart] : copyIndices) {
    std::memcpy(dstBytePtr + physicalIdxStart, srcBytePtr + logicalIdxStart,
                rowSize);
  }
}
...
Tensor createOwnedHostTensor(const void *data, const TensorDesc &desc) {
  ...
  // Skip aligning & padding iff the physical shape isn't in the default row
  // shape anymore, since even logical inputs like 1xN needs to be padded to
  // 32xN.
  if (desc.physicalShape2D[0] != 1) {
    owned = utils::malloc_shared(desc.sizeBytes());
  ...
  TensorDesc alignedDesc(desc.shape, desc.stride, desc.itemsize, desc.dataType,
                         physicalShape2D);
  owned = utils::calloc_shared(alignedDesc.sizeBytes()); // Default zero-fill
  alignAndPadPhysical2DTensorData(data, owned.get(), alignedDesc);
...
std::vector<Tensor> toHost(Tensor tensor, bool untilize, bool blocking) {
  ...
  std::visit(
      utils::overloaded{
          [&](const TensorDesc &d) {
            fprintf(stderr, "-- toHost: No-Op of shape [%u %u] stride [%u %u]",
                    d.shape[0], d.shape[1], d.stride[0], d.stride[1]);
            fprintf(stderr, " physicalShape2D [%zu %zu] size %zu\n",
                    d.physicalShape2D[0], d.physicalShape2D[1], d.sizeBytes());
          },
          [&](const MeshBuffer &) {
...
void memcpy(void *dst, Tensor src,
  ...
  fprintf(stderr, "-- memcpy(dt): shape [%u %u] size %zu\n", hostSrc.shape[0],
          hostSrc.shape[1], hostSrc.sizeBytes());
...
void memcpy(Tensor dst, Tensor src) {
  ...
  fprintf(stderr, "-- memcpy(no-dt):");
  fprintf(stderr, " srcShape [%u %u] srcStride [%u %u] phyShape [%zu %zu]",
          hostSrc.shape[0], hostSrc.shape[1], hostSrc.stride[0],
          hostSrc.stride[1], hostSrc.physicalShape2D[0],
          hostSrc.physicalShape2D[1]);
  fprintf(stderr, " dstShape [%u %u] dstStride [%u %u] phyShape [%zu %zu]",
          hostDst.shape[0], hostDst.shape[1], hostDst.stride[0],
          hostDst.stride[1], hostDst.physicalShape2D[0],
          hostDst.physicalShape2D[1]);
  fprintf(stderr, " minSize/copySize %zu\n", copySize);

...
inline std::shared_ptr<tt_metal::Buffer> createBufferFromBufferRef(
    tt_metal::IDevice *device, target::metal::BufferRef const *bufferRef,
    DeviceAddressValidator const &deviceAddressValidator) {
  target::metal::BufferDesc const *bufferDesc = bufferRef->desc();
  target::metal::ShardedBufferConfig const *shardedBufferConfig =
      bufferDesc->sharded_buffer_config();
  target::metal::ShardSpecBuffer const *shardSpecBuffer =
      shardedBufferConfig->shard_spec_buffer();
  target::metal::ShardSpec const *shardSpec = shardSpecBuffer->shard_spec();
  ...
  assert(bufferDesc->memory_space() == target::MemorySpace::DeviceDRAM ||
...
inline std::string kernelConfigTypeString(
    std::variant<tt_metal::DataMovementConfig, tt_metal::ComputeConfig,
                 tt_metal::EthernetConfig> const &kernelConfig) {
...
inline tt_metal::KernelHandle
createKernel(tt_metal::Program &program, std::string const &kernelSource,
             CoreRangeSet const &coreRangeSet,
             std::variant<tt_metal::DataMovementConfig, tt_metal::ComputeConfig,
                          tt_metal::EthernetConfig> const &kernelConfig,
             char const *currentProgramName, char const *programDebugInfo,
             char const *kernelDebugInfo) {
  ...
  for (auto const *kernelArg : *args) {
    ...
    case target::metal::KernelArgType::KernelArgSemaphore: {
      LOG_ASSERT(!isCompileTime,
                 "Semaphores are not supported in compile time");
...
createKernelConfig(
    target::metal::KernelConfig const *kernelConfig,
    const flatbuffers::Vector<flatbuffers::Offset<tt::target::metal::BufferRef>>
        *buffers,
    std::unordered_map<std::uint32_t, DeviceBuffer> const &deviceBuffers,
    const flatbuffers::Vector<flatbuffers::Offset<tt::target::metal::CBRef>>
  ...
  std::vector<uint32_t> compileArgs =
      processCompileArgs(kernelConfig->args()->ct_args(), buffers,
                         deviceBuffers, cbs, deviceAddressValidator);
...
inline tt_metal::CircularBufferConfig createCircularBufferConfig(
    target::metal::CBRef const *cbRef,
    std::unordered_map<std::uint32_t, DeviceBuffer> const &deviceBuffers) {

...
std::vector<std::uint32_t> processKernelArgs(
    const flatbuffers::Vector<flatbuffers::Offset<target::metal::KernelArg>> *args,
    const flatbuffers::Vector<target::metal::ArgRef> *argRefsType,
    const flatbuffers::Vector<flatbuffers::Offset<void>> *argRefs,
    const std::unordered_map<std::uint32_t, std::shared_ptr<distributed::MeshBuffer>> &meshBuffers,
    const std::unordered_map<std::uint32_t, tt_metal::GlobalSemaphore> &global_semaphores_cache,
    const flatbuffers::Vector<flatbuffers::Offset<tt::target::metal::CBRef>> *cbs,
    const DeviceAddressValidator &deviceAddressValidator,
    std::function<std::uint32_t(std::uint32_t)> createSemaphoreFn) {
  
  // We want to create a vector of uint32_t args to pass to the runtime kernel launch APIs.
  ...
  for (const auto *kernelArg : *args) {
    ...
    case target::metal::KernelArgType::KernelArgCBPort: {
      ...
      LOG_ASSERT(arg->operand_idx() < cbs->size(), "invalid operand ", arg->operand_idx());
    ...
    case target::metal::KernelArgType::KernelArgBufferAddress: {
      ...
      LOG_ASSERT(argRefsType->Get(arg->operand_idx()) == target::metal::ArgRef::BufferRef);
      const target::metal::BufferRef *buffer = reinterpret_cast<const target::metal::BufferRef *>(argRefs->Get(arg->operand_idx()));
      ...
      LOG_ASSERT(bufferDesc->buffer_detail_type() == target::metal::BufferDetail::MetalBuffer);
      const target::metal::MetalBuffer *metalBuffer = bufferDesc->buffer_detail_as_MetalBuffer();
      argsVec.push_back(deviceAddressValidator(buffer->address(), metalBuffer->buffer_type()));
      break;
    }
    ...
    case target::metal::KernelArgType::KernelArgGlobalSemaphore: {
      ...
      LOG_ASSERT(argRefsType->Get(arg->operand_idx()) == target::metal::ArgRef::GlobalSemaphoreRef);
      const tt::target::metal::GlobalSemaphoreRef *global_semaphore_operand = reinterpret_cast<const target::metal::GlobalSemaphoreRef *>(argRefs->Get(arg->operand_idx()));
      LOG_ASSERT(
      ...
      argsVec.push_back(deviceAddressValidator(global_semaphores_cache.at(global_semaphore_operand->global_id()).address(),

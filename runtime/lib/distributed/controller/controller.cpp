...
// Merges system descriptors generated by each worker on its local device.
// Currently the three fields that need to be merged are:
// - chip_descs
// - chip_capabilities
// - chip_desc_indices
...
mergeSystemDescs(const std::vector<::tt::runtime::SystemDesc> &systemDescs) {
  ...
  auto unpackSystemDesc = [](const ::tt::runtime::SystemDesc &systemDesc)
      -> std::unique_ptr<::tt::target::SystemDescRootT> {
    return std::unique_ptr<::tt::target::SystemDescRootT>(
        ::tt::target::UnPackSizePrefixedSystemDescRoot(
            systemDesc.handle.get()));
  };

  std::unique_ptr<::flatbuffers::FlatBufferBuilder> fbb =
      std::make_unique<::flatbuffers::FlatBufferBuilder>();

  // Collect chip descriptors by capability
  ...
  for (const ::tt::runtime::SystemDesc &systemDesc : systemDescs) {
    ...
    for (size_t i = 0; i < systemDescObj.chip_desc_indices.size(); i++) {
      const ::tt::target::ChipCapability chipCapability =
          systemDescObj.chip_capabilities[i];
  ...
  std::vector<::tt::target::ChipCapability> allChipCapabilities;
  std::vector<uint32_t> allChipDescIndices;

  for (size_t i = 0; i < mmioChipDescs.size(); i++) {
    allChipDescs.emplace_back(std::move(mmioChipDescs[i]));
    allChipCapabilities.emplace_back(::tt::target::ChipCapability::HostMMIO);
    allChipDescIndices.emplace_back(allChipDescIndices.size());
  }

  for (size_t i = 0; i < nonMmioChipDescs.size(); i++) {
    allChipDescs.emplace_back(std::move(nonMmioChipDescs[i]));
    allChipCapabilities.emplace_back(::tt::target::ChipCapability::NONE);
    allChipDescIndices.emplace_back(allChipDescIndices.size());
  }

  std::unique_ptr<::tt::target::SystemDescRootT> mergedSystemDesc =
      unpackSystemDesc(systemDescs[0]);

  mergedSystemDesc->system_desc->chip_descs = std::move(allChipDescs);
  mergedSystemDesc->system_desc->chip_capabilities =
      std::move(allChipCapabilities);
  ...
  debug::verifyFlatbuffer(*fbb,
                          ::tt::target::VerifySizePrefixedSystemDescRootBuffer);
...
void Controller::launch(const ::tt::runtime::DistributedOptions &options) {
  ...
  case DistributedMode::LocalSubprocess: {
    ...
    numWorkers = 1;
    command = ::tt::runtime::distributed::utils::getWorkerExecutableCommand(
        bindPort, options.workerPath);
    break;
  }
  case DistributedMode::MultiProcess: {
    ...
    command = ::tt::runtime::distributed::utils::getTTRunCommand(
        bindPort, multiProcessArgs, options.workerPath);
    break;
  }
  }
...
Controller::getTensorDesc(const ::tt::runtime::Tensor &tensorHandle) {
  auto commandBuilder = std::make_unique<::flatbuffers::FlatBufferBuilder>();

  uint64_t commandId =
      CommandFactory::buildGetTensorDescCommand(*commandBuilder, tensorHandle);

  std::shared_ptr<::tt::runtime::TensorDesc> resultDescHandle =
      std::make_shared<::tt::runtime::TensorDesc>();

  auto awaitingHandles = std::make_unique<std::vector<std::shared_ptr<void>>>();
  awaitingHandles->push_back(std::static_pointer_cast<void>(resultDescHandle));

  auto awaitingPromise = std::make_unique<std::promise<void>>();
  std::future<void> awaitingFuture = awaitingPromise->get_future();

  pushToCommandAndResponseQueues(
      commandId, fb::CommandType::GetTensorDescCommand,
      std::move(commandBuilder), std::move(awaitingHandles),
      std::move(awaitingPromise));
...
void Controller::handleGetTensorDescResponse(
    const std::vector<SizedBuffer> &responseBuffers,
    std::unique_ptr<AwaitingResponseQueueEntry> awaitingResponse) {
  ...
  debug::checkResponseTypes(responseBuffers,
                            fb::ResponseType::GetTensorDescResponse);

  const fb::GetTensorDescResponse *response =
      getResponse(responseBuffers[0])->type_as_GetTensorDescResponse();

  auto [awaitingHandles, awaitingPromise] =
      awaitingResponse->popAwaitingState();

  DEBUG_ASSERT(awaitingHandles && awaitingHandles->size() == 1,
               "GetTensorDesc: Awaiting handles must be populated and contain "
               "exactly one handle");
  DEBUG_ASSERT(awaitingPromise, "Awaiting promise must be populated");

  std::shared_ptr<::tt::runtime::TensorDesc> tensorDescHandle =
      std::static_pointer_cast<::tt::runtime::TensorDesc>(
          awaitingHandles->at(0));

  // Extract fields from FlatBuffer TensorDescFlat into runtime TensorDesc
  const fb::TensorDescFlat *tensorDescFlat = response->tensor_desc();

...
// directly from the TTNN tensor. Ideally, we should be able to get all of
// this info directly from the flatbuffer using the "inSystemMemory" API below
...
bool inSystemMemory(const ::tt::target::ttnn::TensorRef *tensorRef) {
  ...
  return ttnn_op_invoke::operations::utils::inSystemMemory(tensorRefNative);
}
...
toTTNNCoreRangeSet(const tt::target::ttnn::CoreRangeSet &coreRangeSet) {
  tt::target::ttnn::CoreRangeSetT coreRangeSetNative;
  coreRangeSet.UnPackTo(&coreRangeSetNative);
  return ttnn_op_invoke::operations::utils::toTTNNCoreRangeSet(
      coreRangeSetNative);
}
...
createMemoryConfigIfNeeded(const ::tt::target::ttnn::MemoryConfig *memcfg) {
  ...
  ::tt::target::ttnn::MemoryConfigT memcfgNative;
  memcfg->UnPackTo(&memcfgNative);
  return ttnn_op_invoke::operations::utils::createMemoryConfigIfNeeded(
      memcfgNative);
}

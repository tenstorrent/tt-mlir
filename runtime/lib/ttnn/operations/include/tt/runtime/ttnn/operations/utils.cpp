...
createConv2dConfig(const ::tt::target::ttnn::Conv2dConfig *memcfg) {
  ...
  if (memcfg->dtype()) {
    conv2dConfig.dtype =
        ::tt::runtime::ttnn::utils::toTTNNDataType(memcfg->dtype().value());
  }

  if (memcfg->weights_dtype()) {
    conv2dConfig.weights_dtype =
        ::tt::runtime::ttnn::utils::toTTNNDataType(*memcfg->weights_dtype());
  }

  if (memcfg->activation()) {
    conv2dConfig.activation = memcfg->activation()->str();
  }

  if (memcfg->input_channels_alignment()) {
    conv2dConfig.input_channels_alignment = *memcfg->input_channels_alignment();
  }

  if (memcfg->deallocate_activation()) {
    conv2dConfig.deallocate_activation = *memcfg->deallocate_activation();
  }

  if (memcfg->reallocate_halo_output()) {
    conv2dConfig.reallocate_halo_output = *memcfg->reallocate_halo_output();
  }

  if (memcfg->act_block_h_override()) {
    conv2dConfig.act_block_h_override = *memcfg->act_block_h_override();
  }

  if (memcfg->act_block_w_div()) {
    conv2dConfig.act_block_w_div = *memcfg->act_block_w_div();
  }

  if (memcfg->reshard_if_not_optimal()) {
    conv2dConfig.reshard_if_not_optimal = *memcfg->reshard_if_not_optimal();
  }

  if (memcfg->override_sharding_config()) {
    conv2dConfig.override_sharding_config = *memcfg->override_sharding_config();
  }
  ...
  if (memcfg->transpose_shards()) {
    conv2dConfig.transpose_shards = *memcfg->transpose_shards();
  }

  if (memcfg->output_layout()) {
    conv2dConfig.output_layout =
        ::tt::runtime::ttnn::utils::toTTNNLayout(*memcfg->output_layout());
  }

  if (memcfg->preprocess_weights_on_device()) {
    conv2dConfig.preprocess_weights_on_device =
        *memcfg->preprocess_weights_on_device();
  }

  if (memcfg->always_preprocess_weights()) {
    conv2dConfig.always_preprocess_weights =
        *memcfg->always_preprocess_weights();
  }

  if (memcfg->enable_act_double_buffer()) {
    conv2dConfig.enable_act_double_buffer = *memcfg->enable_act_double_buffer();
  }

  if (memcfg->enable_weights_double_buffer()) {
    conv2dConfig.enable_weights_double_buffer =
        *memcfg->enable_weights_double_buffer();
  }

  if (memcfg->enable_split_reader()) {
    conv2dConfig.enable_split_reader = *memcfg->enable_split_reader();
  }

  if (memcfg->enable_subblock_padding()) {
    conv2dConfig.enable_subblock_padding = *memcfg->enable_subblock_padding();
  }

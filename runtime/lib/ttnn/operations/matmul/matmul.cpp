...
createSparseMatmulProgramConfig(const ::ttnn::Tensor &inputA,
  ...
  ::ttnn::operations::matmul::MatmulMultiCoreReuseMultiCast1DProgramConfig config;
...
void run(const ::tt::target::ttnn::SparseMatmulOp *op, ProgramContext &context) {
  ...
  if (sparsity_orig.layout() == ::ttnn::Layout::TILE) {
    // Move to host, untilize, then back to device
    auto hostTensor = ::ttnn::from_device(sparsity_orig, /*blocking=*/true);
    auto untilizedTensor = ::ttnn::to_layout(hostTensor, ::ttnn::Layout::ROW_MAJOR, std::nullopt, std::nullopt);
    sparsity = ::ttnn::to_device(untilizedTensor, sparsity_orig.device(), sparsity_orig.memory_config());
  }
  ...
  ::ttnn::Tensor output = ::ttnn::sparse_matmul(
      a, b, sparsity,
      /*nnz=*/nnz,

...
void run(const ::tt::target::ttnn::MatmulOp *op, ProgramContext &context) {
  ...
  std::cout << op->out()->desc()->layout()->memory_desc()->tile_shape() << std::endl;  

  std::optional<::ttnn::operations::matmul::MatmulProgramConfig> matmulOpConfig = std::nullopt;
  ...
  if (outputMemoryConfig.is_sharded()) {
    CoreCoord x = outputMemoryConfig.shard_spec->grid.ranges().begin()->end_coord;
    ...
    matmulOpConfig = ::ttnn::operations::matmul::MatmulMultiCoreReuseMultiCastProgramConfig {

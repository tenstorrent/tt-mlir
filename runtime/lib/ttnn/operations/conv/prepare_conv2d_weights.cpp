...
void run(const ::tt::target::ttnn::PrepareConv2dWeightsOp *op,
  ...
  const ::ttnn::Tensor &weight_tensor =
      tensorPool.getAndValidate(op->weight_tensor());

  ::ttnn::MemoryConfig input_memory_config;
  if (auto config = ::tt::runtime::ttnn::utils::createMemoryConfigIfNeeded(
          op->input_memory_config())) {
    input_memory_config = *config;
  }
  ...
  ::ttnn::Tensor out;
  if (std::holds_alternative<std::reference_wrapper<::ttnn::IDevice>>(
          targetDevice)) {
    auto *device =
        &std::get<std::reference_wrapper<::ttnn::IDevice>>(targetDevice).get();
    out =
        ::ttnn::operations::conv::conv2d::prepare_conv_weights<::ttnn::IDevice>(
            weight_tensor, input_memory_config,
            ...
            stride, padding, dilation, op->has_bias(), op->groups(), device,
            conv2dConfig, std::nullopt);
  } else if (std::holds_alternative<std::reference_wrapper<::ttnn::MeshDevice>>(
                 targetDevice)) {
    auto *device =
        &std::get<std::reference_wrapper<::ttnn::MeshDevice>>(targetDevice)
             .get();
    out = ::ttnn::operations::conv::conv2d::prepare_conv_weights<
        ::ttnn::MeshDevice>(
        weight_tensor, input_memory_config,
        ::tt::runtime::ttnn::utils::toTTNNLayout(op->input_tensor_layout()),
        op->weights_format()->str(), op->in_channels(), op->out_channels(),

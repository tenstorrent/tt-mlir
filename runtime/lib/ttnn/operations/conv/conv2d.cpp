...
void run(const ::tt::target::ttnn::Conv2dOp *op, ProgramContext &context) {
  ...
  //
  // When TTNNPrepareConv2dWeightsAndBias (compile-time pass) is enabled, the
  // PrepareConv2dWeightsOp runs before this op and places the weight into
  // device memory (DRAM). Requesting re-preparation in that case double-
  ...
  // Request prepared weights on cache miss, regardless of whether the weight
  // is already on device. When trace is enabled, the weight may be uploaded to

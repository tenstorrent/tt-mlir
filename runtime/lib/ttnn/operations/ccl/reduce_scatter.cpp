...
void run(const ::tt::target::ttnn::ReduceScatterOp *op,
  ...
  int32_t scatterDimension = op->scatter_dimension();
  int32_t numLinks = op->num_links();
  int32_t clusterAxis = op->cluster_axis();
  auto reduceType = static_cast<::ttnn::operations::reduction::ReduceType>(op->reduce_type());

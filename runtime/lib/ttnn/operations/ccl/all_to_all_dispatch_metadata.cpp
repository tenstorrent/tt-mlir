...
// newly-unrouted slots. (Ported from the GPT-OSS e2e branch.)
...
// produces a fully-replicated topology, but the dispatch kernel expects an
// axis-0 sharded view. (Ported from the GPT-OSS e2e branch.)
...
void run(const ::tt::target::ttnn::AllToAllDispatchMetadataOp *op,
  ...
  // Persistent pre-zeroed outputs. Our flatbuffer op has no
  // optional_*_output_tensor operands, so always allocate via createZeroOutput.
  // All three need axis-0 sharded topology (ttnn::zeros defaults to replicated).
  ::ttnn::Tensor dispatchedOutput = createZeroOutput(op->dispatched(), context);
  ::ttnn::Tensor indicesOutput = createZeroOutput(op->indices(), context);
  ::ttnn::Tensor scoresOutput = createZeroOutput(op->scores(), context);
  ...
  // drain_sync_tilizer_core (forcing one was observed to collapse the dispatch).

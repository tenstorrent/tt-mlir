...
static void executeTrace(const ::tt::target::ttnn::TraceOp *op,
  ...
  for (const ::tt::target::ttnn::TensorRef *input : *op->inputs()) {
    ::tt::runtime::Tensor inputRuntimeTensor =
        context.getTensorPool().getRuntimeTensorAndValidate(input);
    ...
    inputs.push_back(::ttnn::from_device(inputTensor));
  }

  LOG_ASSERT(inputs.size() == traceData.inputTensors.size());
  for (size_t i = 0; i < inputs.size(); i++) {
    ::ttnn::Tensor &inputSlot =
        ::tt::runtime::ttnn::utils::getTTNNTensorFromRuntimeTensor(
            traceData.inputTensors[i]);
    DEBUG_ASSERT(inputSlot.is_allocated());

    ::ttnn::Tensor &inputData = inputs[i];
    // Copy input data to the trace input slot
    ::tt::tt_metal::write_tensor(inputData, inputSlot, ::ttnn::DefaultQueueId);
  }

...
static void runTraceProgramAndCaptureTrace(
  ...
  // Input/output slots must not overlap with intermediates allocated during
  // trace capture. Intermediates use the default bottom-up allocation (low
  // addresses); we flip all EmptyOp allocations in this capture program to
  // top-down so slots live at high addresses and can't collide with them.

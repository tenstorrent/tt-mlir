...
void CallbackTensor::updateTensor(CallbackContext programContext) {
  ...
  ::ttnn::Tensor dstTensor = tensorPool.getAndValidate(tensorRef);
  ...
  tensorPool.insertAndValidate(tensorRef, srcTensor);
}
...
getOpOutputTensor(OpContext opContextHandle,
  ...
  const auto &outPtr = &tensorPool.getAndValidate(tensorRefPtr.value());
...
getOpInputTensors(OpContext opContextHandle,
  ...
  for (const auto &tensorRef : tensorRefs) {
    ...
    const auto &inPtr = &tensorPool.getAndValidate(tensorRef);
    ...
    tensors.push_back(std::make_unique<CallbackTensor>(
        Tensor(std::move(hostTensor), nullptr, DeviceRuntime::TTNN),

...
Device openMeshDevice(const MeshDeviceOptions &options) {
  ...
  ::ttnn::MeshShape shape{options.meshShape[0], options.meshShape[1]};
  ...
  if (options.meshOffset.has_value()) {
    ...
    offset = ::ttnn::MeshCoordinate{meshOffset[0], meshOffset[1]};
  }
  ...
  bool enableAsyncTTNN = options.enableAsyncTTNN.value_or(false);

  ::ttnn::MeshDeviceConfig meshConfig;
  ...
  meshDevice->enable_async(enableAsyncTTNN);

...
void memcpy(Tensor dst, Tensor src) {
  ...
  ::tt::runtime::ttnn::memcpy(dstPtr, src);
}
...
std::vector<Tensor> submit(Device deviceHandle, Binary executableHandle,
  ...
  std::vector<::ttnn::Tensor *> inputs;
  inputs.reserve(inputHandles.size());
  std::transform(inputHandles.begin(), inputHandles.end(),
                 std::back_inserter(inputs),
                 [](const Tensor &input) -> ::ttnn::Tensor * {
                   LOG_ASSERT(input.matchesRuntime(DeviceRuntime::TTNN));
                   return static_cast<::ttnn::Tensor *>(input.handle.get());
                 });

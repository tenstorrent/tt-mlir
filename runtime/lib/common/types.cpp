...
MultiProcessArgs &MultiProcessArgs::withRankBindingPath(std::string_view path) {
  rankBindingPath_ = path;
  return *this;
}
...
std::string MultiProcessArgs::toArgString() const {
  ...
  oss << "--rank-binding-path " << rankBindingPath_;
  ...
  // Extra MPI args
  if (!extraMpiArgs_.empty()) {
    oss << " ";
    for (size_t i = 0; i < extraMpiArgs_.size(); i++) {
      oss << extraMpiArgs_[i];
      if (i != extraMpiArgs_.size() - 1) {
        oss << " ";
      }
    }
  }

...
| Layouts we emit | `TTNNLayout` default: DRAM interleaved, tiled per `shouldTilizeResult(op)` | DRAM interleaved only, tile or row-major (rule books can ask for RM siblings) | anything, subject to rule books and constraint queries |
...
| Rule books | not consulted | consulted; only page-layout parts matter since every candidate is DRAM interleaved | fully live: input filters, output hints, reshard exploration |
| Compile-time check of metal asserts | **none** | `OperationValidationAndFallback`: constraint query per OpModel op, then fallbacks | same, plus every candidate the optimizer considers is constraint-queried |
| Fallback set on query failure | n/a | original IR config (including a bfp8 dtype) tried first; then inputs: {bf16, f32, u32, i32} x {tile, row-major} x DRAM interleaved; output forced DRAM interleaved | same |
...
- **Level 0 has no compile-time net.** A violated assert is a device fault. Only operand
  workarounds, decomposition workarounds, and TTIR verifiers stand between the IR and the
  kernel. Buffer-type and memory-layout asserts are mostly moot here because everything is
  DRAM interleaved, unless a workaround itself sets something else.
...
- **Silent wrong results are never caught by queries.** If metal returns garbage instead
  of asserting (its comment or the linked issue will say "incorrect results"), only a
  rule-book filter can own that, and the uplift diff cannot tell you when it is safe to
  drop. Those TIGHT candidates need a silicon check.
- **Compute-kernel-config asserts (math fidelity, fp32 dest acc) are reachable only where a
  ...
  frontend forces a knob (tt-xla forwards `math_fidelity` / `fp32_dest_acc_en` compile
  options). At levels >= 1 with nothing forced, TTNN's own defaults are what the kernel
  sees, so an assert there is on metal's defaults, not on anything tt-mlir sent.
...
| shape, rank, dim relations, padding | TTIR op verifier, `lib/Dialect/TTIR/IR/TTIROps.cpp`; decomposition workaround under `include/ttmlir/Dialect/TTNN/Transforms/Workarounds/Decomposition/` | `git grep -n "<Op>::verify" origin/main`; ls the Decomposition dir |
| scalar attribute values (`dropout_probability == 0.0F`) | the same verifier. A `.td` default is **not** enforcement | `TTIROps.td` + verifier |
...
Only refusals inside `getOpConstraints` matter; `getOpRuntime` refusals (creation ops,
weight-preparation ops) affect scoring, not validation. Do not read the file with a plain
`grep -B`, the window lands on the previous function's name. On main the always-refusing
set is `DeallocateOp`, `MoeGptOp`, `PrepareConv3dWeightsOp`, `ToTensorSpecOp`;
`TypecastOp` refuses only for a system-memory input. A refusing op is skipped by
`OperationValidationAndFallback` and makes `GreedyL1SpillManagement` evict everything live
from L1. Every cell for such an op at level >= 1 is `unprotected` unless a workaround or
verifier forces the condition. Ops without the OpModel interface at all are skipped the
same way.
